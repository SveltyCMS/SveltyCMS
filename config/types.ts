...
import {
  object, string, number, boolean, array, optional, minLength, minValue, maxValue,
  literal, union, safeParse
} from 'valibot';
...
export const publicConfigSchema = object({
  ...
  SEASON_REGION: optional(union([
    literal('Western_Europe'),
    literal('South_Asia'),
    literal('East_Asia'),
    literal('Global')
  ])),  // Restricted to Western_Europe, South_Asia, or East_Asia
  ...
  LOG_LEVELS: array(union([
    literal('fatal'), literal('error'), literal('warn'), literal('debug'),
    literal('info'), literal('trace'), literal('none')
  ]), [minLength(1)]),
...
function formatPath(path: Array<{ key: string | number }> | undefined): string {
  ...
  return path.map(p => p.key).filter(Boolean).join('.');
}
...
export function validateConfig(schema: unknown, config: unknown, configName: string): unknown {
  ...
  if (result.success) {
    ...
    for (const issue of result.issues) {
      ...
      const isMissingField = issue.message.toLowerCase().includes('required') ||
...
function performConditionalValidation(config: unknown, configName: string): unknown {
  ...
  // Language validation
  if (config.DEFAULT_CONTENT_LANGUAGE && config.AVAILABLE_CONTENT_LANGUAGES &&
  ...
  if (config.DEFAULT_SYSTEM_LANGUAGE && config.AVAILABLE_SYSTEM_LANGUAGES &&
