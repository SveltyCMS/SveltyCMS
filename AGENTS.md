...
- **Modern Stack**: Latest TypeScript (^6.0.3, TS7 pending [language-tools#3063](https://github.com/sveltejs/language-tools/issues/3063)), Node.js (>=24), Svelte 5 (^5.56.10), Vite 8 (^8.2.2), Bun (3-4x faster runtime)
- **Code Quality**: SveltyCMS uses a **tiered validation pipeline** via git hooks:
  - **Pre-commit** (`git commit`): check-test-db-safety → format+lint → lint-staged → **full risk audit** (`bun run risk:audit`: global security scan across all of `src/` incl. all 4 DB adapters + secret misuse + slop + `bun audit` + OSV.dev global check) → **unit tests** (~40s) → **SBOM sync** (auto-regenerates `sbom.json` when `bun.lock`/`package.json` changed). Docs-only commits skip risk audit + unit tests.
  ...
  - **No double-run rule**: avoid re-running the full unit suite on pre-push (pre-commit already ran it).
  - **Private config policy (live data safety)**: Automated local work (tests, builds, Playwright) must **never read or write `config/private.ts`** — that file is the developer's live CMS and using it can destroy real data. Always **`config/private.test.ts`**. CI may create ephemeral `private.ts` on the runner only (never pushed). See `src/utils/private-config-policy.ts`.
...
> [!CAUTION]
> **All files and folders MUST use kebab-case.** PascalCase filenames (e.g. `ConfigTile.svelte`) will fail on Linux CI runners because the filesystem is case-sensitive. Windows may mask this issue, but CI will reject it. Format + lint (`bun run check`) catch naming violations.
>
...
| `scan-security-risk.ts` | `bun run scripts/scan-security-risk.ts --strict` | SQL value/concat interpolation, Mongo `$where`/`$function`, eval/new Function, shell interpolation, path traversal, SSRF, XSS sinks, regex injection, SvelteKit CSRF/cookie config (entire `src/`, all 4 adapters) |
...
2. **Seed via Testing API**: Use `POST /api/testing` with `TEST_API_HEADERS` from `tests/e2e/helpers/api.ts`. Prefer helpers in that file:
   - `seedWebhook` / `deleteWebhook`
   - `seedAutomation` / `deleteAutomation`
   - `enablePlugin` (core plugins expected in CI)
   - `seedExpiredPasswordReset` — past-expiry `password_reset` token (TOKEN_EXPIRED toast E2E)
   - `seedMediaWithMetadata` — media rows with distinct `metadata` for `?jsonPath=` tests
   - Existing: `seedTestUsers`, `prepareTestUser`, `setTestSetting`, `seedInviteToken`, …
   - Handler actions live in `src/routes/api/[...path]/handlers/testing.ts` (`seed-webhook`, `seed-automation`, `seed-expired-password-reset`, `seed-media-with-metadata`, `enable-plugin`, …). Add new seed actions there when a control map needs deterministic state.
...
|               | `bun run test:integration`                | Build + integration (SQLite; harness)                       |
...
| **CI Parity** | `bun run test:doctor` then `bun run gate` | Local green before push; full matrix is GitHub Actions only |
...
- **Pre-commit enforcement**: The `.githooks/pre-commit` and `.githooks/pre-push` hooks enforce the quality pipeline. No convenient local `GIT_HOOK_SKIP` (only real CI envs bypass). Using raw `git --no-verify` requires explicitly invoking the system git binary. **If the gate fails, FIX the underlying issue — never bypass the gate.**
