...
describe("benchmark-db-purge", () => {
  it("resolves database purge scopes", () => {
    ...
    expect(resolveBenchmarkDbScope("benchmark_shared.sqlite")).toBe("benchmark-shared");
    expect(resolveBenchmarkDbScope("bench_tmp_sqlite_123.sqlite")).toBe("test-ephemeral");
    expect(resolveBenchmarkDbScope("SveltyCMS_healing_test_sqlite.sqlite")).toBe("test-ephemeral");
  });

  it("detects stale collection tables by scope", () => {
    expect(shouldPurgeDatabaseCollection("mockcollection42", "user")).toBe(true);
    expect(shouldPurgeDatabaseCollection("mock_collection_1", "user")).toBe(true);
    ...
    expect(shouldPurgeDatabaseCollection("mockcollection42", "benchmark-shared")).toBe(true);
    expect(shouldPurgeDatabaseCollection("BenchmarkStable", "benchmark-shared")).toBe(false);
    expect(shouldPurgeDatabaseCollection("benchacid", "benchmark-shared")).toBe(true);
    expect(shouldPurgeDatabaseCollection("benchacid", "test-ephemeral")).toBe(true);
  });

  it("maps collection table names to ids", () => {
    expect(collectionTableToId("collection_mockcollection0")).toBe("mockcollection0");
    expect(collectionTableToId("collection_posts")).toBe("posts");
  });

  it("detects stale content_nodes paths", () => {
    expect(
      isStaleCollectionContentNode("/collection/mock collection 0", "mock collection 0", "user"),
    ).toBe(true);
    expect(isStaleCollectionContentNode("/collection/posts", "posts", "user")).toBe(false);
  });
