	...
	describe('Execution Logic (Basic)', () => {
		it('should execute a log operation successfully', async () => {
			const flow = {
				id: 'flow-1',
				name: 'Log Flow',
				operations: [{ id: 'op-1', type: 'log', config: { message: 'Hello', level: 'info' } }]
			} as any;

			const payload = { event: 'test', timestamp: new Date().toISOString() } as any;
		...
		it('should stop chain on condition failure', async () => {
			const flow = {
				...
				name: 'Condition Flow',
				operations: [
					{ id: 'op-1', type: 'condition', config: { field: 'status', operator: 'equals', value: 'published' } },
