...
describe("Relation Widget — Single", () => {
  it("validates single ID", () => {
    const f = RelationWidget({ label: "Author", relation: "users", required: true });
    const s = getSchema(f);
...
describe("Relation Widget — Multiple", () => {
  it("validates array of IDs", () => {
    const f = RelationWidget({ label: "Tags", relation: "tags", multiple: true });
    const s = getSchema(f);
  ...
  it("enforces min relations", () => {
    const f = RelationWidget({ label: "Tags", relation: "tags", multiple: true, min: 2 });
    const s = getSchema(f);
    expect(safeParse(s, ["t1", "t2"]).success).toBe(true);
  ...
  it("enforces max relations", () => {
    const f = RelationWidget({ label: "Tags", relation: "tags", multiple: true, max: 3 });
    const s = getSchema(f);
