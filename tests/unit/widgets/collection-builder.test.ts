...
describe("Collection Builder Widget Integration", () => {
  ...
  describe("Widget Selection", () => {
    test("should filter widgets by active status", () => {
      ...
      const filteredWidgets = availableWidgets.filter((key) =>
        mockWidgetStore.activeWidgets.includes(key),
      );
    ...
    test("should handle widget search functionality", () => {
      ...
      const filteredBySearch = Object.keys(widgets).filter(
        (key) =>
          ...
          widgets[key].Description?.toLowerCase().includes(searchTerm.toLowerCase()),
      );
  ...
  describe("Widget Configuration", () => {
    test("should access widget GUI schema for forms", () => {
      ...
      const specificOptions = Object.keys(properties).filter(
        (option) => !standardFields.includes(option),
      );
  ...
  describe("Widget Management Integration", () => {
    test("should handle widget activation/deactivation", () => {
      ...
      const availableForCollections = Object.keys(widgets).filter((key) =>
        mockWidgetStore.activeWidgets.includes(key),
      );
      ...
      const updatedAvailable = Object.keys(widgets).filter((key) =>
        mockWidgetStore.activeWidgets.includes(key),
      );
