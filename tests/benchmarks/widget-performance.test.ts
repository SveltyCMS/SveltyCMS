...
const richSchema = {
  ...
  fields: [
    ...
    { name: "publishedAt", type: "text", widget: { Name: "DateTime" } },
    {
      name: "relatedPost",
      type: "relation",
      widget: { Name: "Relation", relation: COLLECTION_ID },
    },
  ],
};
...
export async function runWidgetBenchmark() {
  ...
  try {
    ...
    const schemaPath = pathMod.join(compiledDir, `${COLLECTION_ID}.js`);
    await fsP.rm(schemaPath, { force: true });

    // 🚀 Scaffold schema file so the content system discovers it
    await fsP.mkdir(compiledDir, { recursive: true });
    await fsP.writeFile(schemaPath, `export default ${JSON.stringify(richSchema, null, 2)};`);
      ...
      for (let i = 0; i < 20; i++) {
        await cms.collections.create(
          COLLECTION_ID as any,
          {
            ...
            content:
              "<p>Rich text with <strong>formatting</strong> and <em>styles</em>.</p>".repeat(5),
    ...
    // 🧹 Final Cleanup
    const compiledDir = pathMod.join(process.cwd(), ".compiledCollections");
    await fsP.rm(pathMod.join(compiledDir, `${COLLECTION_ID}.js`), { force: true });

    if (dbAdapter.collection?.deleteModel) {
      await dbAdapter.collection.deleteModel(COLLECTION_ID).catch(() => {});
    }
