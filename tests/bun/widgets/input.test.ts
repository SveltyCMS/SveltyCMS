...
import Input from '@src/widgets/core/input/Input.svelte';
import widget from '@src/widgets/core/input';
...
describe('Text Widget', () => {
	...
	test('should handle validation errors', async () => {
		...
		const { getByTestId, findByRole } = render(Input, { field });
	...
	test('should handle multilingual data', async () => {
		...
		const { getByTestId } = render(Input, { field });
		const input = getByTestId('text-input');
	...
	test('should update character count', async () => {
		...
		const { getByTestId, getByRole } = render(Input, { field });
	...
	test('should handle store cleanup', async () => {
		...
		const { unmount } = render(Input, { field });
		await unmount();
