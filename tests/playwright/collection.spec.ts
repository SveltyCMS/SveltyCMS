...
test.describe('Full Collection & Widget Flow', () => {
	...
	test('Login, create collection, perform actions, and add widget', async ({ page }) => {
		// 1. Login
		await page.goto('http://localhost:5174/login', { timeout: 60000 });
		await page
		...
		await expect(page).toHaveURL('http://localhost:5174/en/Collections/Names');
		...
		await expect(page).toHaveURL('http://localhost:5174/dashboard', { timeout: 15000 });
	});
});
