        ...
        await page.route('**/login', (route) => {
            ...
            if (request.method() === 'POST' && request.postData()?.includes('OAuth')) {
                ...
                route.fulfill({
                    status: 302,
                    headers: {
                        'Location': 'https://accounts.google.com/o/oauth2/v2/auth?access_type=online&scope=email%20profile%20openid&redirect_uri=http://localhost:5173/login/oauth&client_id=test'
                    }
                });
            } else {
                route.continue();
