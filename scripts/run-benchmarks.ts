 ...
 * | DB_TYPE           | sqlite        |                                        |
 * | RESULTS_DIR       | tests/benchmarks/results | Where to write JSON result files   |
...
function parseCli() {
  ...
  const globalTimeoutMs = timeoutFlag
    ...
    : Number(process.env.GLOBAL_TIMEOUT_MS ?? 450_000);
...
function writeTestConfig(secret: string): void {
  ...
  const content =
    [
      ...
      `  DB_NAME: ${JSON.stringify(process.env.DB_NAME ?? "sveltycms_test_benchmark.db")},`,
...
async function startPreviewServer(): Promise<void> {
  ...
  const fallbackPath = join(rootDir, ".svelte-kit", "output", "server", "index.js");

  if (!existsSync(serverPath) && !existsSync(fallbackPath)) {
    throw new Error(
      `Server build not found at ${serverPath} or ${fallbackPath}. ` +
        `Run 'bun run build' first or ensure the build succeeded.`,
    );
  }
...
async function waitForPortFree(port: number, timeoutMs: number): Promise<void> {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      await sleep(500); // ⚡ Increased for Windows stability
    } catch {
      return; // Fetch failed — port is free.
    }
  }
  console.warn(
    pc.yellow(`⚠️  Port ${port} did not free within ${timeoutMs}ms. Proceeding anyway.`),
  );
}

/**
 * Polls the health endpoint until the server reports an actionable state.
 * Works with the Unified Dispatcher + OpenAPI dynamic generation.
 */
async function waitForServer(): Promise<void> {
  const maxAttempts = 80; // Increased slightly for safety
  ...
  console.log(pc.dim(`⏳ Waiting for Unified Dispatcher at ${API_BASE_URL}/api/system/health…`));

  for (let i = 0; i < maxAttempts; i++) {
    try {
      const res = await fetch(`${API_BASE_URL}/api/system/health`, {
        signal: AbortSignal.timeout(8000),
        headers: { "x-test-mode": "true" }, // Your dispatcher already respects this
      });

      if (!res.ok) {
        if (i % 15 === 0) console.log(pc.dim(`   Attempt ${i}: HTTP ${res.status}`));
        await sleep(800);
        continue;
      }
      ...
      // Also verify OpenAPI is working (optional but recommended)
      if (i > 5 && i % 10 === 0) {
        try {
          const openapiRes = await fetch(`${API_BASE_URL}/api/openapi.json`, {
            signal: AbortSignal.timeout(3000),
          });
          if (openapiRes.ok) {
            console.log(pc.green(`✅ OpenAPI spec available`));
          }
        } catch {}
      }
      ...
      if (
        ...
        console.log(pc.green(`✅ Server ready via Unified Dispatcher (status: ${status || "OK"})`));
        return;
      }
      ...
      if (i % 12 === 0) {
        console.log(pc.dim(`   Attempt ${i}: not ready yet (${err.message?.slice(0, 60)})`));
      }
    }
  ...
  throw new Error(`Server did not become ready within ~70s (Unified Dispatcher + OpenAPI).`);
}

/**
 * Verifies that the OpenAPI spec actually contains dynamic collections.
 */
async function verifyOpenAPI() {
  console.log(pc.dim("🔍 Verifying OpenAPI endpoint..."));
  try {
    const res = await fetch(`${API_BASE_URL}/api/openapi.json`, {
      signal: AbortSignal.timeout(3000),
    });
    if (!res.ok) {
      console.warn(pc.yellow(`⚠️  OpenAPI endpoint returned non-200: ${res.status}`));
      return;
    }
    const spec = await res.json();
    const pathCount = Object.keys(spec.paths || {}).length;
    if (pathCount < 10) {
      console.warn(pc.yellow(`⚠️  OpenAPI spec looks incomplete (${pathCount} paths)`));
    } else {
      console.log(pc.green(`✅ OpenAPI 3.1.0 spec ready (${pathCount} paths)`));
    }
  } catch (err) {
    console.warn(pc.yellow(`⚠️  Failed to verify OpenAPI: ${(err as Error).message}`));
  }
}
...
function persistBenchmarkMeta(meta: BenchmarkMeta): void {
  const resultsDir = join(rootDir, process.env.RESULTS_DIR ?? "tests/benchmarks/results");
...
async function main(): Promise<void> {
  ...
  const { code: setupCode } = await runCommand(pkgManager, ["run", "scripts/setup-system.ts"], {
    env: {
      DB_TYPE: dbType,
      DB_HOST: dbHost,
      DB_NAME: process.env.DB_NAME ?? "sveltycms_test_benchmark.db",
  ...
  const isTestFile = cli.benchmarkFile!.endsWith(".test.ts");
  const subCommand = isTestFile ? "test" : "run";

  const { code: benchCode } = await runCommand("bun", [subCommand, cli.benchmarkFile!], {
    env: {
      ...process.env,
