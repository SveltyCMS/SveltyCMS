...
> **Naming rules (enforced by `bun run check`)**
>
> - Folder = package id: kebab-case (`request-rate`), unique, never renamed.
> - Entry component: **`index.svelte`** — fixed name for every package (like `src/widgets/custom/*/index.ts`).
> - Marketplace docs: **`readme.mdx`** — fixed name for every package.
> - The **folder id** is the registry key saved in user layouts — the entry filename is an implementation detail and can never break a layout.
...
See [Architecture → widget.json](./architecture.mdx#2-the-widgetjson-manifest) for the full schema. `component` is always `"index"` — the checker verifies it matches `index.svelte`.
...
> **Hot add/remove (dev)**: the Vite plugin watches `src/routes/(app)/dashboard/widgets/**` — creating a new package folder (or removing one) triggers a full reload, and the new widget appears in the picker. No manual restart needed.
...
- [Widget Development Guide](../widgets/development.mdx)
