...
1. **Client runtime** — `+page.svelte` globs `./widgets/*/*.svelte` to build the lazy-loading registry. The registry is keyed by the **package folder id** (`widget.json.id`); each entry carries the entry filename (`index.svelte`) for the dynamic import:

```typescript
// +page.svelte — path like "./widgets/system-health/index.svelte"
