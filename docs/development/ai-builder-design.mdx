...
> [!IMPORTANT]
> **Status: Phase 0 + Phase 1 Collection Designer shipped.** Widget / page / automation designers (later phases) are still open. Each feature below lists the building blocks that already exist, the gaps to close, and the acceptance criteria that define "done". Implementation order follows the phases in [Rollout Plan](#rollout-plan).
...
| **Automation Builder**                | `/config/automations`, `docs/guides/content/automations.mdx` | Event-driven flows with `agentic_task` operations                                                       |
| **Workflow Engine (FSM)**             | `/config/workflows`, `docs/guides/content/workflows.mdx`     | Visual finite-state-machine content lifecycles                                                          |
...
### Flow — AI Widget Builder {#widget-builder-flow}
...
### Flow — AI Automation & Workflow Designer {#automation-designer-flow}
...
- **AEO score per page**: is the content structured to be citable (FAQ schema, `handle-security-headers.ts` already emits AEO headers) — with AI-generated fix suggestions.
...
| **1 — AI Collection Designer** ✅              | `generateCollectionSourceFromProposal` AST writer + `approveCollection` disk write / `syncContentState` / audit. `POST /api/ai-builder/approve-collection`                                                                                         | L      | Phase 0           |
...
| `src/plugins/webmcp/tools/builder.ts`                        | WebMCP headless builder tools (`design_collection`, `scaffold_widget`, etc.) ✅ shipped                                                                                                                                                                |
...
- [GUI Automation System](../guides/content/automations.mdx) — Automation Builder, extended by the AI Automation Designer
- [Visual Workflow Engine](../guides/content/workflows.mdx) — FSM engine, extended by the AI Workflow Designer
- [Site Starter](./site-starter.mdx) — target of the Page/Site Designer code mode
- [Widget System](./widgets/index.mdx) — 3-pillar architecture generated by the Widget Builder
