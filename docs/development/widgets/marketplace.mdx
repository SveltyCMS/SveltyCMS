...
- **Must declare a valid SemVer `version` (e.g., `"1.0.0"`) and `author`** in `createWidget`.
...
## Naming Convention & Versioning (required) {#naming-convention-required}
...
| **Version**        | SemVer string            | `version: "1.0.0"`                |
...
Helpers live in `src/widgets/widget-naming.ts`. Registration (widget store, `widgetRegistryService`) **fails closed** for custom/marketplace when naming is invalid, so loaders never look for the wrong path.

### Scaffold

```text
src/widgets/marketplace/phone-number/
  index.ts          # createWidget({ Name: "PhoneNumber", version: "1.0.0", … })
  input.svelte
  display.svelte
  tests/
```
...
export default createWidget({
  ...
  version: "1.0.0", // Semver version for marketplace verification
  author: "Vendor Name",
  Icon: "mdi:phone",
  // …
});
