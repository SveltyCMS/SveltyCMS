...
> [!NOTE] Competitive comparisons based on publicly available documentation as of June 2026. Performance data self-measured via `bun run benchmark` (see `scripts/benchmark-matrix/`).
...
| **Security** | ✅ Draft airgap + prompt injection shield + honeypot IP flagging | ✅ API Key + granular opt-in | ❌ (just permissions) | ✅ OAuth flow |
| **Local inference (server)** | ✅ Ollama + LanceDB | ❌ | ❌ | ❌ |
| **Browser inference (client)** | ✅ LiteRT.js (WebGPU/XNNPACK) | ❌ | ❌ | ❌ |
...
| `design_collection` | Builder | AI-assisted collection schema design (proposal draft) |
| `refine_collection` | Builder | Refine collection proposal via natural language instructions |
| `diff_collection_schema` | Builder | Structural AST diff between active schema and AI proposal |
| `scaffold_widget` | Builder | Scaffold complete 3-pillar Svelte 5 widget files |
| `list_available_widgets` | Builder | Discover all registered system widgets and field types |

> [!IMPORTANT]
> **Draft-by-Default Airgap**: All mutation and builder tools (`create_entry`, `update_entry`, `design_collection`, `scaffold_widget`) operate under strict airgap boundaries. Entries are saved as `draft`, and collection schemas are proposed as drafts requiring human review and compilation via the Collection Builder GUI. This ensures AI agents cannot publish unvetted code or destroy content.
...
// Output (folder is the kebab-case form of the widget Name):
...
> Pillar filenames must be lowercase: the repository gate `scripts/check-widget-naming.mjs:74-77` requires `input.svelte` and `display.svelte` in every widget folder.
...
const anomalies = detectContentAnomalies(); // Run periodically or on-demand
```
...
```typescript
import { reorderByUsage } from "@src/utils/adaptive-ui";
...
- **Tarpit Trap**: Automated AI crawlers attempting to scrape sensitive system files are identified via honeypot routes and immediately IP-flagged for firewall rejection on the next request (no socket-holding delay — that was a Slowloris/DDoS vector).
...
| `read_source_code`      | Read the full content of any source file from the SveltyCMS project.                                             | `filePath` (string, e.g., `src/widgets/widget-factory.ts`)                                                                    |
...
| :----------------------------- | :----- | :----------------------------------------------------- |
| `/api/chat`                    | POST   | Real-time chat with AI Assistant (+ EventBus dispatch) |
...
| Global Search      | `src/utils/global-search-index.svelte.ts`                   | ✅ via semantic  |

## Related

- [State Management](../../reference/architecture/state-management.mdx) — SystemMonitor + hardware-aware rate limiting
