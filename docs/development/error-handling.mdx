...
Standard error codes (`KnownAppErrorCode`) used across the application:

| Code                      | Status | Description                         |
| ------------------------- | ------ | ----------------------------------- |
| `VALIDATION_FAILED`       | 400    | Invalid input data (Valibot schema) |
| `VALIDATION_ERROR`        | 400    | Invalid input data                  |
| `BAD_REQUEST`             | 400    | Malformed request or missing params |
| `UNAUTHORIZED`            | 401    | Authentication required             |
| `REAUTH_REQUIRED`         | 401    | Fresh credentials required          |
| `FORBIDDEN`               | 403    | Insufficient permissions            |
| `NOT_FOUND`               | 404    | Resource not found                  |
| `CONFLICT`                | 409    | Resource already exists             |
| `RATE_LIMITED`            | 429    | Request rate exceeded               |
| `INTERNAL_ERROR`          | 500    | Server error                        |
| `UNEXPECTED_SYSTEM_ERROR` | 500    | Unhandled runtime error             |

---

## Server-Side Usage

### Declarative Request Validation with Valibot

In the new architecture, all mutating API logic is encapsulated in handlers within `src/routes/api/[...path]/handlers/` using `validateRequestBody`:

```typescript
// src/routes/api/[...path]/handlers/auth.ts (mapped as user: { handler: "auth", fn: "handleAuthUserRoutes" })
import type { RequestEvent } from "@sveltejs/kit";
import { AppError } from "@utils/error-handling";
import { validateRequestBody, successResponse } from "./base";
import * as v from "valibot";

const CreateUserSchema = v.object({
  email: v.pipe(v.string(), v.email()),
  role: v.optional(v.string(), "editor"),
});
...
export async function handleAuthUserRoutes(
  ...
  const body = await validateRequestBody(event, CreateUserSchema);
  const user = await cms.users.create(body);
  return successResponse(event, user, 201);
}
```
...
# Run the error-handling tests only
bun run test:unit -- tests/unit/utils/error-handling.test.ts

# Run with coverage
bun run test:unit -- --coverage
```

> Vitest is the canonical runner — import `{ describe, test, expect }` from `"vitest"`, never `bun:test`.
...
export function wrapError(error: unknown, message?: string, status?: number): AppError;
...
`raise(status, message, code?)` is a thin `never`-returning wrapper around `new AppError(message, status, code)` (`src/utils/error-handling.ts:271`). The `code` is **not** derived from the status — it defaults to `"INTERNAL_ERROR"` (`AppError` constructor, `:80`), so pass a semantic code explicitly (`raise(404, "Entry not found", "NOT_FOUND")`) when clients or log filters need to branch on it. The `handleError` hook attaches userId, tenantId, and request path to every error log.
