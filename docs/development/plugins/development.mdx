---
path: "docs/development/plugins/development.mdx"
...
```stackblitz project="SveltyCMS/plugin-starter" file="src/plugin/index.ts" title="Plugin Starter" height="600"
...
Because SveltyCMS features **System Discovery & Autoloading**, there is no need to manually register or import your plugin inside any central files. Simply saving your plugin code inside the `src/plugins/` directory is enough for the CMS to scan and register it on the next boot.

**Activation is lazy**: the boot only _registers_ every discovered plugin (so it shows in **Config → Extensions**), but **fully activates only enabled plugins** — merging the `index.server` module (hooks/migrations), resolving structured parts (routes, capabilities, sugar types, settings), and running migrations. A plugin that ships with `metadata.enabled: false` (the default for commerce-style optional plugins) stays inert until a tenant enables it in the admin UI or via `pluginRegistry.togglePlugin()` — which then lazily wires the server module, parts, and migrations in the same call. No reboot is required to activate an installed plugin.

## Plugin Interface & Versioning

Every plugin must declare a valid **SemVer version** in `metadata.version`. Whenever new features, database migrations, hooks, or bug fixes are added, increment this version. `marketplace.sveltycms.com` checks this version to verify compatibility, manage update notifications, and validate package signatures.

```typescript
interface Plugin {
  metadata: PluginMetadata; // Required: id, name, version (semver), enabled, author
...
hooks: {
  ...
  afterAuthenticate: async (event) => {
    ...
    //
    // event.user    — the authenticated user object
    // event.method  — "password" | "oauth" | "passkey" | "api_key" | "token" | "magic_link"
    // event.ip      — client IP address
    // event.userAgent — User-Agent header
    // event.userHas2FA — whether user already has 2FA enabled
    // event.tenantId — tenant ID (or null)
    //
...
Plugins can be enabled/disabled per tenant. **Enabling is a lazy activation**: `togglePlugin` merges the plugin's `index.server` module, resolves its structured parts, and runs pending migrations (pass the DB adapter as the 5th argument so migrations can run on first enable):

```typescript
// Toggle via registry — lazy-activates on enable
await pluginRegistry.togglePlugin("my-plugin", true, tenantId, userId, dbAdapter);

// Check state
const state = await pluginRegistry.getPluginState("my-plugin", tenantId);
```

The DB adapter argument is optional: without it the plugin is still enabled and parts are resolved, but pending migrations run on the next boot instead of inline.
...
// ❌ Wrong: Missing tenantId — data will be invisible in multi-tenant mode
hooks: {
  beforeSave: async (context, collection, data) => {
    return data; // No tenantId — orphaned across tenants
  },
}
```
    ...
    up: async (dbAdapter, context) => {
      await dbAdapter.schema.ensureCollection(
        "plugin_my-plugin_results",
        {
          fields: [
            { label: "Score", name: "score", type: "number" },
            { label: "Tenant", name: "tenantId", type: "string" }, // ← Include tenant field
          ],
          status: "publish",
        },
...
> **Why this matters**: When `MULTI_TENANT` is enabled, all DB queries automatically filter by `tenantId`. Data written without a `tenantId` becomes invisible — it's stored but never returned by any query. This is functionally equivalent to data loss.
...
| Unified Data Hub                  | Multi-source virtual collections     | [unified-data-hub.mdx](/src/plugins/unified-data-hub/unified-data-hub.mdx) |
...
- **Superadmin Demo Keys**: Superadmins can provide trial keys starting with `SLM-DEMO-` that verify as valid 14-day trial activations.
- **License Walls**: Completely prevent specific plugin hooks, background tasks, or workspaces from executing once the trial has expired and no valid license key is present.
...
{:else}
	<UpgradePrompt extensionId="plugin:my-plugin-id" price="€19.99" />
{/if}
```
...
- [Marketplace System](../../../reference/architecture/marketplace.mdx) — Plugin distribution
