...
This page is how to build a plugin: slots, hooks, admin pages, and the sandbox. Boot order, capabilities, and encrypted settings are the [plugin runtime](../../reference/architecture/plugin-architecture.mdx).
...
### Boot-time activation is lazy (enabled plugins only)

`initializePlugins()` (called from `db-init.ts`) **registers every discovered plugin** — so the admin Extensions list, `pluginRegistry.get()`, and metadata capabilities stay consistent — but only **enabled** plugins are fully activated at boot:

- **Enabled** (DB `pluginStates` row `enabled: true`, else `metadata.enabled`): the `index.server` module is imported (hooks/migrations merged), structured parts are resolved (`{ type: "route" }` → `pluginRouteRegistry`, capabilities, sugar types, settings), and pending migrations run.
- **Disabled** (default: commerce, stripe, taxjar, shipping-live, editable-website, focal-point, pagespeed, unified-data-hub, webmcp): the plugin stays inert — no server-module import, no parts resolution, no migrations.

**Enabling a plugin at runtime goes through `togglePlugin()`, which lazily activates it**: merges `index.server`, resolves parts, and runs pending migrations (when the caller passes the DB adapter). Enablement is checked in one `findMany` on `pluginStates` (L1-cached), so boot adds no per-plugin query. A plugin that was never enabled therefore contributes no routes/capabilities until a tenant turns it on — defense-in-depth: disabled-plugin URLs 404 instead of being reachable-but-403.
