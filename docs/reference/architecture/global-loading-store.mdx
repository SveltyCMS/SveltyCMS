---
path: "docs/reference/architecture/global-loading-store.mdx"
...
```mermaid
flowchart TB
    subgraph Consumers["Loading Triggers"]
        NAV[Navigation]
        FETCH[Data Fetch]
        AUTH[Authentication]
        CONFIG[Config Save]
    ...
    subgraph Store["loading-store.svelte.ts"]
        MAP["SvelteMap&lt;string, LoadingEntry&gt;"]
        IS_LOADING["isLoading ($state)"]
        TIMEOUT["Auto-Timeout (30s)"]
        SSR_GUARD["Browser Guard"]
    end

    subgraph UI["global-loading.svelte"]
        OVERLAY["Full-Screen Overlay"]
        RINGS["4 Animated Loader Rings"]
        LOGO["SveltyCMSLogo"]
        TEXT["Contextual Paraglide Messages"]
        PROGRESS["Progress Bar"]
        ELAPSED["Elapsed Time"]
    end

    subgraph Lifecycle["When This Runs"]
        POST_BOOT["Post-Boot Operations<br/>App is running, user is active"]
    end

    subgraph WarmingUp["Pre-Boot State"]
        WU_PAGE["warming-up/+page.svelte<br/>Shown during INITIALIZING"]
        WU_POLL["Polls /api/health every 1s"]
        WU_SERVICES["Shows service health status"]
    end
    ...
    WU_PAGE -.->|"Same visual identity<br/>(Logo, Rings, Typography)"| UI
```

> **Two loading screens, one visual identity:** `global-loading.svelte` handles post-boot operations (navigation, data fetching, form submission) with contextual messages. The **Warming-Up Dashboard** handles pre-boot initialization with service health status. Both share the SveltyCMSLogo component, animated loader rings, and typography patterns for brand consistency.
...
The global-loading component handles post-boot operations. For the pre-boot screen during system initialization, see the Warming-Up Dashboard which shares the same visual identity (SveltyCMSLogo, loader rings) but displays service health status.
