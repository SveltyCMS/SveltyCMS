...
| **Security at query builder**      | Compile to portable `where` / `whereBetween` / `search` / `orWhere` — never raw `{ contains }` objects into SQL `eq` |
| **Multi-column sort**              | `sort=title:asc,createdAt:desc` → `QueryBuilder.orderBy` (schema-whitelisted)                                        |
| **AND/OR groups**                  | `filterLogic` JSON (size-capped) compiles to `whereIn` or `orWhere` after FLAC                                       |
