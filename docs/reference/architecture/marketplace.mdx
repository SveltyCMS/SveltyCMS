...
flowchart TD
    subgraph Local["Local CMS"]
        ...
        INSTALL["installPlugin() / installTheme()"]
    ...
    API --> SVC
    SVC -->|"local themes + plugin stubs"| LOCAL_LIST["src/themes / plugins"]
...
### Local CMS license gates

Which plugins/widgets call `checkExtensionLicense` and how fail-open works is documented in:

**[License Gate Inventory](../security/license-endpoint-inventory.mdx)**
...
1. **Always** include local built-in themes (`adminThemeService.listThemes`) and known plugin stubs (e.g. Unified Data Hub listing).
...
| Path                    | How                                                     | Best For                                 |
| ----------------------- | ------------------------------------------------------- | ---------------------------------------- |
| **Marketplace Install** | Browse (in-app or CLI) → Download → `installPlugin()`   | Ready-made plugins, themes, integrations |
