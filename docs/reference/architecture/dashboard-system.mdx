...
- **`system.ts`** — Bridges the internal `EventBus` to connected WebSocket clients with tenant isolation. System events (content changes, user activity, cache invalidations) stream to all authorized dashboard widgets.
- **`chat.ts`** — Real-time AI chat for the dashboard co-pilot. Uses per-room streaming with individual access controls.
- **`ws-platform.ts`** — Shared WebSocket platform reference for global broadcasting. Initialized by `src/hooks.ws.ts` during server startup.
