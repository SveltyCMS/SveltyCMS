...
1. **Client** (`+page.svelte`) — Vite `import.meta.glob("./widgets/*/*.svelte")` builds the runtime registry for lazy-loaded rendering. The registry maps the component filename → package folder for dynamic imports.
...
// → { "./widgets/system-health/system-health-widget.svelte": () => import(...), … }

// Server discovery in +page.server.ts
const _widgetModules = import.meta.glob<{ widgetMeta?: … }>("./widgets/*/*.svelte", { eager: true });
```

The **component filename** (not the folder) is the registry key persisted in `system-preferences`, so saved layouts survive package upgrades. The `widget.json` manifest powers the marketplace catalog and telemetry via `manifest-registry.ts`.
...
#### **unified-metrics-widget.svelte** ⭐ _New_
...
#### **user-online-widget.svelte** ⭐ _Enhanced_
...
2. **Add the Component**: `my-widget.svelte` inside the folder, with a `widgetMeta` export
3. **Add the Manifest**: `widget.json` (id, name, icon, license, defaultSize, …)
4. **Extend base-widget**: Use the provided component infrastructure (`../../base-widget.svelte`)
5. **Auto-Registration**: The widget appears automatically in the picker after the next build/restart
