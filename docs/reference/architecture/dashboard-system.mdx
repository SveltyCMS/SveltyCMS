...
1. **Client** (`+page.svelte`) — Vite `import.meta.glob("./widgets/*.svelte")` builds the runtime registry for lazy-loaded rendering.
2. **Server** (`+page.server.ts`) — `readdirSync` scans all `*.svelte` files in `widgets/` and dynamically imports `widgetMeta` for the widget picker.
