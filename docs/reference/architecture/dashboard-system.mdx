---
path: "docs/reference/architecture/dashboard-system.mdx"
...
This page is the grid, the registry, and how a card is loaded. Building a card is [Dashboard widget development](../../development/dashboard/index.mdx).
...
Each widget is a **self-contained package** in its own kebab-case folder: `widgets/<folder>/<component>.svelte` plus a `widget.json` manifest. Widgets use **dual discovery**:

1. **Client** (`+page.svelte`) — Vite `import.meta.glob("./widgets/*/*.svelte")` builds the runtime registry for lazy-loaded rendering. The registry is keyed by the **package folder id**; each entry stores its entry filename (`index.svelte`) for the dynamic import.
2. **Server** (`+page.server.ts`) — eager glob over `widgets/*/*.svelte` imports each file's `widgetMeta` for the widget picker.

```
// Client registry in +page.svelte
const modules = import.meta.glob("./widgets/*/*.svelte");
// → { "./widgets/system-health/index.svelte": () => import(...), … }

// Server discovery in +page.server.ts
const _widgetModules = import.meta.glob<{ widgetMeta?: … }>("./widgets/*/*.svelte", { eager: true });
```

The **folder id** (not the filename) is the registry key persisted in `system-preferences`, so saved layouts survive entry renames. In dev, the Vite plugin watches `src/routes/(app)/dashboard/widgets/**` and full-reloads on package add/remove so new widgets appear in the picker without a manual restart. The `widget.json` manifest powers the marketplace catalog and telemetry via `manifest-registry.ts`.
...
#### **unified-metrics/index.svelte** ⭐ _New_
...
#### **database-pool-diagnostics/index.svelte** ⭐ _New_
...
#### **user-online/index.svelte** ⭐ _Enhanced_
...
The dashboard handler (`src/routes/api/[...path]/handlers/dashboard.ts`) exposes 16 RESTful sub-routes under `/api/dashboard/*`. All require session authentication and the `dashboard:read` permission (mapped in `ENDPOINT_PERMISSIONS`). Paths use kebab-case (`system-info`, `last5-content`, `online-user`, `commerce-orders`, `commerce-inventory`). Freemium commerce widgets also require an active 14-day trial or marketplace license (`403 LICENSE_REQUIRED` after expiry).
...
- **Tenant-scoped queries** use `locals.tenantId` for every database operation
- **System-wide widgets** (health, metrics, system-info) return global data visible to all tenants
- **Per-tenant widgets** (last5-content, audit, security) filter results by `locals.tenantId`
- **Dashboard layout preferences** are stored per-user per-tenant (compound key `tenantId:userId`)
- **Cache keys** include `tenantId` to prevent cross-tenant cache leakage
...
#### 13. **GET /api/dashboard/commerce-orders**

Recent `orders` collection rows plus a status mix for the Commerce Orders widget. Returns `{ available: false }` when the ecommerce preset is not installed (not 404).

**Authentication**: Required (`dashboard:read`). **License**: freemium (`dashboard:commerce-orders`, 14-day trial).

**Response**: `{ available, total, byStatus, recent: [{ id, orderNumber, total, status, createdAt, customerEmail }] }`

#### 14. **GET /api/dashboard/commerce-inventory**

Low-stock and out-of-stock products (variant-aware `inventory`) for the Commerce Inventory widget. Returns `{ available: false }` when the `products` collection is missing.

**Authentication**: Required (`dashboard:read`). **License**: freemium (`dashboard:commerce-inventory`, 14-day trial).

**Response**: `{ available, tracked, outOfStock, lowStock: [{ id, title, sku, qty, threshold }] }`
...
1. **Create Package Folder**: `src/routes/(app)/dashboard/widgets/my-widget/`
2. **Add the Component**: `index.svelte` inside the folder, with a `widgetMeta` export
3. **Add the Manifest**: `widget.json` (id, name, icon, license, defaultSize, …)
4. **Add the Docs**: `readme.mdx` marketplace description (required)
5. **Extend base-widget**: Use the provided component infrastructure (`../../base-widget.svelte`)
6. **Auto-Registration**: The widget appears automatically in the picker after the next build/restart

See [Dashboard Widget Development](../../development/dashboard/index.mdx) for the full guide.

### Widget Development Best Practices

```
// Example widget structure — widgets/my-widget/my-widget.svelte
...
Dashboard widgets receive live updates over **Server-Sent Events (SSE)** — the adapter-node-compatible stream at `GET /api/content/events` (alias `/api/events`):

- `handleContentEventsStream` (`src/routes/api/[...path]/handlers/content.ts`) subscribes to the internal `EventBus` and pushes tenant-filtered events (`content:update`, `cache:invalidate`, `config:change`) as `text/event-stream`.
- Built-in consumers: `content-sse.svelte.ts` (content-tree sync), `global-settings.svelte.ts` (live settings), and the collaboration `SseProvider`.
- For collaborative editing, see [Real-Time Updates](../api/graphql-subscriptions.mdx) (Yjs over SSE + optional `/ws`).

```
// Dashboard widget subscribing to real-time events (browser)
const events = new EventSource("/api/content/events", { withCredentials: true });
events.addEventListener("content_update", (e) => {
  const event = JSON.parse(e.data); // { type, payload, timestamp, tenantId }
