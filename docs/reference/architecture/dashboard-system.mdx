...
Requests pass through the API dispatcher (`+server.ts`) which enforces authentication and `checkEndpointPermission` before reaching the dashboard handler. Tenant-scoped queries use `locals.tenantId`.
