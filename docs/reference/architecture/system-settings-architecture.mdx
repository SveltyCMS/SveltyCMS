---
path: "docs/reference/architecture/system-settings-architecture.mdx"
...
This page is how settings are stored and read. Changing them in the admin is the [System settings guide](../../guides/configuration/system-settings.mdx).
...
- **Bootstrap Layer (`/config/private.ts`)**: Reserved exclusively for the primary database connection strings and security essential (JWT secrets). This is the "static" foundation required for the CMS to boot.
- **Runtime Layer (`system_preferences` DB Table)**: Infrastructure scaling parameters like **Read Replica URLs** and **Cloudflare CDN tokens** are managed dynamically in the database.
...
1.  It detects changes in the `DB_REPLICA_URLS` JSON array.
2.  It calls the database adapter's `configureReplicas()` method.
3.  The adapter gracefully closes old connections and initializes new regional connection pools.
4.  The system immediately begins routing read queries to the new replicas based on the `x-svelty-region` header.
