...
## 6. Route Resource State Machine

The **Route Resource State Machine** (`src/services/core/route-resource-state-machine.ts`) classifies every URL into precise operational resource profiles in **`< 10 microseconds`**:

| Resource Lane    | Routes                         | Required Cache Categories     | Memory Profile     | Middleware Bypass                                     |
| :--------------- | :----------------------------- | :---------------------------- | :----------------- | :---------------------------------------------------- |
| **`bootstrap`**  | `/login`, `/setup`, `/auth/*`  | `SETTINGS`                    | **`< 2MB` RAM**    | Skips `media`, `preferences`, `scim`, `collaboration` |
| **`media`**      | `/mediagallery`, `/media/*`    | `MEDIA`, `SETTINGS`           | Focused Media RAM  | Skips `scim`, `collaboration`                         |
| **`collection`** | `/collections/*`, `/content/*` | `SCHEMA`, `ROLES`, `SETTINGS` | Schema & Field RAM | Skips `scim`                                          |
| **`dashboard`**  | `/dashboard`, `/admin`         | `SETTINGS`, `SYSTEM_STATE`    | Layout Metrics RAM | Skips `scim`                                          |
| **`settings`**   | `/settings`, `/config`         | `SETTINGS`, `ROLES`           | Config RAM         | Skips `media`, `collaboration`                        |
