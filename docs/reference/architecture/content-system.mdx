...
| `schema-contract.ts`              | Post-load schema assert (shape, duplicate fields) + optional structure fingerprint helpers  |
...
- Hard-fail: non-object, missing `fields` array, duplicate `db_fieldName`
...
- Rejects missing `name` or empty `fields` before cache/DB
- Invalid schemas are skipped — last-good runtime state retained
