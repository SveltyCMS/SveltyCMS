...
**Schema hot flags** (cached on first `getSchema`): `_hasActiveWidgets`, `_hasNumberFields`, `_hasSanitizableFields`, `_hasHooks`, `_hasEncryptedFields` — so create/update/find skip field walks, `modifyRequest`, and AES-256-GCM when the schema does not need them.

When widgets _are_ active, `modify-request.ts` caches the resolved `{ field, widget, name }` list on `fields._activeWidgets` and reuses one data accessor + one request context across entries (no per-field object spread). Document writes in `post-write.ts` do **not** purge `cms:content_structure` keys — schema LRU stays warm.
