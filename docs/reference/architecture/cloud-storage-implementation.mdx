...
- **Performance**: Implemented ETag caching. Uses `getMetadata` to fetch the ETag from cloud storage.
- Returns **304 Not Modified** if the request's `If-None-Match` header matches the cloud asset's ETag, saving significant bandwidth and latency.
- Constructs full cloud URL with MEDIA_FOLDER prefix and returns a **307 redirect** if the cache is stale or missing.
...
- Consolidated `save-avatar` and `delete-avatar` into the system handler.
- Now trusts `saveAvatarImage()` to return correct URL
- Simplified logic by delegating to the unified dispatcher.
