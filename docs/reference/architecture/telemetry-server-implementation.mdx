---
path: "docs/reference/architecture/telemetry-server-implementation.mdx"
...
interface CheckUpdateRequest {
  // Authentication fields (REQUIRED — top level, NOT inside payload)
  ...
  payload: {
    ...
    // Extension ecosystem (optional, aggregate)
    widgets?: string[]; // custom content widget names
    dashboard_widgets?: string[]; // installed dashboard widget package ids
...
> ⚠️ Wire contract: the receiver reads `signature`/`timestamp` from the **top level** of the body (`{ payload, signature, timestamp }`). Embedding them inside `payload` makes the server respond `403` with "Missing authentication fields".
...
(Client reference: `src/services/observability/telemetry-service.ts` signs the same string and sends `{ payload, signature, timestamp }`.)
