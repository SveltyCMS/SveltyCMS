...
- **READY pipeline** (16 hooks): Full production middleware chain
...
const pipeline = [
  ...
  handleTurboPipeline, //  2. CONSOLIDATED GATEWAY (Classify, State, Setup, Health, CORS, Security Headers, Static Cache)
  handleTestIsolation, //  3. CI ISOLATION (Triple-lock: env + localhost + crypto secret)
  handleSecurity, //  4. Firewall, Rate Limit, GraphQL Shield, Load Shedding
  handleSystemState, //  5. AUTHORITATIVE GATEKEEPER (Boot, Setup, MAINTENANCE)
  handleTurboGet, //  6. 🚀 TURBO GET FAST-PATH (Session auth cache + pre-compressed response skip — BEFORE auth)
  handleRedirects, //  7. SEO Redirects (Materialized View + Negative Cache)
  handleCompression, //  8. ESM-Native Streaming (zlib + Web Streams fallback)
  handleUserPreferences, //  9. i18n + SSR Theme (skip transformPageChunk for non-dark)
  handleAuthentication, // 10. Identity, Sessions, API Tokens, Multi-Tenancy
  handleAuthorization, // 11. Role-Based Access Control (cached dynamic imports)
  handleLocalSdk, // 12. Zero-Latency CMS Bridge (LocalCMS injection)
  handleContentInitialization, // 13. Content Manager per Tenant + Fresh Install Redirects
  handleAuditLogging, // 14. Fire-and-Forget Audit Trails (mutation-only, non-blocking)
  handleApiRequests, // 15. API Caching (ETags, 2-Layer Cache, SWR Pre-warming)
  handleTokenResolution, // 16. RBAC Token Replacement in API responses
];
```

> **2026-06-05 Consolidation**: `handleSecurityHeaders` and `handleStaticAssetCaching` are now inlined into `handleTurboPipeline`'s post-resolve phase, reducing Promise chain depth by 2 hooks. `handleTurboGet` moved earlier — right after security gates, before auth/authz — maximizing its cache-hit fast-path coverage.
>
> **2026-06-14 Performance Optimizations**: `HOOK_TIMING_ENABLED` gate added to `wrapHandle` — per-request `performance.now()` + `traceSpan` + Map writes are now disabled in production and under `SVELTY_BENCHMARK_SUITE`, saving measurable per-hook overhead. `handleFnRef()` resolved once at pipeline build time instead of per-request. `handleAuditLogging` converted to true fire-and-forget with context captured before `await resolve(event)` — audit logging no longer blocks the mutation response. `handleTurboGet` and `handleApiRequests` now serve pre-compressed (br/gzip) wire bytes on cache HITs, avoiding re-stringify and re-compress on every repeated request. Compression metrics (avg sizes, ratios) captured by the benchmark harness for matrix trend tracking. See [Cache System](./cache-system.mdx) for pre-compression details.
>
> **2026-06-27 Security Hardening**: `handleRateLimit` wired into the pipeline between `handleSecurity` and `handleSystemState` — per-IP sliding window rate limiting with adaptive cost multipliers. Rate limiting now gates BEFORE authentication, preventing brute-force attacks. Test secret comparison hardened with `crypto.subtle.timingSafeEqual`. API CSP tightened (`style-src 'unsafe-inline'` removed). Honeypot routes return empty `200 OK` with no JSON body leak.
