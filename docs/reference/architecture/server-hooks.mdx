...
const apiPipeline = [
  ...
  handleTestIsolation, //  2. CI ISOLATION (TEST_MODE only)
  handleSecurity, //  3. Firewall, GraphQL Shield, WAF Load Shedding
  handleRateLimit, //  4. Per-IP Fixed-Window Rate Limiting (adaptive pressure)
  handleSystemState, //  5. AUTHORITATIVE GATEKEEPER
  handleTurboGet, //  6. 🚀 TURBO GET (Session Cache + responseCache L1 for GETs)
  handleCompression, //  7. ESM-Native Streaming (Zstd/Brotli/Gzip)
  handleAuthentication, //  8. Identity, Session Token Validation (__Host-auth_sessions)
  handleAuthorization, //  9. Role-Based Access Control (Bitset & Roles)
  handleLocalSdk, // 10. Zero-Latency CMS Bridge (LocalCMS)
  handleContentInitialization, // 11. Content Manager per Tenant
  handleAuditLogging, // 12. Fire-and-Forget Audit Trails (microtask outbox)
  handleApiRequests, // 13. API Caching (ETags, L1 Sync Cache)
  handleTokenResolution, // 14. RBAC Token Replacement
];
```
