...
### handleSecurityHeaders - Defense in Depth (Inlined into handleTurboPipeline) {#10-handlesecurityheaders-defense-in-depth}
...
| `handleContentNegotiation` | Content negotiation hook | Implicit | AI agent `Accept: text/markdown` path |
| `handleSecurityHeaders` | `tests/unit/hooks/security-headers.test.ts` | 8+ | Header validation + CSP tests |
