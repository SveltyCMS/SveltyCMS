...
const pipeline = [
  ...
  handleRateLimit, //  5. Per-IP Fixed-Window Rate Limiting (adaptive pressure)
  handleSystemState, //  6. AUTHORITATIVE GATEKEEPER
  handleTurboGet, //  7. 🚀 TURBO GET (session cache + responseCache L1)
  handleRedirects, //  8. SEO Redirects (tenant header post-auth only)
  handleCompression, //  9. ESM-Native Streaming
  handleAeoHeaders, // 10. Automatic Early Opinions
  handleUserPreferences, // 11. i18n + SSR Theme
  handleAuthentication, // 12. Identity, Sessions, x-test-secret bypass
  handleAuthorization, // 13. Role-Based Access Control
  handleLocalSdk, // 14. Zero-Latency CMS Bridge
  handleContentInitialization, // 15. Content Manager per Tenant
  handleAuditLogging, // 16. Fire-and-Forget Audit Trails (microtask)
  handleApiRequests, // 17. API Caching (ETags, 2-Layer Cache)
  handleTokenResolution, // 18. RBAC Token Replacement
];
```

> **2026-08-10**: READY pipeline runs **18 hooks**; immutable asset caching is served by `handleTurboPipeline` + the `FAST_STATIC` lane; `wrapHandle` passes resolved handles directly into `sequence()` when hook timing is disabled (zero per-request wrapper overhead).

> **2026-08 Request Lane Router**: O(1) `classifyRequest` + `x-svelty-lane` on all responses; HEALTH/FAST_STATIC short-circuit before the sequence; HYPER_TURBO keys off `auth_sessions` cookie; user-scoped GraphQL/REST `responseCache` (`src/services/cache/response-cache.ts`).
