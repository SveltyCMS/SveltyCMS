...
**Full Pipeline End-to-End** (SQLite ledger 2026-07-05; matrix 53/57 tests passed):

| Scenario                         | Avg Latency | p95     | RPS       | Δ vs 2026-07-04 ledger |
| :------------------------------- | :---------- | :------ | :-------- | :--------------------- |
| Static Asset (no hooks)          | **0.085ms** | 0.127ms | **9,776** | within variance        |
| Turbo Pipeline (Light)           | **0.527ms** | 0.652ms | **1,569** | **−11%**               |
| Full Security + Auth Pipeline    | **0.780ms** | 0.883ms | **1,176** | **−19%**               |
| REST with API Caching (warm GET) | **0.765ms** | 1.046ms | **1,113** | −8%                    |
| Mutation + Audit Logging         | **2.924ms** | 3.510ms | **342**   | +63% (audit path)      |

> **Benchmark**: `BENCHMARK_RECORD=1 bun test tests/benchmarks/hooks-performance.test.ts` or `bun run scripts/benchmark-matrix/index.ts --db=sqlite`. Full security stack, no turbo-auth shortcut. Warmed shared-server matrix runs reach **0.555ms** full-auth avg (p95 **0.668ms**). Per-hook timing: `GET /api/system/health` → `hooks`. Ledger: [SQLite Hooks section](../../project/benchmarks/benchmark_sqlite.mdx#section-hooks_trace).
...
| -------------------------- | -------------- | ------------- | ---------------- |
| Full Auth Pipeline (hooks) | 0.96ms         | 0.780ms       | **🚀 19%**       |
