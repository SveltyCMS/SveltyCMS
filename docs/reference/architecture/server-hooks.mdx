...
The pipeline is **dynamically dispatched** based on system state, with **two cached sequences** (after lane classification):
...
```typescript
// READY Pipeline (production order) — after classifyRequest + HEALTH/STATIC fast-paths:
const pipeline = [
  handleTurboPipeline, //  1. CONSOLIDATED GATEWAY
  handleTestIsolation, //  2. CI ISOLATION (TEST_MODE only)
  handleSecurity, //  3. Firewall, GraphQL Shield
  handleRateLimit, //  4. Per-IP Fixed-Window Rate Limiting (adaptive pressure; `/api/commerce` lane)
  handleSystemState, //  5. AUTHORITATIVE GATEKEEPER
  handleTurboGet, //  6. 🚀 TURBO GET (session cache + responseCache L1)
  handleRedirects, //  7. SEO Redirects (tenant header post-auth only)
  handleCompression, //  8. ESM-Native Streaming
  handleAeoHeaders, //  9. Automatic Early Opinions
  handleUserPreferences, // 10. i18n + SSR Theme
  handleAuthentication, // 11. Identity, Sessions
  handleAuthorization, // 12. Role-Based Access Control
  handleLocalSdk, // 13. Zero-Latency CMS Bridge
  handleContentInitialization, // 14. Content Manager per Tenant
  handleAuditLogging, // 15. Fire-and-Forget Audit Trails (microtask)
  handleApiRequests, // 16. API Caching (ETags, 2-Layer Cache)
  handleTokenResolution, // 17. RBAC Token Replacement
];
```

> **August 2026 Pipeline Hardening & Zero-Overhead Dispatch**:
>
> - **Zero-Overhead Dispatch**: `wrapHandle` passes raw resolved handlers directly to SvelteKit `sequence()`, eliminating 15+ nested async timing closures per request unless `ENABLE_HOOK_TIMING=1` is explicitly provided.
> - **Rate Limit Fast-Exit**: `handleRateLimit` checks `GET`/`HEAD`/`OPTIONS` on line 1, skipping IP parsing for >90% of traffic.
> - **HTML-Only Middleware Bypass**: `handleAeoHeaders` and `handleUserPreferences` immediately return on `/api/` endpoints.
...
> - **Asynchronous Audit Outbox**: `handleAuditLogging` enqueues mutation entries into a bounded in-memory ring buffer flushed via microtasks, decoupling 200/201 response flushes from audit disk/Merkle writes.

> **2026-08-11**: `handleHyperTurbo` (benchmark auth forge) was **removed**. Benchmarks run `NODE_ENV=production` with real session cookies — `BENCHMARK` grants no request-path bypass. READY pipeline runs **17 hooks**.
