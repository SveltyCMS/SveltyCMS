...
**Before** the READY/SETUP sequence runs, `export const handle` classifies every request with `classifyRequest()` (`src/hooks/request-classifier.ts`) and stamps the response with **`x-svelty-lane`**.
...
> **Benchmark**: `BENCHMARK_RECORD=1 bun test tests/benchmarks/hooks-performance.test.ts`. Dead static-asset section removed in v2026-07-07 — contributed to 18–24% latency reduction across all pipeline stages.
