---
path: "docs/reference/architecture/collection-store-dataflow.mdx"
...
This consolidation replaces the previous fragmented approach (`status-store`, `modeStateMachine`) to provide a single source of truth for **active collection data**.

> [!NOTE]
> **Distinction**: The `collection-store` manages the lifecycle of the _currently viewed/edited_ collection and its entries. The **Content System (5-Pillar Architecture)** manages the global tree of all collections and their metadata, orchestrated via `src/content/index.ts` / `content-registry.svelte.ts`.
>
> **HMR / recompile (2026):** Soft content updates call `invalidate("app:content")` only. They do **not** reset `mode`, `activeValue`, or `consent-store`. Use `patchActiveSchema(schema)` when the active collection definition changes in place so the open editor stays coherent without a full page reload. Structure updates use an FNV fingerprint circuit-breaker (not full `JSON.stringify`) to avoid reactivity loops.
...
sequenceDiagram
    ...
    participant CS as Content Manager (index.ts)
    ...
    PageServer->>CS: getFirstCollection()
    CS-->>PageServer: Target Collection URL
...
- `PATCH` / `PUT` `/api/collections/[collectionId]/[entryId]` - Update entry
- `DELETE /api/collections/[collectionId]/[entryId]` - Delete entry
- `PATCH /api/collections/[collectionId]/[entryId]/status` - Update status (optional `entries[]` for bulk)
- `POST /api/collections/[collectionId]/batch` - Batch operations (`action`: `delete`, `status`, `clone`)
- `POST /api/collections/[collectionId]/batch-clone` - Bulk clone (`entries[]` or `entryIds[]`)

Batch **status** and **delete** use one tenant-scoped statement (`UPDATE`/`DELETE WHERE _id IN (...)`). Clone-by-id is `findByIds` + `insertMany` (two queries, no per-row widget pass on the source).
...
- `src/databases/cache/cache-service.ts` - Redis caching layer
- `src/utils/modify-request.ts` - Data enrichment
...
- `src/content/index.ts` - Content Manager facade
