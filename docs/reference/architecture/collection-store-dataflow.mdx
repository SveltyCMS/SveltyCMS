...
> [!NOTE]
> **Distinction**: The `collection-store` manages the lifecycle of the _currently viewed/edited_ collection and its entries. The **Content System (5-Pillar Architecture)** manages the global tree of all collections and their metadata, orchestrated via `src/content/index.ts`.
