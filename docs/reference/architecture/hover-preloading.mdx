---
path: "docs/reference/architecture/hover-preloading.mdx"
...
| Strategy     | Trigger                                         | Delay         | Data Attribute            | Server Pre-Warming          |
| :----------- | :---------------------------------------------- | :------------ | :------------------------ | :-------------------------- |
| **hover**    | Mouseenter + 150ms dwell                        | 150ms         | `data-preload="hover"`    | `/api/system/prewarm-route` |
| **viewport** | Link enters viewport (200px margin)             | 0ms perceived | `data-preload="viewport"` | `/api/system/prewarm-route` |
| **predict**  | Cursor heading toward link (200px, 30 deg cone) | 0ms perceived | `data-preload="predict"`  | `/api/system/prewarm-route` |
| **smart**    | Predict cone + behavioral hot-path priority     | 0ms perceived | `data-preload="smart"`    | `/api/system/prewarm-route` |
...
### Server prewarm (what actually happens)

`GET /api/system/prewarm-route?path=…` runs **LocalCMS** `findById`/`find` (all four adapters — never an internal HTTP hop), stringifies `{success,data}` once, and stores a `Uint8Array` on `responseCache`. The next matching authenticated GET is `X-Cache: TURBO-HIT` (zero DB, no per-request JSON).

Route classification is an exact-path Map plus `startsWith` (1024-entry LRU), **not** a frozen radix trie. Adaptive prediction requires **≥5 samples and 30% accuracy** before it fires. Extinction is 0.8 on non-chosen alternatives at navigation time. Transition heat is capped at **10,000 path-pairs per tenant**.
