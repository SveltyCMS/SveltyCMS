...
In development mode, the `sveltyCmsPlugin()` invokes the compiler on file changes:

```typescript
// vite.config.ts — handleHmr inside sveltyCmsPlugin
...
await compile({
  ...
  targetFile: isLikelyMove ? undefined : file, // Full compile on move
});
```

When a file is moved/renamed (detected by `unlink` + `.ts`), the compiler runs a **full build** with orphan cleanup to ensure the old compiled file is removed and the new one is created.
...
The output of this pipeline is monitored by the **Content Engine (`src/content/engine.server.ts`)**, which reconciles the compiled JavaScript modules into the live system state.
...
- [Collection Builder Guide](/docs/guides/content/collection-builder) - Visual collection builder with data loss warnings
- [Initialization Workflow](./initialization-workflow.mdx) - How compilation fits into system startup
- [Content System Architecture](./content-system.mdx) - Consumption of compiled modules
- [Widget Architecture](/docs/development/widgets/development) - Understanding widget transformation
