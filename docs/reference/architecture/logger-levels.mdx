...
> **Note:** `src/utils/logger.server.ts` implements an optional **file sink** (rotation, batching, HMAC chain). It is **not** the day-to-day import. File logging only activates if that module is loaded (e.g. registered from server boot). Prefer stdout + platform log shipping in containers. See [Server file sink](#server-file-sink-optional) below.
