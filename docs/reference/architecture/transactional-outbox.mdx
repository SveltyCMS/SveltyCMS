...
2. **Buffered (non-transactional) emits** — `emit()` / `enqueueBuffered()` outside a transaction push into an in-memory buffer. Persistence is a **trailing 25ms debounce** (reset on every emit) so a write burst becomes **one** `insertMany` after the burst, not interleaved with content INSERTs. A hard cap of 512 events forces a `setImmediate` flush — never a same-tick DB write. The event `_id` (UUID) is stamped at **emit time**.
