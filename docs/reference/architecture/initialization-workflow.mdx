...
Run the headless setup script (requires the server to be running) or use the setup wizard at `/setup`:

```
# Replicates the setup wizard's seedDatabase action (recommended for CI/CD)
bun run scripts/setup-system.ts

# Or through the setup wizard at /setup
```

The script is environment-driven (`DB_TYPE`, `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`) and safe to re-run.

### Step 6: Create Admin User

The same script runs the wizard's `completeSetup` action, which creates the initial admin user:

```
# Uses ADMIN_USERNAME / ADMIN_EMAIL / ADMIN_PASSWORD (safe to re-run)
bun run scripts/setup-system.ts
...
│   └── setup-system.ts        # Headless setup (seed + admin) for CI/CD
├── src/
...
jobs:
  test:
    ...
    steps:
      ...
      # Seed database + create admin AFTER the server is up
      - run: bun run scripts/setup-system.ts
        env:
...
For CI/CD environments, SveltyCMS provides one script that replicates the setup wizard end to end.

### Script: Headless System Setup

**File:** `scripts/setup-system.ts`

The script waits for the running server to become healthy, then POSTs the wizard's `seedDatabase` action followed by `completeSetup` (which creates the initial admin user):

```
# Requires the server to be running (default: http://localhost:4173)
bun run scripts/setup-system.ts

# With an explicit API base and database target
API_BASE_URL=http://localhost:5173 DB_TYPE=mongodb DB_HOST=localhost DB_PORT=27017 \
  DB_NAME=SveltyCMS bun run scripts/setup-system.ts
```

Configuration is environment-driven: `ADMIN_USERNAME` / `ADMIN_EMAIL` / `ADMIN_PASSWORD`, `MULTI_TENANT`, `DEMO`, `USE_REDIS`, `PRESET`, and `TEST_API_SECRET` (falls back to `tests/e2e/.auth/test-secret.txt`).
...
✅ **Reuses setup logic** - Calls the actual setup wizard actions over HTTP
...
- [Complete Workflow Guide](/docs/reference/architecture/initialization-workflow.mdx) - This document (installation, setup, testing, CI/CD)
