---
path: "docs/reference/architecture/cryptography-module.mdx"
...
**Field-level encryption at rest (September 2026):** collection fields flagged
`encrypt: true` are encrypted in `write-pipeline.ts` and decrypted in
`read-pipeline.ts` using a `v1:iv:tag:ciphertext` AES-256-GCM envelope bound
with AAD (`tenantId:collectionId:fieldName`). See
[Field-Level Encryption](../security/field-level-encryption.mdx).
...
- [Build-Time Security Plugin](/docs/reference/architecture/security-plugin) - Prevents private settings exposure to client
- [Authentication System](/docs/reference/security/authentication-system) - Complete authentication implementation
- [Import/Export System](/docs/reference/database/index.mdx) - Uses crypto module for sensitive data
- [Security Best Practices](/docs/reference/security/index) - General security guidelines
...
In addition to password-based Argon2id hashing, SveltyCMS provides a **static-key encryption** system for plugin settings via `@src/plugins/settings-crypto.ts`.
...
### Key Differences from password-based encryption

| Feature         | Password-based (legacy)  | `encryptSecret` (Static-key)                               |
| :-------------- | :----------------------- | :--------------------------------------------------------- |
| Key source      | Removed (unused in src/) | `SECRET_ENCRYPTION_KEY` env var                            |
| Key derivation  | —                        | Direct hex/base64 key (or HKDF-SHA-256; SHA-256 dual-read) |
| Use case        | —                        | Automated plugin settings                                  |
| Interactive     | —                        | No (fully automated)                                       |
| Envelope format | —                        | `version:iv:authTag:ciphertext`                            |

### API Reference {#plugin-settings-api-reference}
...
See [Plugin Settings Crypto](/src/plugins/settings-crypto.ts) and [Plugin Settings Declaration](/src/plugins/settings-declaration.ts) for implementation details.

---

Passphrase-shaped `SECRET_ENCRYPTION_KEY` values (not 64-char hex / strict base64) are derived with **HKDF-SHA-256** (`info = sveltycms-plugin-settings`). Decrypt tries that key first, then the pre-2026-08 SHA-256(raw) digest so existing rows keep working. Derivation is cached for the process — not per request. Prefer a 64-char hex key in production.
