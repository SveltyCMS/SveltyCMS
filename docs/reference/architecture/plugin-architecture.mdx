...
description: "How a plugin boots: capabilities, encrypted settings, parts, and admin-shell extensions."
order: 99
...
This page is the engine: boot, capabilities, encrypted settings, parts, and admin-shell extensions. Building a plugin is [Plugin architecture](../../development/plugins/architecture.mdx) and [Plugin development](../../development/plugins/development.mdx). Types live in `src/plugins/types.ts`.
...
```typescript
import { registerAdminAreaExtension } from "@src/plugins/admin-zone-registry.svelte.ts";
...
registerAdminAreaExtension(sidebarWidget);
```

### Rendering

Admin shell zones are rendered by `<AdminZone>` (`src/components/system/admin-zone.svelte`), which also renders `adminTool` plugin parts into `sidebar` · `toolbar` · `dashboard` · `config`. Zones are wired into the layout (`header`/`toolbar`/`footer`), `LeftSidebar` (`sidebar`), the dashboard and config pages, and `AdminPageShell` (`content-header`/`content-footer`). Entries are lazy-loaded, ordered, and gated behind optional `condition(context)` and a client capability checker (the server enforces 403 regardless).
