...
Compiled UI locales (from `project.inlang/settings.json`) are **`en`**, **`de`**, and **`ar`**. Arabic is the RTL locale used to verify mirroring (`getTextDirection("ar")` → `rtl`). Adding a locale still requires a Paraglide compile (`bun run paraglide`).
