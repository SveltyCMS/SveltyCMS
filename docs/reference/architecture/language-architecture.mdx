...
This page is how admin catalogs are compiled. Adding a locale in the admin is [System languages](../../guides/configuration/system-languages.mdx).
...
Fields are marked translatable with `translated: true` — either in code (below) or in the admin UI via the **Translated** toggle on a field's edit dialog (Collection Builder → field → Default tab, next to _Required_). Both paths round-trip: the GUI toggle persists to the collection definition, and the GUI → code generator emits `translated: true`, so code- and GUI-defined collections stay in sync.
