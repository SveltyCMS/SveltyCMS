...
Client IP address resolution is centralized through `getClientIp(event)` (`src/utils/hook-utils.ts:266`) across all authentication, rate limiting, and audit logging flows. Rather than trusting raw `X-Forwarded-For` request headers directly (which is vulnerable to client-side IP spoofing), `getClientIp` enforces reverse-proxy boundary verification and memoizes the result on `event.locals.__clientIp` so the IP is resolved at most once per request instead of 3–5× per hook chain. Scanner rule `ip-spoofing-x-forwarded-for` in `scripts/scan-security-risk.ts` enforces this across the entire server codebase. Additionally, session records capture the resolved IP address and User-Agent at creation time for Active Device tracking.
