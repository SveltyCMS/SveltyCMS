...
- **A2**: Added programmatic focus to the `#twofa-code` input field using `tick()` when Two-Factor Authentication is required (see [2FA documentation](/docs/reference/security/authentication-system.mdx#4-advanced-security-features)).
...
2. **Session cache** (in-memory LRU, 10k entries) — validated sessions with a 24h trust window.
3. **Distributed cache** (Redis `session:{tenant}:{id}`) + in-memory/Redis session store — cross-node fallback.
4. **Database** (`getSessionTokenData` + `getUserById`) — cold path, **single-flight coalesced** (concurrent cold requests await one validation instead of racing).
