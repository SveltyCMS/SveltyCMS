...
- **Small files (<5MB)**: Uses `arrayBuffer()` → `Buffer.from()` → `sniffMimeType(buffer.subarray(0, 2048))` to verify binary signatures before accepting
- **Large files (≥5MB, non-SVG)**: Reads first 2048 bytes via `file.slice(0, 2048).arrayBuffer()`, runs `sniffMimeType()`, and rejects on major MIME category mismatch (e.g., client sends `image/png` but binary signature indicates `application/pdf`)
- **SVG (any size ≤ MAX_SVG_BYTES)**: Always buffered and passed through `sanitizeSvg()` — never pure-streamed. Oversized SVGs are rejected. Misdeclared SVG content is sanitized or blocked on the stream path.
...
- **Remote URL uploads**: `saveRemoteMedia` + egress guard only (no raw `fetch(userUrl)`)
