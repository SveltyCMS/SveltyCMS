...
- **Large files (≥5MB)**: Reads first 2048 bytes via `file.slice(0, 2048).arrayBuffer()`, runs `sniffMimeType()`, and rejects on major MIME category mismatch (e.g., client sends `image/png` but binary signature indicates `application/pdf`)
...
**Security Review Status**: ✅ All Critical Fixes Implemented & API Dispatch Hardened (June 2026 Audit)
