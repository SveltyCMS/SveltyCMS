---
path: "docs/reference/security/widget-security.mdx"
...
**Implementation Status: ✅ 12/12 Major Security Fixes Fully Implemented**
...
### 11. Media Asset Publish-State Gating ✅

**Files**:

- `src/utils/media/media-service.server.ts` — `getPublishedReferences()` / `isReferencedByPublishedContent()`
- `src/services/sdk/namespaces/media-namespace.ts` — SDK layer exposure
- `src/routes/api/[...path]/handlers/media.ts` — Handler-level gate checks
- `src/routes/(app)/mediagallery/` — UI-layer disabled states (grid, table, virtual grid)
...
```
// Layer 1 — Service: Queries collection_* tables for entries with status="publish"
...
// Layer 2 — Handler: 409 Conflict before any mutation
await checkMediaNotReferencedByPublishedContent(cms, mediaId, tenantId);
// Throws AppError with detailed references summary

// Layer 3 — UI: Disabled edit/delete buttons + tooltip + lock badge
// See media-grid.svelte: disabled={isPublishedReferenced}

// Layer 4 — Server Load: Batched pre-check for all visible media items
...
- UI confusion — clear "Referenced by published content" feedback

**Gated Mutation Handlers**:
| Handler | Operation | Status |
|---|---|---|
| `handleDeleteRoutes` | Direct asset deletion | 409 Conflict |
| `handleMediaPostDelete` | POST-based deletion | 409 Conflict |
| `handleMediaManipulate` | Image manipulation | 409 Conflict |
| `handleMediaVersionCreate` | New version creation | 409 Conflict |
| `handleMediaVersionUpload` | Version file upload | 409 Conflict |
| `handleMediaVersionRestore` | Version restoration | 409 Conflict |

**UI Indicators**:

- Edit button: `disabled` + tooltip "Referenced by published content"
- Delete button: `disabled` + tooltip "Referenced by published content"
- Card badge: Lock icon + "Published" label at top-left

### 12. Media Upload - Binary MIME Sniffing ✅

**Files**: `src/utils/media/media-service.server.ts`, `src/utils/media/slim-sniffer.server.ts`

**Vulnerability**: MIME type spoofing — attackers could upload a file with a benign MIME type (e.g., `image/png`) while the file content is actually executable or malicious (e.g., PHP shell).

**Implementation**:

- **Small files (<5MB)**: Uses `arrayBuffer()` → `Buffer.from()` → `sniffMimeType(buffer.subarray(0, 2048))` then `assertMimeAgreement()` before accepting
- **Large files (≥5MB, non-SVG)**: Reads first 2048 bytes via `file.slice(0, 2048).arrayBuffer()`, runs `sniffMimeType()`, and rejects family/scriptable mismatches (`MIME_MISMATCH`)
- **SVG (any size ≤ MAX_SVG_BYTES)**: Always buffered and passed through `sanitizeSvg()` — never pure-streamed. Oversized SVGs are rejected. JPEG-named SVG polyglots are rejected at write time.
- Uses `slim-sniffer.server.ts` (~2KB) magic-byte detection (JPEG, PNG, GIF, WebP, SVG, HTML, TIFF, AVIF, HEIC, MP4, WebM, PDF, DOCX) plus `isAllowedUploadMime()` (no `image/*` prefix matching)
- **Remote URL uploads / importer**: `saveRemoteMedia` and smart-importer use `resolveRemoteAssetMime()` after `validateEgressUrl` + `safeFetch` (no raw `fetch(userUrl)`). `Content-Type: octet-stream` is treated as unknown so sniffing can recover the type.
...
- Stored XSS via large unsanitized SVG (streaming bypass class)
- SSRF via remote media URL upload (internal / metadata targets)
...
- SVGs always sanitized via `bufferAndSanitizeSvg` / `sanitizeSvg()` (never stream-raw)
- `/files/` SVG responses get CSP `script-src 'none'` defense-in-depth
...
- ✅ SVG sanitization (MediaUpload server-side)
...
- [ ] Check all collection entries with status="publish" before delete/edit
...
- [Widget System Overview](/docs/development/widgets/widget-system-overview.mdx)
- [Widget Architecture](../../../development/widgets/architecture.mdx)
- [Widget Development Guide](/docs/development/widgets/widget-development-guide.mdx)

### Related Security Docs

- [Authentication System](/docs/reference/security/authentication-system.mdx)
- [Cryptography Module](/docs/reference/architecture/cryptography-module.mdx)
- [Security Plugin](/docs/reference/architecture/security-plugin.mdx)
