...
description: "SveltyCMS 3-layer session caching, secure rotation, RBAC, Multi-Tenancy 2.0, 2FA, Enterprise SSO (SAML 2.0 + OIDC), API Keys, Magic Links, Guest Auth, and WebAuthn/Passkeys."
order: 6
...
SveltyCMS uses high-entropy, cryptographically secure random tokens for session IDs:

- **32 characters** (alphanumeric), generated via `generateRandomToken()` using `crypto.getRandomValues()` (CSPRNG).
- **Stored directly** in secure cookies and the database.
- **Rotated automatically** on login and renewal. On password change or explicit logout, all active sessions across all devices are immediately invalidated.

### Session & CSRF Cookie Security Model

SveltyCMS employs dual-cookie architecture enforcing strict browser boundaries:

| Cookie      | Production Name        | Dev Name        | Flags                                                                | Security Role                                                                                      |
| :---------- | :--------------------- | :-------------- | :------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------- |
| **Session** | `__Host-auth_sessions` | `auth_sessions` | `httpOnly: true`, `secure: true`, `sameSite: "strict"`, `path: "/"`  | Carries user authentication session token. Inaccessible via JavaScript.                            |
| **CSRF**    | `__Host-csrf_token`    | `csrf_token`    | `httpOnly: false`, `secure: true`, `sameSite: "strict"`, `path: "/"` | Carries 256-bit CSRF token read by client fetch utilities for `X-CSRF-Token` double-submit header. |

- **`__Host-` Cookie Prefix (RFC 6265bis)**: In production HTTPS contexts, session and CSRF cookies use the `__Host-` prefix. This mandates `secure: true`, `path: "/"`, and forbids `domain` inheritance — guaranteeing subdomains cannot inject, overwrite, or hijack cookies.
- **Cookie Invalidation Flag Parity**: When terminating sessions (`clearSessionCookies`), deletion calls match creation flags (`path: "/"`, `httpOnly: true`, `sameSite: "strict"`, `secure: true`) across all prefix variations (`__Host-`, `__Secure-`, plain), guaranteeing full browser cookie revocation.
- **CSRF Token Rotation**: Mutating endpoints (`POST`, `PUT`, `PATCH`, `DELETE`) require matching `X-CSRF-Token` headers and rotate the CSRF cookie upon successful execution to prevent token reuse. Safe read methods (`GET`, `HEAD`, `OPTIONS`) bypass CSRF checks entirely (0µs overhead).

### Session Record (all engines)

Every engine persists the same authoritative record — `auth_sessions` (Mongo: the sessions collection) carries:

| Column                                 | Purpose                                                                                                         |
| :------------------------------------- | :-------------------------------------------------------------------------------------------------------------- |
| `_id`                                  | 32-char CSPRNG session token, rotated on login and renewal.                                                     |
| `user_id`                              | Owning user; all sessions are purged when the user is deleted.                                                  |
| `expires`                              | Absolute expiry — the L0/L1 caches never outlive it.                                                            |
| `tenantId`                             | Tenant scope: a session is valid only inside the tenant it was created for.                                     |
| `userAgent` / `deviceId` / `ipAddress` | Device binding and the session listing in the admin UI.                                                         |
| `amr`                                  | Authentication Method References (RFC 8176), e.g. `["pwd"]` or `["pwd","mfa"]` — drives the `mfaRequired` gate. |
| `mfaVerifiedAt`                        | When the second factor was proven for this session; `null` until then.                                          |

- **Cross-engine parity**: the columns are declared once in the system schema spec and materialised per adapter (Drizzle schemas plus boot legacy-tail `ALTER`s for existing databases, Mongo `SessionSchema`). MariaDB stores `mfaVerifiedAt` as `DATETIME(3)` so millisecond precision survives the round-trip; MariaDB also double-encodes JSON columns, so reads normalise `amr` through the shared `parseJsonField` boundary.
- **Rotation carries the MFA proof**: `rotateToken` copies `amr`, `mfaVerifiedAt`, and the device metadata onto the new row — renewing a cookie never downgrades or silently upgrades the session's authentication strength.
...
## 4. Advanced Security Features (opt-in)

> [!IMPORTANT]
> **Enterprise auth extras are optional.** Core login (password session + RBAC) always runs.
> SAML, OIDC/SSO providers, SCIM, 2FA/TOTP, passkeys, and session device policy only apply
> when configured in System Settings / `config/private.ts` (or the matching DB-driven settings).
> Unconfigured features do **not** run on the request hot path — no SAML/SCIM/2FA tax unless enabled.

| Feature                     | Default                      | Enable via                           |
| :-------------------------- | :--------------------------- | :----------------------------------- |
| Password + session cookies  | On                           | Always (setup)                       |
| RBAC / endpoint permissions | On                           | Roles & permissions                  |
| Rate limit / WAF            | On (middleware)              | Settings; bypass in test/benchmark   |
| 2FA / TOTP                  | Off until user/admin enables | User security + optional force-admin |
| SAML 2.0                    | Off                          | `SAML_*` settings + IdP cert         |
| OIDC / social SSO           | Off                          | `SSO_PROVIDERS` / provider keys      |
| SCIM 2.0                    | Off until tokens/routes used | SCIM tokens + `/api/scim`            |
| Session device policy       | Configurable                 | `SESSION_DEVICE_POLICY`              |
| Passkeys / WebAuthn         | Off until registered         | User passkey registration            |

### Two-Factor Authentication (2FA)

- **TOTP-based**: Compatible with Google Authenticator, Authy, etc.
- **AES-256-GCM Encryption**: TOTP secrets encrypted at rest using the instance's `ENCRYPTION_KEY` — never stored as plaintext in the database.
- **TOTP Replay Protection**: Consumed-codes registry prevents code reuse within the 90-second validity window.
- **Backup Codes**: Secure recovery via high-entropy, SHA-256 hashed backup codes.
- **Trusted Devices**: "Remember this device" — HMAC-signed cookie (30-day TTL) allows skipping 2FA on recognized devices.
- **Pending Setup**: Interrupted 2FA setup is automatically resumed — the encrypted secret is persisted immediately.
- **Configurable TOTP Window**: `TOTP_WINDOW` env var (default 1, max 5) for clock-drift tolerance.
- **Plugin-Enforced 2FA**: Plugins can require 2FA for specific users via the `afterAuthenticate` hook.
...
Full **authorization-code** relying-party flow plus **RP-Initiated Logout**, implemented in `src/databases/auth/sso-session.ts` and `handlers/auth.ts`.

| Capability       | Detail                                                                                                 |
| :--------------- | :----------------------------------------------------------------------------------------------------- |
| **Login**        | `GET /api/auth/oidc-login?provider=` → 302 to OP authorize; HttpOnly `state`/`nonce` cookie            |
| **Callback**     | `GET /api/auth/oidc-callback` — code exchange, **JWKS** id_token verify, local session                 |
| **Logout**       | `GET\|POST /api/auth/oidc-logout` — local session clear + optional OP `end_session_endpoint`           |
| **Discovery**    | `/.well-known/openid-configuration` (cached) fills authorize/token/jwks/logout endpoints               |
| **Providers**    | Multi-provider registry via `SSO_PROVIDERS` setting (`SsoProviderConfig`)                              |
| **Provisioning** | Email claim must match an **existing** local user (or invite flow) — no silent create on OIDC callback |

Also supports social OAuth-style providers (**Google**, **GitHub**) where configured as OIDC/OAuth clients in settings. See [Auth API — OIDC](../api/auth.mdx#4-oidc-sso-openid-connect) for request parameters and security notes.
