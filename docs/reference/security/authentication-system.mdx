...
### Cookie Invalidation Parity

When destroying sessions (`clearSessionCookies`), SveltyCMS sets exact matching deletion attributes (`path: "/"`, `httpOnly: true`, `sameSite`, `secure`) matching `setSessionCookie`. This ensures session cookies are cleanly invalidated by user agents across all cross-site or subdomain contexts.
