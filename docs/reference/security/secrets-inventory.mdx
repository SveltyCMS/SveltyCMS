...
| `ENCRYPTION_KEY`    | Encrypts TOTP secrets, plugin settings (via HKDF domains), and collection fields flagged `encrypt: true` | ✅ CSPRNG (32-char) | ⚠️ Requires data re-encryption |
...
| Key                          | Default  | Description                                                           |
| ---------------------------- | -------- | --------------------------------------------------------------------- |
| `COOKIE_PATH`                | `"/"`    | Custom cookie path for session/demo cookies                           |
...
| `SESSION_TTL_HOURS`          | `24`     | Active session lifetime in hours (`sessionTtlMs()`, login + rotation) |
| `SESSION_IDLE_HOURS`         | `0`      | Sign out after N idle hours (`0` = disabled)                          |
| `SESSION_MAX_PER_USER`       | `0`      | Max concurrent sessions per user (`0` = unlimited, LRU eviction)      |
| `SESSION_EXPIRATION_SECONDS` | `86400`  | ⚠️ **Legacy/no-op** — written by the seeder, never read               |

> [!WARNING]
> `SESSION_EXPIRATION_SECONDS` is seeded (`src/routes/setup/seed.ts`) but **never read** anywhere in the codebase — it does not control session lifetime. The effective controls are `SESSION_TTL_HOURS` (read at `src/databases/auth/index.ts`, `src/hooks/handle-authentication.ts`), `SESSION_IDLE_HOURS` (idle-timeout window in `src/hooks/handle-authentication.ts`) and `SESSION_MAX_PER_USER` (concurrent-session cap in `src/databases/auth/index.ts`).
...
| `MAPBOX_API_TOKEN`        | Private | `""`     | Mapbox public token (⚠️ **Must be URL-restricted in Mapbox dashboard**) |
...
| ------------------------ | ----------------------------------------------------------- | --------------------------------- | ------------------------------------------------------------ |
| **Session cookies**      | Browser (`__Host-auth_sessions` / `auth_sessions`)          | `SESSION_TTL_HOURS` (default 24h) | Opaque session id (not a JWT)                                |
...
| **API keys** (`sck_...`) | Database (**HMAC-SHA-256**, keyed with the instance secret) | Permanent until revoked           | Plaintext shown once on creation (`hashApiKey()`)            |
| **Website tokens**       | Database (**SHA-256** digest)                               | Permanent until revoked           | Bearer auth for external clients (`hashCredentialSha256Hex`) |
...
| `LOG_CHAIN_SECRET`   | Every 90 days     | Past `logs/app.log` chains unverifiable; keep old secret to verify archives |
