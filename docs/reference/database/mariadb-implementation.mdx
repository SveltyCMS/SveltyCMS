...
- **Raw single INSERT** (no RETURNING — Drizzle's mysql2 dialect has no `.returning()`): INSERT 219–271 → **279–317 RPS** (+15%, values synthesized from `prepareValues`, identical shape to the base path).
