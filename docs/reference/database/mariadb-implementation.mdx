...
```ts
// Optimized mysql2 pool settings — all timing values configurable via env
{
  host: config.host,
  port: config.port,
  user: config.user,
  ...
  maxIdle: 50,                   // MARIADB_MAX_IDLE (default 50)
  idleTimeout: 0,                // MARIADB_IDLE_TIMEOUT ms (default 0 = permanently warm)
  queueLimit: 0,                 // Unlimited queue
  enableKeepAlive: true,         // TCP keepalive — prevents silent NAT/firewall drops
  keepAliveInitialDelay: 0,      // MARIADB_KEEPALIVE_DELAY ms (0 = immediate)
...
| ----------------------- | ------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `idleTimeout`           | **0**   | `MARIADB_IDLE_TIMEOUT`    | **0 = permanently warm** — eliminates reconnect storms on bursty load. Set to positive ms (e.g. `300000` = 5 min) only behind ProxySQL/MaxScale in transaction mode. |
| `maxIdle`               | 50      | `MARIADB_MAX_IDLE`        | Max idle connections to keep warm in the pool.                                                                                                                       |
| `keepAliveInitialDelay` | 0       | `MARIADB_KEEPALIVE_DELAY` | Delay in ms before first TCP keepalive ping. 0 = immediate (default).                                                                                                |
