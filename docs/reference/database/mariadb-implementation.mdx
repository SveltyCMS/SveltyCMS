...
> [!WARNING]
> The MariaDB adapter is **planned but not yet implemented**. The content below represents the architectural blueprint and implementation plan, not a shipped feature. For production-ready adapters, use [SQLite](./sqlite-implementation.mdx), [PostgreSQL](./postgresql-implementation.mdx), or [MongoDB](./mongodb-implementation.mdx).
