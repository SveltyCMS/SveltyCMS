...
- **Node.js 24+ (V8 TurboFan)**: Recommended for **long-running production servers**. In full-stack CMS workloads involving deep middleware chains, session HMAC cryptography, GraphQL AST parsing, and Drizzle query construction, V8's TurboFan JIT compiler delivers **1.8× to 2.4× higher sustained throughput (4,000+ RPS on `findById`)** and tighter p99 tail latency.
  - **V8 Young Generation Tuning & Soak-Test Hardening**: In high-throughput 60s soak tests (~1,050 RPS, 61,800+ total requests), SvelteKit allocates 30–35 MB/s of ephemeral objects (Request, Response, URL, locals, ASTs). With the Node.js default 16 MB Semi-Space, the Young Generation overflows in milliseconds, forcing V8 to promote short-lived garbage into Old Space (**Premature Tenuring**). This accumulates ~500 MB of garbage and causes major Full GC pauses at ~15s and ~55s.
  - **The Fix**: Setting `NODE_OPTIONS="--max-semi-space-size=128 --max-old-space-size=1024"` expands the Semi-Space to 128 MB. V8 cleans up transient allocations in sub-millisecond minor scavenge cycles (<1 ms) without tenuring them into Old Space, eliminating GC throughput drops and stabilizing memory at ~200–300 MB under sustained load.
  - **Containerization**: The official production `Dockerfile` is built on `node:24-slim` (~450 MB) with these flags pre-configured, coupled with a fast, zero-dependency `curl` healthcheck (~3 ms vs ~200 ms Node evaluation).
