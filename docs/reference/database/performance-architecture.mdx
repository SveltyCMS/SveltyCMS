...
| Raw multi-VALUES `insertMany` (all SQL)                    | SQLite, PostgreSQL, MariaDB                                                           | Single atomic statement per chunk (999-param SQLite / 65k-param PG & MariaDB). SQLite BULK (100) 177 → 248 RPS (+40%); **PG 87 → 233 RPS (+168%, was Drizzle `.returning()`)**; MariaDB is engine-bound (~4.6ms/100 rows) so parity with Drizzle but now raw + `skipReturning` for outbox/seeds. Undefined values bind as column defaults: literal `DEFAULT` (PG/MariaDB) or explicit fills (SQLite — no `DEFAULT` keyword in VALUES; fills `createdAt`/`updatedAt`/`status`/`isDeleted`/`data`, fixing a NULL-`createdAt` bug on the raw path)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Raw single INSERT (MariaDB)                                | MariaDB                                                                               | Drizzle mysql2 has no `.returning()` and pays the AST per insert — raw no-returning `pool.execute` + values synthesis: INSERT 219–271 → 279–317 RPS (+15%)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `skipReturning` bulk (SQLite)                              | SQLite + outbox flush + seeds                                                         | Seed/system-bulk callers get prepared values WITHOUT JSON.parse/flatten (was a no-op win before): bulk profile 401 → 565 batches/s; outbox flush wired to skipReturning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Composite index `(tenantId, updatedAt)` + default sort     | SQLite/PG/Maria `createModel` DDL + `defaultPageSortOption`                           | The status-less tenant list (default list page, `WHERE tenantId=? ORDER BY updatedAt DESC LIMIT n`) previously paid a temp B-tree sort / filesort per page (SQLite ~0.12 ms/1k rows, linear in row count). New composite serves it directly; `defaultPageSortOption` changed `_id: -1` → `updatedAt: -1` (meaningful default: recently updated first; keyset cursor carries the sort value + `_id` tiebreaker). FIND PAGE 0.70 → 0.12 ms, keyset page 0.53 → 0.11 ms                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Date-column filter coercion                                | All SQL `drizzle-sql-helpers` + `prepareValues`                                       | Date/timestamp columns now coerce ISO strings / epoch numbers from filters and keyset cursors into `Date` objects — Drizzle timestamp modes call `value.getTime()` / `value.toISOString()` unguarded, so `{ updatedAt: { $lt: isoString } }` previously threw (keyset pagination on `updatedAt` crashed on SQLite AND MariaDB). Detection reads public getters (sqlite-core) AND `column.config` (mysql-core hides the metadata); accepts ISO-8601 and MariaDB space-separated datetime strings. JSON-field comparisons keep dialect-text semantics                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Flat Delimited Cache Keys (`serializeQueryShape`)          | LocalCMS SDK Read Pipeline                                                            | Replaces `JSON.stringify` on collection query hashing with O(1) delimited string tokens (`q:..                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | l:.. | o:..`), cutting cache key calculation time from ~15µs to <1µs |
...
| Outbox emit (buffered)                                          | crypto UUID + `getDb()` per emit                | crypto UUID stamped at **emit time** (~1µs — part of the emit contract: callers read `event._id`); only the DB write is deferred to the 25ms/64-event bulk flush; buffered path is env-check + push | measured 28µs → ~5µs                                                                                                                  |
