...
**Implementation:** `SqlAdapterCore.findPage`, `MongoCrudMethods.findPage`, pure helpers in `core/page-utils.ts`. Tenant guard forwards `findPage`.
...
### Verified gains (2026-08-04, local Docker / embedded)

Suite: `BENCHMARK_RECORD=1 bun test tests/benchmarks/database-performance.test.ts`  
Hardware: same host as prior adapter baseline. Redis disabled.

| Scenario                             |    SQLite | PostgreSQL |   MariaDB |    MongoDB |
| ------------------------------------ | --------: | ---------: | --------: | ---------: |
| **FIND PAGE (50 hasMore)**           |  0.073 ms |   1.006 ms |  0.660 ms |   0.833 ms |
| **LIST+COUNT (legacy)**              |  0.191 ms |   4.845 ms |  0.928 ms |   4.904 ms |
| **findPage vs legacy**               | **~2.6×** |  **~4.8×** | **~1.4×** |  **~5.9×** |
| **COUNT exact** (filtered, no cache) |  0.119 ms |   2.160 ms |  0.779 ms |   4.607 ms |
| **COUNT ESTIMATE** (unfiltered)      |  0.024 ms |   0.721 ms |  0.423 ms |   0.435 ms |
| **COUNT CACHED** (L1 hit)            |  0.024 ms |   0.024 ms |  0.024 ms |   0.024 ms |
| **estimate vs exact**                |   **~5×** |    **~3×** | **~1.8×** | **~10.6×** |
| **cached vs exact**                  |   **~5×** |   **~90×** |  **~32×** |  **~192×** |

> [!NOTE]
> Networked engines benefit most from dropping the second round-trip (`LIST+COUNT` → `findPage`) and from count cache. SQLite already has a tiny floor; product defaults still remove wasted exact counts under poll storms.
