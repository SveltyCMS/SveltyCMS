...
| `insertReturnsRows` / `updateReturnsRows`         | SQLite + PostgreSQL (Drizzle RETURNING); MariaDB re-reads via fast `findById` | Single RT where dialect allows                                       |
...
**SDK tax suite:** `tests/benchmarks/local-cms-crud.test.ts` measures `cms.collections.create` / `update` / `findById` vs adapter `findOne` (not raw `db.crud` alone).
