...
| Row-store hybrid (scalar fields → columns)                 | All SQL                                                                               | Every scalar field (string/number/boolean widgets — allowlist in `drizzle-sql-helpers`) becomes a physical column via `createModel`; the `data` blob keeps only dynamic fields (object/array widgets). Filters/sorts on materialized fields use real columns/indexes; the read merge treats columns as authoritative (data fills only gaps); legacy rows are backfilled from `data` on column creation (idempotent UPDATE); raw findById selects materialized columns; boolean columns coerce 0/1 → true/false on the raw paths; `atomicIncrement` targets the column when materialized. Also fixed a pre-existing silent ALTER no-op (PRAGMA ran through `stmt.run()`) and PG/Maria dead columns (createModel made them; getTable never registered them) |
