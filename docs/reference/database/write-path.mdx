...
flowchart TD
    ...
    subgraph Shared["Shared for both cold & warm"]
        ...
        ENC{"hot._hasEncryptedFields?"}
        E["AES-256-GCM encrypt<br/>v1:iv:tag:ciphertext"]
...
- `sql-adapter-core.ts` `update` `@1995-2055` (core) + `executeUpdate` `@2030-2130`: `rawUpdateReturning()` → `UPDATE … RETURNING`. **Partial PATCHes intentionally keep `RETURNING`** so untouched physical columns (`status` / `createdAt` / `isDeleted`) remain intact in the response — the response reflects the DB's actual post-update state.
- **Partial updates MERGE the JSON `data` blob** (2026-09): a payload that mentions one dynamic field no longer replaces the other fields stored in `data` (`prepareValues` `@1095` marks the patch, `executeUpdate` `@2066` rides it). PostgreSQL merges in SQL (`SET "data" = COALESCE("data",'{}'::jsonb) || $n::jsonb`) and SQLite/MariaDB use `json_patch` / `JSON_MERGE_PATCH` for scalar/array patches, hydrating the stored blob and merging in JS for nested-object or explicit-`null` patches (both dialect operators are RFC 7396: recursive, and they _delete_ keys patched with `null`). The contract is a **shallow, null-keeping merge — MongoDB `$set` parity**: a patched object replaces wholesale (never recurses), and `null` is a stored value, not a deletion. Full-document writers opt out with `replaceData: true`; `skipReturning: true` already implies it, because that path reconstructs the row from the prepared values.
...
**The one exception (and its scope):** `skipReturning: true` re-constructs the row from the prepared values instead of a read-back. This path is reserved for **seed / benchmark tooling** (e.g. `tests` calling `skipReturning` explicitly) where the caller already knows the row and `RETURNING` is pure overhead. It is **not** the production write path. The benchmark env-var shortcut that previously forced this non-production path was deliberately removed so that benchmarks measure the real `INSERT … RETURNING` fast path. Because it treats the payload as the **complete document**, it also opts out of the partial-update merge above.

> **Merged on every write path (2026-09-22):** the single-entry update, `crud.updateMany`, `batch.bulkUpdate` and the query-builder `updateMany` all merge a JSON `data` patch now. Where the dialect operator can express the patch exactly (`operator` dialects, and scalar/array patches on `subset` dialects) the merge happens **inside the statement** — no read, no extra round trip. A nested-object or explicit-null patch on SQLite/MariaDB cannot be expressed in one `SET`, so those rows are merged individually (each against its own stored blob) instead of silently replacing every unmentioned field; `batch.bulkUpdate` routes them to its per-row fallback loop, which hydrates per row.
