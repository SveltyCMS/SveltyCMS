...
| `upsert` / `upsertMany`                | Atomic update-or-insert. SQL `_id` filters are one `INSERT … ON CONFLICT` / `ON DUPLICATE KEY` (chunked under bind limits). Mongo is `bulkWrite`. Other filters stay findOne + write. |
...
## 🔐 Credential Storage (Website Tokens & API Keys) {#-credential-storage-website-tokens--api-keys}
...
- Credential lookup without request tenant context uses branded `withSystemScope(reason)` only (not `bypassSafeQuery`), so `safeQuery` still applies soft-delete boundaries.
...
5. **Transactions & Soft Failures**: Inside `db.transaction(fn)`, returning `{ success: false, rollback: false }` allows returning non-fatal read/query misses without triggering an automatic outer database transaction rollback.
