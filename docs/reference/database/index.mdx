---
path: "docs/reference/database/index.mdx"
title: "Database Documentation Hub"
description: "Central hub for SveltyCMS database architecture — 4 production adapters, findPage/hasMore, count modes + cache, allocation-floor engine, and zero-tax LocalCMS."
order: 1
icon: "mdi:database"
...
graph TD
    ...
    subgraph Adapters["4 Production Engines"]
        H[(MongoDB)]
        I[(MariaDB)]
        J[(PostgreSQL)]
        K[(SQLite)]
...
> **SQL family**: ~5-8 fewer allocations per filtered query. **MongoDB**: ~2-3 fewer (result pool + fused mapQuery). **All 4**: benefit from shared BaseAdapter optimizations plus the core widget/session/cache pipeline. See [Performance Architecture](./performance-architecture.mdx#core--adapter-communication-all-4-engines).
...
MongoDB for unstructured/high-volume document stores. MariaDB for MySQL-compatible production setups. All four engines are production-ready.

---

**Last Updated**: 2026-08-21 (targeted writes + `createdAt` immutability on all 4 engines, core widget/session/cache pipeline)
