...
| ----------------------------- | ----------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `max`                         | 100         | `DATABASE_MAX_CONNECTIONS` | Maximum connections in the pool. Raise for large servers; keep at 100 on shared hosts to avoid `too many clients`.                                                                                 |
| `idle_timeout`                | **0**       | `DATABASE_IDLE_TIMEOUT`    | **0 = permanently warm (default)** — eliminates reconnect storms when a load wave hits after a quiet period. Set to a positive value (seconds) only when fronted by PgBouncer in transaction mode. |
| `connect_timeout`             | 10s         | `DATABASE_CONNECT_TIMEOUT` | TCP connect timeout. 3× faster failure detection on unreachable hosts vs. the postgres.js default 30s.                                                                                             |
...
> [!TIP]
> **When to change `DATABASE_IDLE_TIMEOUT`:** The default `0` (permanently warm) is optimal for direct
> PostgreSQL connections and most managed-database services (Supabase, RDS, Neon). Set it to a
> positive number of **seconds** (e.g. `600`) only when a connection pooler like PgBouncer in
> **transaction mode** sits between the CMS and Postgres — in that case the pooler handles recycling
> and idle connections on the CMS side waste pooler slots.
