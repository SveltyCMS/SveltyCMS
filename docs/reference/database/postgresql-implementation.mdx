...
- **Raw `INSERT…RETURNING` fast path**: One-round-trip insert via nested-fragment tagged template. postgres.js 3.x removed `sql.join` — the earlier implementation silently fell back to Drizzle on every insert (~225 RPS, 50% of the 1c ceiling). The fix builds the VALUES list with nested fragments, quotes identifiers (unquoted camelCase folds to lowercase), and binds dates/objects as strings (the describe-phase Bind mishandles raw Date/object params under pipelined connections). Measured: 217 → **299 RPS** in the adapter benchmark; the raw SQL alone reaches ~413 RPS (92% of the 450 RPS 1c ceiling).
