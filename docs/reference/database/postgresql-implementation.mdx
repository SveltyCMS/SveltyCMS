...
An unchanged release skips this pass entirely: `bootstrapSystemSchema` compares a sha-256 of `SYSTEM_SCHEMA` (per dialect) stored in `svelty_schema_state` and returns `{ success: true, skipped: true }` without touching the database — measured 2026-09-22: 118 statements / 124 ms per boot → ~2 ms. Any spec change re-runs the pass, and a pass with a failed statement does not store the marker, so a half-applied schema is retried instead of trusted.

### Connection Pool Configuration (2026-09 Hardening)

SveltyCMS configures the `postgres.js` driver with production-tuned connection pool settings. All timing values are configurable via environment variables so operators can tune without code changes:

| Setting                       | Default     | Env Var Override           | Purpose                                                                                                                                                                                         |
| ----------------------------- | ----------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `max`                         | tier-based  | `DATABASE_MAX_CONNECTIONS` | Maximum connections in the pool — `hw.dbPoolSize` (2 per budget core, `DB_POOL_SIZE`), floored at 20 on small hosts and 32 above. Raise for large servers; keep low on shared hosts.            |
| `idle_timeout`                | 30s         | —                          | Idle seconds before a pooled connection is closed; the config object's `idle_timeout` overrides it. Shorten it behind PgBouncer in transaction mode, where idle connections waste pooler slots. |
| `connect_timeout`             | 10s         | —                          | TCP connect timeout. 3× faster failure detection on unreachable hosts vs. the postgres.js default 30s.                                                                                          |
| `max_lifetime`                | 60 min      | —                          | Halves pool recreation overhead vs. the previous 30 min default.                                                                                                                                |
| `keepalive`                   | true        | —                          | Prevents silent connection drops from cloud NAT/proxies/firewalls.                                                                                                                              |
| `keepaliveInitialDelayMillis` | 10 000      | —                          | Starts keepalive pings after 10 s idle.                                                                                                                                                         |
...
| `prepare`                     | true        | `DATABASE_PREPARE=false`   | Server-side prepared statements. Disable for PgBouncer transaction mode (`DATABASE_PREPARE=false`).                                                                                             |
...
> [!TIP]
> **When to change `idle_timeout`:** the 30-second default is optimal for direct PostgreSQL
> connections and most managed-database services (Supabase, RDS, Neon). Shorten it only when a
> connection pooler like PgBouncer in **transaction mode** sits between the CMS and Postgres — there
> the pooler handles recycling and idle CMS connections waste pooler slots.
...
| **Connection Pool** | Keepalive (10s), pipeline (batch), idle 30s, lifetime 60min, timeout 10s |
...
- **GIN Indexes**: the 5 spec GIN indexes on high-query JSONB columns (`content_nodes.data`, `content_nodes.metadata`, `media_items.metadata`, `roles.permissions`, `auth_users.roleIds`), plus a `jsonb_path_ops` `<table>_data_gin` on every dynamic collection table (`createModel`; opt out with `schema.jsonIndex === false`)
...
- **Targeted column UPDATE**: `rawUpdateReturning` builds `SET` only for prepared keys. Shared `prepareUpdateValues()` never writes `createdAt` and omits an empty `data={}` blob on partial updates.
- **Partial updates merge the JSONB blob**: the `data` column's `SET` expression becomes `COALESCE("data",'{}'::jsonb) || $n::jsonb` (shallow, keeps explicit nulls) whenever the write is a patch rather than a complete document — so a `PATCH {views: 1}` no longer replaces `title`/`body`/… in the same row. The replace form is kept for `replaceData: true` / `skipReturning: true` callers, and the same merge covers `crud.updateMany`, `batch.bulkUpdate` and query-builder `updateMany`. See [Write Path](./write-path.mdx#3-does-a-write-verify-the-data-was-100-persisted).
- **Indexed dynamic-field filters (containment + GIN)**: an equality filter on a field that lives in the `data` blob (i.e. not materialized into a column) is emitted as `data @> '{"field": value}'::jsonb` — the form the `jsonb_path_ops` GIN index serves — instead of `data->>'field' = $1`, which can only be a sequential scan. `createModel` provisions `<table>_data_gin` for every dynamic collection table; `schema.jsonIndex === false` opts out for write-only collections. Measured at 100k rows: **7.9–10.9 ms Seq Scan → 0.1–1.7 ms Bitmap Index Scan** (index build 347 ms once; write cost within noise). JSON type fidelity is preserved by probing the raw value, so `{views: 101}` matches a stored number and not a stored `"101"` (SQLite and MongoDB already agreed; MariaDB's text-based JSON comparison remains looser). Dotted paths probe a nested object (`a.b` → `{"a":{"b":…}}`), matching the `data#>>'{a,b}'` extraction form.
- **Prepared JSON/array binds**: `bindPgParam()` serializes objects and arrays as JSON text; JSON/jsonb columns use `$n::jsonb` so postgres.js does not emit PG array literals `{…}` (those force a describe-phase recompile).
- **`crud.aggregate()` (native translation)**: MongoDB-style pipelines compile to one statement via `core/aggregation-translator.ts` — `$match`, `$group` (scalar `_id` plus `$sum`/`$avg`/`$min`/`$max`/`$count`), `$sort`, `$skip`, `$limit`, `$count` and include-only `$project`. `$min`/`$max` on a **non-materialized** field are refused with `error.code === "NOT_SUPPORTED"` (the `->>` extraction is TEXT — `MIN` over `{10, 2}` returns `"10"` — and there is no `min(jsonb)`); declare the field `indexed`/`materialize: true` to aggregate it. `SUM`/`AVG` cast the extraction to `numeric` (`getJsonNumericField`), and any other unsupported stage fails closed the same way.
- **Numeric range filters on blob fields**: `$gt`/`$gte`/`$lt`/`$lte` compile to `jsonb_typeof(data->'x') = 'number' AND data->'x' >= '4'::jsonb` (`getJsonCompare`) — comparing the extracted TEXT was lexicographic (`views >= 4` missed stored `16`/`32`), and the `jsonb_typeof` guard is required because jsonb orders String below Number, so an unguarded `<` would admit stored strings.
...
Implement dialect hooks in `src/databases/postgresql/adapter-core.ts`; the shared CRUD implementation lives in `src/databases/core/sql-adapter-core.ts` and the concrete entry point is `postgres-adapter.ts`. Use the `wrap()` helper for consistent error handling and logging.
