---
path: "docs/reference/database/core-infrastructure.mdx"
...
The `core` directory is not a centralized security interceptor. Tenant isolation, query sanitization, and permission enforcement live inside each adapter's implementation (drizzle helpers, mongo crud-methods).

They are also handled by the `forTenant()` wrapper and higher security layers (safe-query, content engine, API dispatcher).
...
- **SQL Adapter Core**: `sql-adapter-core.ts` — `SqlAdapterCore` abstract class extending `BaseAdapter`, implementing `ISqlAdapter`. Absorbs ~1,300 lines of duplicated CRUD, delegation, module-getter, and `prepareValues` logic shared by all three SQL engines. Each adapter (PostgreSQL, MariaDB, SQLite) now extends this class and implements only dialect-specific hooks (`getTable`, `getJsonField`, `isMissingTableError`, etc.). `prepareUpdateValues()` stamps `isUpdate` so `createdAt` is insert-only and empty `data={}` blobs are omitted on partial UPDATE.
- **Plugin Registry**: `plugin-registry.ts` — topological dependency-aware parallel service boot with **zero settling delay** (no `setTimeout`); one pass into ready/blocked lists per wave.
...
### Key Responsibilities {#db-ts-responsibilities}
...
// 2b. CRUD wrappers (db.ts — after load, before READY)
// adapter.crud = createCountCachedCrud(createTenantGuardedCrud(originalCrud))
// - Tenant guard: fail-closed MULTI_TENANT (zero overhead when single-tenant)
// - Count cache: 30s L1/L2 for crud.count (equal benefit on all engines)
...
// SQLite capabilities
{
  ...
  supportsIndexing: true,
  supportsFullTextSearch: true,
  supportsAggregation: true,         // Documented pipeline subset (aggregation-translator.ts)
...
- [Cache System Files](../architecture/cache-system.mdx) - Cache infrastructure
