...
description: "Complete MongoDB-specific implementation guide covering optimizations, estimatedDocumentCount, findPage, indexes, connection pooling, and performance tuning for SveltyCMS."
order: 4
icon: "mdi:leaf"
...
### Count estimate + `findPage` (shared contract)

Mongo implements the same product APIs as SQL adapters:

| API                                          | Mongo path                                                         | Notes                                                   |
| -------------------------------------------- | ------------------------------------------------------------------ | ------------------------------------------------------- |
| `count(..., { mode: "estimate" \| "auto" })` | `estimatedDocumentCount()` when filter empty **and** no `tenantId` | WiredTiger metadata — avoids full `countDocuments` scan |
| `count` filtered / tenant-scoped             | `countDocuments(secureQuery)`                                      | Exact; short-lived L1 cache still applies               |
| `findPage`                                   | `findMany` with `limit + 1` via `MongoCrudMethods.findPage`        | Prefer over `findMany` + `count` for lists              |

**Measured (2026-08-04, local Docker Mongo, no Redis):**

| Scenario               |          Avg | Gain                               |
| ---------------------- | -----------: | ---------------------------------- |
| FIND PAGE (50 hasMore) | **0.833 ms** | **~5.9×** vs LIST+COUNT (4.904 ms) |
| COUNT exact (filtered) |     4.607 ms | baseline scan path                 |
| COUNT ESTIMATE         | **0.435 ms** | **~10.6×** vs exact                |
| COUNT CACHED (L1)      | **0.024 ms** | **~192×** vs exact                 |

Full cross-engine table: [Performance Architecture](./performance-architecture.mdx#product-layer-list--count-equal-on-all-engines).
...
- **Schema-Guarded Date Conversion (`mapDates`)**: `processDates`' deep walk is **skipped entirely** for generic content collections (`strict:false` schemas that store timestamps as ISO strings via `nowISODateString`) — the walk found nothing and was pure CPU. System collections with real `Date` paths still convert. Detected once per model and cached.
- **3-Key `mapQuery` Fast Path**: `safeQuery` injects `isDeleted:{$ne:true}` on non-fast-path queries, making them 3 keys — the old fast path accepted only ≤2 (`_id`/`token`/`tenantId`) and every findMany/MT-update fell into the full rebuild + sanitize recursion. The fast path now also accepts the exact safeQuery-injected `isDeleted` shape.
...
**Phase 9: Update Parity with the SQL Engines (September 2026)**

- **`strict: false` on every update site**: Mongoose's schema default **drops `$set` paths that are not declared in the model**, so patching a dynamic field the collection schema did not declare was a silent no-op on MongoDB while the SQL adapters store it in the JSON `data` blob. `crud.update` (both `findOneAndUpdate` paths), `crud.updateMany`, `batch.bulkUpdate` (homogeneous `updateMany` and the heterogeneous `bulkWrite`) and `MongoQueryBuilder.updateMany` now pass `strict: false` — the payload is written as sent, and the namespace / field-permission layer stays the schema gate.
- **`queryBuilder()` model resolution fixed**: the builder looked up its Mongoose model with the **raw** collection name while `crud.*` normalizes it (`normalizeCollectionName`). The lookup missed, so `_getOrCreateModel` built a generic model bound to a **differently named** MongoDB collection — every builder operation (find/count/updateMany/deleteMany) silently ran against an empty collection. It now resolves exactly like `MongoCrudModule._getRepo`.
- **`CollectionModel.aggregate` routes through `crud.aggregate`**: the typed collection wrapper no longer runs the raw `model.aggregate` — it calls `crud.aggregate` (tenant `$match` injection, normalized collection resolution) and logs a refused pipeline with its `NOT_SUPPORTED`/error code instead of returning an empty array.
- **Pipeline parity with the SQL engines**: `core/aggregation-translator.ts` now gives SQLite/PostgreSQL/MariaDB the same pipeline contract — `$match`, `$group` (scalar `_id` plus `$sum`/`$avg`/`$min`/`$max`/`$count`), `$sort`, `$skip`, `$limit`, `$count` and include-only `$project` are portable across all four engines. Stages outside that subset (`$lookup`, `$unwind`, `$facet`, compound `$group._id`) stay native here and are `NOT_SUPPORTED` on SQL, where `$min`/`$max` additionally require a materialized field.
...
The MongoDB adapter has been fully modularized to align with the enterprise-grade architecture used by SveltyCMS's SQL engines.
...
| **collection-module.ts**     | Collection wrapper: `aggregate` → `crud.aggregate`.     | Tenant `$match` injection, logged failures.                    |
...
## ⚙️ Connection Pool Configuration (2026-09 Hardening)

### Hardware-Adaptive Connection Pool

The MongoDB adapter scales pool size with the detected host hardware (`@utils/hardware-profile`) and exposes all timing values via environment variables:

```ts
// src/databases/mongodb/adapter-core.ts
const connectOptions: mongoose.ConnectOptions = {
  // Hardware-adaptive pool (scales with CPU/RAM profile: weak VPS → few, large box → 100-ceiling)
  ...
  // Timeout settings
  serverSelectionTimeoutMS: 30_000, // Topology selection
  connectTimeoutMS: 10_000, // TCP connect
  socketTimeoutMS: 45_000, // Prevent zombie connections
  waitQueueTimeoutMS: 10_000, // Max wait for pool slot

  // Idle & heartbeat (2026-09: both env-configurable)
  maxIdleTimeMS: 0, // MONGODB_IDLE_TIMEOUT_MS (0 = driver-managed, no forced disconnect)
  heartbeatFrequencyMS: 10_000, // MONGODB_HEARTBEAT_MS (topology detection cadence)

  // Wire compression (auto-detected: zstd → snappy → none; MONGO_COMPRESSORS=none to disable)
  // compressors: ["zstd", "snappy"],  // only if optional peer packages are installed
};
```

| Setting                | Default     | Env Var                   | Purpose                                                                                                                                          |
| ---------------------- | ----------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `maxPoolSize`          | HW-adaptive | `DB_POOL_SIZE`            | Scales with detected CPU budget. Override via env for fixed sizing.                                                                              |
| `minPoolSize`          | HW-adaptive | `MONGO_MIN_POOL_SIZE`     | Pre-spawned sockets scaled down on weak hosts.                                                                                                   |
| `maxIdleTimeMS`        | **0**       | `MONGODB_IDLE_TIMEOUT_MS` | **0 = driver-managed (no forced disconnect)**. Set to positive ms (e.g. `300000`) when behind a load balancer that silently closes idle sockets. |
| `heartbeatFrequencyMS` | 10 000      | `MONGODB_HEARTBEAT_MS`    | How often the driver pings the server. Lower values detect failovers faster at the cost of extra traffic.                                        |
| `socketTimeoutMS`      | 45 000      | —                         | Prevents zombie connections on hanging queries.                                                                                                  |
| `MONGO_COMPRESSORS`    | `auto`      | `MONGO_COMPRESSORS`       | `zstd,snappy` for WAN deployments; `none`/`off`/`false` for LAN/loopback (saves CPU on small payloads).                                          |

> [!NOTE]
> Pool size is hardware-adaptive by default. On a 1-core VPS the default `dbPoolSize` may be as low as 5;
> on a 16-core server it scales to ~100. Set `DB_POOL_SIZE=N` to override for a specific deployment.
...
Hot writes (`update` / `updateMany` / `upsert`) use atomic `{ $set }` with `lean: true`, `runValidators: false`, `cloneUpdate: false` and `strict: false` — Valibot already ran in core. `createdAt` is stripped from `$set` (insert / `$setOnInsert` still sets it).

All CRUD methods in `crud-methods.ts` use `.lean()`:

```
// File: src/databases/mongodb/crud-methods.ts
...
4. **Website Token Parity**: `website-token-methods.ts` declares `tenantId` and `isDeleted` on the Mongoose schema (`strict: true`), routes all CRUD through `safeQuery` (no `bypassSafeQuery`), and aligns with the SQL `ISystemAdapter.websiteTokens` contract. See [Credential Storage](./database-methods.mdx#-credential-storage-website-tokens--api-keys).

### Runtime Compatibility

1. **Bun Runtime Alignment**: The MongoDB adapter now runs natively under `bun test` — no need for vitest/Node.js. A `v8` shim in `src/utils/v8-shim.ts` patches `process.getBuiltinModule('v8')` to handle the `bson` package's `isBuildingSnapshot()` call, which Bun (JavaScriptCore) doesn't implement. All 4 database adapters (MariaDB, PostgreSQL, SQLite, MongoDB) use `bun test` uniformly.

2. **Reserved Key Warning Suppression**: The generic Mongoose schema in `adapter-core.ts` now passes `suppressReservedKeysWarning: true` to avoid warnings when content documents contain a field named `collection` (a Mongoose reserved pathname).

3. **Self-Healing Shutdown Guard**: During intentional system reinitialization (`POST /api/system/reinitialize`), the `__SYSTEM_SHUTTING_DOWN__` flag prevents the resilience hooks from scheduling competing auto-reconnections, eliminating `MongoNotConnectedError` race conditions during state-machine stress tests.
