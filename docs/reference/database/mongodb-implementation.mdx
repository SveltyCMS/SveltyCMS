...
Hot writes (`update` / `updateMany` / `upsert`) use atomic `{ $set }` with `lean: true`, `runValidators: false`, and `cloneUpdate: false` — Valibot already ran in core. `createdAt` is stripped from `$set` (insert / `$setOnInsert` still sets it).
