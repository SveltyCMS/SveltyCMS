...
| **GraphQL API**   | `/api/graphql`           | `POST` | `collection:write`  |

> [!NOTE]
> Permission ids in this table are the **namespaced API route** vocabulary, authorized fail-closed against `ENDPOINT_PERMISSIONS` in `src/routes/api/[...path]/+server.ts` (the `graphql` entry maps `GET` ⇒ `collection:read`, everything else ⇒ `collection:write`). Admin UI tiles use the separate `manage:*` vocabulary (`CONFIG_TILE_PERMISSIONS` in `src/routes/(app)/config/+page.server.ts`) — the two are not interchangeable.
...
## 2b. Collection List Query Params (SSR + entry-list) {#2b-collection-list-query-params-ssr--entry-list}
...
| -------------------- | ---------------------------------------------------------------------------------- | -------------------------------------- |
| `search`             | In-collection text search (searchable schema fields + system ids; `LIKE`/`$regex`) | `TableFilter` / `globalSearchValue`    |
