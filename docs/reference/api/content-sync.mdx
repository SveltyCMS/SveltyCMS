...
> [!NOTE]
> **Content Sync and Configuration Promotion**:
>
> - **Content Sync** (`/api/content-sync/*`) moves _editorial content entries_ between active environments (e.g. pulling production articles into staging with PII anonymization).
> - **Configuration Promotion** (`/api/config/*`) moves _site structure and settings_ (collections, roles, permissions, widgets, automations) through version control (Git) in a Schema-as-Code workflow.

> [!IMPORTANT]
> All content sync endpoints are **admin-gated**. GET requests require `content:read`, POST requests require `content:sync`. Unmapped namespaces fail-closed via the dispatcher's `ENDPOINT_PERMISSIONS` mapping (`src/routes/api/[...path]/+server.ts`, `"content-sync"` entry), which only distinguishes `POST` — `PATCH` and `DELETE` therefore require `content:read`.
...
| **Create Channel**   | `/api/content-sync/channels`     | `POST`   | `content:sync` |
| **Update Channel**   | `/api/content-sync/channels/:id` | `PATCH`  | `content:read` |
| **Delete Channel**   | `/api/content-sync/channels/:id` | `DELETE` | `content:read` |
| **Create Sync Plan** | `/api/content-sync/plan`         | `POST`   | `content:sync` |
| **Push Content**     | `/api/content-sync/push`         | `POST`   | `content:sync` |
| **Pull Content**     | `/api/content-sync/pull`         | `POST`   | `content:sync` |
