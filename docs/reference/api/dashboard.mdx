...
`/dashboard` itself is lean: picker metadata comes from `widget.json` (no Svelte eval), saved layouts hydrate in `+page.server.ts`, widget Svelte chunks load when they enter the viewport, and `base-widget` polling pauses while the tab is hidden. See [Dashboard Widget Architecture](../../development/dashboard/architecture.mdx).
...
| **Commerce Orders** | `/api/dashboard/commerce-orders` | `GET` | `dashboard:read` + license |
| **Commerce Inventory** | `/api/dashboard/commerce-inventory` | `GET` | `dashboard:read` + license |
...
### Commerce Orders (freemium)

Recent rows from the ecommerce `orders` collection plus a status mix. Returns `{ available: false }` when the preset is not installed (not 404). Gated by `checkExtensionLicense("dashboard", "commerce-orders")` — 14-day key-less trial, then `403 LICENSE_REQUIRED`.

**Endpoint**: `GET /api/dashboard/commerce-orders`

**Parameters**: `?limit=5` — Number of recent orders (default: 5, max: 50). Status mix is computed from up to 20 newest rows.
...
```json
{
  "available": true,
  "total": 12,
  "byStatus": { "pending": 4, "processing": 8 },
  "recent": [
    {
      "id": "…",
...
### Commerce Inventory (freemium)

Low-stock and out-of-stock products from the `products` collection. Variant-aware (preset `inventory` plus importer aliases). Returns `{ available: false }` when `products` is missing. Gated by `checkExtensionLicense("dashboard", "commerce-inventory")`.

**Endpoint**: `GET /api/dashboard/commerce-inventory`

**Parameters**: `?limit=8` — Number of low-stock rows (default: 8).
...
```json
{
  ...
  "outOfStock": 2,
  "lowStock": [
    { "id": "p1:TEE-S", "title": "Tee · Small", "sku": "TEE-S", "qty": 1, "threshold": 5 }
  ]
}
```
...
The admin dashboard is composed of modular Svelte 5 widget packages (one folder per widget), each backed by one or more API endpoints. The widget catalog is **install-specific** — the core packages bundled with SveltyCMS are listed below, and the [marketplace](../architecture/marketplace.mdx) can add more (counts are never fixed):

| Widget              | File                           | Data Source                              |
| :------------------ | :----------------------------- | :--------------------------------------- |
| **System Health**   | `system-health/index.svelte`   | `/api/dashboard/health`                  |
| **Unified Metrics** | `unified-metrics/index.svelte` | `/api/dashboard/metrics`                 |
| **CPU Monitor**     | `cpu/index.svelte`             | `/api/dashboard/system-info?type=cpu`    |
| **Memory Monitor**  | `memory/index.svelte`          | `/api/dashboard/system-info?type=memory` |
| **Disk Monitor**    | `disk/index.svelte`            | `/api/dashboard/system-info?type=disk`   |

| Widget              | File                           | Data Source                            |
| :------------------ | :----------------------------- | :------------------------------------- |
| **Cache Monitor**   | `cache-monitor/index.svelte`   | `/api/dashboard/cache-metrics`         |
| **Performance**     | `performance/index.svelte`     | `/api/dashboard/metrics?detailed=true` |
| **Audit Log**       | `audit-log/index.svelte`       | `/api/dashboard/audit`                 |
| **Activity Logs**   | `logs/index.svelte`            | `/api/dashboard/logs`                  |
| **System Messages** | `system-messages/index.svelte` | `/api/dashboard/system-messages`       |

| Widget                        | File                                     | Data Source                         |
| :---------------------------- | :--------------------------------------- | :---------------------------------- |
| **Last 5 Content**            | `last5-content/index.svelte`             | `/api/dashboard/last5-content`      |
| **Last 5 Media**              | `last5-media/index.svelte`               | `/api/dashboard/last5media`         |
| **Online Users**              | `user-online/index.svelte`               | `/api/dashboard/online-user`        |
| **Security**                  | `security/index.svelte`                  | `/api/dashboard/security`           |
| **Database Pool Diagnostics** | `database-pool-diagnostics/index.svelte` | `/api/database/pool-diagnostics`    |
| **SCIM Status**               | `scim-status/index.svelte`               | `/api/dashboard/scim`               |
| **Media Storage Analytics**   | `media-storage-analytics/index.svelte`   | `/api/media/analytics`              |
| **Tenant Analytics**          | `tenant-analytics/index.svelte`          | `/api/dashboard/tenant-analytics`   |
| **Commerce Orders**           | `commerce-orders/index.svelte`           | `/api/dashboard/commerce-orders`    |
| **Commerce Inventory**        | `commerce-inventory/index.svelte`        | `/api/dashboard/commerce-inventory` |
| **Task List Widget**          | `todo-list/index.svelte`                 | — (client-side, static)             |
| **Calendar View**             | `calendar-view/index.svelte`             | — (client-side, static)             |
...
1. **Client runtime** — `+page.svelte` uses `import.meta.glob("./widgets/*/*.svelte")` to build the widget registry for rendering and lazy loading. The registry is keyed by the **folder id** (`widget.json.id`); each entry stores its entry filename for the dynamic import.
2. **Server metadata** — `+page.server.ts` eagerly globs the same pattern and pre-computes `availableWidgets` (name, icon, description, folder) for the widget picker.
...
- **Licensing check**: Premium widgets are gated on **both** sides, mirroring custom widgets and plugins:
  - **Client-side gating**: Each widget checks `checkExtensionLicense("dashboard", widgetId)` (via `GET /api/system/license-status?type=dashboard&id=<widget-id>`) and renders an upgrade prompt instead of dashboard content when the trial expired and no `SLM-`/`SLM-DEMO-` key is configured.
  - **Server-side gating**: The backing API endpoints call `checkExtensionLicense` and return **`403 LICENSE_REQUIRED`** when the check fails — premium data is never served without entitlement. See `dashboard-license.ts` in the dashboard handler for the endpoint → widget-id map:

| Endpoint                                | Gating widget id            | License  |
| :-------------------------------------- | :-------------------------- | :------- |
| `GET /api/dashboard/audit`              | `audit-log`                 | freemium |
| `GET /api/dashboard/logs`               | `logs`                      | freemium |
| `GET /api/dashboard/security`           | `security`                  | freemium |
| `GET /api/dashboard/scim`               | `scim-status`               | paid     |
| `GET /api/dashboard/cache-metrics`      | `cache-monitor`             | freemium |
| `GET /api/dashboard/online-user`        | `user-online`               | freemium |
| `GET /api/dashboard/metrics`            | `unified-metrics`           | freemium |
| `GET /api/dashboard/commerce-orders`    | `commerce-orders`           | freemium |
| `GET /api/dashboard/commerce-inventory` | `commerce-inventory`        | freemium |
| `GET /api/database/pool-diagnostics`    | `database-pool-diagnostics` | freemium |
