...
Each dashboard widget is a **self-contained package** in its own kebab-case folder under `src/routes/(app)/dashboard/widgets/<folder>/`, containing the entry component `index.svelte`, a `widget.json` manifest, and the required `readme.mdx` marketplace description (enforced by `scripts/check-dashboard-widget-packages.mjs`):
