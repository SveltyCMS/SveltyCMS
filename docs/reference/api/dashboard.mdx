...
| :------------------ | :---------------------------------------------- | :--------------------------------------- |
| **System Health**   | `system-health/system-health-widget.svelte`     | `/api/dashboard/health`                  |
| **Unified Metrics** | `unified-metrics/unified-metrics-widget.svelte` | `/api/dashboard/metrics`                 |
| **CPU Monitor**     | `cpu/cpu-widget.svelte`                         | `/api/dashboard/system-info?type=cpu`    |
| **Memory Monitor**  | `memory/memory-widget.svelte`                   | `/api/dashboard/system-info?type=memory` |
| **Disk Monitor**    | `disk/disk-widget.svelte`                       | `/api/dashboard/system-info?type=disk`   |

| Widget              | File                                            | Data Source                            |
| :------------------ | :---------------------------------------------- | :------------------------------------- |
| **Cache Monitor**   | `cache-monitor/cache-monitor-widget.svelte`     | `/api/dashboard/cache-metrics`         |
| **Performance**     | `performance/performance-widget.svelte`         | `/api/dashboard/metrics?detailed=true` |
| **Audit Log**       | `audit-log/audit-log-widget.svelte`             | `/api/dashboard/audit`                 |
| **Activity Logs**   | `logs/logs-widget.svelte`                       | `/api/dashboard/logs`                  |
| **System Messages** | `system-messages/system-messages-widget.svelte` | `/api/dashboard/system-messages`       |

| Widget                        | File                                                            | Data Source                       |
| :---------------------------- | :-------------------------------------------------------------- | :-------------------------------- |
| **Last 5 Content**            | `last5-content/last5-content-widget.svelte`                     | `/api/dashboard/last5-content`    |
| **Last 5 Media**              | `last5-media/last5-media-widget.svelte`                         | `/api/dashboard/last5media`       |
| **Online Users**              | `user-online/user-online-widget.svelte`                         | `/api/dashboard/online-user`      |
| **Security**                  | `security/security-widget.svelte`                               | `/api/dashboard/security`         |
| **Database Pool Diagnostics** | `database-pool-diagnostics/database-pool-diagnostics.svelte`    | `/api/database/pool-diagnostics`  |
| **SCIM Status**               | `scim-status/scim-status-widget.svelte`                         | `/api/dashboard/scim`             |
| **Media Storage Analytics**   | `media-storage-analytics/media-storage-analytics-widget.svelte` | `/api/media/analytics`            |
| **Tenant Analytics**          | `tenant-analytics/tenant-analytics-widget.svelte`               | `/api/dashboard/tenant-analytics` |

### Widget Discovery (dual registration + package folders)

Each dashboard widget is a **self-contained package** in its own kebab-case folder under `src/routes/(app)/dashboard/widgets/<folder>/`, containing the component, a `widget.json` manifest, and optional `.mdx` docs:

```text
widgets/system-health/
  system-health-widget.svelte   # component (filename = stable registry key)
  widget.json                   # marketplace manifest
```

Widgets are registered in two places:

1. **Client runtime** — `+page.svelte` uses `import.meta.glob("./widgets/*/*.svelte")` to build the widget registry for rendering and lazy loading. The registry maps the component filename → package folder for dynamic imports.
...
The **component filename** (not the folder) is the registry key persisted in `system-preferences`, so saved layouts survive package upgrades and renames. The `widget.json` manifest drives the marketplace catalog and telemetry via `manifest-registry.ts`. See [Dashboard Widget Development](../../development/dashboard/index.mdx).
