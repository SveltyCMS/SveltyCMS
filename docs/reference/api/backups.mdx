...
> [!IMPORTANT]
> All backup endpoints are **admin-gated**. GET requests require `backup:read`, POST requests require `backup:create`. Unmapped namespaces fail-closed via the dispatcher's `ENDPOINT_PERMISSIONS` mapping.
...
| **Create Backup**       | `/api/backups/create`       | `POST` | `backup:create` |
| **Validate Backup**     | `/api/backups/validate`     | `POST` | `backup:create` |
| **Create Restore Plan** | `/api/backups/restore-plan` | `POST` | `backup:create` |
| **Execute Restore**     | `/api/backups/restore`      | `POST` | `backup:create` |
| **Job Status**          | `/api/backups/jobs/:jobId`  | `GET`  | `backup:read`   |

> [!NOTE]
> Permission ids in this table are the **namespaced API route** vocabulary, authorized fail-closed against `ENDPOINT_PERMISSIONS` in `src/routes/api/[...path]/+server.ts` (the `backups` entry maps `POST` ⇒ `backup:create`, `GET`/`OPTIONS` ⇒ `backup:read`). Admin UI tiles use the separate `manage:*` vocabulary (`CONFIG_TILE_PERMISSIONS` in `src/routes/(app)/config/+page.server.ts`) — the two are not interchangeable.
