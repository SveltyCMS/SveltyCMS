...
> [!IMPORTANT]
> All content transfer endpoints are **admin-gated**. GET requests require `content:read`; POST requests require `content:export` under `/api/content-export/*` and `content:import` under `/api/content-import/*`. Unmapped namespaces fail-closed via the dispatcher's `ENDPOINT_PERMISSIONS` mapping.
...
| :-------------------- | :------------------------------------ | :----- | :--------------- |
| **Validate Export**   | `/api/content-export/validate`        | `POST` | `content:export` |
| **Plan Export**       | `/api/content-export/plan`            | `POST` | `content:export` |
| **Run Export**        | `/api/content-export/run`             | `POST` | `content:export` |
