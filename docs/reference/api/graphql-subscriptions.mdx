...
> **Migration Guide:** For standard GraphQL queries and mutations (REST/HTTP), see the [GraphQL API Reference](./graphql.mdx). The WebSocket subscription endpoint has moved from `ws://[domain]/api/graphql` (legacy) to `ws://[domain]/live` (new via `svelte-realtime`).
