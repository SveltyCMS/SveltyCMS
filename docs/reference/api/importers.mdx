...
> [!IMPORTANT]
> All importer endpoints are **admin-gated**. GET requests require `content:read`, POST requests require `content:import`. Unmapped namespaces fail-closed via the dispatcher's `ENDPOINT_PERMISSIONS` mapping.
...
> [!NOTE]
> The SSE endpoint at `/api/migration/import` is the recommended path for full-scale CMS migrations. It combines format detection, preview, and execution into one real-time stream with granular progress events. CLI commands are also supported via `bun run migrate import --file=<export> --format=<source> --collection=<target>`.

---

## Next Steps

- [Configuration Promotion](./configuration-promotion.mdx) — Promote site schemas and settings between environments (Schema as Code).
