---
path: "docs/reference/api/setup.mdx"
...
| **Reinitialize**    | `/api/setup/reinitialize` | `POST` | Administrator session |

> [!NOTE]
> `POST /api/setup/reinitialize` is **not a grantable permission** — it is recovery-only and requires an authenticated administrator session (`SETUP_REINIT_FORBIDDEN`, `src/routes/api/[...path]/handlers/setup.ts`). Permission ids elsewhere in the API are authorized fail-closed against `ENDPOINT_PERMISSIONS`; admin UI tiles use the separate `manage:*` vocabulary (`CONFIG_TILE_PERMISSIONS` in `src/routes/(app)/config/+page.server.ts`) — the two are not interchangeable.

> **Security Gating**: All endpoints except `status` are protected by `isSetupComplete()`. After setup finishes, `/api/setup/test-db`, `/api/setup/seed-db` and `/api/setup/complete` all return `403 SETUP_ALREADY_COMPLETE`; `/api/setup/reinitialize` returns `403 SETUP_REINIT_FORBIDDEN` unless the caller is a real administrator session. This is enforced at two levels: the `handleSetupRoutes` handler entry check, and the `handle-system-state.ts` middleware. See [Server Hooks](../architecture/server-hooks.mdx).
