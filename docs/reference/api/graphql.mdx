...
Clients can send a SHA-256 digest instead of the full query document via `extensions.persistedQuery.sha256Hash`, following the standard APQ handshake:

- **Miss**: only the hash is sent and nothing is registered under it — the gateway answers with a `PERSISTED_QUERY_NOT_FOUND` GraphQL error.
- **Register**: the client repeats the request with both `query` and `extensions.persistedQuery.sha256Hash`; the gateway verifies the pair, stores the query per tenant in the L1 RAM / L2 Redis store (`apq:{hash}`, 7-day TTL, tagged `graphql`/`apq`), then executes it.
- **Hit**: later requests send the hash alone; the stored document is resolved and executed.

The gateway also accepts `extensions.persistedQuery.hash`, a top-level `hash` field, and the GET `extensions` / `hash` / `sha256Hash` parameters.

**Hash binding**: an APQ key is accepted only when it is **exactly 64 hex characters**, and when a `query` accompanies it, `sha256(query)` must equal that key under a constant-time comparison (`timingSafeEqual`) **before** anything is stored. A malformed key fails with `PERSISTED_QUERY_HASH_INVALID`; a non-matching digest fails with `PERSISTED_QUERY_HASH_MISMATCH`; neither response reads from or writes to the APQ store. This closes cache poisoning (a client registering a query under a hash another client will later request) and unbounded persisted-query growth from arbitrary client-chosen keys.

Implementation: `src/routes/api/graphql/+server.ts` (`APQ_HASH_PATTERN`, `apqHashMatchesQuery`). Regression suite: `tests/unit/api/graphql-apq.test.ts`. See also [API Security §3.2 GraphQL Authentication](../security/api-security.mdx#32-graphql-authentication).
...
- **CSRF (parity with REST, 2026-09-24)**: every authenticated `POST /api/graphql` must pass the same-origin fast path or the double-submit token (`X-CSRF-Token` + `csrf_token` cookie, constant-time, rotated on use) — Yoga batching means a POST can hide mutations, so the gate covers every POST rather than only parsed mutation operations. Exempt: `GET` (safe method, queries only), server-to-server API-key/token callers (no cookie an attacker can ride), and `TEST_MODE`. Cross-origin browser clients should use API keys for mutations or send the token; same-origin clients are unaffected.
