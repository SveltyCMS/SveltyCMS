---
path: "docs/reference/components/entrylist-multibutton.mdx"
...
The `entry-list-multi-button` is an enterprise-grade component for managing bulk actions on collection entries with advanced features like action queuing, optimistic UI, keyboard navigation, and selection export.
...
Bulk delete, publish, draft, and clone call `POST /api/collections/:id/batch` with `{ action, entryIds }`. That route is handled as a batch action — it must not fall through to create.
...
| `Alt+D`   | Move to draft                     | ✅                 |
| `Alt+E`   | Bulk edit selected                | ✅                 |
| `Alt+Del` | Delete selected                   | ✅                 |
| `Alt+X`   | Export filtered selection or data | ❌                 |
...
- [Collection Store Dataflow](/docs/reference/architecture/collection-store-dataflow.mdx)
- [Hover Preloading](/docs/reference/architecture/hover-preloading.mdx)
