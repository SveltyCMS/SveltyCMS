...
| **Sortable columns**     | Header click toggles ASC/DESC; `sort`/`order` URL params restored after SSR                       |
| **Compact filter row**   | In-header controls stay inside column cells (no overlapping floating labels)                      |
...
| **Bulk actions**         | `POST /api/collections/:id/batch` (`delete`, `status`, `clone`) via `entry-list-multi-button`     |
