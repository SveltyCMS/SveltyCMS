---
path: "docs/guides/configuration/email-system.mdx"
...
SveltyCMS uses `@better-svelte-email/server` as its email rendering engine, which processes Tailwind CSS v4 classes at runtime. This means you can use classes like `bg-blue-500` or `p-4` in your Svelte files, and they will be converted to inline `style="background-color: #3b82f6; padding: 1rem;"` attributes in the generated email HTML.

### 2. Zero-Configuration Preview (Dev Mode)

When running in development mode or when SMTP is not configured, SveltyCMS skips
actual SMTP delivery. The rendered email HTML is logged to the server console and,
for critical flows like password reset, an **inline reset form** appears
immediately in the browser so the user can proceed without waiting for an email.

The reset link is also printed to the server terminal so an admin can share it
manually if needed.
...
- **[Automation System](../../development/automation-system.mdx)**: Learn how to trigger emails based on CMS events.
- [Webhook Integrations](../../reference/api/system.mdx#3-webhooks-integrations): For outbound events to third-party services.
- **[System Settings](../../reference/api/system.mdx)**: Manage SMTP credentials.
