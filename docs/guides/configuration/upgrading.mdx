...
> [!IMPORTANT]
> **One schema path, no legacy tails (as of September 2026).** The SQL system schema is declared once in `src/databases/system-schema-spec.ts` and applied at boot from that spec only — releases no longer carry per-engine repair tails (`ADD COLUMN IF NOT EXISTS` lists, the v0.0.8-era `security` → `password` / `redirects_mv.from|to` renames, or the MariaDB `mfaVerifiedAt` widening). Fresh databases are provisioned from the spec (measured: 27 tables + 117 indexes, ~330 ms once) and an unchanged spec skips the DDL pass entirely on every later boot (~2 ms).
>
> **Consequence for older installations:** a database created by an earlier release that still needs one of those one-off repairs is _not_ migrated automatically. Upgrade through the release that introduced the change (or export → re-provision → import) if your database predates the current column set; the spec always describes the schema the code expects.
>
...
8. **Codemods**: Automatically executes scripts in `scripts/codemods/`. Scripts starting with an underscore (e.g., `_utils.ts`) are ignored by the runner.
9. **Database & Tests**: Runs `db:push` and `test:unit` to ensure everything is working correctly.
