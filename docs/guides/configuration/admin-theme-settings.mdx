...
description: "Customize the admin panel with multiple themes, density presets, visual styles, custom CSS, and live preview — inspired by Drupal Gin."
order: 7
...
SveltyCMS's admin theme system lets you create, switch, and customize **multiple named themes** — like Drupal's Gin or DXPR themes — each with its own density, visual style, features, and custom CSS. Changes preview instantly before saving.
...
The **Visual Style** tab controls card borders and shadows — directly inspired by Drupal Gin's structural theming.
...
Fresh installs seed the active theme with the **Corporate** workspace from `/themes/corporate.json`:
...
Skeleton's theme generator exports JSON with a `properties` map (`--color-primary-500`, `--color-surface-950`, etc.). SveltyCMS also accepts a **shorthand palette** format in `/themes/*.json` for quick branding:
...
Drop a `.json` file into the `/themes/` directory and Vite auto-imports it to the database on next save. This is the **File + DB hybrid** — files are version-controlled distribution artifacts, DB is the runtime source of truth.
...
**Example — Corporate (default on install)** (`/themes/corporate.json`):

```
{
  "name": "Corporate",
  "description": "Gin-inspired enterprise default — cozy density, bordered cards.",
  "density": "cozy",
  "variant": "bordered",
  "features": {
...
**Example — Developer Workbench** (`/themes/developer-workbench.json`):
...
**Example — Distraction-Free Writer** (`/themes/distraction-free-writer.json`):
...
| :---------------- | :------------- | :--------------- | :------------------------------------------------------- |
| **Top Toolbar**   | `header`       | `global-toolbar` | Theme-injectable global toolbar (Gin Toolbar equivalent) |
...
This architecture matches or exceeds Drupal Gin's toolbar, sidebar, and layout builder modules — all without PHP/Twig overhead, using native Svelte 5 reactivity.
...
Drop a `.json` file into `/themes/` and Vite auto-imports it:
...
| ------------------- | ------------------- | ----------------------------------------------------- |
| `global-toolbar`    | Top header          | Global navigation toolbar (Gin Toolbar equivalent)    |
