...
<Callout type="success">
  **Zero-Restart Architecture**: SveltyCMS uses dynamic in-memory configuration loading
  (`initializeWithConfig()`). While Vite may perform a hot-restart in development when `private.ts`
  is written, the production system transitions from `IDLE` → `READY` state instantly without
  process recycling, ensuring zero downtime.
</Callout>
