...
<Callout type="info">
  **Zero-I/O Optimization**: By injecting the setup status via Vite's `define` constant, we eliminate the need to check for `private.ts` on every request. This reduces I/O pressure on the filesystem, especially critical for high-concurrency environments.
</Callout>
...
<Callout type="success">
  **Zero-Restart Architecture**: SveltyCMS uses dynamic in-memory configuration loading (`initializeWithConfig()`). 
  While Vite may perform a hot-restart in development when `private.ts` is written, the production system transitions 
  from `IDLE` → `READY` state instantly without process recycling, ensuring zero downtime.
</Callout>
