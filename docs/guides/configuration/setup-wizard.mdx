...
- **PII Masking**: All logs during setup (account creation, DB connection) are automatically redacted to prevent leaking emails or passwords in terminal outputs.
- **Secret Redaction**: The configuration loader (`config-state.ts`) prevents character-for-character printing of `private.ts` to any log stream.
- **Write-Mutex Protection**: SQLite writes during the high-pressure setup phase are serialized via a global mutex to prevent "Database is Locked" errors.
...
The following test (`tests/e2e/setup-wizard.spec.ts`) demonstrates how to automate the multi-step configuration, including handling Skeleton v4's modal portals and cookie consent banners:
...
test("Setup Wizard: Configure DB and Create Admin", async ({ page }) => {
  ...
  // Handle "Welcome" popup (Skeleton v4 portal detection)
...
- [Setup Reference](/docs/api/setup.mdx) - Complete API documentation
