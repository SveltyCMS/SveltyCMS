---
path: 'docs/guides/enterprise-media-gallery-implementation'
title: 'Enterprise Media Gallery Implementation Guide'
description: 'Enterprise-grade features for SveltyCMS Media Gallery handling 10,000+ files with optimal performance'
order: 8
...
# Enterprise Media Gallery Implementation Guide
...
**Component**: `DuplicatesModal.svelte`  
**API Endpoints**:

- `GET /api/media/duplicates` - Find all duplicates
- `POST /api/media/duplicates` - Merge duplicates  
  **Utility**: `duplicateDetection.ts`
...
- Visual duplicate grouping with thumbnails
- Storage savings calculator
- Primary file selection (radio buttons)
- Batch merge operations
- Reference updates (all links point to primary file)
- Automatic cleanup of merged files
...
```typescript
{
  "duplicates": [
    {
      "hash": "a1b2c3d4...",
      "files": [MediaBase, MediaBase, MediaBase],
      "count": 3
    }
  ],
  "savings": {
    "totalSavings": 15728640, // bytes
    "percentage": 12.5
  }
}
```
...
**Location**: MediaUpload widget & Gallery upload process
...
**Version**: 1.1.0  
**Status**: Refactored - Hash-based duplicate prevention (removed post-upload detection)
