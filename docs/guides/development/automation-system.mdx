...
SveltyCMS includes a visual **Automation Builder** for creating event-driven workflows — no custom code required. Automations extend the existing [Webhook System](/docs/api/system.mdx) by adding multiple operation types, conditional logic, and a step-by-step editor GUI.
