...
SveltyCMS 2026 integrates with a centralized **Model Context Protocol (MCP)** server to empower the built-in AI Assistant with deep project knowledge.

## Overview

The system uses a **Remote Retrieval-Augmented Generation (RAG)** pipeline. Instead of hosting a heavy vector database inside the CMS repository, the CMS queries a centralized Knowledge Core hosted at `mcp.sveltycms.com`.
...
## Future: Community Bot

The same `mcp.sveltycms.com` server powers the SveltyCMS Discord bot, ensuring that answers given in the community chat match the documentation accessible inside the CMS.
