...
> [!TIP]
> **New**: See [Enterprise Scaling Layers](./scaling-layers.mdx) for the complete composable scaling story — PgBouncer connection pooling, optional Redis, pre-compressed cache HITs (Brotli/zstd), CDN purge, and deployment diagrams. Start simple (single-node SQLite) and add layers as you grow.
