---
path: 'docs/guides/content/per-locale-publishing.mdx'
title: 'Per-Locale Publication Status'
description: 'Guide on independent publication state management for multilingual content.'
order: 10
icon: 'mdi:earth'
author: 'admin'
created: '2026-01-18'
...
```mermaid
graph TD
    User([User Action]) --> Toggle[Toggle Publication Status]
    Toggle --> CheckEnabled{Per-Locale Enabled?}

    CheckEnabled -- No --> Global[Update global 'status' field]
    Global --> Sync[Save to Database]

    CheckEnabled -- Yes --> LocaleAware[Update language-specific status]
    LocaleAware --> StatusByLocale[Update 'statusByLocale' record]
    StatusByLocale --> Sync

    Sync --> UI[Refresh UI with updated state]
```
...
**In Collection Schema** (`config/collections/.../YourCollection.ts`):
