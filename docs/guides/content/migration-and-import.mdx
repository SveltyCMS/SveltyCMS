...
This is the operator task: move content in, out, or between installs. The service layout behind those screens is [Data operations](../../reference/architecture/data-operations.mdx). Endpoint contracts are the [API reference](../../reference/api/index.mdx).
...
| Import a WooCommerce / Shopify catalog         | **Smart Importer**   | `--format=woocommerce` or `shopify` → `products` |
...
> [!NOTE]
> The Smart Importer is a marketplace plugin. See the [plugin documentation](/src/plugins/smart-importer/smart-importer.mdx) for installation and platform-specific guides (WordPress, Drupal). WooCommerce and Shopify catalogs import into the ecommerce **preset** `products` collection — see [E-Commerce Overview](./ecommerce.mdx). After import, the **Commerce Inventory** dashboard widget (14-day trial) shows low-stock SKUs.
...
> [!IMPORTANT]
> **Configuration Promotion moves structure (schemas, settings, roles), not content.** Use [Content Transfer](#2-exporting-importing-content) for editorial content packages, or [Content Sync](#5-content-sync-cross-environment) for real-time channel synchronization.

> **API reference**: [Configuration Promotion](/docs/reference/api/configuration-promotion) — full endpoint documentation with resource types, drift detection, safety modes, and audit logging.
