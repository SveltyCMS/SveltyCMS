...
<Callout type="info">
  The `globalSettings.ts` store is a **server-side read cache**. The admin UI **writes directly** to the database via the API and then invalidates this cache.
</Callout>
...
<Callout type="warning">
  **Security:** The export process must be configured to **exclude** secrets like `SMTP_PASSWORD`. The `_requiredSettings` property on config entities helps identify which settings are environment-specific secrets and should not be exported.
</Callout>
