...
The system uses a Vite plugin (`src/content/vite.ts`) that watches for changes. It first synchronizes the database with `dbAdapter.collection.createModel()`, then regenerates types:
