...
```typescript
import { waitForSystemReady, waitForServiceHealthy } from '@stores/systemState';

// Wait for overall system (intelligent timeout)
try {
...
try {
	await waitForServiceHealthy('database', 5000); // Optional custom timeout
...
```typescript
import { updateServiceHealth, startServiceInitialization } from '@stores/systemState';
...
```typescript
import { setSystemState } from '@stores/systemState';
...
| File                                                           | Purpose                                    |
| -------------------------------------------------------------- | ------------------------------------------ |
| `src/stores/systemState.ts`                                    | State machine implementation (1058 lines)  |
