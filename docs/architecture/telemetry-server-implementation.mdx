...
```json
{
	...
	"installation_id": "8f9d...", // SHA-256 Hash of JWT Secret
	"db_type": "mongodb", // Database type
	"country": "DE", // Country code (resolved client-side)
	"widgets": "[\"weather-widget\"]" // Custom widgets only
}
```
...
**Rate Limit (429):**
If the server returns 429, the client will interpret this as a "soft backoff" and sleep for 12 hours, logging only a warning.
