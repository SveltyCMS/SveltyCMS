---
path: /docs/architecture/global-loading-store
...
1. **`loadingStore.svelte.ts`** - Core state manager with reactive store
2. **`GlobalLoading.svelte`** - UI component displaying full-screen loading overlay
...
globalLoadingStore.startLoading(
	...
	'AdminArea.fetchData', // Optional context for debugging
...
## GlobalLoading UI Component

The `GlobalLoading.svelte` component displays operation-specific messages:
...
### Role Management (AdminRole.svelte)
...
### Token Generation (WebsiteTokens.svelte)
...
```typescript
// ✅ Correct
globalLoadingStore.withLoading(
	asyncFn,
	loadingOperations.dataFetch,
