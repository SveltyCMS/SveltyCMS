...
Connection-aware preloading via `src/utils/connection-detector.ts` — detects slow-2g/2g/3g/4g, Save-Data mode, RTT latency, and online status. Also provides `shouldDisableRealtime()`, `shouldReduceMotion()`, `getRecommendedImageQuality()`, `getRecommendedBatchSize()`.
...
### Client-Side Navigation (Standardized)

SveltyCMS mandates the **"Anchor-First"** pattern for all internal navigation to ensure consistent sub-50ms perceived latency.

#### 1. Core Collection Lists

Recommended for table rows and entry lists:
...
#### 2. Dashboard Widgets (Speculative)

For "At-a-glance" productivity, dashboard widgets use semantic links to prefetch content before the user clicks:

```
<!-- last5-content-widget.svelte -->
<li role="listitem">
    <a href="/en/posts?edit=123" data-sveltekit-preload-data="hover">
        <!-- Content preview -->
    </a>
</li>
```
...
> [!IMPORTANT]
> **Accessibility (A11y)**: When using anchors within a list, always wrap the `<a>` tag inside an `<li>` element. Avoid applying `role="listitem"` directly to the anchor, as it conflicts with its interactive nature.

#### 3. Configuration & Builders

Even complex builders like the **Collection Builder** utilize anchor preloading for editing schemas:

```
<!-- tree-view-node.svelte -->
<a
...
- Edit mode load: **< 50ms (Perceived)**

### Best Practices

### Do's ✅

- ✅ **Standardize on `<a>` tags** with `data-sveltekit-preload-data="hover"`.
- ✅ **Wrap anchors in `<li>`** when building list-based widgets.
...
- ❌ **Avoid `goto()` for primary navigation** (it bypasses speculative preloading).
- ❌ **Don't apply `role="listitem"` to `<a>` tags** (use proper `<li>` nesting).
...
- [URL Pattern Implementation](./url-pattern.mdx): Edit URL structure
