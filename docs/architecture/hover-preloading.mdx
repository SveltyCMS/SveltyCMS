...
When user clicks after preload:

```typescript
// EntryList.svelte - Click handler
const newUrl = new URL(page.url);
newUrl.searchParams.set('edit', originalEntry._id);
goto(newUrl, { keepFocus: true });

// Data already preloaded → instant page transition!
```
...
- ✅ Use intent detection (hover delay)
- ✅ Deduplicate preload requests
- ✅ Clean up stale cache entries
- ✅ Mark preload requests with header (`X-Preload: true`)
- ✅ Leverage SvelteKit's fetch caching
- ✅ Monitor server load impact

### Don'ts ❌

- ❌ Preload on short hovers (< 300ms)
