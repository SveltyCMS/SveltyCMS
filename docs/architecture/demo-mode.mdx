...
1.  **Creation**:
    ...
    - **Method B (With Link)**: User visits site with `demo_tenant_id` cookie.
      - `handleAuthentication` hook detects missing `demo_tenant_id` cookie.
      - Calls `TenantService.create({ plan: 'free', status: 'active' })` to generate a new Tenant.
      - Sets `demo_tenant_id` cookie with the new `_id`.
