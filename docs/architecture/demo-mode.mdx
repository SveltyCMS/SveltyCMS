...
Demo mode is controlled via the `DEMO` setting in your configuration.

1.  **Production**: Edit `config/private.ts` (or `config/private.js` in build):
    ```typescript
    ...
    ```
2.  **Development**: You can also use the environment variable `SVELTYCMS_DEMO=true` or run `bun run dev:demo`.
...
3.  **Expiration & Cleanup**:
    ...
    - It deletes all associated data (collections, users, settings, media) using the `cleanupExpiredDemoTenants` service.

### Seeding Strategy

The `seedDemoTenant` function (`src/routes/api/setup/seed.ts`) performs:
