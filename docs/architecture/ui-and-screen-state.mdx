...
1.  **`screen`** (`screenSizeStore.svelte.ts`): Manages responsive breakpoints, window dimensions, and accessibility preferences.
2.  **`ui`** (`UIStore.svelte.ts`): Manages the visibility of layout components, sidebars, and route-specific UI contexts.
