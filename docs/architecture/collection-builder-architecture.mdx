...
graph TD
    ...
    subgraph Client_UI ["Client UI (Collection Builder)"]
        UI_Store["CollectionStore (Flat List)"]
...
Located at `src/content/ContentManager.ts`, this singleton is responsible for bridging the gap between static files and dynamic database state.
