...
#### Collaborative & Multilingual Data Flow

Widgets in SveltyCMS must support both multilingual contexts and real-time collaborative editing via Yjs.

```mermaid
sequenceDiagram
    participant User
    participant Input as Input Component
    participant YJS as Y.Doc (Shared State)
    participant Store as app Singleton
    participant DB as Database

    Note over User, YJS: Collaborative Sync
    User->>Input: Edit text/value
    Input->>YJS: Update Y.Map/Y.Text
    YJS-->>Input: Remote delta (User B)
    Input->>User: Render remote cursor & text

    Note over User, DB: Multilingual Flow
    User->>Store: Switch language (en → de)
    Store->>Input: Language change notification
    Input->>YJS: Switch active sub-map (de)
    YJS-->>Input: "Guten Tag"
    Input->>DB: Debounced Save (via fields.svelte)
```
...
### 🛡️ The 3-Pillar Performance Shield

Our 3-pillar architecture is designed to protect the CMS core from performance degradation caused by custom or third-party widgets:

- **UI Isolation**: The **Input** and **Display** pillars are strictly client-side. A heavy UI component may slow down the browser, but it cannot block the SvelteKit server or API.
- **Hook Profiling**: The **Definition** pillar (index.ts) contains all server-side logic (`modifyRequest`, `aggregations`). Because these are isolated into a standard interface, SveltyCMS can profile the execution time of each widget's hook.
- **Fail-Safe Dispatching**: If a custom widget's server-side logic exceeds a defined latency threshold (e.g., 50ms), the system can log a performance violation or trigger a "Fast-Path" fallback to protect the rest of the API request.
