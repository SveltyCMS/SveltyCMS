...
- `dbInterface.ts` - Database adapter interface definition (IDBAdapter)
...
<script lang="ts">
	...
	// Reactive language selection - uses contentLanguage for translated fields
...
// Initialize from cookies or environment defaults
let initialContentLanguage = getCookie('contentLanguage') ?? publicEnv.DEFAULT_CONTENT_LANGUAGE ?? 'en';

// Reactive state with Svelte 5 runes
let _contentLanguage = $state<Locale>(initialContentLanguage);

// Store with cookie persistence
export const contentLanguage = {
	...
	set(newValue: Locale) {
		...
		setCookie('contentLanguage', newValue);
		notifyContentLanguageSubscribers();
	},
	subscribe(fn: (value: Locale) => void) {
		contentLanguageSubscribers.add(fn);
		fn(_contentLanguage);
		return () => contentLanguageSubscribers.delete(fn);
	}
};
```
...
2. **URL Navigation**: Server returns `contentLanguage` → Syncs to store
3. **User Toggle**: TranslationStatus dropdown → Updates store → Navigates with `invalidateAll: true`
4. **Widget Display**: Widgets read `contentLanguage.value` reactively

```svelte
<!-- +page.svelte - Syncs server data to client store -->
<script lang="ts">
	import { contentLanguage } from '@stores/store.svelte';
	...
	// Sync store with server-loaded language
	$effect(() => {
...
1. **System Language** (`systemLanguage`) - Admin interface language (menus, buttons, labels)
2. **Content Language** (`contentLanguage`) - Language for viewing/editing content data
...
User Action → Store Update → Server Request → Cache Lookup → Widget Render
                    ↓              ↓               ↓              ↓
             contentLanguage   /:lang/:path   :lang:cache   value[lang]
```
...
2. **Switch to Secondary Language** (e.g., 'de')
   - TranslationStatus dropdown shows completion: "EN: 100% | DE: 0%"
...
│   │   │   ├── +page.svelte     # Collection view (EntryList/Fields)
