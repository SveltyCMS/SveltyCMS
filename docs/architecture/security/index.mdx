...
- ✅ **Defense-in-Depth Handler Checks (May 2026)**: System handlers (`handleSettingsRoutes`, `handleSystemMgmtRoutes`, `handleAutomationRoutes`) enforce admin verification. Media handlers enforce `media:write` and `media:delete` permissions. Page actions use centralized permission guards.
- ✅ **Cookie Prefix Hardening (May 2026)**: `__Host-` prefixed session cookies strictly enforced on secure connections; never leaked on insecure connections.
- ✅ **Setup Completion Gating (May 2026)**: All `/api/setup` endpoints blocked with 403 after initialization; bootstrap routes redirect to login.
