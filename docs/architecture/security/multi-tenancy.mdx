...
- **REST API Dispatcher**: All routes under `/api/[...path]` are centrally gated by a **Fail-Closed** security layer. It validates the required Permission ID against the user's `locals.roles` for the current tenant before any adapter logic is executed.
