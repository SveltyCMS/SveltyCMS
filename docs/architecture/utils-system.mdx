...
| File                         | Key exports                                                    |
| ---------------------------- | -------------------------------------------------------------- |
| `media-utils.ts`             | `getMimeType`, `mediaUrl`, `sanitizedFilename`, `SIZES`        |
| `media-models.ts`            | `MediaBase`, `MediaImage`, `MediaItem`, `MediaType`            |
| `media-service.server.ts`    | `MediaService`                                                 |
| `media-storage.server.ts`    | `saveFile`, `deleteFile`, `getImageSizes`, `saveResizedImages` |
| `cloud-storage.ts`           | `upload`, `download`, `getUrl`, `isCloud`, `getConfig`         |
| `sharing.ts`                 | `createLink`, `validateLink`, `revoke`, `extend`, `newToken`   |
| `bulk-download.ts`           | `createBulkArchive`, `streamArchive`, `cleanupArchive`         |
| `storage-analytics.ts`       | `analyze`, `insights`, `trends`, `quota`, `formatBytes`        |
| `version-history.ts`         | `compareVersions`, `getVersionStats`, `createVersion`          |
| `advanced-search.ts`         | `advancedSearch`, `getSuggestions`, `SearchCriteria`           |
| `streaming-upload.ts`        | `parseMultipartStream`                                         |
| `media-processing.server.ts` | `hashFileContent`, `MediaProcessingService`                    |
| `slim-sniffer.server.ts`     | `sniffMimeType`                                                |
| `webgpu-processor.ts`        | `optimizeImage`                                                |
