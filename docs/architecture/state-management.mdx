...
export const handleSystemState: Handle = async ({ event, resolve }) => {
	...
	// During INITIALIZING state, wait for initialization to complete
	if (systemState.overallState === 'INITIALIZING') {
		...
		// Wait for initialization to complete instead of blocking
		await dbInitPromise;
	...
	// If system is not ready (IDLE without setup), block with 503 error
	if (!isSystemReady()) {
