...
**Classification Overhead** (one-shot flag computation, no downstream work):

| Scenario                   | Overhead    | Optimization Strategy            |
| :------------------------- | :---------- | :------------------------------- |
| **Static Asset Fast-Path** | **0.02 µs** | Regex short-circuit (Early Exit) |
| **API Request Fast-Path**  | **0.05 µs** | Single-pass flag cache           |
...
| Scenario                          | Avg Latency | p95 Latency  | RPS   |
| :-------------------------------- | :---------- | :----------- | :---- |
| **Turbo Pipeline (Light)**        | 1.067 ms    | 2.111 ms     | 891   |
| **Full Security + Auth Pipeline** | 0.950 ms    | 1.625 ms     | 1,044 |
| **Mutation + Audit Logging**      | 0.831 ms    | 1.072 ms     | 1,098 |
| **Combined Middleware p95**       | —           | **1.214 ms** | —     |

> **Benchmark**: `bun test tests/benchmarks/hooks-performance.test.ts` — measures full HTTP round-trip through the middleware chain including Turbo, Security (firewall + rate limit), Auth, Authorization, and Audit logging. Raw DB adapter latency is ~0.089ms p95, so the total middleware tax is ~1.1ms.
...
- **Classification Efficiency**: One-shot flag computation saves ~5-15 regex/prefix evaluations per request across 15+ downstream hooks.
- **Full Pipeline Throughput**: The production middleware chain sustains **1,044 RPS** (Full Auth+Security) and **1,098 RPS** (Mutation+Audit) on SQLite, per the [Hooks Performance Benchmark](../project/benchmarks/benchmark_sqlite.mdx#hooks).
- **Middleware Tax**: Total middleware overhead is ~1.1ms on top of raw DB adapter latency (~0.089ms p95), well within the <2ms enterprise target.
