...
handleTestIsolation, // 3. Test context & worker isolation
handleCompression, // 4. Streaming-safe compression
handleSecurity, // 5. Unified firewall & rate limiting (protects state)
handleSystemState, // 6. Readiness gate & auto-init
handleSetup, // 7. Setup gate
handleUserPreferences, // 8. i18n and SSR theme (combined)
...
handleAuditLogging, // 13. async audit trails (must catch cache hits)
...
**Purpose**: Validates system readiness and authorized host context before processing requests.
...
export const handleSystemState: Handle = async ({ event, resolve }) => {
  ...
  // 1. Initial boot trigger
  if (systemState.overallState === "IDLE" && isSetupComplete()) {
    await waitForInitialization(); // Consolidated promise waiter
  }

  // 2. Host Trust Validation (SSRF Protection)
  if (isBootstrapRoute(pathname) && !isTrustedHost(event)) {
     throw new AppError("Access from untrusted host blocked", 403, "UNTRUSTED_HOST");
  }

  // 3. Readiness Blocking
  const restricted = ["IDLE", "INITIALIZING", "SETUP", "MAINTENANCE", "FAILED"];
  if (restricted.includes(systemState.overallState) && !isBootstrapRoute(pathname)) {
    throw error(503, `System is currenty in ${systemState.overallState} mode.`);
  }
...
- **Phase-Based Gating**: Efficiently distinguishes between high-performance static assets, setup routes, and standard content requests.
- **Trusted Host Check**: Dynamic origin validation for bootstrap flows (Setup, Login) to prevent environmental exposure.
- **Shared Promise Waiter**: Thundering herd protection ensures that concurrent requests during startup all wait for the single `dbInitPromise`.
- **Degraded Enforcement**: Transparently alerts the frontend via `locals.degradedServices` if non-critical dependencies are failing.
...
### 10. handleApiRequests - Modular Dispatcher & Caching
