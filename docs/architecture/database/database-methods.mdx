...
- **MongoDB**: `src/databases/mongodb/` (flattened structure)
- **SQL (Drizzle)**: `src/databases/agnostic/` (base modules) and engine folders (`sqlite/`, `postgresql/`, `mariadb/`).
...
## 💎 Platinum Performance Tiers (2026)

### 1. `bypassSafeQuery` Hook

**Purpose:** Bypass security middleware for internal, trusted lookups.

- **Usage:** Set `bypassSafeQuery: true` in the options object.
- **Impact:** Eliminates ~0.5ms of overhead per request by skipping tenant injection and soft-delete filtering.
- **Safety:** Automatically restricted to internal SDK calls.

### 2. `isLookupQuery` Fast-Path

**Purpose:** Skip Query IR translation for simple lookups.

- **Detection:** Automatically identifies queries targeting only `_id` or `token`.
- **Impact:** Bypasses the entire intermediate representation layer, calling the driver directly.

### 3. `hasChanges` Zero-Allocation

**Purpose:** Prevent unnecessary object cloning.

- **Pattern:** Utility functions (`processDates`) check for modifications before returning a new object.
- **Impact:** Significant reduction in memory allocation and GC pressure.
```
...
Each adapter organizes its logic into specialized modules:

| MongoDB Modular Adapter | SQL (Drizzle) Equivalent        | Responsibility                    |
| :---------------------- | :------------------------------ | :-------------------------------- |
| **AuthModule**          | `modules/auth/authModule.ts`    | Authentication & User management  |
| **CollectionModule**    | `collection/collectionModule`   | Dynamic schema/model management   |
| **CrudModule**          | `crud/crudModule.ts`            | Generic CRUD operations           |
| **ContentModule**       | `modules/content/contentModule` | CMS-specific content workflows    |
| **MediaModule**         | `modules/media/mediaModule`     | Media file operations             |
| **SystemModule**        | `modules/system/` modules       | System preferences & settings     |
| **MonitoringModule**    | `performance/` modules          | Health, metrics & **Versioning**  |
| **TransactionModule**   | `operations/transaction-module` | ACID compliance & native sessions |
