...
- **Security Fast-Path (`bypassSafeQuery`)**: High-performance toggle for internal system lookups, bypassing redundant tenant/security checks.
...
3. **Security Injection**: Automatically injects `tenantId` and `isDeleted: false` filters.
