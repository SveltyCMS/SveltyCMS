...
- **ID Lookup Fast-Path (`isLookupQuery`)**: Direct collection access for ID/token queries, bypassing the Query IR translation pipeline entirely.

---

## 🛡️ Agnostic Query IR Translation

To maintain strict parity with SQL engines, the MongoDB adapter no longer interacts with raw MongoDB filter objects in the domain modules. Instead, it utilizes the **Agnostic Query IR**.
...
| **adapter-core.ts**    | Unified connection and Query IR management.             | Hardened `isLookupQuery` ID-bypass.                  |
