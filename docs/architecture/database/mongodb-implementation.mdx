...
- **Domain-Specific Modules**: Refactored monolithic adapter into specialized modules (`AuthModule`, `CrudModule`, `MediaModule`, `SystemModule`) for improved maintainability and telemetry.
- **Mongoose 9.5.0 Integration**: Bleeding-edge support for MongoDB 8.0/9.0 engine features, including optimized `.lean()` execution and enhanced wire compression (Zstd).
- **Explicit Schema Registration**: Forced string-based `_id` schemas across all system collections to ensure 100% parity with SQL engines and prevent "Cast to ObjectId" errors.
