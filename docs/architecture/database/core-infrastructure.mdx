...
The `core` directory is not a centralized security interceptor — tenant isolation, query sanitization, and permission enforcement live inside each adapter's implementation (drizzle helpers, mongo crud-methods), the `forTenant()` wrapper, and higher security layers (safe-query, content engine, API dispatcher).
...
### Key Responsibilities {#theme-manager-responsibilities}
...
### Usage Examples {#theme-manager-usage}
