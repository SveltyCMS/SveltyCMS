...
- `src/databases/cache/cache-service.ts` (Redis connection)
...
| Per-DB Connection Pool Defaults                   | ✅ Implemented | `config-state.ts` — PG:20, SQLite:1, MariaDB/Mongo:100                            |
| Detailed error logs download                      | ✅ Implemented | API: `/api/logs/download` (admin-only)                                            |

All features above are production-ready. The "Future Roadmap" section later in this document lists not-yet-implemented items (adaptive backoff, distributed coordination, advanced metrics).
...
These items are on the roadmap for future releases:
...
- **Recovery Policy**:
  - After **60 seconds** in `OPEN` state, the breaker automatically moves to `HALF_OPEN`.
    It allows **one** probe request: if successful, it moves to `CLOSED`; if it fails, it returns to `OPEN` for another 60s.
- **Config**:
...
- Redis connection (`src/databases/cache/cache-service.ts`)
...
- **Security**: Sensitive configuration keys (passwords, secrets) are automatically redacted using the `redactConfig` helper before logs are stored or exported.
