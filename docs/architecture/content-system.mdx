...
The SveltyCMS Content System is a high-performance, reactive engine. As of June 2026 the server tier is consolidated into **two modules** — `engine.server.ts` (scan, reconcile, cache, watcher, CRUD) and `loader.server.ts` (path security, native/pooled import, worker pool) — down from 16 files.
