...
4. **Update language switcher UI:**
   ```svelte
   <select bind:value={systemLanguage.value}>
...
```svelte
<!-- TranslationStatus.svelte - Recommended: Anchor tag dropdown -->
<script lang="ts">
	import { contentLanguage } from '@stores/store.svelte';

	function handleLanguageChange(newLang: string) {
		// Update store (syncs with server on navigation)
...
{#each AVAILABLE_CONTENT_LANGUAGES as lang}
	<a
		...
		class:active={lang === contentLanguage.value}>
		{lang.toUpperCase()}
	</a>
{/each}
...
<select onchange={(e) => switchLanguage(e.currentTarget.value)}>
	{#each AVAILABLE_CONTENT_LANGUAGES as lang}
		<option value={lang} selected={lang === contentLanguage.value}>
			{lang.toUpperCase()}
		</option>
	{/each}
</select>
...
**Step 3: Update TranslationStatus Component**
...
The `TranslationStatus` component shows completion percentage:

```svelte
<TranslationStatus entry={currentEntry} availableLanguages={['en', 'de', 'fr', 'es']} currentLanguage={contentLanguage.value} />
...
const title = value[contentLanguage.value]; // ✅ Dynamic
```
...
<button>{contentLanguage.value === 'en' ? 'Save' : 'Speichern'}</button>
