...
```
L0: Mtime Tree + _schemaCache (content-service.server.ts) → skip fs/import when unchanged
...
**Helpers**: `src/content/content-cache.server.ts` (`setSchemaCacheEntry`, `invalidateSchemaCache`, `invalidateNavigationCache`, `notifyContentUpdate`).
...
**Last Updated**: 2026-06-15 (corrected L1/L2 architecture, added stampede protection, single-flight, negative cache, SWR documentation)
