...
sequenceDiagram
    ...
    Setup->>Cache: Warm cache: fetch(baseUrl)
    Note over Cache,SSR: Cache warming happens<br/>BEFORE redirect

    Cache->>SSR: GET / (homepage)
    SSR->>SSR: Server-side render
    SSR->>DB: Load collections, menu, settings
    SSR-->>Cache: Cached HTML response
