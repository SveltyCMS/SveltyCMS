...
NODE_OPTIONS="--max-old-space-size=4096" bun x nx run cms:build
```
...
{
"scripts": {
"build:cms": "cross-env NODE_OPTIONS='--max-old-space-size=4096' nx run cms:build --configuration=production",
"build:all": "cross-env NODE_OPTIONS='--max-old-space-size=4096' nx run-many --target=build --all --configuration=production"
}
}
