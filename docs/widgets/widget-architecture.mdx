...
<script lang="ts">
	...
	const displayValue = value[contentLanguage.value]; // Fails for untranslated fields
</script>
...
<script lang="ts">
	...
	const lang = $derived(field.translated ? contentLanguage.value : 'en');
...
4. **Provide fallbacks for missing translations:**
   ...
   ```typescript
   const text = value[contentLanguage.value] || value.en || value[DEFAULT_CONTENT_LANGUAGE] || '';
   ```
...
2. **Don't assume all fields are translated:**
   ...
   ```typescript
   const value = data[contentLanguage.value]; // ❌ Breaks for untranslated fields
   ```
...
<script>
	...
	$: widgetFn = $widgetFunctions[field.widget];
</script>
