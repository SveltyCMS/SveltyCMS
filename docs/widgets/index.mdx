...
### 5. [Marketplace Readiness](/docs/widgets/Marketplace_Readiness_Guide.mdx) 🚀

**Essential checklist** for publishing widgets.
...
**Read this when:** You are ready to package and share your widget.

### 6. [Multilingual Widget Guide](/docs/widgets/widget-multilingual-guide.mdx)
...
| Install widgets at runtime     | [Runtime Discovery](/docs/widgets/runtime-widget-discovery.mdx)   | Dynamic Loading        |
...
- **[Runtime Widget Discovery](/docs/widgets/runtime-widget-discovery.mdx)** - Zero-downtime widget installation
...
// Initialize widgets (tenant-aware)
await widgets.initialize('tenant-id');

// Check status & metadata
const isActive = widgets.active.includes('SEO');
const canDisable = widgets.canDisable('SEO');
const deps = widgets.getDependencies('SEO');
const isLoaded = widgets.isLoaded;

// Update status
await widgets.updateStatus('SEO', 'active', 'tenant-id');
```
