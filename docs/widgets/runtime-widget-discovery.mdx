...
```typescript
async updateStatus(widgetName: string, status: WidgetStatus, tenantId?: string): Promise<void> {
  // ... validation logic ...
  ...
  try {
    ...
    if (status === 'active') {
       this.#activeWidgets.add(widgetName);
    } else {
       this.#activeWidgets.delete(widgetName);
    }
...
```typescript
async initialize(tenantId?: string, dbAdapter?: DatabaseAdapter | null): Promise<void> {
  // ... existing logic ...
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		// 4. Activate widget
		await widgets.updateStatus(widgetId, 'active');
...
describe('Runtime Widget Discovery', () => {
	...
	it('should discover marketplace widget at runtime', async () => {
		...
		expect(widgets.active).toContain('testWidget');
	});
	...
	it('should load widget without server restart', async () => {
		...
		const widget = widgets.getWidget('testWidget');
...
describe('Widget Cache Integration', () => {
	it('should invalidate collection caches when widget is disabled', async () => {
		...
		await widgets.updateStatus('input', 'inactive');
	...
	it('should re-validate widget health after status change', async () => {
		...
		await widgets.updateStatus('richText', 'inactive');

		// System should detect unhealthy state
		expect(widgets.healthStatus).toBe('unhealthy');

		// Re-enable widget
		await widgets.updateStatus('richText', 'active');

		// System should recover
		expect(widgets.healthStatus).toBe('healthy');
	});
});
```
