...
<script lang="ts">
	...
	const _language = $derived(
		field.translated
			? contentLanguage.value // Use active content language
...
✅ **Do use contentLanguage store:**

```svelte
let currentValue = $derived(value?.[contentLanguage.value] ?? ''); // ✅ Reactive
```
...
<script lang="ts">
	...
	let displayValue = $derived(
		typeof value === 'string'
			...
			: (value?.[contentLanguage.value] ?? '–') // Translated field
	);
</script>
...
class ContentLanguage {
	value = $state<string>('en');

	setValue(lang: string) {
		this.value = lang;
		// Triggers reactive updates in all $derived using contentLanguage.value
	}
}

export const contentLanguage = new ContentLanguage();
```
...
```svelte
<!-- BAD -->
let displayValue = value[contentLanguage.value]; // ❌ Throws if value is null
```

```svelte
<!-- GOOD -->
let displayValue = $derived(value?.[contentLanguage.value] ?? '–'); // ✅ Safe access
```
...
```svelte
<!-- BAD -->
let currentLang = systemLanguage.value; // ❌ UI language, not content language let displayValue = value[currentLang]; // Shows wrong data if UI ≠ Content
language
```

```svelte
<!-- GOOD -->
let currentLang = contentLanguage.value; // ✅ Always use contentLanguage for data let displayValue = value[currentLang];
```
