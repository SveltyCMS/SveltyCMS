...
description: 'Complete guide to the widget marketplace - installation, development, and publishing with 3-pillar architecture and multilingual support'
order: 75
icon: 'mdi:store'
author: 'admin'
created: '2025-08-05'
...
- **📱 Tabbed Interface**: Switch between "Installed Widgets" and "Marketplace"
- **🎯 Unified Experience**: Seamless transition from browsing to installation
- **📊 Real-time Stats**: See your installed vs. available widgets
- **🔄 Status**: Currently in development - "Coming Soon" placeholder active
...
- **🌍 Internationalization** - Translation-enabled widgets, multilingual components

### Multilingual Widget Discovery

Marketplace widgets can be tagged with multilingual support:

```typescript
// Marketplace widget metadata
interface MarketplaceWidget {
	id: string;
	name: string;
	features: {
		multilingual: boolean; // ✅ Supports translated fields
		contentLanguages: string[]; // Supported languages ["en", "de", "fr"]
		uiTranslations: string[]; // UI language support
		translationStrategy: 'field' | 'full'; // Per-field or entire widget
	};
	// ... other metadata
}
```

**Filter by Multilingual Support:**

```typescript
// Search for i18n-ready widgets
GET /api/marketplace/widgets?multilingual=true&languages=de,fr
  ...
  "widgets": [
    {
      ...
      "features": {
        "multilingual": true,
        "contentLanguages": ["en", "de", "fr", "es", "it"],
        "translationStrategy": "field"
      }
    }
  ]
}
...
- **📊 Statistics Cards**: 6 key metrics displayed in a grid
...
- **🎯 Filter Tabs**: Quick filters (All, Active, Inactive, Core, Custom)
- **📋 2-Column Grid**: Professional layout for desktop (responsive)
- **� Widget Cards**: Expandable cards showing 3-pillar architecture details

### Marketplace Tab (Coming Soon)

- **🏪 Widget Browse**: Discover new widgets
- **� Advanced Search**: Filter by category, price, rating
- **⭐ Ratings & Reviews**: Community feedback
- **💳 Payment Integration**: Support for premium widgets
...
1.  **Prepare**: Ensure your widget meets all quality standards.
2.  **Package**: Create a compliant `widget.json` and zip bundle.
3.  **Submit**: Upload via the Developer Portal.

👉 **View the full [Marketplace Readiness Guide](/docs/widgets/Marketplace_Readiness_Guide.mdx)**
