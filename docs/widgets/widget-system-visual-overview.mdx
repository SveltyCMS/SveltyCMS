...
description: 'Complete visual guide to the SveltyCMS widget system with Mermaid diagrams including multilingual data flow'
order: 0
icon: 'mdi:chart-tree'
author: 'admin'
created: '2025-10-17'
...
sequenceDiagram
    ...
    participant DB as Database (Agnostic)

    User->>Store: Switch language (en → de)
    Store->>Input: Language change notification

    Note over Input: field.translated = true
    Input->>Store: Read app.contentLanguage
    Store-->>Input: "de"

    Input->>Widget: Access value[_language]
    Widget-->>Input: German translation

    User->>Input: Edit German text
    Input->>Widget: Update value = {...value, de: newText}

    Widget->>DB: Save multilingual object
    Note over DB: MongoDB: {en: "...", de: "..."}<br/>SQL: translation_table rows

    User->>Store: Switch to French (fr)
    Store->>Input: Language = "fr"
    Input->>Widget: Access value["fr"]
    Widget-->>Input: null (no translation yet)
    Input->>Input: Show empty field

    User->>Input: Add French translation
    Input->>Widget: value = {...value, fr: newText}
    Widget->>DB: Save with 3 languages
```
...
sequenceDiagram
    ...
    Form->>Store: Load widget from widgets singleton
