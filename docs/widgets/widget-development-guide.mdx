...
├── widget.json        # Marketplace Manifest (Required for publishing)
...
> [!TIP]
> Planning to publish your widget? Check out the [Marketplace Readiness Guide](/docs/widgets/Marketplace_Readiness_Guide.mdx) for packaging requirements.
...
<script lang="ts">
	...
	$effect(() => {
		if (field.translated && value && typeof value === 'object') {
			currentValue = (value as Record<string, string>)[app.contentLanguage] || '';
	...
	function handleInput(event: Event) {
		...
		if (field.translated) {
			...
			translatedValue[app.contentLanguage] = inputValue;
...
<script lang="ts">
  ...
  const displayValue = $derived(() => {
    ...
    if (field.translated && typeof value === 'object') {
      return (value as Record<string, string>)[app.contentLanguage] || '';
    }
...
**Step 1: Import the app singleton**
...
```typescript
// Use app.contentLanguage for translated fields, default language otherwise
...
const value = data[app.contentLanguage];
```
...
const value = field.translated ? data[app.contentLanguage] : data;
```
...
value = { [app.contentLanguage]: newValue };
```
...
value = { ...value, [app.contentLanguage]: newValue };
```
...
const value = data[app.systemLanguage]; // Wrong store!
```
...
- [ ] Import `contentLanguage` from `@stores/store.svelte`
- [ ] Import `DEFAULT_CONTENT_LANGUAGE` from `@src/utils/constants`
- [ ] Add `const _language = $derived(field.translated ? app.contentLanguage : DEFAULT_CONTENT_LANGUAGE)`
