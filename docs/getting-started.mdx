...
# Start smart development server (automatically launches setup or CMS)
bun dev
```

### 2. Complete Setup Wizard

The smart dev server detects your environment and launches the **Setup Workspace**:
...
📍 Setup Wizard: http://localhost:5173 (Automatically redirects to CMS)
...
```bash
# Development
bun dev              # Smart dev server (auto-detects mode)
bun build            # Build all apps for production
bun preview          # Preview production build

# Database
bun run seed         # Seed initial data
bun run db:migrate   # Run migrations (Drizzle)
