...
description: Strategic plan for implementing highly flexible, structural admin themes in SveltyCMS.
...
This document presents the detailed architectural blueprint for achieving layout and styling flexibility within the SveltyCMS administration panels.
...
<div
  class="card border-(length:--admin-border-width) shadow-(--admin-shadow-elevation)"
...
Figma Variables support multiple types (Color, Float/Number, String, Boolean) and **Modes** (e.g., Light Mode vs. Dark Mode, or Default Theme vs. Custom Theme).
...
- **Variable Modes:** Changing the layout mode in Figma (e.g., switching the page mode from "Classic" to "Custom Layout") instantly updates padding, radius, and shadow properties in the mockup. SveltyCMS replicates this behavior via DOM data attributes (e.g., `data-admin-theme="custom"`).
...
### 5. Semantic SCSS/CSS Variable Rollout
...
While Figma serves as a designer's sandbox to model themes, SveltyCMS's **injected CSS Variable theme system** is the ultimate future-proof mechanism for achieving high-level admin theme portability:

- **Zero Compilation Overhead:** Traditional setups require compilation tasks (Sass/Webpack/Vite rebuilds) to swap administrative designs. By executing styling entirely via runtime CSS variables, a new custom layout can be loaded instantly by serving a static `.css` file.
...
### 9. Theme-Exclusive Features & Advanced Extension Capabilities

Advanced admin themes do not merely style core layouts; they inject dynamic UI features (e.g. custom shortcuts, dynamic navigation toolbars, sticky action headers, and real-time palette tweaks) that are missing from standard themes.
...
### Phase 1: Design Token Abstraction - [COMPLETED]

- **CSS Variable Mapping:** Extract hardcoded dimensions, shadows, and spacing rules from `src/app.css` into `--admin-*` tokens.
  - _Implementation:_ All dynamic administrative tokens (`--admin-spacing-scale`, radii, elevations, layout heights, and editorial colors) were mapped inside [utilities.css](file:///d:/SveltyCMS/src/utilities.css) (imported by `src/app.css`) to protect variables from being overridden during theme swapping or copy-paste style modifications.
- **Theme Context Setup:** Create the Svelte 5 `ThemeContext` controller and hook it into the admin root layout `(app)/+layout.svelte`.
  - _Implementation:_ Implemented `AdminTheme` reactive controller under [theme-context.svelte.ts](file:///d:/SveltyCMS/src/components/ui/theme-context.svelte.ts) and set it up inside [+layout.svelte](<file:///d:/SveltyCMS/src/routes/(app)/+layout.svelte>) using Svelte 5 context and dynamic layout-density configurations (spacious, cozy, compact). Included Svelte 5 compiler refinements (`untrack`) to ensure clean compilation outputs.

### Phase 2: Component Refactoring - [IN PROGRESS]

- **Component Token Wiring:** Refactor the 36 native UI components (cards, inputs, dropdowns, tables, buttons) to reactively read context properties and layout metrics.
  - _Implementation:_ Refactored core primitives including [Button](file:///d:/SveltyCMS/src/components/ui/button.svelte), [Card](file:///d:/SveltyCMS/src/components/ui/card.svelte), [Input](file:///d:/SveltyCMS/src/components/ui/input.svelte), and [Badge](file:///d:/SveltyCMS/src/components/ui/badge.svelte) to dynamically adjust padding, border radius, border width, and layout metrics based on scaling factors. Applied rounding logic (`Math.round(val * 10) / 10`) to heights and paddings to clean up floating point rendering.
...
- **Accessibility & WCAG Auditing (1 day):** Verify color contrast scales, keyboard focus rings, and screen-reader flow during theme swaps.
  - _Implementation:_ Created automated integration tests in [theme-context.test.ts](file:///d:/SveltyCMS/tests/unit/components/ui/theme-context.test.ts) to verify correct context metrics propagation and scaling in SSR. Passed all checks successfully.

---

## 5. Upgrade: Adaptive Workspaces Architecture

To elevate SveltyCMS from structural branding to adaptive, workflow-aware environments:

- **Role-Based Layout Adjustments:** Dynamically reorganize sidebar options, layout density, and component groupings according to the user's role (e.g., translators receive focus on localization tables; managers receive dashboard metrics).
- **Interactive Density Modes:** Contextual parameters for `compact` (heavy listings and developer logs), `cozy` (standard editing), and `spacious` (distraction-free writer modes).
- **Real-Time Contrast Validation:** Dynamic color scale analysis to verify that focus rings, text elements, and alert states meet contrast safety ratios.
- **Status-Color Tokens:** Standardized theme variables linked directly to content review states (draft, scheduled, review, published).
