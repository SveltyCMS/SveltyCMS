...
### 3.9 Self-Learning Architecture (New!)

- **Status:** **✅ IMPLEMENTED (Adaptive Optimization)**
- **Resiliency:** Traditional CMSs often experience crash loops when the database is briefly unavailable during startup (Race Conditions). SveltyCMS implements a **Self-Learning State Machine** with persistent performance metrics.
- **Adaptive Calibration:** The system records operational "baselines" (startup times, success rates) and uses **Exponential Moving Averages (EMA)** to automatically calibrate anomaly detection thresholds. This allows the system to distinguish between "normal" environmental slowness and genuine service failures.
- **Persistent Knowledge:** By storing performance trends in the `SystemPreferences` collection, the state machine "remembers" its environment across restarts, enabling intelligent pre-warming and proactive health monitoring from the first request.
- **Setup Benchmarking:** Automated benchmarking during the setup phase establishes a "Golden Signal" baseline for the specific hardware/cloud environment SveltyCMS is running on.
...
| **Setup Pipeline** | **Background (⚡)** | Blocking (Slow) | Blocking (Slow) | Blocking       | Blocking   | Blocking   | N/A         |
...
| **Resiliency** | **✅ Self-Learning** | ❌ Crashing | ⚠️ Manual Retry | ⚠️ Limited | ❌ N/A | ⚠️ Caching | ✅ SaaS SLA |
