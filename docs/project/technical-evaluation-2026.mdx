...
_Note: SveltyCMS metrics derived from the enterprise-grade benchmark matrix (`scripts/benchmark-matrix/`) with 48 tests across 9 dimensions. The system includes rolling trend analysis, cross-test correlation, adaptive budgets, and root cause classification. Database metrics reflect raw driver latency. Middleware overhead reflects compiled SvelteKit edge-ready hooks. Media upload time reflects block time (Phase 3 background jobs)._
...
>
> # Record to report (populates docs/project/benchmarks/)
> BENCHMARK_RECORD=1 bun test tests/benchmarks/database-performance.test.ts
>
> # Full SQLite matrix (all 48 tests)
