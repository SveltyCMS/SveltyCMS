...
- **Middleware Performance Breakthrough (April 2026):** Optimized the server hook pipeline with consolidated security handlers and `/api` fast-paths, resulting in a **71% total reduction in hook overhead** (down to **27.16 µs**).
...
- **Throughput:** This positions SveltyCMS as the fastest-responding headless engine in the 2026 landscape, with an achievable **36,000+ req/sec** theoretical throughput for middleware.
...
| -------------------------- | ----------------- | --------------- | ------------------------ | ----------------- | ---------------------------------- |
| **SveltyCMS (API layer)**  | **1.5–6.9**       | **~0.2**        | **1.2–3.5**              | **0.17 (Async)**  | Local API path, compiled hooks     |
...
| **SveltyCMS (raw SQLite)** | —                 | **0.17**        | —                        | —                 | Bun Native SQLite, zero-config     |
