...
- **Zero-Latency REST Stack**: Achieved **0.35ms** entry retrieval latency by eliminating schema-resolution bottlenecks via request-level caching.
- **Bulk Ingestion Limit**: Verified stable ingestion of **10,000 entries in ~1.5s** (6,625 entries/s), enabling massive dataset migrations without server starvation.
...
- **Simulated Form Vitality**: Proved a **sub-0.15ms** server prep time for complex 50-field forms, ensuring the Admin UX feels instantaneous regardless of schema complexity.
...
| -------------------------- | ----------------- | --------------- | ------------------------ | ----------------- | --------------------------------- |
| **SveltyCMS (API layer)**  | **0.35 - 0.9**    | **~0.1**        | **15–18**                | **0.12 (Async)**  | High-Frequency Data Engine        |
...
| **SveltyCMS (raw SQLite)** | **0.08**          | **0.05**        | —                        | —                 | Bun Native SQLite, zero-config    |
