...
A major critique of "Headless" CMSs is the difficulty of extending core functionality without forking the codebase. SveltyCMS addresses this with a formal **Plugin System**.

- **Hook-Based Architecture:** Plugins can inject logic into Server-Side Rendering (SSR) pipelines (`pluginRegistry.getSSRHooks`), enabling data injection without client-side fetches.
- **Automatic Migrations:** Plugins define their own database schemas and migrations, which run automatically on server startup. This solves the "schema management" problem for third-party extensions.
- **Tenant-Aware:** The plugin architecture enforces tenant isolation by default, ensuring that extension data (e.g., SEO metatags, PageSpeed results) is correctly scoped to the active workspace.
...
- **Status:** **Backend Implemented.** The core `AuditLogService` provides immutable logging for critical system events (Imports, Permissions, System Settings). Service-level tracking is active.
- **Gap:** A dedicated Admin UI for visualizing these logs is currently in development. Code-level access to audit trails is fully functional for compliance reporting.
...
| **Admin UI**           | SvelteKit/Skeleton: Compiled        | ✅ SOTA                 | React competitors suffer VDOM overhead.    |
| **Security (Auth)**    | Basic: OAuth/Email. No SAML UI.     | ⚠️ Emerging             | Contentful provides deep SSO flows.        |
| **Data Storage**       | Mongo (Current); SQL (Roadmap)      | ⚠️ Evolving             | Directus/Strapi support all SQL natively.  |
