...
- Dispatcher wired via `ENDPOINT_PERMISSIONS` + `api:api-keys` permission
...
> **Why this matters**: Most systems exhibit positive memory growth under load (slow leaks from unclosed file handles, event listeners, promise chains). A system that _shrinks_ under sustained load has proven its GC hygiene — every allocation is paired with proper cleanup. The negative latency drift is an additional signal: the JIT compiler, connection pool warmup, and L1/L2 cache population all compound to make the system _more_ efficient the longer it runs.
