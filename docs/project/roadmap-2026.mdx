...
- ✅ **Multi-database support** - MongoDB, MariaDB/MySQL (Production Ready), PostgreSQL (Beta)
- ✅ **Live Preview** - Enterprise handshake protocol (iframe + editable.website plugin); see `docs/guides/Live_Preview_Architecture.mdx`
- 🔜 **SQLite Support** - Lightweight adapter via Drizzle ORM for local/embedded deployments (Q1 2026)
...
- ✅ **SCIM 2.0 Endpoints** - Native endpoints scaffolded for IdP integration.
...
**Current State:** ✅ **Native Endpoints Implemented (Beta)**
**Enterprise Standard:** Automated user lifecycle via SCIM 2.0

**Gap Analysis:**

- ✅ Native `/api/scim/v2` endpoints verified in codebase.
- Needs integration with specific IdPs (Okta/Azure) for production hardening.
