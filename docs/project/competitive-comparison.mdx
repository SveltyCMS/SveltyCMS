...
- **Sub-millisecond p95 latencies** for authenticated reads and writes — no other headless CMS publishes comparable numbers.
- **Cache invalidation is 35× faster** (2.9ms vs 100ms+) via prefix-map O(bucket) clearing.
