...
| :----------------------- | :-------------------------------- | :----------------------------- | :--------------------------------- | :----------------------- | :------------------ |
| **Avg API Read Latency** | **0.55–2.9 ms**                   | 4–12 ms (sub-10 ms co-located) | 3–18 ms (avg ~35 ms in some tests) | 6–25 ms (avg 120–150 ms) | N/A (Git-based)     |
| **Avg Insert Latency**   | **0.65–1.6 ms**                   | 3–10 ms                        | 4–12 ms                            | 5–15 ms                  | N/A                 |
| **Middleware Overhead**  | **1.1–8.5 µs** (92 µs total p95)  | ~115 µs                        | ~145 µs                            | ~180 µs                  | N/A (browser-based) |
| **Throughput (RPS)**     | **1,600+ RPS** (Full Stack)       | ~900–2,100 RPS (REST/GraphQL)  | Lower (optimized ~ higher)         | 900–1,200 RPS (REST)     | N/A (static/Git)    |
| **Media Upload**         | **0.12 ms** (async/background)    | 100–350 ms                     | 120–400 ms                         | 150–500 ms               | N/A                 |
| **Cold Start**           | **~1.05 s** (Ready state)         | Variable (Next.js)             | ~5 s                               | ~8 s                     | Very fast (<300 ms) |
| **Bundle Size (Brotli)** | **~842 KB** (smallest)            | 3–5 MB+                        | Medium/Large                       | Largest                  | <300–500 KB         |
| **Auth Latency**         | **< 0.8 ms** (3-layer cache)      | Not specified                  | Not highlighted                    | Not highlighted          | Fast (GitHub OAuth) |
...
In our previous tests, we measured raw adapter throughput hitting ~2,300 RPS on SQLite. Our April 2026 "Zero-Mock Production Audit" measures the **fully-loaded framework**. We now route requests through full Authentication, Session Validation, the 5-Pillar Hook pipelines, and intelligent deduplication logging.

Even with this realistic, heavy production workload, SveltyCMS sustains **over 1,600 requests per second** with a p95 latency of just **6ms**. That is blazing fast for a fully secured, multi-tenant capable CMS. Furthermore, our Cold Starts have improved dramatically, dropping from 1,600ms down to ~1,050ms.
