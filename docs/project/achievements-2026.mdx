...
- **Widgets**: `ComponentLoader` / tree-shakeable loaders. Dedicated Slug input/display pillars (`src/widgets/core/slug/input.svelte` + `display.svelte`) instead of reusing the generic Input widget.
- **Compiled RTL locale (September 2026)**: Arabic (`ar`) added beside `en`/`de` in `project.inlang/settings.json` + `src/messages/ar.json`. Admin `dir` still comes from `getTextDirection()` (`ar` is already in the RTL set).
