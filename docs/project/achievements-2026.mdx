...
- **Interactive Admin Session Console (September 2026):** Shipped a comprehensive user session management modal (`modal-user-sessions.svelte` and `DELETE /api/user/sessions`) inside User Management. Administrators can inspect concurrent active sessions per user account, complete with client metadata (OS, browser type, IP address, and current active session indicator) and execute one-click remote session revocation with instantaneous authorization cache invalidation. Tests: `tests/unit/auth/session-manager.test.ts`.
