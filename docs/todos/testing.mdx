---
path: 'docs/todos/testing.mdx'
title: 'Enterprise Testing Infrastructure'
description: 'Complete documentation for SveltyCMS enterprise testing infrastructure and implementation plan.'
...
### ⚠️ Technical Debt / Stale Tests Removed

The following integration and hook tests were removed or excluded because they were stale, relied on legacy architectures (pre-Tailwind refactor), or consistently failed due to strict security middleware. They should be refactored to align with the new security and UI architecture.

**Removed API Tests (Legacy/Stale):**

- `tests/bun/api/theme.test.ts` (Legacy theme file architecture)
- `tests/bun/api/dashboard.test.ts` (Stale metrics/dashboard paths)
- `tests/bun/api/system.test.ts` (Legacy system endpoints)
- `tests/bun/api/media.test.ts` (Stale media processing tests)
- `tests/bun/api/token.test.ts` (Stale token management)
- `tests/bun/api/auth-2fa.test.ts` (Legacy 2FA flow)

**Excluded Hook Tests (Stale/Brittle):**

- All tests in `tests/bun/hooks/` (Excluded in `scripts/run-integration-tests.ts`)
