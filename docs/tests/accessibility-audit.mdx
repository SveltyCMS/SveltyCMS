...
We write dedicated accessibility specs (e.g., `tests/e2e/routes/login/accessibility.spec.ts`) to audit critical user flows:
...
npx playwright test tests/e2e/routes/login/accessibility.spec.ts
```text
````
