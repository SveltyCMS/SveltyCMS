...
- **`tests/e2e/accessibility.spec.ts`** — Keyboard focus indicator visibility test: verifies that the focused element has a visible focus ring (not `outline: none`) when navigating the admin interface via Tab key.
- **`tests/e2e/routes/login/accessibility.spec.ts`** — Login flow accessibility audit.
