...
description: "Testing 2026: shell + structure persistence + golden E2E; utils/persistence also in unit + integration."
order: 16
...
> [!IMPORTANT]
> Do **not** re-grow a multi-file Playwright suite for builder. Empty board, widget catalog, and federation stay in unit/integration (or plugin-specific tests). One extra seed-backed structure journey is allowed on the same spec.
...
| **E2E structure** | Category create → reload → name still on the board                              | `builder.spec.ts` — `structure: category create survives reload`        |
...
| Org structure save          | structure unit  | structure-persistence matrix                  | **structure** (category reload) |
