...
- **World Life Data**: High-fidelity diagnostics including **Success/Failure Latency Splits**, the **per-row lane/cache/hook-count context table**, and the **rejection-path cost split** — rejected requests land at **0.35–0.39 ms**, i.e. parity with the authenticated rows (**0.33–0.36 ms** turbo-cache served, **0.29–0.33 ms** full chain; `tests/benchmarks/auth-performance.test.ts`, SQLite, measured **2026-09-22**, five runs each side). Both sides are labelled by lane, because they are different work: `Auth Validation @ 1c` is served from the turbo cache (4 hooks) while a rejection runs the full chain (7). Resolved as _Rejection-path cost parity_ in the [2026 achievements](../project/achievements-2026.mdx).
...
```bash
# Run the full matrix across all 4 databases (add USE_REDIS=true for the 4 Redis variants)
bun run scripts/benchmark-matrix/index.ts
```

The cross-adapter comparison table in `docs/project/benchmarks/index.mdx` is patched by the matrix itself after each adapter (the `SUMMARY_MATRIX` zone in `reporting.ts`) — there is no separate leaderboard command.
...
- **Percentile Tracking**: Standardized on **p50 (Median)**, **p95 (User Experience)**, and **p99 (Tail Risk)**. Percentiles and min/max are computed on **raw** samples — GC spikes and OS jitter stay visible in the tail instead of being trimmed away. The **mean** is IQR-trimmed (robust central tendency), and small-sample confidence intervals use the t-distribution (z = 1.96 only for n ≥ 30).
...
> [!IMPORTANT]
> **Collection isolation:** Filesystem benchmarks write only to `.compiledCollections/test/<workspace>/`. API benchmarks seed via LocalCMS. The matrix runs `purgeBenchmarkCollectionArtifacts()` before each audit. See [Benchmark Collection Isolation](./benchmark-isolation.mdx).

### Unified benchmark runtime flag

GraphQL, scanners, and `compile.ts` all use `isBenchmarkRuntime()` from `preset-collections.server.ts` (checks the single `BENCHMARK=true` flag). Standalone `bun test tests/benchmarks/*` and the matrix server set the same flag so behavior is consistent.
