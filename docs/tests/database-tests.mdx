...
| `find-page-count-contract.test.ts`  | 9     | **findPage** hasMore/total/**keyset cursor**, **count** modes, count cache |
| `auth-session-amr-contract.test.ts` | 6     | Session `amr` + `mfaVerifiedAt` multi-engine persistence & rotation        |
| `content-nodes-contract.test.ts`    | 8     | Node hierarchy, parent traversal, path queries, physical column selection  |
| `health-contract.test.ts`           | 12    | Connection health, pool stats, queryBuilder, tenant policy                 |

### Three-layer mapping: list / count product APIs {#three-layer-mapping-list--count-product-apis}

Respect the pyramid in [Testing index](./index.mdx) — **do not** put adapter contracts in Playwright.

| Layer                               | What                                                            | Files                                                                       | Command                                                                     |
| :---------------------------------- | :-------------------------------------------------------------- | :-------------------------------------------------------------------------- | :-------------------------------------------------------------------------- |
| **Unit** (white-box)                | Pure eligibility + page slicing + count-cache proxy (mocked L1) | `tests/unit/core/page-utils.test.ts`, `tests/unit/core/count-cache.test.ts` | `bun run test:unit` / pre-commit                                            |
| **Integration** (black-box adapter) | Real `ensureFullInitialization` + live CRUD on active `DB_TYPE` | `tests/integration/databases/find-page-count-contract.test.ts`              | `bun run test:integration` / pre-push SQLite; CI multi-DB                   |
| **E2E**                             | Only after UI/list routes **call** `findPage` (golden journey)  | _none yet_ — not an adapter-only concern                                    | —                                                                           |
| **Benchmark**                       | Latency truth tables (not a gate substitute)                    | `tests/benchmarks/database-performance.test.ts`                             | `BENCHMARK_RECORD=1 bun test tests/benchmarks/database-performance.test.ts` |

```bash
# Unit only (pure helpers)
bun test tests/unit/core/page-utils.test.ts tests/unit/core/count-cache.test.ts

# Integration contract (SQLite local default)
bun test --timeout 300000 tests/integration/databases/find-page-count-contract.test.ts

# Same contract on a Docker engine
DB_TYPE=postgresql bun test --timeout 300000 tests/integration/databases/find-page-count-contract.test.ts
```
