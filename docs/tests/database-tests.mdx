...
3. **Safety Guard**: `src/databases/config-state.ts` fails startup if `TEST_MODE` tries to use a non-isolated `DB_NAME`.
...
### 3. Automated 4-DB Gate (Pre-Push)

You do not need to run each adapter manually before every push. The pre-push hook (`scripts/precheck.ts --tier=push`) runs integration and core benchmarks for **all four adapters** when code, scripts, or database files changed:

```bash
bun run verify:push       # same as git push hook
bun run verify:full       # always runs full matrix (manual)
```

Env blocks and credentials are centralized in `src/utils/test-db-credentials.ts` — the same source used by `ci.yml`, `run-integration-tests.ts`, and `run-core-benchmarks.ts`.
...
## Benchmark Execution Strategy (June 2026)
