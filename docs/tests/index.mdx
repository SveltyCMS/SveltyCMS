...
# Black-box integration runner (choose DB; use --no-build if build/ exists)
bun run scripts/run-integration-tests.ts --db=sqlite
bun run scripts/run-integration-tests.ts --db=sqlite --no-build  # skip rebuild
...
# Run the test DB safety check (also runs in pre-commit and pre-push)
