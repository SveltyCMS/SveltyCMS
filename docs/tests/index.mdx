...
**Core Gates:** White-box unit tests, black-box integration tests, and Playwright E2E (`bun run test:e2e` = CI-parity preview)  
...
- **[Test Status (dashboard)](./test-status.mdx)** — live health, open gaps, commands
- [Test Status Archive](./test-status-archive.mdx) — inventories & history
...
````bash
# 1. White-box unit gate (Vitest via vitest.config.ts)
bun run test:unit
bun run check

# 2. Production build used by black-box integration and Playwright webServer
bun run build

# 3. Black-box integration runner (SQLite needs no Docker)
bun run test:integration
# or: bun run scripts/run-integration-tests.ts --db=sqlite

# 4. E2E CI-parity (adapter-node preview :4173 — not Vite)
bun run test:e2e
# Dev shortcut only: bun run test:e2e:dev
bun run scripts/run-integration-tests.ts --db=sqlite --suite=db
bun run scripts/run-integration-tests.ts --db=sqlite --no-build

# 4. Playwright E2E — default is CI-parity (preview :4173)
bun run test:e2e
# Dev-server shortcut only (Vite :5173 — not CI-identical):
bun run test:e2e:dev

# 5. Smart runner - auto-detects what to test from git diff
bun run test:smart
...
Run DB-backed suites after the matching profile is healthy:
...
| Integration | `scripts/run-integration-tests.ts`, `scripts/check-test-db-safety.ts` | `tests/integration/**/*.test.ts` | `config/private.test.ts`, `tests/e2e/.auth/test-secret.txt`, `config/collections/test/integration_test_collection.ts`, `.compiledCollections/test/integration_test_collection.js` |
| E2E | `playwright.config.ts`, `tests/e2e/global.setup.ts`, `tests/e2e/helpers/` | `tests/e2e/**/*.spec.ts`, `tests/e2e/auth.setup.ts` | `config/private.test.ts`, `tests/e2e/.auth/`, temp SQLite DB names containing `e2e` or `test` |
...
- `config/private.ts` is never renamed, copied from, deleted, or used by local integration tests. `run-integration-tests.ts` builds with `TEST_MODE=true`, so Vite aliases `@config/private` to `config/private.test.ts`.
...
- `bun run ci:local` backs up `config/private.ts` (and related paths), runs the full precheck manifest, optionally Playwright E2E, then restores your live config. A warning is printed when live CMS artifacts are detected.

### Tiered Local Testing

SveltyCMS enforces a **three-tier** local testing strategy. All tiers share the task manifest in `scripts/precheck-shared.ts`:

```bash
# Tier 1: Pre-Commit (automatic, ~2–3s) — lint-staged on staged files + DB safety
git commit -m "feat: my change"

# Tier 2: Pre-Push (automatic, ~15–45 min) — build + 4-DB integration + benchmarks
bun run git push origin my-branch

# Tier 3a: Full CI core (manual) — same checks as push, always-on scans
bun run verify:full

# Tier 3b: Full CI + E2E (manual, ~30–60 min)
bun run ci:local
...
| Command | Build | 4-DB integration | Benchmarks | E2E |
| :------ | :---: | :--------------: | :--------: | :-: |
| `git push` (hook) | when code changed | when code changed | when code changed | — |
| `bun run verify:push` | same as hook | same | same | — |
| `bun run verify:full` | always | always | always | — |
| `bun run ci:local` | always | always | optional `--skip-benchmarks` | optional `--skip-e2e` |
...
2. `format`, `lint`, `check`, `unit`, `security-audit` — White-box quality gates.
3. `build` — Production bundle generation.
4. `db-tests` — Black-box integration matrix across SQLite, MongoDB, MariaDB, and PostgreSQL.
5. `bench-core` — Core performance benchmarks per adapter (also runs on pre-push precheck).
6. `e2e-wizard`, `e2e-auth`, `e2e-app` — Playwright browser validation (local: `bun run ci:local` only).

Pre-push precheck mirrors steps 2–5. Only sharded E2E remains GitHub-only unless you run `ci:local`.
