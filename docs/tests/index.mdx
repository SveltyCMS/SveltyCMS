...
**Core Gates:** Pre-commit (unit + `risk:audit`) → pre-push (build 4 adapters + tenant + SQLite integration) → CI multi-DB + E2E  
**Start here:** `bun run test:doctor` (prints the gate map and runs unit + SQLite integration)  
**Primary Goal:** [100% three-layer completeness](./three-layer-completeness.mdx) (critical inventory, not vanity line %)  
**Primary Safety Rule:** Local and CI tests must never use live credentials or production data  
**Recent hot area:** Request Lane Router + response cache — see [Test Status](./test-status.mdx) and [performance-architecture](../reference/database/performance-architecture.mdx)

## Layer ownership (where new work goes)

| Concern                                                       | Unit               | Integration                                         | E2E                      |
| :------------------------------------------------------------ | :----------------- | :-------------------------------------------------- | :----------------------- |
| Pure helpers / eligibility (`page-utils`, count-cache keying) | ✅                 | —                                                   | —                        |
| Lane classifier + response-cache keying                       | ✅                 | ✅ `request-lane-headers`                           | ✅ `hooks-lane-smoke`    |
| Adapter contracts (`findPage`, count modes, DB result shapes) | mocks only         | ✅ `tests/integration/databases/*-contract.test.ts` | —                        |
| Full HTTP middleware + API envelopes                          | dispatcher helpers | ✅ `tests/integration/api/`                         | golden journeys only     |
| Browser UX / control-map routes                               | —                  | —                                                   | ✅ seed-first Playwright |

> [!IMPORTANT]
> Do **not** add Playwright specs for adapter-only APIs. Product APIs like `crud.findPage` / `count({ mode })` are covered by **unit + database integration contracts** until a real admin list control-map uses them — then one seed-backed E2E golden path is enough. See [Database tests — list/count mapping](./database-tests.mdx#three-layer-mapping-list--count-product-apis).

> [!TIP]
> Live dashboard: **[Test Status](./test-status.mdx)**. Architecture: **[ADR Testing 2026](./adr-testing-2026.mdx)** (includes A++ suite contract + residual golden closure).
...
- **[ADR: Testing 2026](./adr-testing-2026.mdx)** — pyramid, fixtures, soft-skip ban, testid policy (webhooks reference)
- **[Three-Layer Completeness (100% Goal)](./three-layer-completeness.mdx)** 🎯
- **[Headless-First Test Inventory](./headless-test-inventory.mdx)** 🧠 — P0/P1 tiers, session→page, import-boundary
...
- **[E2E Architecture & Flow](./e2e-architecture.mdx)** 🏗️
...
- **[Test Status (dashboard)](./test-status.mdx)** — live health, hot areas, open gaps, commands
- [API Coverage Report](./api-testing.mdx)
- [API Unit Helpers](./api-unit-helpers.mdx) (shared `invokeApi` / mock events)
...
- [Dashboard Tests (widget-agnostic)](./routes/dashboard.mdx) 📊
- [Media Gallery Tests](./routes/mediagallery.mdx) 🖼️
- [Access Management Tests](./routes/access-management.mdx) 🛡️
- [System Settings Tests](./routes/system-settings.mdx) ⚙️
...
# 1. White-box unit (Vitest 5)
bun run test:unit             # full suite
bun x vitest --watch          # interactive TDD watch mode
bun x vitest --ui             # Vitest web UI (@vitest/ui is installed)
bun run test:unit:changed     # smart incremental testing on git changes
bun run test:unit:area        # scoped run on the real Vitest runner
bun run test:security         # focused hooks security suite
bun x vitest run --coverage   # P0 gate coverage (outputs to .vitest/coverage)
bun run check
...
# 3. Playwright E2E — CI-parity (preview :4173)
bun run test:e2e
# Dev-server shortcut only (Vite :5173 — not CI-identical):
bun run test:e2e:dev
# Reuse existing build:
bun run test:e2e:quick

# 4. Smart runner — auto-detects suites from git diff
bun run test:smart
...
Run DB-backed suites after the matching profile is healthy. Integration uses the **in-suite harness** (`tests/integration/harness.ts`) — set `DB_TYPE` (and credentials) then run Bun test:
...
- `bun run test:doctor` / hooks / CI use isolated test config and DBs only — they must never back up/replace or connect through live `config/private.ts` (see `private-config-policy.ts`). CI may write an ephemeral `private.ts` on the runner only.
...
```bash
# Tier 1: Pre-Commit (automatic, ~40s) — DB safety → format+lint → lint-staged → unit
git commit -m "feat: my change"

# Tier 2: Pre-Push (automatic, ~5 min) — production build + SQLite integration
...
| :-------------------------- | :--------: | :----------------: | :----------------------: |
| `git commit` (hook)         |     —      |         —          |        unit only         |
| `git push` / `bun run gate` |   always   |       always       |       — (CI-only)        |
| `bun run test:doctor`       | if missing |       always       |  optional `--with-e2e`   |
| GitHub Actions              |   always   |       always       |       full matrix        |

See **[Git Workflow & Quality Gate](./git-workflow.mdx)** for Docker prerequisites, credential parity, and scripts audit.
...
2. `whitebox` — format, lint, check, unit, CVE audit, secret misuse, deploy-backdoor probe.
3. `build` — Production bundle (`COMPILE_ALL_ADAPTERS=true`).
4. `db-tests` — Black-box integration matrix across SQLite, MongoDB, MariaDB, and PostgreSQL.
5. `bench-core` — Core performance benchmarks per adapter (CI-only; not pre-push).
6. `e2e-prep` — wizard + auth-setup, then `e2e` × 6 named groups (Playwright).
7. `all-green` — consolidates job results.
...
- **New:** `tests/unit/core/` — 23 dedicated tests for `proxy-utils.ts` and `relational-utils.ts` covering previously untested database layer patterns.
...
- **Tools:** 62-module Benchmark Matrix (`scripts/benchmark-matrix/`) — module inventory in [Performance Benchmarks](../project/benchmarks/index.mdx).
...
| **websocket-broadcast**         | Benchmarks network-layer broadcast latency (SSE vs WebSocket).                                  | Critical performance audit for Broadcast.            | ~20s      |
...
graph TD
    subgraph levels [Testing Levels]
        ...
        Int[Integration<br/>harness + bun test]
...
1. **Setup**: `tests/benchmarks/seed-runner.test.ts` calls `seedBenchmarkState()` before boot (in-process `LocalCMS`, mirroring `/api/testing action=seed`)
2. **Stable data**: `ensureStableTestData()` creates `BenchmarkStable` in DB/content store only
3. **Cleanup**: PID-scoped temp DB; `cleanupAllBenchmarkWorkspaces()` (`benchmark-utils.ts` `afterAll`) clears the sandbox workspaces and the matrix kills its server child on adapter exit
