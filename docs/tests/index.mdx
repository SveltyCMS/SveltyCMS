...
| :------ | :--- | :------------------------- |
| `mongodb` | `27017` | `DB_USER=testuser`, `DB_PASSWORD=testpass`, `authSource=admin` |
...
- `bun run ci:local` simulates a fresh CI runner and refuses to run when live setup artifacts are present unless `SVELTYCMS_DISPOSABLE_TEST_WORKSPACE=true` marks the checkout as disposable.
...
Run Tier 3 only in a disposable clone/worktree. It intentionally resets local CI state and now fails fast if it detects `config/private.ts`, top-level `config/collections/*.ts`, or top-level `.compiledCollections/*.js`.
