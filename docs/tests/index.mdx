...
bun run test:unit
bun run test:watch            # interactive TDD watch mode
bun run test:ui               # Vitest 5 visual web UI with trace viewer
bun run test:changed          # smart incremental testing on git changes
bun run test:security         # focused hooks security suite
bun run test:unit:coverage    # P0 gate coverage (outputs to .vitest/coverage)
bun run check
...
| **sse-broadcast**               | Measures EventBus → SSE stream broadcast latency and handshake timing.                          | Critical performance audit for Real-Time Broadcast.  | ~20s      |
