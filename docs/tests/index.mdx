...
- `bun run ci:local` / full precheck use isolated test config and DBs only — they must never back up/replace or connect through live `config/private.ts` (see `private-config-policy.ts`).
