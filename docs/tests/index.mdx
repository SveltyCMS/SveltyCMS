...
bun x playwright test --project=wizard  # (runs tests/e2e/routes/setup/setup-wizard.spec.ts via project testMatch)
...
| **content-incremental-reload**  | Compares surgical 1-file `fullReload` vs full reconciliation + SCHEMA cache wipe.              | Critical performance audit for Hot Reload.           | ~4s       |
