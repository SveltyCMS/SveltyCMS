...
The gate runs the full unit test suites (both Vitest and Bun Native) and the Production Build locally before every commit to ensure zero CI regressions. SQLite Integration Tests are also run locally on staged changes.

Key features:

- **Parallel Execution**: Linting, Type-Checking, and Testing run simultaneously.
- **Full Test Suite & Build Verification**: Runs the complete unit test suites (Vitest & Bun Native) and the Production Build locally to prevent broken builds in CI.
...
2.  **Full Unit Testing**: Runs Vitest (`bun run test:unit --run`) and Bun Native (`bun run test:unit:bun`) suites.
