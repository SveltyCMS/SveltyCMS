...
#### Pre-push (~15–45 min) — CI core parity

`.githooks/pre-push` runs `bun run verify:push`. When source, infra, or database files changed:

| Check                                                                | CI job           |
| :------------------------------------------------------------------- | :--------------- |
| Format + tree-clean verification                                     | `format`         |
| Slop scanner + import validation                                     | `lint`           |
| oxlint + admin-theme lint (if admin routes changed)                  | `lint`           |
| Docs lint (if docs changed)                                          | `docs-lint`      |
| `bun audit --severity=high` (if `package.json` / `bun.lock` changed) | `security-audit` |
| `bun run check`                                                      | `check`          |
| Full unit tests (**always**)                                         | `unit`           |
| Production build (`COMPILE_ALL_ADAPTERS`)                            | `build`          |
| Integration × 4 (sqlite, mongodb, mariadb, postgresql)               | `db-tests`       |
| Core benchmarks × 4                                                  | `bench-core`     |

**Prerequisites:** Docker Desktop with MongoDB (27017), MariaDB (3306), and PostgreSQL (5432) on `127.0.0.1`. Credentials match `src/utils/test-db-credentials.ts` and `tests/docker-compose.yml` image defaults.
...
**Still runs only on GitHub:** sharded Playwright E2E (`e2e-wizard`, `e2e-auth`, `e2e-app`).
...
flowchart LR
    ...
    C["git push"] -->|"15–45 min"| D["Pre-Push precheck<br/>build + 4-DB<br/>+ benchmarks"]
...
| Check                             | Tool                                  |
| :-------------------------------- | :------------------------------------ |
| Full static analysis + unit tests | `precheck-shared.ts` manifest         |
| Production build                  | `vite build` + `COMPILE_ALL_ADAPTERS` |
| 4-DB integration                  | `run-integration-tests.ts --no-build` |
| Core benchmarks × 4               | `run-core-benchmarks.ts`              |
...
3.  **Push**: `bun run git push origin feat/my-feature` — pre-push precheck runs build + 4-DB integration + benchmarks.
4.  **Before PR**: Run `bun run ci:local` (or `bun run verify:full --skip-e2e` for faster DB-only parity).
