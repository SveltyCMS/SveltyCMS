...
#### Pre-commit (~2–3s) — staged files + security suite (once)

`.githooks/pre-commit` runs:

1. **Test config safety** — `scripts/check-test-db-safety.ts` blocks unsafe `private.test.ts`, live `private.ts` pointing at test DBs, and **matching live/test DB names** (user data protection).
2. **Format check** + **lint-staged** — `oxfmt`, `oxlint`, `slop-scanner` on staged `src/`; `lint-docs` on staged `docs/`.
3. **Security regression (pre-commit only)** — `bun run scripts/security-regression.ts` (hooks + RBAC + setup gating).

> [!IMPORTANT]
> **No double-run rule:** security regression runs on **pre-commit only**. Pre-push excludes those files from unit tests.
>
...
#### Pre-push (~1–3 min) — build + unit (excludes security suite)

`.githooks/pre-push` runs `bun run verify:push` via `scripts/precheck.ts`. Diff base is `@{upstream}` / `origin/next`.

| Check                                                                 | CI job   | Est.                                    |
| :-------------------------------------------------------------------- | :------- | :-------------------------------------- |
| Test Config Safety                                                    | —        | 1s                                      |
| Format / oxlint / `bun run check` (when paths changed)                | whitebox | ~20s                                    |
| Slop + imports + secret scan (source/infra)                           | whitebox | ~10s                                    |
| Docs / benchmark MDX lint (docs or infra)                             | —        | ~4s                                     |
| Unit tests via `test-smart --unit-only` (**excludes security files**) | whitebox | ~3–60s                                  |
| Production build (`COMPILE_ALL_ADAPTERS`, when source/infra)          | build    | ~45–120s                                |
| Deploy backdoor double-build                                          | whitebox | only if vite/testing-strip paths change |

**Security regression is not re-run on push.** DB integration + benchmarks are CI-only. Opt in with `--include-db-tasks`.
...
`ci:local` / full precheck must use **`config/private.test.ts` only** for automated runs — they must **not** rename, overwrite, or connect through the developer's `config/private.ts` (live user data). Isolation is env + policy (`private-config-policy.ts`), not backup/restore of live config.
