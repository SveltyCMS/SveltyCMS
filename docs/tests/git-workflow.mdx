...
In April 2026, the pre-commit chain was upgraded to a **Parallel TypeScript Orchestrator**, reducing validation from ~150s to **~34s**.

In May 2026, the gate was further optimized for local development by introducing **Tired Check Routing** using the `PRE_COMMIT="true"` environment flag. This achieves an ultra-fast commit feedback loop of **under 10 seconds** locally.

Key features:

- **Parallel Execution**: Linting, Type-Checking, and Testing run simultaneously.
- **Related Test Filtering**: Uses `vitest related` to only run tests affected by staged changes.
- **Selective Local Runs**: Automatically skips heavy operations (Production Build, Bun Native Unit Tests, and SQLite Integration Tests) during local commits, delegating them to the robust GitHub Actions CI workflows.
...
2.  **Incremental Unit Testing**: `vitest related` run on staged files.
