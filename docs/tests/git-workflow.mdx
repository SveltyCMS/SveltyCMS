...
The gate runs the full unit test suite (1,100+ tests via Vitest with jsdom) and the Production Build locally before every commit to ensure zero CI regressions. SQLite Integration Tests are also run locally on staged changes.

Key features:

- **Parallel Execution**: Linting, Type-Checking, and Testing run simultaneously.
- **Full Test Suite & Build Verification**: Runs the complete 1,100+ unit suite (Vitest) and the Production Build locally to prevent broken builds in CI.
