...
- **Purpose**: Multi-database verification of the full middleware chain (Firewall → Auth → RBAC → Handler → DB).
- **Runner**: `scripts/run-integration-tests.ts` — orchestrates build, preview server, DB seeding, test execution, and cleanup.
- **Databases**: SQLite, MongoDB, PostgreSQL, MariaDB (same suite against all 4 engines).
- **Command**: `bun run test:integration`
- **Isolation**: Generates `config/private.test.ts` with isolated credentials. Never touches production data.
