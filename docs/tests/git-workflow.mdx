...
#### Pre-commit (`DB safety` → format+lint → lint-staged → **risk:audit** → unit → SBOM)
...
4. **`risk:audit`** — security-risk scan (all adapters) + secret misuse + slop + `bun audit` + OSV (skipped if only docs changed).
5. **Unit tests** — `vitest run` (skipped if only docs changed).
6. **SBOM sync** — when `bun.lock` / `package.json` changed.

> [!IMPORTANT]
> **No double-run rule:** the full unit suite (including hooks security tests) runs on **pre-commit only**. Pre-push runs build + post-build verifiers + tenant + SQLite integration — do not re-run the entire unit suite on push. Focused re-run: `bun run test:security`.
>
...
#### Pre-push (~5–8 min) — production build + verifiers + tenant + SQLite integration
...
```mermaid
flowchart LR
    A["git commit"] -->|"~40–60s"| B["Pre-Commit<br/>DB safety → format+lint<br/>→ lint-staged → risk:audit → unit"]
    C["git push"] -->|"~5–8 min"| D["Pre-Push<br/>Build 4 adapters → backdoor/bundle<br/>→ check → secrets → tenant → SQLite integ"]
```
...
> [!NOTE]
> **Gate vs. inner loop:** the pre-commit unit run (`bun run test:unit`) is the gate — always
> exhaustive, intentionally not selective. For iteration, pick the smallest scope that covers the
> change: `bun run test:unit:area tests/unit/<dir>/` (fastest: one area, no changed-graph build) or
> `bun run test:unit:changed` (selective: unpushed commits + dirty tree; prints the resolved diff base
> and affected-test count, warns loudly when nothing is affected, exits 3 when no base resolves, and
> escalates to the full suite on setup/config changes).
...
| `test-unit-changed.ts`          | Selective Vitest runner (resolves base)  | `bun run test:unit:changed`    |
