...
> **Note**: API test coverage documentation is actively being expanded. See the [Test Status Overview](./test-status.mdx) for the latest counts, and the [Hook Test Coverage](./hook-test-coverage.mdx) and [E2E Coverage Matrix](./e2e-coverage-matrix.mdx) for route-level detail.
