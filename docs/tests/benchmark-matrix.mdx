...
flowchart TB
    ...
    subgraph Tests["tests/benchmarks/ (76 files)"]
        BMs["62 Registered + 14 Standalone"]
...
| `tests/benchmarks/`         | 62 registered + 14 standalone test modules  | 76 files   |
| `tests/benchmarks/modules/` | Shared utility modules                      | 11 files   |
...
> [!NOTE]
> Matrix registration is for **performance** benchmarks. Harness/metadata tests
> that validate the benchmark tooling itself (e.g. `statistics-accuracy.test.ts`,
> the `modules/*.test.ts` suites) run standalone via `bun test tests/benchmarks/`
> and do **not** enter the matrix — they never measure a database adapter.
