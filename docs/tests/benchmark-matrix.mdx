...
The SveltyCMS benchmark matrix is a CI-integrated performance validation system that runs 58+ benchmark test modules against 8 database configurations (4 adapters × 2 Redis states), detects regressions via statistical analysis, and generates per-adapter MDX reports with trend visualizations.
...
bun run scripts/benchmark-matrix/index.ts --sql --redis

# Run a single benchmark only
bun run scripts/benchmark-matrix/index.ts --sql --only=relational

# Skip build (already built)
bun run scripts/benchmark-matrix/index.ts --sql --no-build
```
