...
Counts drift with every wave — [`test-status.mdx`](./test-status.mdx) is the single source of truth for file/test counts.

| Layer       | Scale (approx.)       | Health signal                                            |
| :---------- | :-------------------- | -------------------------------------------------------- |
| Unit        | see `test-status.mdx` | Local full unit green in a single run                    |
| Integration | see `test-status.mdx` | SQLite strong; multi-DB skew tracked                     |
| E2E         | see `test-status.mdx` | Flakes on builder; smoke + journey hardening in progress |
...
| Layer       | Suite                                                 | Path                                                   |
| ----------- | ----------------------------------------------------- | ------------------------------------------------------ |
| Unit        | Session cookies (loopback Secure)                     | `tests/unit/auth/sessions/session-cookies.test.ts`     |
| Unit        | Client import boundary (collaboration / client roots) | `tests/unit/live/client-import-boundary.test.ts`       |
