...
- ✅ `rich-text.test.ts`
- ✅ `select.test.ts`
- ✅ `slug.test.ts` — validation plus dedicated `input.svelte` / `display.svelte` pillar paths
- ✅ `tests/unit/utils/form-sections.test.ts` — editor section grouping, pairing, and width parsing (`src/utils/form-sections.ts`)
- ✅ `tests/unit/utils/locale-map.test.ts` — locale-map unwrap used by list cells and SEO
...
- ✅ `tests/unit/widgets/custom/seo.test.ts` — validation plus SERP/heatmap helpers (`seo-serp.ts`)
