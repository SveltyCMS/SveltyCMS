...
> [!IMPORTANT]
> **Source of truth:** `src/utils/benchmark-paths.ts` and `src/routes/setup/preset-collections.server.ts` (`purgeBenchmarkCollectionArtifacts()` → `src/utils/benchmark-db-purge.ts`). Matrix lifecycle: `scripts/benchmark-matrix/index.ts`.
...
1. **Pre-seed** (`index.ts` → `seed-runner.test.ts`): the pre-seed child runs before boot so roles, admin and benchmark collections exist when the server starts.
2. **Server boot** (`index.ts`): ONE production server per adapter, started from `serverEnv`.
3. **Child tests**: `API_BASE_URL` points at that server; `setupBenchmarkServer()` reuses it (`{ stop: async () => {} }`) instead of spawning a second one.
4. **Workspace cleanup** (`tests/benchmarks/modules/benchmark-utils.ts`): `afterAll` calls `cleanupAllBenchmarkWorkspaces()` per test file — except under `BENCHMARK_MATRIX=1`, where the matrix owns cleanup.
5. **Exit** (`index.ts`): the adapter's server is killed (SIGTERM → 300 ms grace → SIGKILL) before the next adapter starts.

```bash
# Single benchmark via matrix (isolated temp DB, shared production server)
bun run scripts/benchmark-matrix/index.ts --db=sqlite --only=relational --no-build
```

> [!WARNING]
> **`--only` narrows the tests, not the reporting.** The run still rewrites the per-adapter
> ledger MDX for the adapters it touches, patching the subset into reports assembled from
> whatever is already on disk. Foreign sections can change without having been measured —
> after a measurement-only run, discard the doc churn with
...
# Inspect SQLite pollution (collection_* tables per database in config/database/)
bun -e 'import path from "node:path"; import { readdirSync } from "node:fs"; import { Database } from "bun:sqlite"; const dir = path.resolve("config/database"); const isDb = (n) => n.endsWith(".db") || n.endsWith(".sqlite"); for (const f of readdirSync(dir).filter(isDb)) { const db = new Database(path.join(dir, f), { readonly: true }); console.log(f, db.query("SELECT name FROM sqlite_master WHERE type = ? AND name LIKE ?").all("table", "collection_%").map((r) => r.name)); }'
```
...
| Artifact purge call sites         | Done — `purgeBenchmarkCollectionArtifacts()` is the only matcher          |
