...
| :--------------- | :----------------------------------------------------------- |
| `BENCHMARK=true` | `benchmark-utils.ts`, matrix server env, run-core-benchmarks |
...
Standalone `bun test tests/benchmarks/*` sets `BENCHMARK=true` (and `TEST_MODE=true`) in `benchmark-utils.ts` so GraphQL and scanners stay aligned.
