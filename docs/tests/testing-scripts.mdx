...
### `scripts/scan-db-risk.ts` — DB-Layer Risk Scanner (all 4 adapters)

Static scanner for injection classes in OUR adapter code — the layer no external database can cover. Scopes `src/databases/`, `src/plugins/`, `src/services/`:
...
| Non-httpOnly cookie          | error    | `httpOnly: false` on auth/session cookies                                                                              |
| SQL identifier interpolation | warning  | Unescaped identifiers (skipped in files with `assertSafeSqlIdentifier`-style guards)                                   |
| Regex interpolation          | warning  | `new RegExp(\`...${x}...\`)`without the`\$&` escape idiom                                                              |

```bash
bun run scripts/scan-db-risk.ts          # scan (warnings tolerated)
bun run scripts/scan-db-risk.ts --strict # exit 1 on errors (pre-commit)
```
...
1. `scan-db-risk --strict` — our DB code (4 adapters) + SvelteKit config
