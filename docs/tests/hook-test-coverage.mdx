...
In addition to correctness, all hooks are benchmarked for execution overhead using `tests/benchmarks/hooks-performance.ts`. Following the April 2026 optimizations, SveltyCMS achieved **High-Frequency Middleware** status.

| Metric                     | Value    | Target (2026) |
| -------------------------- | -------- | ------------- |
| Static Regex Check Latency | 0.085 µs | < 0.200 µs    |
| API Fast-path Skip Latency | 0.031 µs | < 0.100 µs    |
| Compression Guard Latency  | 0.044 µs | < 0.100 µs    |
| **Total API Hook Overhead**| **7.16 µs**| < 15.00 µs    |

**Note**: Overhead is measured in microseconds (µs). Total overhead is **21x faster** than the original 2026 budget (0.150ms).
