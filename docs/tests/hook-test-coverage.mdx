...
| ---------------------------- | :-------------: | :----: | --------------------------- |
| **Turbo Pipeline (Light)**   |   0.81ms avg    |  <2ms  | Health check — fastest path |
| **Full Security + Auth**     |   1.47ms avg    |  <2ms  | JWT + session + RBAC        |
| **Mutation + Audit Logging** |   1.28ms avg    |  <2ms  | Write with SHA-256 audit    |
| **Auth Overhead**            |   **0.66ms**    |   —    | Turbo → Full Auth delta     |

**Note**: Auth overhead (0.66ms) = Full Auth (1.47ms) - Turbo (0.81ms). Optimized May 2026 by eliminating redundant setup-state check on every authenticated request.
