...
## 2026 product-route standard

Admin/config routes should follow **[ADR: Testing 2026](./adr-testing-2026.mdx)**:

- Shared **`fetchApi`** mutations (CSRF always)
- Domain **`*-api.ts` + `*-utils.ts`** (unit) + golden E2E journey
- **No soft-skip** for empty install on control-map rows — seed via `/api/testing`
- **Reference route:** `/config/webhooks`
...
**Headless-first placement** (adapters + session + RBAC before admin E2E): **[Headless-First Test Inventory](./headless-test-inventory.mdx)**.
...
Files: `tests/unit/**/*.test.ts` | Runner: `bun run test:unit` | Tests: 4,500+ (515+ files — run `bun run test:unit` for current count)
...
Runner: `bun run test:integration` | Tests: 660 tests (71 files)
...
Files: `tests/e2e/*.spec.ts` | Runner: `npx playwright test` | Scenarios: ~44 spec files, golden-first paths
