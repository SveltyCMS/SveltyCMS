...
| `e2e-chromium` (×6) | `chromium`                          | Parallel (6 shards) | Run all browser-based tests against one server    |
...
- Sharded into 6 parallel jobs using Playwright's `--shard=N/6`
- Each shard reuses the `storageState` files from `auth-setup`
- All 6 shards connect to the same preview server instance
