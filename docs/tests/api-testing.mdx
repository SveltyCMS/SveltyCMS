...
- **User Lifecycle**: Creation, login/logout, multi-tenant isolation, and role-based access. `POST /api/user` and `GET /api/user` strictly require `user:write`/`user:read` authorization, stripping non-admin privilege fields (`role`, `roles`, `isAdmin`, `roleIds`, `permissions`).
- **Token System**: JWT creation, rotation, and revocation. `/api/token/*` enforces a strict positive allowlist (only public route is `GET /api/token/validate-token/{value}`).
- **Payload & Page Limits**: Streaming body counter enforces 15 MB `API_MAX_BODY_SIZE_BYTES` limit even on chunked requests; `MAX_PAGE_SIZE = 200` enforced centrally.
