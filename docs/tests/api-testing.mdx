...
```bash
# Fast local loop (~1 min SQLite once preview artifacts exist)
bun run scripts/run-integration-tests.ts --db=sqlite --no-build

# API suite only (when runner supports --suite)
bun run scripts/run-integration-tests.ts --db=sqlite --suite=api --no-build

# Full matrix parity (slower; use before PR that touches adapters/auth)
...
> [!TIP]
> First run after a clean checkout needs `bun run build` (or omit `--no-build`). Day-to-day, `--db=sqlite --no-build` is the short path.
