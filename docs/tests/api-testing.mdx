...
1. **Config Isolation**: Local integration/E2E/precheck use **`config/private.test.ts` only**. They must never read or write the developer's **`config/private.ts`** (live CMS DB — e.g. `sveltycms.db`). See `src/utils/private-config-policy.ts`.
