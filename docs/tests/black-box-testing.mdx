...
> [!IMPORTANT]
> **Safety Protocol**: Black-box tests must use test-only configuration. `config/private.test.ts` is the local source of truth. Local integration and E2E runs must never rename, copy from, delete, or otherwise mutate the user's `config/private.ts`. CI may create an ephemeral `config/private.ts` only on fresh runners for build parity, and that file must be a mirror of the isolated test config.
>
