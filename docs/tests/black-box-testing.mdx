...
- **Benchmark Stability**: Benchmark runtimes (`isBenchmarkRuntime()` — the single `BENCHMARK=true` flag; `TEST_MODE` is the separate test-harness token) scan benchmark collections and keep API-seeded fixtures (`BenchmarkStable`, `benchmark_authors`) synchronized in the temp DB via `refreshContent({ mode: "schemas" })` — the default refresh mode under `TEST_MODE` (the legacy `BENCHMARK_MODE`/`BENCHMARK_STABLE`/`SVELTY_BENCHMARK_SUITE` tokens were consolidated into `BENCHMARK`). Filesystem scan benchmarks use isolated paths under `.compiledCollections/test/` — see [Benchmark Collection Isolation](./benchmark-isolation.mdx).
