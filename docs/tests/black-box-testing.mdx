...
>
> **Pre-push does not run the 4-DB matrix or benchmarks by default** (CI-only). Opt in: `bun run scripts/precheck.ts --tier=push --include-db-tasks`. Credentials: `src/utils/test-db-credentials.ts`.
>
> Shared classifier: `src/utils/test-db-safety.ts` (server, integration runner, hooks). User live collections under `config/collections/` (outside `test/`) and root `.compiledCollections/` must not be written by benchmarks — see [Benchmark Collection Isolation](./benchmark-isolation.mdx).
