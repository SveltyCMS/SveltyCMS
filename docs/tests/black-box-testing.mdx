...
The GitHub Actions E2E matrix is intentionally curated around maintained smoke coverage:
...
Legacy exploratory Playwright specs remain in `tests/e2e/`, but they are not PR-gating until their routes, selectors, and redirect expectations are rewritten against the current admin UI.
