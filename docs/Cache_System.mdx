---
path: 'docs/dev-guide/cache-system.mdx'
title: 'Cache System'
...
```typescript
// When updating a post, clear related caches
async function updatePost(postId: string, data: any) {
	await db.posts.update(postId, data);

	// Clear specific caches
	await cacheStore.delete(`dynamic:post:${postId}`);
	await cacheStore.delete('query:posts:published');
	await cacheStore.delete('query:posts:recent');
}
```
