...
5. **[MariaDB Implementation](./MariaDB_Implementation.mdx)** 🆕
...
6. **[PostgreSQL Implementation](./PostgreSQL_Implementation.mdx)** 🆕 (Beta)
   ...
   - **Status**: Beta (adapter scaffolding complete, stub methods)

**Coming Soon:**

- SQLite Implementation (via Drizzle ORM) - Lightweight embedded database for local development and edge deployments
...
│  • Progressive Phases (READY → WARMING → WARMED)        │
│  • Sub-1s API Readiness (Critical Path)                 │
│  • Background Feature Activation (Non-Blocking)          │
└───────────────────────┬─────────────────────────────────┘
                        │
┌───────────────────────▼─────────────────────────────────┐
│       Layer 2: Database Adapter (dbInterface.ts)        │
│  • Capability-Based Dependency Guards                   │
│  • Promise-Based Startup Locking                        │
│  • Unified QueryCriteria translation                    │
└───────────────────────┬─────────────────────────────────┘
                        │
        ┌───────────────┴───────────────┬────────────────┐
        │                               │                │
