---
path: 'docs/database/cache-system.mdx'
...
SveltyCMS features a sophisticated **dual-layer caching system** with Redis L1 (fast, in-memory) and Database L2 (persistent) caching. Any supported database (MongoDB, MariaDB, PostgreSQL, SQLite) serves as the second persistent layer, ensuring performance and recovery even after Redis cold-starts.
        ...
        └──────────┬───────────┘
         ...
         Cache          Cache
          Hit            Miss
            ...
            │             ↓
            │   ┌──────────────────────┐
            │   │ Check Database (L2)  │ ← Medium (5-10ms)
            │   │ Persistent Cache     │
            │   └──────────┬───────────┘
            │              │
            │       ┌──────┴──────┐
...
| **L2: Database** | Disk    | 5-10ms   | Permanent   | Warm data, recovery       |
| **Source DB**    | Disk    | 50-100ms | Permanent   | Cold data, cache miss     |

**Cache Hit Rates (Steady State)**:

- **Static Layer (Warmed)**: 99.9% (Schemas, Roles, Navigation)
- **L1 (Redis)**: 85-92% of dynamic requests
- **L2 (Database)**: 5-10% of requests
- **Source DB**: <1% after full hydration
...
## 📁 File 1: `src/databases/cache-service.ts`
...
1. **Dual-Layer Caching**: Redis L1 + Database L2
...
5. **Predictive Prefetching**: Pattern-based prefetching with custom fetchers
...
### ContentManager Optimizations

In addition to the global `CacheService`, `ContentManager` implements specific optimizations for high-performance content delivery:

- **LRU Cache for Collections**: Maintains a dedicated in-memory LRU cache (referenced `lru-cache` library, max 500 items, 20s TTL) for the most frequently accessed collection schemas, ensuring optimal access patterns and reducing overhead.
- **Batch Processing**: Content initialization and cache population are processed in batches (default size 100) to prevent event loop blocking during large dataset loads.
- **Smart Initialization**: Uses an exponential backoff strategy with jitter for robust startup, ensuring the system recovers gracefully from temporary database unavailability.
- **Reactive Snapshots**: Exposes `getReactiveNavStore()` for Svelte 5 rune-compatible integration on the client side.
...
class CacheService {
	...
	// Example usage of predictive prefetching
	// cacheService.registerPrefetchPattern({
	//   pattern: /^user:(\d+)$/,
	//   prefetchKeys: (key) => {
	//     const userId = key.split(':')[1];
	//     return [`user:${userId}:permissions`, `user:${userId}:profile`];
	//   },
	//   fetcher: async (keys) => {
	//     // Fetch data for keys
	//     return {
...
The following statistics are typical for production environments across all supported databases (MongoDB, PostgreSQL, MariaDB, SQLite):

| Category    | Hit Rate | Reason                                     |
| ----------- | -------- | ------------------------------------------ |
| **Schema**  | 99%      | Changes rarely after startup               |
| **Widget**  | 95%      | Configs are shared across many records     |
| **Theme**   | 98%      | Static design data rarely changes          |
| **Content** | 85%      | Dynamic, but frequently accessed           |
| **User**    | 90%      | Permissions checked on every request       |
| **Session** | 95%      | High volume of repetitive token validation |
| **API**     | 75%      | Custom queries with specific payloads      |
| **Media**   | 80%      | Metadata caching for assets                |
...
1. **cache-service.ts (546 lines)** - Core caching infrastructure
...
- [Core Infrastructure](./core-infrastructure.mdx) - Database core files
- [Authentication System](./authentication-system.mdx) - Auth infrastructure
- [MongoDB Implementation](./mongodb-implementation.mdx) - MongoDB adapter
