...
```
1. Application Starts (IDLE)
   ↓
2. First Request / Manual Trigger (INITIALIZING)
   ├─ loadPrivateConfig() - In-memory config preferred
   └─ loadAdapters() - Dynamic import based on DB_TYPE
   ↓
3. Critical Path Handshake (READY)
   ├─ dbAdapter.connect() - Connection with retry
   ├─ setupAuthModels() - Parallelized model caching
   └─ loadSettingsFromDB() - Critical system preferences
   ↓
4. SYSTEM IS OPERATIONAL (READY signaled)
   └─ Returning first request in <1s
   ↓
5. Background Task Scheduling (WARMING)
   ├─ ThemeManager.initialize() - UI hydration
   ├─ WidgetRegistry.scan() - Component discovery
   └─ ContentManager - Remains LAZY until first use
   ↓
6. FULL HYDRATION (WARMED)
   └─ All background caches and registries active
```
