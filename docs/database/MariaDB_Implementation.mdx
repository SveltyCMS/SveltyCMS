...
The MariaDB adapter is optimized for relational data integrity and high-concurrency connection pooling. High-level improvements like the **92% cache hit rate** are inherited from the [Unified Caching Layer](./index.mdx#unified-caching-flow).

| Operation                      | Response Time | Cache Hit Rate |
| :----------------------------- | :------------ | :------------- |
| **Simple lookup (cached)**     | 1.2ms         | 92%            |
| **Relational Join (2 tables)** | 14ms          | 88%            |
| **Batch Insert (100 records)** | 48ms          | N/A            |
| **System Preference Update**   | 5ms           | N/A            |
| **Cold Start (READY)**         | ~950ms        | N/A            |
