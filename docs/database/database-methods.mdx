...
> **Note:** This document details the **MongoDB reference implementation**. For the MariaDB/SQL implementation using Drizzle ORM, please see the [MariaDB Implementation Guide](./MariaDB_Implementation.mdx).
...
```mermaid
sequenceDiagram
    participant Setup as /api/setup/complete
    ...
    Note over Setup,State: ✅ System fully operational<br/>NO RESTART NEEDED
```
...
stateDiagram-v2
    ...
    INITIALIZING --> DEGRADED: Some services unhealthy
    INITIALIZING --> FAILED: Critical service failed

    READY --> DEGRADED: Service becomes unhealthy
    DEGRADED --> READY: Service recovered
    DEGRADED --> FAILED: Critical service failed
...
- **Adapter** - Implements IDBAdapter for specific database (MongoDB, PostgreSQL, Drizzle, etc.)
...
// Future SQL/Drizzle adapter transforms to relational:
// posts table: id, slug, createdAt, updatedAt
// post_translations table:
//   - post_id, field='title', lang='en', value='Hello'
//   - post_id, field='title', lang='de', value='Hallo'
//   - post_id, field='description', lang='en', value='World'
//   - post_id, field='description', lang='de', value='Welt'
//
...
| **mongoDBCacheUtils**          | Caching utilities                   |
...
const entries = await withCache(cacheKey, async () => await db.crud.findMany('posts', { collectionId }), 180, 'CONTENT');
