...
- 🛠️ **[For Developers & AI Agents](#for-developers-and-ai-agents)** — Non-negotiable technical rules, canonical locations, frontmatter spec, and PR process.
...
All competitive comparisons and feature references in public-facing documentation must comply with **EU Directive 2006/114/EC** and **German UWG §6 (Unfair Competition Act)**:

- **Use Verifiable Sources**: Every competitive claim must cite public, objective sources (e.g. competitor documentation, CVE databases).
- **Date-Stamp Performance Metrics**: Include measurement date, hardware specifications, and reproduction commands.
- **Avoid Disparaging Language**: Describe architectural differences neutrally (e.g. "React-based VDOM reconciliation" vs. "Svelte compile-time reactivity"). Never use disparaging terms like "fail/s to", "broken", or "suffer/s from".
- **Qualify Absolutes**: Claims like "first", "best", or "only" must be qualified (e.g., "to our knowledge", "based on public documentation as of [Date]").
...
> 6. **Widget API Sync Rule**: Any change to `src/routes/api/[...path]/handlers/widgets.ts` or `src/stores/widget-store.svelte.ts` — adding, removing, or renaming an endpoint, or changing a response shape — **must** update `/docs/api/Widget_API.mdx` in the same PR.
>
>    This means refreshing the `updated` date, updating the status table, and removing any obsolete endpoints. Never leave these out of sync.
>
> 7. **Thin API Pattern**: All data access logic should reside in the **Local SDK** (`src/routes/api/cms.ts`) or a dedicated service. The Unified Gatekeeper handlers in `src/routes/api/[...path]/handlers/` should only handle request parsing, calling `locals.cms`, and returning a JSON response.
...
> 9. **"Next Steps" footer required**: Every guide and architecture page **must** end with a `## Next Steps` or `## Related` section linking to 2-3 relevant pages. This keeps users flowing through the docs instead of dead-ending.
> 10. **Frontmatter `updated` date**: Update the `updated` field in frontmatter with every content change. Stale dates erode trust.
...
| **Concepts**         | `/docs/concepts/`                                  | Mental model overviews — 3-4 short pages.        |
| **Migration Guides** | `/docs/guides/migration/`                          | "Moving from X" guides (WordPress, Strapi, etc). |
...
| :------------ | :----- | :------------------------------------------------------------- | :------------------------------------------ |
| `path`        | string | File path from project root for routing and identification.    | `"docs/api/Collection_API.mdx"`             |
| `title`       | string | H1 heading used in nav, browser tab, and search results.       | `"Collection API"`                          |
| `description` | string | SEO summary for link previews.                                 | `"CRUD operations for collection entries."` |
| `order`       | number | Sidebar position relative to siblings. Lower = higher.         | `15`                                        |
...
| `created`     | string | Creation date in `YYYY-MM-DD`. Set once, never change.         | `"2025-10-05"`                              |
| `updated`     | string | Last modification date. Update with every PR.                  | `"2026-03-27"`                              |
| `tags`        | array  | Lowercase kebab-case keywords for search.                      | `["api", "collections", "crud"]`            |
...
4. **Audit & CI parity**: Run the full check suite and Audit matrix (if code changed).

```

# Run pre-commit checks (staged files, ~2-3s)

bun run gate:fast

# Run pre-push checks (type check + unit tests, ~15-45s)
...
# Run performance audit (if applicable)
...
7. **AI agent commits**: Commit messages authored or co-authored by AI agents **must NOT** include `Co-Authored-By` or AI tags, keeping all commit histories clean and uniform.
...
- [ ] Linter passes: `bun ./tests/lint-docs.ts` and `bun run slop`
- [ ] **915+ Test Suite passes**: All unit tests are green (`bun run test:unit`).
- [ ] **Audit Budget verified**: No regressions in benchmark matrix.
- [ ] **If `src/routes/api/widgets/` or `src/stores/widget-store.svelte.ts` changed**: `Widget_API.mdx` updated in this same PR.
- [ ] Rendered correctly in the CMS preview.
...
- [CI/CD Performance Metrics](../tests/ci-performance-metrics.mdx)
