...
> 5.  **Widget API Sync**: Any change to `src/routes/api/widgets/` or `src/stores/widgetStore.svelte.ts` (add/remove/rename endpoint, response shape change) MUST update `/docs/api/Widget_API.mdx` in the same PR (refresh `updated`, status table, remove obsolete endpoints). Never leave docs out-of-sync.
