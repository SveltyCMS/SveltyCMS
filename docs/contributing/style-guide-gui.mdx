...
- Deterministic unique IDs for ARIA/controls pairing: use Svelte 5's `$props.id()` rune (`const uid = $props.id()`), never `crypto.randomUUID()` at render time which creates SSR/hydration ID mismatches
- Localized date/time display: use `formatDate()`, `formatDateTime()`, `formatTime()` from `@utils/format-date` or `@utils/date` passing `systemLanguage` to avoid hydration text flash
...
- ❌ `crypto.randomUUID()` inside component scripts or prop defaults for DOM/ARIA IDs — use `$props.id()` rune
- ❌ Unpinned `date.toLocaleDateString()` / `date.toLocaleString()` without explicit locale — use `@utils/format-date` helpers
