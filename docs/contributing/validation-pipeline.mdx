---
title: "Validation Pipeline"
description: "SveltyCMS tiered validation architecture for contributors — fast commits, safe pushes, deterministic CI"
path: "contributing/validation-pipeline"
updated: "2026-06-25"
---

# Validation Pipeline

SveltyCMS uses a **three-tier validation architecture** that balances developer velocity with code safety. Every tier catches a different class of defect at the right stage of the development lifecycle.

## Architecture Overview

```mermaid
flowchart TB
    subgraph FAST["⚡ Fast Loop — Pre-commit ~2-3s"]
        direction LR
        A1["git add"] --> A2["git commit"]
        A2 --> A3["lint-staged"]
        A3 --> A4["Format staged files"]
        A3 --> A5["Lint staged files"]
        A3 --> A6["Slop scan staged files"]
    end

    subgraph MEDIUM["🛡️ Medium Loop — Pre-push ~15-45s"]
        direction LR
        B1["git push"] --> B2["quality-gate.ts"]
        B2 --> B3["Check (format + lint)"]
                B2 --> B4["Unit Tests (conditional)"]
        B2 --> B5["Full Lint + Slop"]
        B2 --> B6["Docs + Admin Lint"]
    end

    subgraph DEEP["🏗️ Deep Loop — CI Pipeline ~5-20min"]
        direction LR
        C1["GitHub PR"] --> C2["ci.yml"]
        C2 --> C3["Production Build"]
        C2 --> C4["DB Matrix x4"]
        C2 --> C5["E2E x18 Projects"]
        C2 --> C6["Benchmarks"]
    end

    FAST --> MEDIUM --> DEEP
```

## The Three Loops

| Loop       | Trigger      | What Runs                                    | Time     | Skip          |
| :--------- | :----------- | :------------------------------------------- | :------- | :------------ |
| **Fast**   | `git commit` | Format, lint, slop scan — staged files only  | ~2-3s    | `--no-verify` |
| **Medium** | `git push`   | Type check, unit tests, full lint, docs lint | ~15-45s  | `--no-verify` |
| **Deep**   | GitHub PR    | Build, DB matrix, E2E sharding, benchmarks   | ~5-20min | N/A           |

## Fast Loop — Pre-commit

When you `git commit`, the pre-commit hook runs `lint-staged` which checks **only your staged files**:

```mermaid
sequenceDiagram
    participant Dev as Developer
    participant Git as git commit
    participant LS as lint-staged
    participant Tools as oxfmt / oxlint / slop

    Dev->>Git: git commit -m "fix: ..."
    Git->>LS: Pre-commit hook
    LS->>LS: Get staged files from git
    LS->>Tools: Run on staged files only
    Tools-->>LS: Pass / Fail
    LS-->>Git: Allow / Block commit
    Git-->>Dev: ✅ Committed (~2s)
```

**What runs on staged source files** (`src/**/*.{ts,js,svelte}`):

- `oxfmt` — Format with oxfmt
- `oxlint --deny correctness --deny perf` — Critical lint rules only
- `slop-scanner.ts --strict --files` — Svelte 5 quality, RTL, accessibility, security

**What runs on staged docs** (`docs/**/*.{md,mdx}`):

- `lint-docs.ts` — Documentation compliance, EU advertising law, broken links

**Why staged files only?** Editing one variable name shouldn't trigger a scan of 500+ unrelated files.

## Medium Loop — Pre-push

When you `git push`, the pre-push hook runs `quality-gate.ts` across the **full codebase**:

```mermaid
sequenceDiagram
    participant Dev as Developer
    participant Git as git push
    participant QG as quality-gate.ts
    participant CI as GitHub Actions

    Dev->>Git: git push origin feature/x
    Git->>QG: Pre-push hook
    QG->>QG: Detect changed file types
    QG->>QG: Format verification
    QG->>QG: Full lint + slop scan
    QG->>QG: check (format + lint)
    QG->>QG: all unit tests (conditional on file changes)
    QG-->>Git: Allow / Block push
    Git->>CI: Push reaches remote
    Git-->>Dev: ✅ Pushed (~30s)
```

**What runs:**

- Format verification — ensures tree is clean
- Slop Scanner — full `src/` scan
- Lint — full project lint
- Admin Theme Lint — if admin routes changed
- Docs Lint — if documentation changed
- Check — runs `bun run check` (format verification + oxlint, not svelte-check)
- Unit Tests — all tests (conditionally skipped when only docs changed)

**What does NOT run** (deferred to CI):

- ❌ Production builds
- ❌ Integration tests (Docker DB containers)
- ❌ E2E tests (Playwright browser automation)
- ❌ Benchmarks

## Deep Loop — CI Pipeline

The GitHub Actions workflow provides **deterministic verification** on Ubuntu Linux runners:

```mermaid
flowchart LR
    subgraph Bootstrap
        B1["Install deps"]
        B2["Codegen"]
    end

    subgraph Quality["Quality Gates — parallel"]
        Q1["Format"]
        Q2["Lint"]
        Q3["Type Check"]
        Q4["Unit Tests"]
        Q5["Dep Audit"]
    end

    subgraph Build
        BD["Production Build"]
    end

    subgraph Integration["DB Matrix — parallel"]
        D1["SQLite"]
        D2["MongoDB"]
        D3["MariaDB"]
        D4["PostgreSQL"]
    end

    subgraph E2E["E2E — sharded, parallel"]
        E1["Wizard"]
        E2["Auth"]
        E3["RBAC"]
        E4["Media"]
        E5["... 14 more"]
    end

    Bootstrap --> Quality
    Bootstrap --> Build
    Build --> Integration
    Build --> E2E
```

## Manual Commands

| Command              | What It Does                             |
| :------------------- | :--------------------------------------- |
| `bun run gate:fast`  | Run pre-commit checks manually           |
| `bun run gate`       | Run pre-push checks manually             |
| `bun run test:smart` | Run only tests affected by your git diff |
| `bun run ci:local`   | Simulate the full CI pipeline locally    |
| `bun run slop`       | Run full slop scanner                    |
| `bun run test:unit`  | Run all unit tests                       |

## Skipping Hooks

```bash
git commit --no-verify   # Skip pre-commit (fast loop)
git push --no-verify     # Skip pre-push (medium loop)
```

> [!WARNING]
> Skipping hooks is acceptable for WIP branches, but CI will still catch all issues on PR. Never merge a PR that fails CI.

## Why This Architecture?

This follows the same pattern used by **SvelteKit**, **Vite**, **Prisma**, and **Next.js**:

```mermaid
quadrantChart
    title Validation Speed vs Scope
    x-axis "Narrow Scope" --> "Full Scope"
    y-axis "Slow" --> "Fast"
    quadrant-1 "Sweet spot"
    quadrant-2 "Pre-commit ideal"
    quadrant-3 "Anti-pattern"
    quadrant-4 "CI territory"
    "lint-staged": [0.2, 0.95]
    "quality-gate": [0.7, 0.6]
    "ci.yml": [0.95, 0.15]
    "Old pre-commit": [0.9, 0.05]
```

- **SvelteKit** uses `lint-staged` on pre-commit; browser automation tests run only in CI
- **Prisma** does not force developers to spin up Docker containers locally; the DB matrix is CI-only
- **Vite** runs formatting/linting on pre-commit; the massive integration test suite runs concurrently in CI

The key insight: **local machines vary wildly** (Windows/macOS/Linux, different specs), but **CI provides deterministic environments** where parallelized cloud runners handle long-running processes.
