...
SveltyCMS dynamically generates a comprehensive **OpenAPI 3.1.0** (formerly Swagger) specification. This document acts as the machine-readable "source of truth" for your entire API surface, including dynamic collections and system handlers.
...
| **Auth Required** | No (Specification is public, Endpoints are guarded) |

---

## 1. The Goal

Enable **Automated SDK Generation** and **Contract-First Development**. By providing a standard OpenAPI spec, SveltyCMS allows developers to generate type-safe clients for any programming language (TypeScript, Python, Go, Rust, etc.) without manually writing HTTP wrappers.
...
The specification is not static. Whenever you create a new collection in the **Collection Builder**, the OpenAPI service automatically adds:
...
For collections with **translated fields**, the specification accurately describes the multilingual object structure:
...
- [🔐 **API Access Tokens**](./api-access-tokens.mdx)
