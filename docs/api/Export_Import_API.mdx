...
curl -X GET https://old.example.com/api/export/full > backup.json.enc

# Import to PostgreSQL instance
curl -X POST https://new.example.com/api/import/full \
  -d @backup.json.enc
```
...
```bash
# Export tenant data
curl -X GET "https://cms.example.com/api/export/full?tenantId=tenant-a" \
  > tenant-a-backup.json.enc

# Import to new tenant
curl -X POST https://new-cms.example.com/api/import/full \
...
```javascript
// Export full system
const exportResponse = await fetch('/api/export/full?format=json', {
	credentials: 'include'
});
...
// Import data
const importResponse = await fetch('/api/importData', {
	method: 'POST',
	credentials: 'include',
...
- `src/routes/api/export/full/+server.ts` - Full export endpoint
