...
The Media API provides a robust and type-safe system for managing media files (images, videos, documents) in SveltyCMS. All operations are handled by the **MediaService** and **MediaProcessingService**, which use a database-agnostic adapter and support multi-tenancy.
