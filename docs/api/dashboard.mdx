...
Dashboard endpoints use lightweight queries and cached data where possible. The metrics and cache-metrics endpoints aggregate pre-computed statistics rather than scanning large datasets. Recent content uses cross-collection search with a configurable limit (default 5).
