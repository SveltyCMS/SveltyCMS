...
await cms.websiteTokens.create({ name: "API Key", permissions: ["content:read"], user, tenantId });
...
const result = await cms.collections.bulkCreate("products", largeDataArray, { tenantId });
```
