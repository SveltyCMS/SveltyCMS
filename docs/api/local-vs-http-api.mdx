...
| :-------------------------------------- | :-------------------------------------- | :--------- | :------------------------------------------------------------- |
| **Client-side** (`.svelte`, `+page.ts`) | **HTTP / [GraphQL API](./graphql.mdx)** | 50–150 ms  | Browser environment; requires auth, CORS, and network.         |
...
The `locals.cms` object provides a unified, typed interface to all system capabilities:

```typescript
// Content Management
await locals.cms.collections.find("posts", { limit: 10 });
await locals.cms.media.find("images", { limit: 5 });

// Identity & Access
await locals.cms.auth.listUsers();
await locals.cms.websiteTokens.create({
  /* ... */
});

// System & Infrastructure
await locals.cms.system.getPreference("theme");
await locals.cms.widgets.list();

// Advanced APIs
await locals.cms.graphql("{ posts { title } }");
