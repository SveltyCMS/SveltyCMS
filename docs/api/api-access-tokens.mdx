...
description: 'Secure programmatic access to SveltyCMS APIs via API access tokens for external integrations.'
...
> [!NOTE]
> For management of **Website Tokens** (long-lived integrations), please refer to the **[Access Management Guide](/docs/guides/configuration/access-management.mdx#website-tokens)**.
...
export async function syncToSveltyCMS(data) {
	const response = await fetch('https://your-cms.com/api/import/full', {
		method: 'POST',
		headers: {
...
- [Token API Endpoints](./token-api-endpoints.mdx) - Detailed endpoint documentation
