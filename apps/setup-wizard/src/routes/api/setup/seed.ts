...
export async function seedCollectionsForSetup(
	...
	try {
		...
		logger.debug(
			`⏱️  Collection scan: \x1b[32m${scanTime.toFixed(2)}ms\x1b[0m (found ${collections.length})`
		);
		...
		for (const schema of collections) {
			try {
				...
				logger.info(
					`✅ Created collection model: ${schema.name || 'unknown'} (\x1b[33m${createTime.toFixed(2)}ms\x1b[0m)`
				);
				successCount++;
				...
				if (!firstCollection && schema.path && schema.name) {
					...
					logger.debug(
						`First collection identified: \x1b[34m${collectionName}\x1b[0m at ${collectionPath}`
					);
				}
			} catch (error) {
				...
				if (errorMessage.includes('already exists') || errorMessage.includes('duplicate')) {
					...
					logger.error(
						`❌ Failed to create collection '${schema.name || 'unknown'}': ${errorMessage}`
					);
		...
		if (error instanceof Error) {
			logger.error(
				`Failed to seed collections after \x1b[32m${overallTime.toFixed(2)}ms\x1b[0m: ${error.message}`
			);
			if (error.stack) {
				logger.debug('Stack trace:', error.stack);
			}
			...
			logger.error(
				`Failed to seed collections after \x1b[32m${overallTime.toFixed(2)}ms\x1b[0m:`,
				error
			);
		}
...
export const defaultPublicSettings: Array<{ key: string; value: unknown; description?: string }> = [
	...
	{ key: 'DEFAULT_THEME_NAME', value: 'SveltyCMSTheme', description: 'Name of the default theme' },
	{
		...
		value: '/src/themes/SveltyCMS/SveltyCMSTheme.css',
...
export async function seedSettings(dbAdapter: DatabaseAdapter): Promise<void> {
	...
	logger.info(
		`🌱 Seeding \x1b[34m${settingsToSeed.length}\x1b[0m missing settings (${Object.keys(existingSettings).length} already exist)...`
	);
...
export async function exportSettingsSnapshot(
	...
	const settingsResult = await dbAdapter.systemPreferences.getMany(
		allSettingKeys,
		'system' as string
	);
