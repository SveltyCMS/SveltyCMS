...
export const POST: RequestHandler = async ({ request, cookies, url }) => {
	...
	try {
		...
		try {
			...
			logger.info('✅ Database adapter and Auth service initialized successfully', {
				correlationId
			});
		} catch (initError) {
		...
		try {
			...
			if (existingUser && existingUser._id) {
				...
				logger.debug('User attributes updated, fetching updated user...', {
					correlationId,
					userId: existingUser._id
				});
				...
				logger.debug('Updated user retrieved, creating session...', {
					correlationId,
				...
				logger.info('Creating new admin user and session (optimized)', {
					correlationId,
					email: admin.email
				});
			...
			if (!adminUser || !session) {
				throw new Error(
					`Missing data after auth operations - adminUser: ${!!adminUser}, session: ${!!session}`
				);
			}
		...
		try {
			...
			if (firstCollection && firstCollection.path) {
				...
				logger.info(
					`✨ Fast redirect using pre-seeded collection: \x1b[34m${firstCollection.name}\x1b[0m at \x1b[34m${redirectPath}\x1b[0m`,
					{
						correlationId
					}
				);
