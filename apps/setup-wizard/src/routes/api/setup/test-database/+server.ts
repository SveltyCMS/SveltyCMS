...
async function testMongoDbConnection(dbConfig: DatabaseConfig) {
	...
	if (!importedUtils || typeof importedUtils.buildDatabaseConnectionString !== 'function') {
		logger.error(
			'buildDatabaseConnectionString is missing or not a function after import:',
			importedUtils
		);
		throw new Error(
			'Internal error: buildDatabaseConnectionString is not available. Check for circular dependencies or build issues.'
		);
	}
	...
	try {
			...
			return json(
				{
					...
					userFriendly:
						'MongoDB Atlas requires a username and password. Please provide your database credentials.',
					classification: 'credentials_required',
					latencyMs: durationMs,
					details:
						'Atlas clusters always require authentication. Please provide your database username and password.'
				},
				{ status: 400 }
			);
		}
		...
		try {
			...
			if (status?.authInfo?.authenticatedUsers)
		...
		if (authProvided && !warnings.includes('ping_failed')) {
			...
			success = true;
			message = authProvided
				? m.api_db_test_success_authenticated()
				: m.api_db_test_success_implicit_auth();
			if (!authProvided) {
		...
		resultPayload = {
			...
			collectionsSample,
			stats: dbStats
				? { collections: dbStats.collections, objects: dbStats.objects, dataSize: dbStats.dataSize }
				: null,
			authenticatedUsers,
			classification,
			userFriendly
		...
		if (conn) {
			await conn
				.close()
				.catch((closeError) => logger.warn('Failed to close connection after error:', closeError));
		}
		...
		if (conn) {
			await conn
				.close()
				.catch((closeError) => logger.warn('Failed to close connection:', closeError));
		}
	}
...
async function testPostgresConnection(dbConfig: DatabaseConfig) {
	...
	try {
			...
			return json(
				{
					...
					details:
						'To test PostgreSQL connections, install the postgres package: npm install postgres'
				},
				{ status: 400 }
			);
		}
		...
		const { classification, raw, userFriendly } = classifyDatabaseError(
			error,
			'postgres',
			dbConfig
		);
			...
			return json(
				{
					...
					details:
						'To test MySQL/MariaDB connections, install the required packages: npm install mysql2 mariadb'
				},
				{ status: 400 }
			);
		}
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const {
			...
			issues,
			output: dbConfig
		} = safeParse(databaseConfigSchema, body);
		if (!validationSuccess || !dbConfig) {
			logger.error('Invalid database configuration received for testing.', { issues });
			return json(
				{ success: false, error: 'Invalid database configuration.', details: issues },
				{ status: 400 }
			);
		}
