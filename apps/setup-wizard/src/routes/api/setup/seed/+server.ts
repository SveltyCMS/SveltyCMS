...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const firstCollection =
			collections.length > 0 ? { name: collections[0].name, path: collections[0].path } : null;
		logger.info(
			`Found ${collections.length} collections. First collection for redirect: ${firstCollection?.name}`
		);
		...
		const seedProcess = async () => {
			try {
				...
				logger.info(
					'🌱 Starting background seeding of default data (settings, themes, collections)...'
				);
