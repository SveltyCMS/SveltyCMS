 ...
 * - Monorepo-aware: Finds workspace root automatically
 */
...
async function findWorkspaceRoot(startDir: string = process.cwd()): Promise<string> {
	...
	while (currentDir !== path.parse(currentDir).root) {
		try {
			...
			await fs.access(path.join(currentDir, 'nx.json'));
			return currentDir;
		} catch {
	...
	// Fallback to process.cwd() if no workspace root found
	return process.cwd();
}
...
export async function compile(options: CompileOptions = {}): Promise<void> {
	...
	// Define collection paths relative to workspace root
	const {
		userCollections = path.posix.join(workspaceRoot, 'config/collections'),
		compiledCollections = path.posix.join(workspaceRoot, 'compiledCollections')
	} = options;
