...
<div class="space-y-6">
		...
		<label class="label">
			<div class="mb-1 flex items-center gap-1 text-sm font-medium">
				...
				<button
					...
					title={m.setup_email_help_provider()}
		...
		<label class="label">
			<div class="mb-1 flex items-center gap-1 text-sm font-medium">
				...
				<button
					...
					title={m.setup_email_help_password()}
					class="ml-1 text-slate-400 hover:text-tertiary-500 hover:dark:text-primary-500"
				>
					<iconify-icon icon="mdi:help-circle-outline" width="14" aria-hidden="true"></iconify-icon>
				</button>
	...
	<div class="space-y-3">
			...
			<div class="card preset-outlined-primary-500 p-4">
				...
				<div class="flex items-start gap-3">
					...
					<button
						...
						aria-expanded={showSuccessDetails}
						aria-label={showSuccessDetails ? 'Hide details' : 'Show details'}
					>
						<iconify-icon icon={showSuccessDetails ? 'mdi:chevron-up' : 'mdi:chevron-down'} class="text-2xl text-surface-500" aria-hidden="true"
						></iconify-icon>
					</button>
				</div>
				...
				{#if showSuccessDetails}
					<div class="ml-9 mt-2 space-y-2 text-sm text-surface-600 dark:text-surface-300">
						<p>{m.setup_email_test_success_desc()}</p>
						{#if testEmailSent}
							<p>
								{m.setup_email_test_email_sent({ email: wizard.adminUser.email })}
							</p>
						{/if}
						<div class="flex gap-2 pt-2">
							<span class="badge preset-filled-success-500">{m.setup_email_badge_verified()}</span>
							<span class="badge preset-filled-surface-500">{m.setup_email_badge_saved()}</span>
						</div>
					</div>
				{/if}
			</div>
		...
		{#if testError}
			<div class="card preset-outlined-error-500 flex items-start gap-3 p-4" role="alert">
				<iconify-icon icon="mdi:alert-circle" class="mt-0.5 text-xl text-error-500" aria-hidden="true"></iconify-icon>
				<div>
					<p class="font-semibold text-error-700 dark:text-error-300">{m.setup_email_connection_failed()}</p>
					<p class="mt-1 text-sm text-error-600 dark:text-error-200">{testError}</p>
				</div>
			</div>
		{/if}
	</div>
