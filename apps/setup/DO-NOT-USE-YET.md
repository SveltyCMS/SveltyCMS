...
❌ Do not run `nx dev setup` or `bun dev` from this directory  
...
1. **Migration Step**: Code from `src/routes/setup` is moved to `apps/setup/src/routes`
2. **Structure Created**: 
   ```
   apps/setup/
   ├── src/
...
4. **Paraglide Setup**: i18n messages configured for setup workspace
...
nx dev setup         # ❌ No code migrated yet
cd apps/setup && bun dev  # ❌ No src/ directory
```
