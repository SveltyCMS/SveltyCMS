/**
 * @file apps/cms/src/content/ContentManager.ts
...
import type { ContentNode, Schema, ContentNodeOperation, DatabaseId, IContentManager, NavigationNode } from '@cms-types';
...
import { CacheCategory } from '@shared/database/CacheCategory'; // ✅ Safe for client - no Redis imports
...
class ContentManager implements IContentManager {
	 ...
	 */
	public async getFirstCollection(tenantId?: string, forceRefresh: boolean = false): Promise<Schema | undefined | null> {
		// Check cache first
		const now = Date.now();
		if (
			!forceRefresh &&
	...
	public async getCollection(identifier: string, tenantId?: string): Promise<Schema | undefined | null> {
		if (this.initState !== 'initialized') {
			throw new Error('ContentManager is not initialized.');
		}
