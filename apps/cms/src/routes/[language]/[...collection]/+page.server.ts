/**
 * @file apps/cms/src/routes/[language]/[...collection]/+page.server.ts
...
export const load: PageServerLoad = async ({ locals, params, url }) => {
	...
	try {
		...
		if (isUUID) {
			...
			currentCollection = await contentManager.getCollectionById(collection!, tenantId);
			...
			currentCollection = await contentManager.getCollection(collectionPath, tenantId);
		}
		...
		if (editEntryId && currentCollection.revision && currentCollection._id) {
			try {
