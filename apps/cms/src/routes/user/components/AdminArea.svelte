<!-- 
@file shared/features/src/user/components/AdminArea.svelte
@component 
...
<script lang="ts">
	...
	function normalizeMediaUrl(url: string | null | undefined): string {
		...
		try {
			...
			// Handle Default_User.svg specifically to be safe
			if (url.toLowerCase().includes('default_user.svg')) return DEFAULT_AVATAR;
			...
			if (clean === 'files' || clean === '') return DEFAULT_AVATAR;
	...
	function modalTokenUser() {
		modalState.trigger(
			ModalEditToken as any,
			{
				title: m.adminarea_createtoken_title(),
				body: m.multibuttontoken_modalbody(),
...
<div class="flex flex-col">
	...
	<div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
		<button onclick={modalTokenUser} aria-label={m.adminarea_emailtoken()} class="gradient-primary btn w-full items-center text-white sm:w-auto">
			<iconify-icon icon="material-symbols:mail" color="white" width="18" class="mr-1"></iconify-icon>
			<span class="whitespace-nowrap">{m.adminarea_emailtoken()}</span>
		</button>
		...
		>
			<button
				...
				class="gradient-secondary btn w-full items-center text-white sm:w-auto"
			>
				<iconify-icon icon="material-symbols:key-outline" color="white" width="18" class="mr-1"></iconify-icon>
				<span class="whitespace-nowrap">{showUsertoken ? m.adminarea_hideusertoken() : m.adminarea_showtoken()}</span>
			</button>
			...
			{#if showUsertoken && !showUserList && tableData}
				...
				{#if expiredTokens.length > 0}
					...
					>
						...
						<span class="whitespace-nowrap">{showExpiredTokens ? 'Hide Expired' : 'Show Expired'}</span>
					</button>
				{/if}
			{/if}
		</PermissionGuard>
		...
		<button
			...
			class="gradient-tertiary btn w-full items-center text-white sm:w-auto"
		>
			<iconify-icon icon="mdi:account-circle" color="white" width="18" class="mr-1"></iconify-icon>
			<span class="whitespace-nowrap">{showUserList ? m.adminarea_hideuserlist() : m.adminarea_showuserlist()}</span>
		</button>
	</div>
									...
									<th>
										<div class="flex items-center justify-between">
											<FloatingInput
												...
												name={header.key}
												onInput={(value: string) => handleInputChange(value, header.key)}
											/>
										</div>
									</th>
								{/each}
