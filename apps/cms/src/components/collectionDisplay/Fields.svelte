<!--
@file shared/components/src/collectionDisplay/Fields.svelte
@component
...
<script lang="ts">
	...
	import { widgetFunctions as widgetFunctionsStore } from '@cms/stores/widgetStore.svelte';
	...
	import { activeInputStore } from '@cms/stores/activeInputStore.svelte.js';

	// Token Picker
	import TokenPicker from '@cms/components/TokenPicker.svelte';
...
<Tabs value={localTabSet} onValueChange={(e) => (localTabSet = e.value)} class="flex flex-1 flex-col items-center">
	...
	>
		...
		{#if collection.value?.revision}
			<Tabs.Trigger value="1" class="flex-1">
				<div class="flex items-center justify-center gap-2 py-2">
					...
					{m.applayout_version()} <span class="preset-filled-secondary-500 badge">{revisions?.length ?? 0}</span>
				</div>
			</Tabs.Trigger>
		{/if}
		...
		{/if}
		<Tabs.Indicator class="preset-filled-primary-500 rounded-md" />
	</Tabs.List>
							...
							{#if field.widget}
								...
								{#if loadedWidget}
									...
									{#key currentContentLanguage}
										...
										<!-- Show inline preview for tokens -->
										<TokenPreview value={currentCollectionValue[fieldName]} entry={currentCollectionValue} />
									{/key}
								{:else}
		...
		<div class="p-4">
			{#if !revisions || revisions.length === 0}
				<div class="flex flex-col items-center justify-center p-8 text-center text-surface-500">
					<iconify-icon icon="mdi:history" width="48" class="mb-2 opacity-50"></iconify-icon>
					<p class="font-semibold">No Revision History</p>
					<p class="text-sm">Revisions will appear here after you save changes to this entry.</p>
				</div>
			{:else}
				...
				<div class="rounded-lg border p-4 dark:text-surface-50">
					<h3 class="test-center mb-3 text-lg font-bold text-tertiary-500 dark:text-primary-500">Changes from Selected Revision</h3>
					{#if selectedRevision}
						...
						{:else if selectedRevisionId}
							<p class="text-center text-surface-500 dark:text-surface-50">No differences found.</p>
						{:else}
							<p class="text-center text-surface-500 dark:text-surface-50">Select a revision to see what's changed.</p>
						{/if}
					{:else}
						<p class="text-center text-surface-500 dark:text-surface-50">Select a revision to see what's changed.</p>
					{/if}
