<!--
@file shared/components/src/collectionDisplay/EntryList.svelte
@component
...
<script lang="ts">
	...
	// Stores
	import { collection, collectionValue, mode, setCollectionValue, setMode, setModifyEntry, statusMap } from '@cms/stores/collectionStore.svelte';
	...
	function onFilterChange(filterName: string, value: string) {
		filterDebounce(() => {
			...
			Object.entries(newFilters).forEach(([key, val]) => {
				filterUpdates[`filter_${key}`] = (val as string) || null;
			});
	...
	setModifyEntry(async (status?: keyof typeof statusMap): Promise<void> => {
		...
		showStatusChangeConfirm({
			...
			onConfirm: async () => {
				await setEntriesStatus(selectedIds as string[], status as StatusType, onActionSuccess);
			}
		});
	});
	...
	const onPublish = () => setEntriesStatus(getSelectedIds() as string[], StatusTypes.publish, onActionSuccess);
	const onUnpublish = () => setEntriesStatus(getSelectedIds() as string[], StatusTypes.unpublish, onActionSuccess);
	const onDraft = () => setEntriesStatus(getSelectedIds() as string[], StatusTypes.draft, onActionSuccess);
	const onDelete = (isPermanent = false) => {
		const selectedIds = getSelectedIds() as string[];
		if (!selectedIds.length) {
			showToast('No entries selected', 'warning');
			return;
		}
			...
			onConfirm: async () => {
				try {
					if (willDelete) {
						...
						await setEntriesStatus(selectedIds as string[], StatusTypes.archive, () => {});
					}
					onActionSuccess();
				} catch (error) {
	...
	const onSchedule = (date: string) => {
		...
		setEntriesStatus(getSelectedIds() as string[], StatusTypes.draft, onActionSuccess, payload);
	};
											...
											{:else if (header as TableHeader).name === 'createdAt' || (header as TableHeader).name === 'updatedAt'}
												<div class="flex flex-col text-xs">
													<div class="font-semibold">
														{formatDisplayDate(entry[(header as TableHeader).name] as string | number | Date, 'en', {
															year: 'numeric',
															month: 'short',
															day: 'numeric'
														})}
