<!--
@file shared/components/src/media/tagEditor/TagEditorModal.svelte
@component
				...
				<section class="p-3 border border-primary-500/30 rounded bg-primary-50/50 dark:bg-primary-900/10">
					<div class="flex justify-between items-center mb-2">
						...
						{#if !file.metadata?.aiTags?.length}
							<button class="btn btn-sm preset-filled-secondary" onclick={handleAITagging} disabled={isGenerating}>
								{#if isGenerating}
					...
					<div class="flex flex-wrap gap-2 mb-3">
						{#if file.metadata?.aiTags?.length}
							{#each file.metadata.aiTags as tag, i}
								...
								{:else}
									<button
										class="badge preset-filled-secondary flex items-center gap-1 cursor-pointer hover:ring-2 hover:ring-secondary-300"
					...
					<div class="flex gap-2">
						...
						/>
						<button class="btn btn-sm preset-filled-surface" onclick={addManualTag} disabled={!newTagInput.trim()} aria-label="Add Tag">
					...
					{#if file.metadata?.aiTags?.length}
						<div class="mt-3 pt-3 border-t border-primary-500/20">
							<button class="btn btn-sm preset-filled-success w-full" onclick={saveAITags} disabled={isSaving}>
					...
					<div class="flex flex-wrap gap-2">
						{#if file.metadata?.tags?.length}
							{#each file.metadata.tags as tag, i}
								...
								{:else}
									<button
										class="badge preset-filled-surface-500 flex items-center gap-1 cursor-pointer hover:ring-2 hover:ring-surface-400"
