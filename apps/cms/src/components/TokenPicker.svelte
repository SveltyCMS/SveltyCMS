...
<script lang="ts">
	import { activeInput } from '@cms/stores/activeInputStore.svelte.js';
	import { TokenRegistry, replaceTokens } from '@cms/services/token/engine';
	import { modifierMetadata } from '@cms/services/token/modifiers';
	import { page } from '$app/state';
	import { collection, collectionValue } from '@cms/stores/collectionStore.svelte.js';
	import { ui } from '@cms/stores/UIStore.svelte.js';
	import { publicEnv } from '@shared/stores/globalSettings.svelte';
	import { fade, slide } from 'svelte/transition';
	import { nowISODateString } from '@shared/utils/dateUtils';
	import type { TokenDefinition, ModifierMetadata } from '@cms/services/token/types';
...
{#if activeInput.current}
	<div
		class="token-window card fixed z-9999 flex max-h-[80vh] w-full max-w-md flex-col overflow-hidden rounded border border-surface-200-700 bg-surface-100-900 p-4 shadow-xl"
	...
	>
						...
						{#if openCategories[cat] || search}
							<div transition:slide class="mt-2 space-y-1">
								{#each tokens as t}
									...
									<div
										class="card preset-filled-surface-500 hover:preset-filled-secondary cursor-pointer p-2 transition-colors"
									...
									>
										<div class="flex items-start justify-between gap-2">
											<div class="flex-1 text-left">
												<div class="text-sm font-medium dark:text-primary-500">{t.name}</div>
												<code class="code text-[10px] opacity-70">{t.token}</code>
											</div>
											<div class="flex items-center gap-1">
												<span class="badge preset-tonal-secondary text-[10px] uppercase">{t.type}</span>

												<Tooltip positioning={{ placement: 'left' }}>
													<Tooltip.Trigger>
														<button
															onclick={(e: MouseEvent) => toggleInfo(t.token, e)}
															class="btn-icon btn-icon-sm preset-tonal-surface-500"
													...
													<Portal>
														<Tooltip.Positioner>
															<Tooltip.Content class="card w-64 p-4 shadow-xl z-20 bg-surface-100-800-token border border-surface-500/30">
																<div class="text-xs">
		...
		{:else if selectedToken}
			<div class="flex-1 space-y-4 overflow-y-auto pr-2">
				<!-- Token Info -->
				<div class="card bg-primary-500 border border-primary-500/30 p-4">
				...
				{#if selectedModifiers.length > 0}
					<div class="space-y-2">
						...
						{#each selectedModifiers as mod, i}
							<div class="card preset-surface-500 group relative p-3">
								<div class="mb-2 flex items-center justify-between">
				...
				<div>
					...
					<div class="flex flex-wrap gap-2">
						{#each availableModifiers as m}
							<button onclick={() => addModifier(m)} class="chip preset-filled-surface-500 hover:preset-filled-secondary transition-colors">
			...
			<div class="mt-4 space-y-3 border-t border-surface-500/30 pt-4">
				...
				<div class="flex gap-2">
					<button onclick={deleteToken} class="btn preset-tonal-error" title="Clear input">
						<iconify-icon icon="mdi:trash-can-outline"></iconify-icon>
					</button>
