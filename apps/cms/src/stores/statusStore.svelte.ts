/**
 * @file shared/stores/src/statusStore.svelte.ts
...
import { contentLanguage } from '@shared/stores/store.svelte'; // Check path
...
function getEffectiveStatus(): StatusType {
	...
	if (isPerLocaleEnabled() && cv?.statusByLocale) {
		const localeStatus = (cv.statusByLocale as Record<string, string>)[contentLanguage.value];
		if (localeStatus) {
...
export const statusStore = {
	...
	async toggleStatus(newValue: boolean, componentName: string = 'Component'): Promise<boolean> {
		...
		try {
			...
			if (collections.activeValue?._id && collections.active?._id) {
				...
				const result = await updateEntryStatus(String(collections.active._id), String(collections.activeValue._id), newStatus, payload);
				...
				if (result.success) {
					...
					showToast(
						newValue
							? perLocale
								? `Entry published for ${locale}`
								: 'Entry published successfully'
							: perLocale
								? `Entry unpublished for ${locale}`
	...
	get isScheduled(): boolean {
		...
		if (perLocale && cv?._scheduledByLocale) {
			return !!(cv._scheduledByLocale as Record<string, string>)[contentLanguage.value];
		}
