import { g as getSystemState, S as SERVICE_BASELINE_TIMES } from './state.js';
function getHealthCheckReport() {
	...
	return {
		...
		initializationTime:
			state.initializationCompletedAt && state.initializationStartedAt ? state.initializationCompletedAt - state.initializationStartedAt : void 0,
...
function identifyBottlenecks() {
	...
	Object.entries(state.services).forEach(([name, service]) => {
		const baseline = SERVICE_BASELINE_TIMES[name];
...
function getRecommendedTimeouts() {
	const state = getSystemState();
...
export { getPerformanceSummary as a, getRecommendedTimeouts as b, getHealthCheckReport as g, identifyBottlenecks as i };
