const name = 'sveltycms';
const description = 'A powerful Headless CMS with sveltekit power';
const version = '0.0.6';
const type = 'module';
const engines = { node: '>=22.0.0' };
const scripts = {
	'-- DEVELOP -------------------------------------------': '',
	dev: 'bun scripts/smart-dev.ts',
	devv: "LAUNCH_EDITOR='code' vite --open",
	'-- BUILD ---------------------------------------------': '',
	build: 'bun run build:all',
	'build:setup': 'nx build setup --configuration=production',
	'build:cms': 'nx build cms --configuration=production',
	'build:all': 'nx run-many --target=build --all --configuration=production',
	'build:server': 'bun run build:all',
	'-- CHECK & TEST ---------------------------------------': '',
	check: 'svelte-kit sync && svelte-check --tsconfig ./tsconfig.json --threshold error',
	'check:watch': ' svelte-kit sync && svelte-check --tsconfig ./tsconfig.json --watch',
	test: 'bun run test:unit',
	'test:unit': 'bun test --preload ./tests/bun/setup.ts tests/bun/services tests/bun/utils tests/bun/stores tests/bun/widgets',
	'test:integration': 'bun run scripts/run-integration-tests.ts',
	'test:integration:run':
		'wait-on http://localhost:4173 && bun test --preload ./tests/bun/setup.ts tests/bun/api tests/bun/databases tests/bun/hooks',
	'test:e2e': 'concurrently -k -s last "bun run dev" "bun run test:e2e:run"',
	'test:e2e:run': 'wait-on http://localhost:4173 && bun x playwright test tests/playwright',
	...
	paraglide: 'paraglide-js compile --project ./project.inlang --outdir ./shared/paraglide/src --emit-ts-declarations',
	translate: 'bun x @inlang/cli machine translate --project ./project.inlang',
	'db:push': 'drizzle-kit push',
	'db:migrate': 'drizzle-kit migrate',
	'db:studio': 'drizzle-kit studio',
	'-- NX MONOREPO ----------------------------------------': '',
	'nx:dev:setup': 'nx dev setup',
	'nx:dev:cms': 'bun scripts/smart-dev.ts cms',
	'nx:build:setup': 'nx build setup',
	'nx:build:cms': 'nx build cms',
...
const homepage = 'https://SveltyCMS.com';
const repository = { type: 'git', url: 'https://github.com/SveltyCMS/SveltyCMS' };
const bugs = { url: 'https://github.com/SveltyCMS/SveltyCMS/issues' };
const keywords = [
	'cms',
...
const devDependencies = {
	...
	'@skeletonlabs/skeleton-svelte': '^4.10.0',
...
const optionalDependencies = { mariadb: '^3.4.5', mongoose: '^9.1.4', mysql2: '^3.16.1', postgres: '^3.4.8' };
const pkg = {
	name,
	description,
	version,
