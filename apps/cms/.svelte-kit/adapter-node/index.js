...
import {
	...
	e as create_remote_key,
	p as parse_remote_arg,
	b as stringify,
	g as deserialize_binary_form,
...
import { b as base64_encode, t as text_decoder, c as text_encoder, g as get_relative_path } from './chunks/utils3.js';
...
async function handle_error_and_jsonify(event, state, options2, error2) {
	...
	return (await with_request_store({ event, state }, () => options2.hooks.handleError({ error: error2, event, status, message }))) ?? { message };
}
...
async function call_action(event, event_state, actions) {
	...
	return record_span({
		...
		fn: async (current2) => {
			...
			const result = await with_request_store({ event: traced_event, state: event_state }, () => action(traced_event));
...
function serialize_data(fetched, filter, prerendering = false) {
	...
	const attrs = ['type="application/json"', 'data-sveltekit-fetched', `data-url="${escape_html(fetched.url, true)}"`];
	if (fetched.is_b64) {
		attrs.push('data-b64');
	}
...
class BaseProvider {
	...
	get_header(is_meta = false) {
		...
		if (this.#style_src_attr.length > 0) {
			directives['style-src-attr'] = [...(directives['style-src-attr'] || []), ...this.#style_src_attr];
		}
		if (this.#style_src_elem.length > 0) {
			directives['style-src-elem'] = [...(directives['style-src-elem'] || []), ...this.#style_src_elem];
		}
		if (this.#script_src.length > 0) {
			directives['script-src'] = [...(directives['script-src'] || directives['default-src'] || []), ...this.#script_src];
		}
		if (this.#script_src_elem.length > 0) {
			directives['script-src-elem'] = [...(directives['script-src-elem'] || []), ...this.#script_src_elem];
		}
...
async function handle_remote_call(event, state, options2, manifest, id) {
	return record_span({
		...
		fn: (current2) => {
			const traced_event = merge_tracing(event, current2);
			return with_request_store({ event: traced_event, state }, () => handle_remote_call_internal(traced_event, state, options2, manifest, id));
		}
	});
}
async function handle_remote_call_internal(event, state, options2, manifest, id) {
	...
	try {
		...
		const data = await with_request_store({ event, state }, () => fn(parse_remote_arg(payload, transport)));
		return json(
			/** @type {RemoteFunctionResponse} */
			{
				type: 'result',
	...
	async function serialize_refreshes(client_refreshes) {
		...
		if (client_refreshes) {
			for (const key2 of client_refreshes) {
				...
				refreshes[key2] = with_request_store({ event, state }, () => fn2(parse_remote_arg(payload, transport)));
			}
		}
		...
		return stringify(
			Object.fromEntries(await Promise.all(Object.entries(refreshes).map(async ([key2, promise]) => [key2, await promise]))),
			transport
		);
	}
}
async function handle_remote_form_post(event, state, manifest, id) {
	return record_span({
		...
		fn: (current2) => {
			const traced_event = merge_tracing(event, current2);
			return with_request_store({ event: traced_event, state }, () => handle_remote_form_post_internal(traced_event, state, manifest, id));
		}
	});
}
							...
							return record_span({
								...
								fn: (resolve_span) => {
									return with_request_store(null, () =>
										resolve2(merge_tracing(event2, resolve_span), page_nodes, opts).then((response2) => {
	...
	async function resolve2(event2, page_nodes, opts) {
		try {
			...
			if (route) {
				...
				let response2;
				if (is_data_request) {
					response2 = await render_data(event2, event_state, route, options2, manifest, state, invalidated_data_nodes, trailing_slash);
					...
					if (!page_nodes) {
						...
						response2 = await render_page(event2, event_state, route.page, options2, manifest, state, page_nodes, resolve_opts);
					} else {
