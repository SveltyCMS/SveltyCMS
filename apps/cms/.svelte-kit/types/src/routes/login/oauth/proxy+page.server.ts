// @ts-nocheck
/**
 * @file apps/cms/src/routes/login/oauth/+page.server.ts
 * @description Server-side logic for the OAuth page.
 *
 * ### Features
 * - Handles OAuth authentication for Google
...
export const load = async ({ url, cookies, fetch, request }: Parameters<PageServerLoad>[0]) => {
	try {
		await dbInitPromise;
...
export const actions = {
	OAuth: async ({ request }: import('./$types').RequestEvent) => {
