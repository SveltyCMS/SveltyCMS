...
export const actions = {
	// Save Collection
	saveCollection: async ({ request }: import('./$types').RequestEvent) => {
		try {
	...
	// Save config
	saveConfig: async ({ request }: import('./$types').RequestEvent) => {
		try {
			const formData = await request.formData();
	...
	// Delete collection
	deleteCollections: async ({ request }: import('./$types').RequestEvent) => {
		try {
			const formData = await request.formData();
