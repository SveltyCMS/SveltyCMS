...
function createScreenSizeStores() {
  ...
  //
  const currentSize = $derived(currentSizeStore.value);
  ...
  function updateScreenSize() {
    if (typeof window !== 'undefined') {
      ...
      heightStore.set(window.innerHeight);
      currentSizeStore.set(getScreenSizeName(window.innerWidth));
      isMobileStore.set(isMobile)
      isTabletStore.set(isTablet)
      isDesktopStore.set(isDesktop)
      isLargeScreenStore.set(isLargeScreen)
    }
  }
