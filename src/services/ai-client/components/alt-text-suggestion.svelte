...
<script lang="ts">
	...
	let { imageData, mimeType, currentAltText = '', onsuggestion }: Props = $props();
	...
	const backendLabel = $derived.by(() => {
		...
		switch (suggestion.backend) {
			case 'litert':
				return 'Browser AI (LiteRT.js)';
			case 'ollama':
				return 'Server AI (Ollama)';
			default:
				return 'Unavailable';
		}
	});
...
<div class="ai-alt-text">
	...
	{:else}
		<button
			...
			title={hasManualContent
				? 'Overwrite existing alt-text with AI suggestion'
