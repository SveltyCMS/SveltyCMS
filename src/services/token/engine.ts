...
class TokenRegistryService {
  ...
  private readonly cache = new Map<
    string,
    { timestamp: number; data: Record<TokenCategory, TokenDefinition[]> }
  >();
  ...
  getTokens(
    ...
    if (config.includeSystem !== false) {
      add({
        ...
        type: "date",
        description:
          "Full ISO 8601 timestamp (e.g., 2025-11-24T14:30:00Z). Use with |date() modifier to format.",
      ...
      Object.entries(publicEnv).forEach(([key, val]) => {
        if (typeof val === "string" || typeof val === "number" || typeof val === "boolean") {
          add({
            ...
            example:
              key === "SITE_NAME" ? "{{entry.title}} | {{site.SITE_NAME}}" : `{{site.${key}}}`,
