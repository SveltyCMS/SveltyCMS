...
<script lang="ts">
	...
	function updateMapping(source: string, target: string) {
		fieldMappings = fieldMappings.map((m) => (m.source === source ? { ...m, target } : m));
	}
			...
			while (true) {
				...
				for (const block of blocks) {
					...
					if (payload.type === 'progress') {
						...
						importPercent =
							payload.percent ?? (importTotal > 0 ? (importCurrent / importTotal) * 100 : 0);
						...
						addLog(
							'success',
							`Import complete: ${payload.imported || 0} imported, ${payload.failed || 0} failed`
						);
...
>
	<AdminCard
		class="border border-surface-500/30 bg-white p-6 shadow-xs dark:border-surface-500/40 dark:bg-surface-900/20"
	>
		<div class="space-y-6">
			<!-- Step Indicators -->
			<div
				class="flex items-center justify-center gap-2 text-sm"
				aria-label="Migration wizard steps"
			>
			...
			{#if step === 1}
				{#if !file}
					...
					>
						<div class="flex flex-col items-center gap-4" in:fade={{ duration: 300 }}>
							<div
								class="flex h-16 w-16 items-center justify-center rounded-full bg-surface-500/10 dark:bg-surface-800"
							>
								<iconify-icon
							...
							<div>
								...
								<p class="mt-1 text-sm text-surface-500">
									36+ platforms: WXR, JSON, YAML, CSV, NDJSON
								</p>
							</div>
						...
						<input
							...
							type="file"
							accept=".xml,.wxr,.json,.yaml,.yml,.csv,.ndjson"
							class="hidden"
				...
				{:else}
					<!-- File Detected -->
					<div
						class="rounded border border-surface-500/30 bg-surface-500/10 p-6 dark:border-surface-500/40 dark:bg-surface-800/50"
						in:slide={{ duration: 300 }}
					>
						<div class="flex items-start justify-between">
							<div class="flex items-center gap-4">
								<div
									class="flex h-14 w-14 items-center justify-center rounded-xl bg-tertiary-500/10 dark:bg-primary-500/10"
								>
									<iconify-icon
										icon={confidenceLabels[detectedFormat] || 'mdi:file-outline'}
										width="28"
								...
								<div>
									...
									{#if detectedFormat !== 'unknown'}
										<span
											class="mt-2 inline-flex items-center gap-1 rounded-full bg-tertiary-500/10 px-2.5 py-1 text-xs font-medium text-tertiary-600 dark:bg-primary-500/10 dark:text-primary-500"
										>
										...
										{#if estimatedCount > 0}
											<span class="ms-2 text-xs text-surface-500"
												>~{formatNumber(estimatedCount)} entries</span
											>
										{/if}
									{/if}
								</div>
							...
							<div class="mt-5 border-t border-surface-500/30 pt-4 dark:border-surface-500/40">
								<div class="flex items-center justify-between mb-2">
									...
									<button
										...
										class="text-xs text-tertiary-500 hover:underline"
									>
						...
						<div class="mt-4">
							<Input
								...
								type="text"
								placeholder="Auto-detected from source content type (e.g. post, page)"
			...
			{:else if step === 2}
				<div class="space-y-4">
					<div class="flex items-center justify-between">
						<div>
							...
							<p class="text-sm text-surface-500">
								{fieldMappings.length} fields detected — adjust targets below
							</p>
						</div>
						<div class="flex gap-2 text-xs">
							<span
								class="px-2 py-1 rounded bg-tertiary-500/10 text-tertiary-600 dark:bg-primary-500/10 dark:text-primary-500"
							>
					...
					<!-- Mapping Table -->
					<div
						class="max-h-96 overflow-y-auto rounded border border-surface-500/30 dark:border-surface-500/40"
					>
						<table class="w-full text-sm" aria-label="Field mapping table">
							<thead class="sticky top-0 bg-surface-500/10 dark:bg-surface-800">
								<tr>
									<th class="px-4 py-2 text-start text-xs font-medium text-surface-500 w-[40%]"
										>Source Field</th
									>
									...
									<th class="px-4 py-2 text-start text-xs font-medium text-surface-500 w-[40%]"
										>Target Field</th
									>
									<th class="px-4 py-2 text-center text-xs font-medium text-surface-500"
										>Confidence</th
									>
								</tr>
							</thead>
							<tbody class="divide-y divide-surface-100 dark:divide-surface-700">
								{#each fieldMappings as mapping (mapping.source)}
									<tr class="hover:bg-surface-500/10 dark:hover:bg-surface-800/50">
										<td class="px-4 py-2.5 font-mono text-xs text-surface-600 dark:text-surface-400"
											>{mapping.source}</td
										>
										...
										<td class="px-4 py-2.5">
											<input
												...
												oninput={(e) =>
													updateMapping(mapping.source, (e.target as HTMLInputElement).value)}
										...
										<td class="px-4 py-2.5 text-center">
											<span
												class="inline-flex items-center gap-1 text-xs font-medium {getConfidenceColor(
													mapping.confidence
												)}"
											>
												{mapping.confidence}%
			...
			{:else if step === 3}
				<div class="text-center space-y-6 py-8">
					<div
						class="flex h-20 w-20 items-center justify-center rounded-full bg-warning-500/10 mx-auto"
					>
						<iconify-icon
					...
					<div>
						...
						<p class="mt-2 text-sm text-surface-500 max-w-md mx-auto">
							...
							{availableContentTypes.length > 0
								? ` Checking ${selectedContentTypes.size} content type(s).`
								: ''}
						</p>
					</div>
					...
					{#if isProcessing}
						<div class="flex justify-center py-4">
							<div
								class="h-6 w-6 animate-spin rounded-full border-2 border-tertiary-500 border-t-transparent"
							></div>
						</div>
					{/if}
				</div>
			...
			{:else if step === 4}
				<div class="space-y-6">
					...
					<div class="space-y-2">
						...
						{#if importCurrent > 0}
							<p class="text-xs text-surface-500">
								{formatNumber(importCurrent)} / {formatNumber(importTotal)}
							</p>
						{/if}
					</div>
					...
					{#if importLog.length > 0}
						<div
							class="max-h-48 overflow-y-auto rounded border border-surface-500/30 bg-surface-500/10 p-3 font-mono text-xs dark:border-surface-500/40 dark:bg-surface-900/50"
						>
					...
					<div class="flex justify-center gap-3">
						...
						{:else}
							<Button variant="primary" onclick={() => goToStep(5)}>View Results →</Button>
						{/if}
					</div>
				</div>
			...
			{:else if step === 5}
				<div class="space-y-6">
					<div class="text-center">
						<div
							class="flex h-16 w-16 items-center justify-center rounded-full bg-tertiary-500/10 mx-auto mb-3"
						>
							<iconify-icon
								icon="mdi:check-all"
								width="32"
					...
					<div class="grid grid-cols-3 gap-4">
						<div class="rounded-lg bg-tertiary-500/10 p-4 text-center dark:bg-primary-500/10">
							<p class="text-2xl font-bold text-tertiary-600 dark:text-primary-500">
						...
						<div class="rounded-lg bg-surface-500/10 p-4 text-center dark:bg-surface-800">
							<p class="text-2xl font-bold text-surface-600 dark:text-surface-400">
								{estimatedCount}
							</p>
					...
					{#if transactionToken}
						<div
							class="rounded border border-surface-500/30 bg-surface-500/10 p-4 dark:border-surface-500/40 dark:bg-surface-800/50"
						>
							<p class="text-xs font-mono text-surface-500">Transaction: {transactionToken}</p>
							<p class="mt-1 text-xs text-surface-500">
								All entries are in <strong class="text-warning-500">Draft</strong> status. Review before
								publishing.
							</p>
						</div>
					{/if}
					...
					<div class="flex flex-wrap justify-end gap-2">
						...
						{#if transactionToken}
							<Button variant="error" onclick={rollbackMigration}>↩ Rollback This Migration</Button>
						{/if}
					</div>
				</div>
			{/if}
