...
>
	...
	<div class="flex flex-col items-center gap-0.5">
		<p
			class="w-full truncate text-xs font-medium uppercase tracking-wide text-surface-600 group-hover:text-tertiary-600 dark:text-primary-500 dark:group-hover:text-tertiary-500 lg:text-sm"
		>
			Migration
		</p>
		...
		{#if isPro}
			<span
				class="mt-0.5 text-[9px] px-2 py-px rounded-full bg-warning-500/10 text-warning-500 border border-warning-500/30 font-bold tracking-wider"
			>
				PRO
			</span>
		{:else}
			<span
				class="mt-0.5 text-[9px] px-2 py-px rounded-full bg-surface-500/10 text-surface-500 dark:bg-surface-700 dark:text-white font-medium"
			>
				Free
			</span>
		{/if}
