...
<script lang="ts">
	...
	function handleKeydown(e: KeyboardEvent) {
		...
		if (e.key === 'ArrowDown') {
			focusedIndex = Math.min(focusedIndex + 1, filteredNodes.length - 1);
			e.preventDefault();
		}
		if (e.key === 'ArrowUp') {
...
<div class="transformation-tree space-y-4" onkeydown={handleKeydown}>
	<!-- Toolbar -->
	<div
		class="flex flex-wrap items-center gap-2 p-3 bg-surface-500/10 dark:bg-surface-900 border border-surface-500/30 dark:border-surface-500/40 rounded-xl"
	>
		<div
			class="flex items-center gap-1 bg-white dark:bg-surface-500/10 border border-surface-500/30 dark:border-surface-500/40 rounded-lg px-2 py-1 flex-1 min-w-30 sm:min-w-37.5"
		>
			...
			<input
				...
				class="bg-transparent border-0 p-0 text-xs text-surface-600 dark:text-surface-400 focus:outline-none w-full"
		...
		<select
			...
			class="bg-white dark:bg-surface-500/10 border border-surface-500/30 dark:border-surface-500/40 text-xs rounded-lg px-2 py-1 text-surface-600"
		...
		<button
			...
			class="text-[10px] font-bold uppercase px-2 py-1 rounded-lg border border-surface-500/30 dark:border-surface-500/40 text-surface-500 hover:bg-surface-500/10 dark:hover:bg-surface-800 transition"
		...
		<div class="flex items-center gap-1">
			...
			{#if undoStack.length > 0}
				<button
					onclick={undo}
					class="text-[10px] px-1.5 py-1 rounded-lg border border-surface-500/30 text-surface-400 hover:text-surface-600 transition"
		...
		{#if selectedNode}
			<div class="rounded-xl border border-tertiary-500/30 bg-tertiary-500/10 p-3 space-y-1">
				<div class="flex items-center gap-1.5 mb-2">
					...
					<span class="text-[10px] font-bold uppercase text-tertiary-500"
						>{selectedNode.label} → {selectedNode.suggestedTarget}</span
					>
					<span class="text-[9px] text-surface-400"
						>({selectedNode.action}, {selectedNode.confidence}%)</span
					>
				</div>
				{#if selectedNode.sampleValue}
					<div
						class="text-[10px] text-surface-600 dark:text-surface-400 bg-surface-500/10 dark:bg-surface-900 rounded-lg p-2 font-mono"
					>
						{selectedNode.sampleValue}
					</div>
				{:else}
					<div class="text-[10px] text-surface-400 italic">
						No sample data — click Import to see transformed values
					</div>
				{/if}
			</div>
		{/if}
...
{#snippet TreeNode(props: {
	...
	{@const nodeRecommendations = recommendations.filter((r: any) =>
		r.affectedFields?.some(
			(f: string) => f === node.label || node.label.includes(f) || f.includes(node.label)
		)
	)}
	...
	>
		<div
			class="flex flex-col gap-1.5 p-2.5 border rounded-lg transition shadow-xs cursor-pointer {isSelected
				? 'bg-tertiary-500/10 dark:bg-tertiary-500/10 border-tertiary-500'
				: 'bg-white dark:bg-surface-900 border-surface-500/30 dark:border-surface-500/40'} {node.confidence <
				50 && !isSelected
				? 'border-warning-500'
				: ''}"
		...
		>
			<div class="flex items-center gap-2 min-w-0">
				<input
					...
					class="shrink-0 h-3.5 w-3.5 rounded border-surface-500/30 text-tertiary-500"
				...
				{#if node.children && node.children.length > 0}
					...
					>
						<iconify-icon icon={isOpen ? 'mdi:chevron-down' : 'mdi:chevron-right'} width="14"
						></iconify-icon>
					</button>
				{/if}
				...
				<iconify-icon
					...
					width="12"
					class="shrink-0 {node.confidence >= 80
						? 'text-success-500'
						: node.confidence >= 50
							? 'text-warning-500'
							: 'text-error-500'}"
				...
				></iconify-icon>
				<span
					class="text-[9px] uppercase font-bold px-1.5 py-0.5 rounded-full border shrink-0 {typeColors[
						node.type
					]}">{node.type.replace('_', ' ')}</span
				>
				<span class="font-semibold text-xs text-surface-600 dark:text-surface-100 truncate flex-1"
					>{node.label}</span
				>

				{#if node.aiSuggestion}
					<span
						class="shrink-0 text-[9px] px-1.5 py-0.5 rounded-full border {node.aiSuggestion
							.level === 'critical'
							? 'bg-error-500/10 text-error-600 border-error-500/30'
							: 'bg-warning-500/10 text-warning-600 border-warning-500/30'}"
						title={node.aiSuggestion.message}>AI</span
					>
				{/if}

				{#if node.sampleValue}
					<span
						class="text-[9px] text-surface-400 italic truncate max-w-20"
						title={node.sampleValue}
						>{node.sampleValue.slice(0, 20)}{node.sampleValue.length > 20 ? '…' : ''}</span
					>
				{/if}

				<span
					class="shrink-0 text-[10px] font-mono border px-1.5 py-0.5 rounded-full {confidenceColor(
						node.confidence
			...
			<div
				role="presentation"
				class="flex items-center gap-1.5 ms-7"
				onclick={(e) => e.stopPropagation()}
			>
				...
				<input
					...
					class="bg-surface-500/10 dark:bg-surface-500/10 border border-surface-500/30 dark:border-surface-500/40 rounded-md px-1.5 py-0.5 text-[10px] font-mono text-surface-600 dark:text-surface-400 focus:outline-none focus:border-tertiary-500 w-24"
				...
				<select
					...
					class="bg-surface-500/10 dark:bg-surface-500/10 border border-surface-500/30 dark:border-surface-500/40 text-[10px] rounded-md px-1.5 py-0.5 text-surface-600 focus:outline-none"
				>
					<option value="map">Map</option>
			...
			{#if nodeRecommendations.length > 0}
				<div class="flex flex-wrap gap-1 ms-7">
					{#each nodeRecommendations as rec (rec.title)}
						<div
							class="text-[9px] px-1.5 py-0.5 rounded border flex items-center gap-1 {rec.level ===
							'critical'
						...
						>
							<iconify-icon
								icon={rec.level === 'critical' ? 'mdi:alert-octagon' : 'mdi:lightbulb'}
								width="10"
							></iconify-icon>
							{rec.title}
						</div>
