---
path: "src/plugins/smart-importer/docs/wordpress-migration"
...
| :-------------------------------------- | :------------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Plugins** (Yoast, checkout PHP, etc.) | PHP code, not data        | [Plugins](/docs/reference/architecture/plugin-architecture.mdx) — Stripe, SEO, etc. WooCommerce **catalog data** does migrate (see [WooCommerce Products](#woocommerce-products) below). |
| **Themes**                              | PHP/CSS templates         | [Design System](/docs/guides/configuration/admin-theme-settings) — admin theme + Tailwind v4                                                                                             |
| **Widgets** (sidebar)                   | PHP widgets               | [Widget System](/docs/development/widgets) — Svelte 5 native                                                                                                                             |
| **Shortcodes**                          | PHP-rendered placeholders | [Token System](/docs/reference/architecture/token-system.mdx) — auto-converted during import                                                                                             |
| **User Accounts**                       | Different auth system     | Manually recreate in [Access Management](/docs/guides/configuration/access-management)                                                                                                   |
| **Menus**                               | Navigation structure      | Rebuild as collections plus a rendered navigation template: [Collection Builder](/docs/guides/content/collection-builder)                                                                |
| **Gutenberg Blocks**                    | React-based editor        | RichText widget (HTML preserved, block structure in `rawCustomFields`)                                                                                                                   |
| **Custom Post Type Definitions**        | Register in PHP           | [Collection Builder](/docs/guides/content/collection-builder.mdx) — visual schema builder                                                                                                |
...
Prefer the dedicated catalog parser (nests variations, fills `ecommerce` SKU/price/stock):

```bash
bun run migrate import --file=export.xml --format=woocommerce --collection=products
# or Woo REST JSON:
bun run migrate import --file=products.json --format=woocommerce --collection=products
```

A product-only WXR is auto-detected as `woocommerce`. Mixed WordPress dumps (posts + products) still use `--format=wordpress`; product items now also get an `ecommerce` block from `_sku` / `_price` / `_stock` meta.

Imported SKUs land in the ecommerce **preset** `products` collection. Add the **Commerce Inventory** dashboard widget (14-day trial) to watch low stock — see [E-Commerce Overview](/docs/guides/content/ecommerce.mdx). Checkout PHP / cart sessions do **not** migrate; those belong in the optional Commerce plugin.
...
- [Token System Documentation](/docs/reference/architecture/token-system.mdx)
