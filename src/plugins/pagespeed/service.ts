...
export async function storeResult(
	...
	try {
		...
		if (existing.success && existing.data) {
			...
			const updateResult = await dbAdapter.crud.update<PageSpeedResult>(
			...
			if (!updateResult.success) {
				logger.error('Failed to update PageSpeed result', { error: updateResult.error });
				return false;
			}
		} else {
			// Insert new record
			const insertResult = await dbAdapter.crud.insert<PageSpeedResult>(
				'plugin_pagespeed_results',
				result
			);
			
			if (!insertResult.success) {
				logger.error('Failed to insert PageSpeed result', { error: insertResult.error });
				return false;
			}
		}
