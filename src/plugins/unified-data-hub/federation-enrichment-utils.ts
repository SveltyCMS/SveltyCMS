/**
 * @file src/plugins/unified-data-hub/federation-enrichment-utils.ts
 * @description Pure validation helpers for native collection federationEnrichments config.
 * Shared between server flows and the Collection Builder picker (client) —
 * kept OUTSIDE any `server` directory so SvelteKit 3's server-only module
 * guard does not block browser imports.
 *
 * Features:
