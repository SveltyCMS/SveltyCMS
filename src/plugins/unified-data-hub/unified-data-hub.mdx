---
path: "src/plugins/unified-data-hub/unified-data-hub.mdx"
title: "Unified Data Hub Plugin"
...
1. Migrate away from a legacy CMS → [Smart Importer](/docs/guides/content/smart-importer.mdx)
...
Coverage map for MVP / v1.5 / v2.0 scope and query-planner phase gates.

| Area                                                                       | Status          | Implementation                                                                      |
| -------------------------------------------------------------------------- | --------------- | ----------------------------------------------------------------------------------- |
| Passthrough executor (single-source read)                                  | ✅ Shipped      | `virtual-query-engine.ts`                                                           |
| SQL + Mongo connectors (read + write)                                      | ✅ Shipped v2.3 | `postgres.ts`, `mariadb.ts`, `sqlite.ts`, `mongodb.ts`, `sql-dialect.ts`            |
| REST connector (read + opt-in writes)                                      | ✅ Shipped v2.4 | `rest-openapi.ts`, `rest-egress.ts`, `writesEnabled` config                         |
| Single-source write-back                                                   | ✅ Shipped v2.3 | `virtual-write-engine.ts`, `POST/PATCH/DELETE` + LocalCMS `create/update/delete`    |
| LocalCMS `virtualCollections` namespace                                    | ✅ Shipped      | `virtual-collections-namespace.ts`                                                  |
| HTTP `GET /api/virtual-collections`                                        | ✅ Shipped      | `handlers/virtual-collections.ts`                                                   |
| GraphQL `virtualCollections` / `virtualCollection` / `virtualEnrich`       | ✅ Shipped      | `resolvers/virtual-collections.ts`                                                  |
| GraphQL `createVirtualEntry` / `updateVirtualEntry` / `deleteVirtualEntry` | ✅ Shipped v2.3 | Same LocalCMS write path as REST `POST/PATCH/DELETE`                                |
| AST normalization + capability pre-check (v1.5 P0)                         | ✅ Shipped      | `query-planner.ts`                                                                  |
| Postgres pool cache (v1.5 P1, p95 ~12ms)                                   | ✅ Shipped      | `postgres-pool-cache.ts`                                                            |
| WordPress REST filter/sort pushdown (v1.5 P2)                              | ✅ Shipped      | `query-planner.ts` + `wordpress-rest.ts`                                            |
| Circuit breaker 3 failures / 60s (v1.5 P3)                                 | ✅ Shipped      | `connector-circuit-breaker.ts`                                                      |
| Cache tenant index (v1.5 P4)                                               | ✅ Shipped      | `cache.ts`                                                                          |
| Same-source virtual joins `?include=` (v1.5 P5)                            | ✅ Shipped      | `virtual-join.ts`                                                                   |
| Native stitch enrich (v2.0 Phase C)                                        | ✅ Shipped      | `native-virtual-stitch.ts`, `/enrich`                                               |
| Entry preview sidebar + enrichment picker                                  | ✅ Shipped      | `virtual-entry-preview.svelte`, `federation-enrichment-picker.svelte`               |
| N+1 stitch telemetry                                                       | ✅ Shipped      | `stitch-telemetry.ts`, response headers                                             |
| Decomposition plan tree (v3.0)                                             | ✅ Shipped      | `federated-decomposition.ts`, `meta.decomposition`                                  |
| Per-source cursor pagination (v3.0 stable)                                 | ✅ Shipped      | `federated-cursor.ts`, `meta.nextCursor`                                            |
| WebMCP virtual tools                                                       | ✅ Shipped      | `list_virtual_collections`, `query_virtual_collection`, `enrich_virtual_collection` |
| Topology + content graph extension                                         | ✅ Shipped      | `mcp-extension.ts`, `navigation.ts`                                                 |
| Headless API contracts                                                     | ✅ Shipped      | `headless-contracts.ts`, `getHeadlessContracts` action                              |
| Cross-source join execution (v3.1 alpha)                                   | ✅ Opt-in       | `enableCrossSourceAlpha` + Pro license                                              |
| Tier limits (Community 1 connector / 3 collections)                        | ✅ Shipped      | `tier-limits.ts`                                                                    |
| Crypto-chained federation audit                                            | ✅ Shipped      | `audit.ts` → `plugin_unified-data-hub_federation_audit`                             |

### Explicitly deferred (not in v2.3.0)

| Feature                                  | Planned phase | Notes                                                         |
| ---------------------------------------- | ------------- | ------------------------------------------------------------- |
| REST write-back hardening                | v2.5+         | OAuth2 forwarding, response schema validation, retry policies |
| Row/column policy engine                 | v2+           | Collection-level RBAC today                                   |
| Visual Studio query builder              | v2+           | Workspace covers connector/schema CRUD                        |
| GraphQL supergraph / federation spec     | v3+           | Per-collection GraphQL queries only                           |
| `search_entries` + `includeVirtual`      | v1.5+         | Not shipped                                                   |
| AI Command Bar cross-source NL queries   | v1.5+         | WebMCP tools provide governed reads                           |
| Materialized views                       | v2+           | TTL LRU cache today                                           |
| GraphQL / SaaS OAuth connectors          | Post-MVP      | MariaDB, SQLite, MongoDB shipped in v2.3                      |
| Hosted MCP `search_federation_docs` tool | Roadmap       | Plugin `.mdx` indexed via hosted MCP docs                     |
...
| ----------------- | ---------- | ------------------- | ----------------------------------------------------------- |
| Community / trial | 1          | 3                   | Health widget, basic reads                                  |
| Pro               | Unlimited  | Unlimited           | WebMCP tools, cross-source alpha, SaaS connectors (roadmap) |
| Enterprise        | Unlimited  | Unlimited           | Custom connectors, compliance dashboard (roadmap)           |
...
- [Smart Importer](/docs/guides/content/smart-importer.mdx) — one-time migration sibling
