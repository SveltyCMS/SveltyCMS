...
<div class="flex h-full flex-col gap-4 p-4 md:p-6" data-testid="udh-workspace">
	<div class="flex items-center justify-between gap-4">
		<div>
			...
			<p class="text-sm text-surface-500">
				Federate Postgres, MariaDB, SQLite, MongoDB, and REST sources as governed virtual
				collections.
			</p>
			{#if tierLabel}
				...
				{:else if connType !== 'rest'}
					<div class="grid gap-3 sm:grid-cols-2">
						...
						<Input
							...
							data-testid="udh-connector-database"
						/>
			...
			>
					...
					{:else}
						<div class="grid gap-3 sm:grid-cols-2">
							<Input
								...
								bind:value={vcTable}
								required
								data-testid="udh-collection-table"
							/>
				...
				<Button
					...
					disabled={savingCollection ||
						!vcConnectorId ||
						((vcPreset === 'sql-table' || vcPreset === 'mongodb-collection') &&
							(!vcName.trim() ||
								!vcSlug.trim() ||
								(vcPreset === 'sql-table' ? !vcTable.trim() : !vcCollection.trim())))}
			...
			{:else}
				<ul class="space-y-2 text-sm">
					{#each collections as vc (vc._id)}
						<li
							class="flex justify-between gap-2 border-b border-surface-100 pb-2 dark:border-surface-500/40"
						>
	...
	<AdminCard title="Headless API">
		<p class="text-sm text-surface-600 dark:text-surface-400">
			...
			<code class="text-xs">cms.virtualCollections.find()</code>. Writable SQL/Mongo connectors
			support
			<code class="text-xs">POST/PATCH/DELETE</code> and
			<code class="text-xs">cms.virtualCollections.create/update/delete()</code>.
		</p>
	</AdminCard>
</div>
