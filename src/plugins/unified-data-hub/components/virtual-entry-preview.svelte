...
<script lang="ts">
	...
	const enrichments = $derived((collection?.federationEnrichments ?? []) as FederationEnrichment[]);
	...
	function displayFields(row: FederatedRow, enrichment: FederationEnrichment): [string, unknown][] {
		...
		if (preferred?.length) {
			return preferred.filter((f) => f in row).map((f) => [f, row[f]]);
		}
...
{#if showPreview}
	...
	>
		<div
			class="flex items-center justify-between gap-2 border-b border-surface-500/30 pb-2 dark:border-surface-600"
		>
		...
		<div class="mt-3 space-y-4">
			{#each enrichments as enrichment (previewKey(enrichment))}
				...
				<article
					class="rounded border border-surface-500/30 bg-surface-500/10 p-3 dark:border-surface-500/40 dark:bg-surface-900/20"
				>
					...
					{#if state?.meta?.stitchWarning}
						...
						>
							{state.meta.message ??
								'High stitch key count — batch enrich requests to avoid N+1 latency.'}
						</p>
					{/if}
					...
					{:else}
						<dl class="space-y-1 text-xs">
							{#each displayFields(state.row, enrichment) as [field, value] (field)}
								<div class="flex justify-between gap-2">
									<dt class="font-medium capitalize text-surface-600 dark:text-surface-400">
										{field}
									</dt>
									<dd
										class="truncate text-end font-semibold text-tertiary-500 dark:text-primary-500"
									>
										{String(value ?? '—')}
									</dd>
								</div>
						...
						{#if state.meta?.staleness}
							<p class="mt-2 text-[10px] uppercase tracking-wide text-surface-500">
								{state.meta.staleness}{#if state.meta.matched !== undefined}
									· matched {state.meta.matched}{/if}
							</p>
						{/if}
					{/if}
				</article>
