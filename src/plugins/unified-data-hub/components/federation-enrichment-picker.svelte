...
<script lang="ts">
	...
	const collectionKey = $derived(String(collections.active?._id ?? collections.active?.name ?? ''));
	...
	<Card data-testid="federation-enrichment-picker">
		<div class="mb-4 flex flex-wrap items-center justify-between gap-2">
			<div>
				...
				<p class="mt-1 text-xs text-surface-500 dark:text-surface-400">
					Link a native field to a virtual collection. Entry editor sidebar shows live federated
					data.
				</p>
			</div>
		...
		{:else if virtualOptions.length === 0}
			<p class="text-sm text-surface-500">
				...
				<a
					href="/config?plugin=unified-data-hub"
					class="text-primary-500 underline"
					data-preload="hover"
					aria-label="Open Unified Hub workspace"
				>
					Open Unified Hub
				</a>
			</p>
