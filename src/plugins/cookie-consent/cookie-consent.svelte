	...
	>
		...
		>
			...
			<div class="mt-6 flex flex-col gap-3 sm:flex-row">
				{#if showDetails}
					<Button onclick={handleSavePreferences} variant="primary" class="flex-1">
						{cookie_button_save()}
					</Button>
				{:else}
					<Button onclick={() => (showDetails = true)} variant="secondary" class="flex-1">
						{cookie_button_customize()}
					</Button>
					<Button onclick={handleRejectAll} variant="secondary" class="flex-1">
						{cookie_button_reject()}
					</Button>
					<Button
