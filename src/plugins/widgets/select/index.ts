/**
 * @file src/widgets/custom/select/index.ts
...
import { literal, nullable, union, string, type InferInput as ValibotInput } from "valibot";
...
const validationSchema = (field: FieldInstance & SelectProps) => {
  // Handle empty options to prevent union error
  if (!field.options || field.options.length === 0) {
    return field.required ? string("At least one option must be configured.") : nullable(string());
  }
...
const SelectWidget = createWidget<SelectProps>({
  ...
  inputComponentPath: "/src/widgets/custom/select/input.svelte",
  displayComponentPath: "/src/widgets/custom/select/display.svelte",
