...
export const createValidationSchema = (field: TagsProps) => {
...
const TagsWidget = createWidget<TagsProps>({
  ...
  aggregations: {
    filters: async ({ field, filter }: { field: any; filter: string }) => [
      {
        $match: {
    ...
    sorts: async ({ field, sortDirection }: { field: any; sortDirection: number }) => ({
