...
function fieldSignature(norm: any): string {
  ...
  const widget =
    typeof norm.widget === "string" ? norm.widget : (norm.widget?.Name ?? norm.widget?.key ?? "");
      ...
      for (const [key, codeField] of codeFields) {
        ...
        if (codeField.required && !dbField.required) {
          changes.push({
            ...
            suggestion:
              "Ensure all existing entries have a value for this field to avoid validation errors.",
          });
        }
