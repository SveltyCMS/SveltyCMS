...
import { button_cancel, button_confirm } from "@src/paraglide/messages";
...
export function showConfirm(options: ConfirmModalOptions): void {
  modalState.trigger(
    ConfirmDialog,
    {
      ...
      body: options.body,
      buttonTextConfirm:
        options.buttonTextConfirm || options.confirmText || button_confirm?.() || "Confirm",
...
// Schedule modal
export function showScheduleModal(options: {
...
// Clone confirmation
export function showCloneModal(options: {
  count?: number;
  onConfirm: () => void | Promise<void>;
  onCancel?: () => void;
}): void {
