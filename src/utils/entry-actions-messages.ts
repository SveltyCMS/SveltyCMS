...
export const entryMessages = {
  ...
  entriesArchived: (count: number) =>
    entries_archived?.({ count }) ||
    `${count} ${count === 1 ? "entry" : "entries"} archived successfully`,

  entriesPublished: (count: number) =>
    entries_published?.({ count }) ||
    `${count} ${count === 1 ? "entry" : "entries"} published successfully`,

  entriesUnpublished: (count: number) =>
    entries_unpublished?.({ count }) ||
    `${count} ${count === 1 ? "entry" : "entries"} unpublished successfully`,

  entriesSetToTest: (count: number) =>
    entries_set_to_test?.({ count }) ||
    `${count} ${count === 1 ? "entry" : "entries"} set to test successfully`,

  entriesDeleted: (count: number) =>
    entries_deleted?.({ count }) ||
    `${count} ${count === 1 ? "entry" : "entries"} deleted successfully`,

  entriesScheduled: (count: number) =>
    entries_scheduled?.({ count }) ||
    `${count} ${count === 1 ? "entry" : "entries"} scheduled successfully`,

  entriesCloned: (count: number) =>
    entries_cloned?.({ count }) ||
    `${count} ${count === 1 ? "entry" : "entries"} cloned successfully`,

  entriesUpdated: (count: number, status: string) =>
    entries_updated?.({ count, status }) ||
    `${count} ${count === 1 ? "entry" : "entries"} updated to ${status}`,

  // Error messages
  updateFailed: (operation: string) =>
    update_failed?.({ operation }) || `Failed to ${operation} entries`,

  deleteFailed: (operation: string) =>
    delete_failed?.({ operation }) || `Failed to ${operation} entries`,
  ...
  entryStatusUpdated: (status: string) =>
    entry_status_updated?.({ status }) || `Entry status updated to ${status}`,
  ...
  // Admin permissions
  onlyAdminsCanDelete: () =>
    only_admins_can_delete?.() || "Only administrators can delete archived entries",

  statusReservedForSystem: (status: string) =>
    status_reserved_for_system?.({ status }) ||
    `${status} status is reserved for system operations`,
  ...
  errorSavingDraft: (error: string) =>
    error_saving_draft?.({ error }) || `Error saving draft: ${error}`,
  ...
  errorScheduling: (error: string) =>
    error_scheduling?.({ error }) || `Error scheduling entry: ${error}`,
