...
export class MediaService {
  ...
  public async manipulateMedia(
    ...
    const { rotation, flipH, flipV, crop, filters, blurRegions, watermarks, annotations, saveBehavior } = manipulations;
    ...
    if (crop && crop.width > 0 && crop.height > 0) {
      ...
      if (crop.shape === "circle") {
        ...
        const circleMask = await sharp(maskPixels, { raw: { width: workW, height: workH, channels: 4 } }).png().toBuffer();
    ...
    if (Array.isArray(blurRegions) && blurRegions.length > 0) {
      for (const region of blurRegions) {
        ...
        const blurBuf = await sharp(originalBuffer)
          ...
          .toBuffer();
        composites.push({ input: blurBuf, left: compositeLeft, top: compositeTop, blend: "over" });
      }
    }

    // — Watermarks —
    const escXml = (s: string) =>
      s.replace(/[<>&"]/g, (c: string) => (({ "<": "&lt;", ">": "&gt;", "&": "&amp;", '"': "&quot;" } as Record<string, string>)[c] ?? c));
    ...
    if (Array.isArray(watermarks) && watermarks.length > 0) {
      for (const wm of watermarks) {
        if (wm.type === "text" && wm.text) {
          ...
          const opacity = Math.round((wm.opacity ?? 0.8) * 255).toString(16).padStart(2, "0");
          ...
          try {
            ...
            if (wmW > 0 && wmH > 0) {
              const wmBuf = await sharp(Buffer.from(wm.imageUrl.split(",")[1] ?? "", "base64"))
                ...
                .ensureAlpha()
                .toBuffer();
              composites.push({ input: wmBuf, left: wmLeft, top: wmTop, blend: "over" });
            }
          } catch {
    ...
    if (Array.isArray(annotations) && annotations.length > 0) {
      const shapes = annotations.map((a: any) => {
    ...
    const outputFilename = isCircle && !existing.filename.endsWith(".png")
    ...
    if (saveBehavior === "new") {
      ...
      if (!uploadResult.success || !uploadResult.data) throw new Error("Failed to create new media record");
