...
revisionSchema.statics = {
  ...
  async getRevisionHistory(contentId: string): Promise<DatabaseResult<ContentRevision[]>> {
    try {
      ...
      logger.error(
        `Error retrieving revision history for content ID: ${contentId}: ${err.message}`,
      );
      return {
        success: false,
        message,
  ...
  async createRevision(
    revisionData: Omit<ContentRevision, "_id" | "createdAt" | "updatedAt">,
  ...
  async updateRevision(
    revisionId: DatabaseId,
    updateData: Partial<ContentRevision>,
