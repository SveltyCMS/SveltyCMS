...
SYSTEM_PREFERENCES_SCHEMA.statics = {
  ...
  async getPreferenceByLayout(
    ...
    try {
      const query: mongoose.QueryFilter<SystemPreferencesDocument> = {
        userId,
        layoutId,
        scope: "user",
      };
      ...
      logger.error(
        `Error retrieving system preference for userId: ${userId}, layoutId: ${layoutId}`,
        error,
      );
      return {
        success: false,
  ...
  async setPreference(
    ...
    try {
      ...
      logger.error(
        `Error setting system preference for userId: ${userId}, layoutId: ${layoutId}`,
        error,
      );
      return {
        success: false,
