  ...
  },
  {
    ...
    statics: {
      ...
      async bulkUpdateFolderOrder(
        parentId: string,
        orderUpdates: Array<{ path: string; order: number }>,
        ...
        try {
          ...
          const result = await this.bulkWrite(
            bulkOps as mongoose.AnyBulkWriteOperation<SystemVirtualFolder>[],
          );
          logger.info(
            `Updated order for ${result.modifiedCount} virtual folders under parent: ${parentId}`,
          );
      ...
      async updateVirtualFolder(
        ...
        try {
          const folder = await this.findByIdAndUpdate(
            folderId,
            { $set: updateData },
      ...
      async deleteVirtualFolder(folderId: string): Promise<DatabaseResult<void>> {
        try {
          ...
          const folderIdsToDelete = [
            folder._id,
            ...children.map((c: SystemVirtualFolder) => c._id),
          ];
