...
themeSchema.statics = {
  ...
  async storeThemes(
    themes: Omit<Theme, "_id" | "createdAt" | "updatedAt">[],
    generateId: () => string,
  ): Promise<void> {
    if (themes.length === 0) {
      return;
    ...
    try {
      ...
      const result = await this.bulkWrite(operations as mongoose.AnyBulkWriteOperation<Theme>[], {
        ordered: false,
      });
...
export const ThemeModel =
  (mongoose.models?.Theme as Model<Theme> | undefined) ||
  mongoose.model<Theme>("Theme", themeSchema);

// --- Merged schema from deleted theme.ts ---

// --- Merged from theme.ts (model + statics) into methods for file reduction pilot ---
