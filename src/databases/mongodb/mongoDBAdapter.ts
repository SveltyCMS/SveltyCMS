...
export class MongoDBAdapter implements dbInterface {
	...
	async moveMediaToFolder(mediaId: string, folderId: string): Promise<boolean> {
		try {
			...
			const updateResult = await Promise.all(mediaTypes.map((type) => this.updateOne(type, { _id: mediaId }, { folderId: folderId })));
	...
	async getContentStructureChildren(parentId: string): Promise<Document[]> {
		try {
			const children = await ContentStructureModel.find({ parent: parentId }).sort({ order: 1 }).exec();
