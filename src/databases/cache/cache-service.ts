...
export class CacheService {
  ...
  /**
   * Test-only: await the pending batched L2 writes so cross-instance assertions
   * are deterministic without fixed sleeps (the write batcher flushes on its own
   * interval, which under load made `L2 contract` assertions flaky).
   * Mirrors the flush `cleanup()` performs — without tearing the clients down.
   */
  async flushL2WritesForTest(): Promise<void> {
