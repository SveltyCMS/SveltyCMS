...
export class ContentModule {
	...
	nodes = {
		...
		createMany: async (nodes: Omit<ContentNode, 'createdAt' | 'updatedAt'>[]): Promise<DatabaseResult<ContentNode[]>> => {
			return this.core.wrap(async () => {
				...
				const results = await this.db.insert(schema.contentNodes).values(values).returning();
		...
		deleteMany: async (paths: string[]): Promise<DatabaseResult<{ deletedCount: number }>> => {
			return this.core.wrap(async () => {
				const results = await this.db.delete(schema.contentNodes).where(inArray(schema.contentNodes.path, paths)).returning();
	...
	drafts = {
		...
		createMany: async (drafts: Omit<ContentDraft, '_id' | 'createdAt' | 'updatedAt'>[]): Promise<DatabaseResult<ContentDraft[]>> => {
			return this.core.wrap(async () => {
				...
				const results = await this.db.insert(schema.contentDrafts).values(values).returning();
		...
		getForContent: async (contentId: DatabaseId, options?: PaginationOptions): Promise<DatabaseResult<PaginatedResult<ContentDraft>>> => {
			return this.core.wrap(async () => {
				const conditions = [eq(schema.contentDrafts.contentId, contentId as string)];
				let q = this.db
					.select()
		...
		deleteMany: async (draftIds: DatabaseId[]): Promise<DatabaseResult<{ deletedCount: number }>> => {
			return this.core.wrap(async () => {
				const results = await this.db.delete(schema.contentDrafts).where(inArray(schema.contentDrafts._id, draftIds)).returning();
				return { deletedCount: results.length };
	...
	revisions = {
		...
		deleteMany: async (revisionIds: DatabaseId[]): Promise<DatabaseResult<{ deletedCount: number }>> => {
			return this.core.wrap(async () => {
				const results = await this.db.delete(schema.contentRevisions).where(inArray(schema.contentRevisions._id, revisionIds)).returning();
				return { deletedCount: results.length };
		...
		cleanup: async (contentId: DatabaseId, keepLatest: number): Promise<DatabaseResult<{ deletedCount: number }>> => {
			return this.core.wrap(async () => {
				...
				const results = await this.db.delete(schema.contentRevisions).where(inArray(schema.contentRevisions._id, idsToDelete)).returning();
				return { deletedCount: results.length };
