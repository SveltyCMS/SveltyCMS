...
export class WebsiteTokensModule {
  ...
  async getAll(options: {
    ...
    return this.core.wrap(async () => {
      ...
      if (options.sort) {
        ...
        const column = (
          schema.websiteTokens as unknown as Record<string, import("drizzle-orm").Column>
        )[options.sort];
        if (column) {
  ...
  async delete(tokenId: DatabaseId): Promise<DatabaseResult<void>> {
    return this.core.wrap(async () => {
      await this.db
        .delete(schema.websiteTokens)
        .where(eq(schema.websiteTokens._id, tokenId as string));
