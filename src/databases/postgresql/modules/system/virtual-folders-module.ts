...
export class VirtualFoldersModule {
  ...
  async getContents(folderPath: string): Promise<
    DatabaseResult<{
      folders: SystemVirtualFolder[];
      files: import("../../../db-interface").MediaItem[];
    }>
  > {
    return this.core.wrap(async () => {
      const folders = await this.db
        .select()
        .from(schema.systemVirtualFolders)
        .where(eq(schema.systemVirtualFolders.path, folderPath));
