 ...
 * @description CRUD operations module for PostgreSQL, inheriting from RelationalCrudModule.
 */
...
export class CrudModule extends RelationalCrudModule implements ICrudAdapter {
  ...
  /**
   * 🚀 OPTIMIZATION: PostgreSQL-specific findOne with prepared statement and read-replica awareness.
   */
  ...
  async find<T extends BaseEntity>(
    ...
    if (options.rawSql && options.sql) {
      ...
      return this.adapter
        .wrap(async () => {
          const db = options?.tx || (this.adapter as any).getDb("read");
