...
for (const [tableName, columns] of Object.entries(
  helpers.SYSTEM_LITERAL_COLUMNS,
)) {
...
export abstract class SQLiteAdapterCore
  extends SqlAdapterCore
  implements ISqlAdapter
{
      ...
      try {
        ...
        if (client) {
          ...
          if (client.query) {
            row = client
              .query(
                `SELECT data FROM content_nodes WHERE _id = '${cleanName}' LIMIT 1`,
              )
              .get();
  ...
  public createDynamicTableDefinition(
    name: string,
    ...
    return sqliteTable(name, columns, (t) => {
      ...
      if (columnsToAdd) {
        for (const colName of columnsToAdd.keys()) {
          if (t[colName])
            idxs[`${colName}Idx`] = index(`${name}_${colName}_idx`).on(
              t[colName],
            );
        }
      }
        ...
        return await db.transaction(async (tx: any) => {
          ...
          if (this._insertManyReturningSupported !== false) {
            try {
              ...
              if (process.env.BENCHMARK !== "true") {
                logger.warn(
                  "[SQLite] insertMany returning fallback invoked due to error:",
                  err,
                );
              }
              await (query as any);
  ...
  private async _warmTableRegistry(): Promise<void> {
    ...
    try {
      ...
      if (client.query) {
        rows = client
          .query(
            "SELECT _id, data FROM content_nodes WHERE _id NOT LIKE 'system_%'",
          )
          .all();
      } else if (client.prepare) {
        rows = client
    ...
    return this.wrap(
      async () => {
        ...
        try {
          ...
          logger.debug(
            `SQLite RETURNING failed, using inline SELECT fallback: ${err.message}`,
          );
        }
    ...
    await this.wrap(
      async () => {
        ...
        if (debugMode && !isBenchSuite)
          console.log(
            `[DB Provision] [SQLITE] Executing DDL for ${physicalName}`,
          );
  ...
  private async createDriver(dbPath: string) {
    ...
    if (isBun) {
      try {
        ...
        logger.warn(
          `[SQLite] Bun driver failed: ${e.message}. Falling back to node:sqlite...`,
        );
        if (process.platform === "win32") {
    ...
    if (nodeVersion) {
      ...
      if (major > 22 || (major === 22 && minor >= 5)) {
        try {
          ...
          logger.info(
            `[SQLite] Using native 'node:sqlite' driver (shimmed via sqlite-proxy).`,
          );
    ...
    throw new Error(
      `No compatible SQLite driver found (bun:sqlite or node:sqlite).`,
    );
  }
