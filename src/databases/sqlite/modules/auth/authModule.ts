...
import type { DatabaseResult, PaginationOption, Role, Session, Token, User } from '../../../dbInterface';
...
export class AuthModule {
	...
	async consumeToken(
		...
		return (this.core as any).wrap(async () => {
			...
			return { status: result.changes > 0, message: result.changes > 0 ? 'Token consumed' : 'Token not found or already consumed' };
