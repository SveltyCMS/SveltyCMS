...
export class AdapterCore {
	...
	public async connect(config: string | { connectionString?: string; filename?: string }): Promise<DatabaseResult<void>> {
		try {
			...
			// Try bun:sqlite first; fall back to better-sqlite3 for plain Node.js environments.
			// We use try/catch import rather than checking globals because Vite's SSR module
			// runner does not expose the Bun global even when the process is Bun.
			const bunSqlite = 'bun:sqlite';
			let bunDatabase: any;
			try {
				const mod = await import(/* @vite-ignore */ bunSqlite);
			...
			if (bunDatabase) {
				this.sqlite = new bunDatabase(dbPathResolved) as SQLiteClient;
				...
				// Fallback to better-sqlite3 in plain Node.js environments
