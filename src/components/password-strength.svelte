...
<script lang="ts">
	...
	const COLOR_CLASSES: Record<number, string> = {
		0: 'bg-surface-300 text-surface-600 dark:bg-surface-600 dark:text-surface-400',
		1: 'bg-error-500 text-white',
		2: 'bg-warning-500 text-surface-900',
		3: 'bg-warning-500 text-surface-900',
		4: 'bg-success-500 text-white',
	...
	function getBarColor(barIndex: number, score: number): string {
		if (score === 0) {
			return 'bg-surface-500/20 dark:bg-surface-700';
		}
		if (barIndex === 0) {
			return score >= 1 ? 'bg-error-500' : 'bg-surface-500/20 dark:bg-surface-700';
		}
		if (barIndex === 1) {
			return score >= 2 ? 'bg-warning-500' : 'bg-surface-500/20 dark:bg-surface-700';
		}
		return score >= 3 ? 'bg-success-500' : 'bg-surface-500/20 dark:bg-surface-700';
	}
...
{#if showStrength}
	...
	>
		...
		<div class="relative h-4 w-full overflow-hidden rounded-sm bg-surface-500/20 dark:bg-surface-700">
			...
			<div class="min-w-0 flex-1">
				{#if !showMatchIndicator}
					<div class="flex items-center gap-2">
						<span class="text-xs text-[var(--admin-text-muted)]"> Strength </span>
						{#if showRequirements}
		...
		{#if showRequirements && showRequirementsList}
			<div
				...
				class="rounded border border-[var(--admin-border-default)] bg-surface-500/10 p-3 dark:border-surface-500/40 dark:bg-surface-800"
				transition:slide={{ duration: prefersReducedMotion ? 0 : 200 }}
			>
				<h4 class="mb-2 text-xs font-semibold text-surface-600 dark:text-surface-400">Password Requirements</h4>
				<ul class="space-y-1 text-xs" role="list">
					<li class="flex items-center gap-2">
						<span
							class="flex h-4 w-4 items-center justify-center rounded-full text-[10px] {complexityChecks.hasMinLength
								...
								: 'bg-surface-300 text-[var(--admin-text-muted)] dark:bg-surface-600 dark:text-surface-400'}"
							aria-hidden="true"
						>
							{complexityChecks.hasMinLength ? '✓' : '○'}
						</span>
						<span class={complexityChecks.hasMinLength ? 'text-success-600 dark:text-success-400' : 'text-[var(--admin-text-muted)]'}>
					...
					<li class="flex items-center gap-2">
						...
						<span class={complexityChecks.hasUpper ? 'text-success-600 dark:text-success-400' : 'text-[var(--admin-text-muted)]'}>
					...
					<li class="flex items-center gap-2">
						...
						<span class={complexityChecks.hasLower ? 'text-success-600 dark:text-success-400' : 'text-[var(--admin-text-muted)]'}>
					...
					<li class="flex items-center gap-2">
						...
						<span class={complexityChecks.hasNumber ? 'text-success-600 dark:text-success-400' : 'text-[var(--admin-text-muted)]'}> One number </span>
					</li>
					<li class="flex items-center gap-2">
						<span
						...
						<span class={complexityChecks.hasSpecial ? 'text-success-600 dark:text-success-400' : 'text-[var(--admin-text-muted)]'}>
