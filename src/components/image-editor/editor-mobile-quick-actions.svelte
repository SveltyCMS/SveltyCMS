...
{#if visible}
	<div
		class="grid shrink-0 grid-cols-3 gap-0 items-center justify-items-center w-full max-w-48 mx-auto py-0.5 pb-1 bg-transparent border-b-0 [&_iconify-icon]:block [&_iconify-icon]:leading-[0]"
		role="toolbar"
		aria-label="Transform quick actions"
	>
		<button
			type="button"
			class="inline-flex items-center justify-center size-8 p-0 text-[rgba(255,255,255,0.94)] cursor-pointer bg-transparent border-none rounded-full transition-[color,opacity] duration-150 hover:text-white active:opacity-72"
