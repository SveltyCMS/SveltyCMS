...
<script lang="ts">
	...
	const tools = $derived.by(() => {
		...
		return MOBILE_RAIL_ORDER.map((key) => byKey.get(key))
			.filter((tool): tool is { id: string; name: string; icon: string } => !!tool)
			.map((tool) => ({
				...tool,
...
<nav
	class="flex shrink-0 gap-0 items-center justify-evenly w-full min-h-12 py-0.75 px-1 pb-[max(0.375rem,env(safe-area-inset-bottom,0px))] overflow-visible bg-[--editor-chrome-bg] border-t-0"
	aria-label="Editing tools"
>
	{#each tools as tool (tool.id)}
		<button
			type="button"
			class="inline-flex flex-[0_1_auto] items-center justify-center size-10 text-[rgba(255,255,255,0.72)] cursor-pointer bg-transparent border-none rounded-full transition-[background,color] duration-150 hover:not-disabled:text-[rgba(255,255,255,0.9)] disabled:cursor-not-allowed disabled:opacity-35 {selectedToolId ===
			tool.id
				? 'bg-white/[0.14]'
				: ''}"
