...
{#if !loading && workflow}
	<AdminCard
		class="p-4 border border-surface-500/30 dark:border-surface-500/40 shadow-sm space-y-4"
	>
		<div class="flex items-center justify-between">
			<div class="flex items-center gap-2">
				<iconify-icon
					icon="mdi:state-machine"
					class="text-tertiary-500 dark:text-primary-500 text-xl"
				></iconify-icon>
				<h3 class="text-sm font-bold uppercase tracking-wider opacity-50">Workflow Status</h3>
			</div>
			{#if currentState}
				<span
					class="inline-flex items-center gap-2 px-3 py-1 font-bold text-xs rounded-full"
					style="background-color: {currentState.color}20; color: {currentState.color}; border: 1px solid {currentState.color}40;"
				>
					<Badge
						variant="surface"
						class="flex items-center gap-2 font-bold text-xs bg-transparent!"
					>
		...
		{#if !instance}
			<div
				class="p-4 rounded bg-surface-500/10 dark:bg-surface-500/10 text-center border-2 border-dashed border-surface-500/30 dark:border-surface-500/40"
			>
		...
		{:else}
			<div class="space-y-3">
				<div class="flex flex-wrap gap-2">
					{#each availableTransitions as trans (trans.id)}
						<Button
							variant="primary"
							onclick={() => {
								selectedTargetStateId = trans.to;
								showComment = true;
							}}
					...
					{#if availableTransitions.length === 0}
						<p class="text-xs italic opacity-40">
							This entry has reached a final state or no transitions are available.
						</p>
					{/if}
				</div>
				...
				{#if showComment}
					<div
						class="space-y-3 pt-3 border-t border-surface-500/30 dark:border-surface-500/40"
						transition:slide
					>
						<Textarea
							bind:value={comment}
						...
						<div class="flex justify-end gap-2">
							<Button
								...
								size="sm"
								class="text-[10px]">Cancel</Button
							>
							<Button
								variant="tertiary"
								onclick={() => triggerTransition(selectedTargetStateId)}
								size="sm"
								class="text-[10px]">Confirm Move</Button
							>
						</div>
					</div>
				{/if}
						...
						{#if instance && instance.history}
							{#each instance.history.slice(-3).reverse() as log, i (i)}
								<div class="text-[10px] flex items-center justify-between opacity-60">
									<span
										>{workflow.states.find((s) => s.id === log.fromState)?.label} ➔ {workflow.states.find(
											(s) => s.id === log.toState
										)?.label}</span
									>
