...
{#if updateAvailable}
	...
	<div class="card preset-filled-primary-500 fixed bottom-4 right-4 z-50 max-w-sm p-4 shadow-xl">
		...
		<div class="mt-3 flex gap-2">
			<button class="preset-tonal-surface border border-surface-500 btn btn-sm" onclick={updateServiceWorker}> Update Now </button>
			<button class="preset-tonal border border-surface-500 btn btn-sm" onclick={() => (updateAvailable = false)}> Later </button>
		</div>
	</div>
{/if}
...
{#if import.meta.env.DEV && browser}
	<div class="card preset-tonal-surface fixed bottom-4 left-4 z-50 p-2 text-xs">
		<button class="preset-tonal border border-surface-500 btn btn-sm" onclick={clearCache}> Clear Cache </button>
	</div>
{/if}
