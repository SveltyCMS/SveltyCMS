...
function handleHeatmapGenerated(event) {
    const { heatmapData, keywordDensity } = event.detail;
    // You can use this data for further analysis or display if needed
    console.log('Heatmap data:', heatmapData);
    console.log('Keyword density:', keywordDensity);
  }

  function handleKeywordsInput(event) {
    seoKeywords = event.target.value.split(',').map(k => k.trim().toLowerCase());
  }
