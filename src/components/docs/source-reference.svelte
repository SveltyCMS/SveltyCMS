...
<script lang="ts">
	...
	let { path, moduleName = 'Source Code', githubBranch = 'next' }: SourceRefProps = $props();

	const githubUrl = $derived(`https://github.com/SveltyCMS/SveltyCMS/blob/${githubBranch}/${path}`);
</script>

<div
	class="my-4 inline-flex items-center gap-2 rounded-lg border border-surface-500/30 bg-surface-500/10 px-3.5 py-2 text-xs text-surface-600 transition-all hover:border-primary-500/30 hover:bg-primary-500/50 dark:border-surface-500/40 dark:bg-surface-900/60 dark:text-surface-400 dark:hover:border-primary-500/40 dark:hover:bg-primary-900/20"
>
