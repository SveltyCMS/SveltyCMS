...
<script lang="ts">
	...
	const classes = $derived(
		cn(
			...
			!finalPreset &&
				!isCustomColor &&
				'border border-surface-500/30 dark:border-surface-500/40 bg-surface-500/10 dark:bg-surface-900 text-surface-900 dark:text-surface-50 shadow-sm',
			className
		)
	);
</script>
...
<div class={classes} style={customStyles} {...rest}>
	{#if header}
		<div
			class="flex flex-col space-y-1.5 p-6 border-b border-surface-500/30 dark:border-surface-500/40"
		>
			{@render header()}
		</div>
	{/if}
