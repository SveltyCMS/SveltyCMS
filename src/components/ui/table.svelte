...
<script lang="ts">
	...
	const virtualVisibleStart = $derived(
		Math.max(0, Math.floor(virtualScrollTop / rowHeight) - VIRTUAL_OVERSCAN)
	);
	const virtualVisibleEnd = $derived(
		Math.min(
			data.length,
			Math.ceil((virtualScrollTop + virtualHeight) / rowHeight) + VIRTUAL_OVERSCAN
		)
	);
...
<div
	class={cn(
		'w-full flex flex-col rounded-2xl border border-surface-500/30 dark:border-surface-500/40 bg-white dark:bg-surface-900 shadow-xl overflow-hidden transition-all duration-300',
		className
	)}
>
	{#if header}
		<div
			class="bg-surface-500/10 dark:bg-surface-900/20 border-b border-surface-500/30 dark:border-surface-500/40"
		>
			{@render header()}
		</div>
	{/if}
			...
			>
				<table class="w-full text-start border-collapse min-w-full table-fixed">
					<thead class="sticky top-0 z-10">
						<tr
							class="bg-surface-500/10 dark:bg-surface-800/90 backdrop-blur-md border-b border-surface-500/30 dark:border-surface-500/40"
						>
							{#if selectable}
								<th class="w-12 p-4">
									<input
										...
										type="checkbox"
										class="size-4 rounded border-surface-500/30 dark:border-surface-600 accent-primary-500 transition-all"
										checked={allSelected}
							...
							{#each columns as col (col.key)}
								<th
									class={cn(
										'font-bold uppercase tracking-widest text-[10px] text-surface-600 dark:text-surface-400 select-none transition-colors',
										densityClass,
										col.sortable && 'hover:text-tertiary-500 dark:text-primary-500',
										col.class
									)}
									...
									<div class="flex items-center gap-2">
										...
										{#if col.sortable && sortKey === col.key}
											<iconify-icon
												...
												class="text-tertiary-500 dark:text-primary-500 animate-in fade-in zoom-in duration-300"
											></iconify-icon>
										{/if}
									</div>
								</th>
					...
					<tbody class="divide-y divide-surface-100 dark:divide-surface-800">
						...
						{#each virtualData as row, vi (row._id || row.id || vi)}
							...
							{:else}
								<tr
									class={cn(
										'group transition-all duration-200 hover:bg-tertiary-500  dark:hover:bg-tertiary-500 dark:bg-primary-500',
										selectedIds.has(row._id || row.id) && 'bg-tertiary-500 dark:bg-primary-500',
										onrowclick
									)}
									...
									onkeydown={(e) => {
										...
										if (e.key === ' ') {
											e.preventDefault();
											toggleSelectRow(row._id || row.id);
										}
									}}
								>
									{#if selectable}
										<td class="p-4" onclick={(e) => e.stopPropagation()}>
											<input
												...
												type="checkbox"
												class="size-4 rounded border-surface-500/30 accent-primary-500 transition-all hover:scale-110"
									...
									{#each columns as col (col.key)}
										<td
											class={cn(
												'text-surface-600 dark:text-surface-400 font-medium whitespace-nowrap overflow-hidden text-ellipsis',
												densityClass,
												col.class
											)}
										>
											{#if cell}{@render cell({ row, column: col })}{:else}{row[col.key] ??
													'-'}{/if}
										</td>
									{/each}
								</tr>
							{/if}
							{#if expand}
								<tr class="bg-surface-500/30 dark:bg-surface-800/20">
					...
					{#if loading}
						{#each Array(5) as _, i (i)}
							<tr class="animate-pulse">
								{#if selectable}<td class="p-4"
										><div class="size-4 bg-surface-200 dark:bg-surface-700 rounded-sm"></div></td
									>{/if}
								{#each columns as _, ci (ci)}<td class={densityClass}
										><div class="h-4 w-full bg-surface-200 dark:bg-surface-700 rounded"></div></td
									>{/each}
							</tr>
						{/each}
					{:else if data.length === 0}
						<tr>
							<td
								...
								class="p-20 text-center text-surface-400"
							>
								<div
									class="flex flex-col items-center gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500"
								>
					...
					{:else}
						{#each data as row, index (row._id || row.id || index)}
							...
							{:else}
								<tr
									class={cn(
										'group transition-all duration-200 hover:bg-tertiary-500 dark:bg-primary-500 dark:hover:bg-tertiary-500 ',
										selectedIds.has(row._id || row.id) && 'bg-tertiary-500 dark:bg-primary-500',
										onrowclick
									)}
									onclick={() => onrowclick?.(row)}
