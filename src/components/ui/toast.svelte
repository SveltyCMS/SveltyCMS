...
interface Props {
    toast: Toast;
    index: number;      // track position in stack
    count: number;      // determine reverse order
...
// This assumes the newest toast is at the end of your store array
const reverseIndex = $derived(count - 1 - index);
...
<!-- Only visually render the top 3 items to avoid off-screen overflow clutter -->
{#if reverseIndex < 3}
<div
    ...
    style:transform="translateY(-{reverseIndex * 12}px) scale({1 - reverseIndex * 0.05})"
    style:z-index={100 - reverseIndex}
    style:opacity={1 - reverseIndex * 0.25}
