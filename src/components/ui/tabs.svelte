...
<div class="w-full" data-testid={testId}>
  <div
    ...
    class="flex gap-1 {variant === 'pills' ? 'p-1 bg-surface-500/10 dark:bg-surface-800 rounded-xl' : variant === 'underline' ? 'border-b border-surface-500/30 dark:border-surface-500/40' : ''} overflow-x-auto"
  >
    {#each tabs as tab, i (tab.id)}
      <Button
        ...
        class="flex items-center gap-2 whitespace-nowrap px-4 py-3 text-sm font-medium transition-all duration-200 {variant === 'pills'
          ...
          : variant === 'underline'
            ? 'border-b-2 -mb-px ' + (activeTab === tab.id ? 'border-primary-500 text-primary-600 dark:text-primary-500' : 'border-transparent text-surface-500 hover:text-surface-600 dark:hover:text-surface-400 hover:border-surface-500/30')
            : 'rounded-lg ' + (activeTab === tab.id ? 'bg-primary-500/10 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' : 'text-surface-500 hover:text-surface-600 dark:hover:text-surface-400')
        }"
      >
        {#if tab.icon}
