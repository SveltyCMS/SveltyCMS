...
{#snippet treeNode(node: TreeItem, depth: number)}
    ...
    -->
		<div
			...
			role="treeitem"
			id={`treenode-${node.id}`}
			tabindex={isFocused || selectedId === node.id || (focusedNodeId === null && depth === 0)
				? 0
				: -1}
		...
		>
			...
			{#if dragOverNode?.id === node.id && dropPosition === 'before'}
				<div
					class="absolute -top-0.5 inset-s-0 inset-e-0 h-0.5 bg-tertiary-500 dark:bg-primary-500 z-10 rounded-full"
					transition:scale={{ duration: transitionDuration }}
				></div>
			{/if}
			...
			<svelte:element
				...
				class={cn(
					...
					dragOverNode?.id === node.id &&
						dropPosition === 'inside' &&
						'bg-tertiary-500/20! dark:bg-primary-500/20! border-tertiary-500! dark:border-primary-500!',
				...
				{#if showChevron}
					{#if node.isLoading}
						<div class="flex items-center justify-center {densityTokens.dummy}">
							<div
								class="h-3 w-3 animate-spin rounded-full border-2 border-surface-500 border-t-transparent"
								aria-label="Loading"
							></div>
						</div>
					{:else}
			...
			{#if dragOverNode?.id === node.id && dropPosition === 'after'}
				<div
					class="absolute -bottom-0.5 inset-s-0 inset-e-0 h-0.5 bg-tertiary-500 dark:bg-primary-500 z-10 rounded-full"
					transition:scale={{ duration: transitionDuration }}
				></div>
			{/if}
		</div>
		...
		{#if hasChildren}
			...
			>
				...
				{#if isMedia && isRoot && expanded}
					<div
						class={cn(
							'pointer-events-none absolute bottom-0 inset-s-5.75 top-0 w-px',
							mediaGuideLine
						)}
						aria-hidden="true"
					></div>
...
>
	{#if filteredItems.length === 0}
		<div
			class="flex flex-col items-center justify-center gap-2 p-6 text-center text-surface-500 dark:text-surface-400"
		>
			<iconify-icon icon="mdi:file-tree-outline" width="32" class="opacity-40" aria-hidden="true"
			></iconify-icon>
