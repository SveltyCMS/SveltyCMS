...
<script lang="ts">
	...
	const variantClass = $derived.by(() => {
		...
		switch (variant) {
			...
			case 'ghost':
				return 'hover:bg-surface-200/50 dark:bg-white/10 dark:hover:bg-white/20 text-surface-600 dark:text-surface-400';
			case 'transparent':
...
>
	...
	<div
		class="absolute inset-0 rounded-[inherit] bg-linear-to-tr from-white/10 to-transparent pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity"
	></div>
