...
<script lang="ts">
	...
	const themeClasses = $derived.by(() => {
		switch (calculatedVariant) {
			case 'success':
				return 'bg-success-500/10 text-success-600 dark:text-success-400 border-success-500/20';
			case 'warning':
				return 'bg-warning-500/10 text-warning-600 dark:text-warning-400 border-warning-500/20';
			case 'error':
				return 'bg-error-500/10 text-error-600 dark:text-error-400 border-error-500/20';
			case 'info':
				return 'bg-tertiary-500 text-white dark:bg-primary-500/10 dark:text-primary-500 border-tertiary-500 dark:border-primary-500/20';
			case 'ghost':
				return 'bg-transparent text-surface-500 border-surface-500/30 dark:border-surface-500/40';
			default:
