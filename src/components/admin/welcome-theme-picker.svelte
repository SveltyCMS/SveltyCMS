...
<script lang="ts">
	...
	let dismissed = $state(
		typeof localStorage !== 'undefined' &&
			localStorage.getItem('svelty-welcome-picker-dismissed') === 'true'
	);
	...
	const presets = [
		{
			...
			description:
				'Professional enterprise workspace. Cozy density, bordered cards, warm teal/blue palette.',
			density: 'cozy',
			variant: 'bordered',
			accent: 'bg-teal-600',
			ring: 'ring-teal-500'
		},
		{
			...
			icon: 'mdi:pen',
			description:
				'Distraction-free writing environment. Spacious layout, elevated cards, clean typography.',
			density: 'spacious',
			variant: 'elevated',
		...
		},
		{
			...
			icon: 'mdi:chart-line',
			description:
				'High-density data cockpit. Compact layout, flat cards, maximum information density.',
			density: 'compact',
			variant: 'flat',
	...
	async function activate(presetId: string) {
		...
		try {
			...
			if (res.ok) {
				toast.success(
					`Workspace set to ${presets.find((p) => p.id === presetId)?.name}. Reloading...`
				);
...
{#if !dismissed}
	<div transition:fade={{ duration: 200 }}>
		<AdminCard
			class="p-6 border-2 border-dashed border-tertiary-500/30 dark:border-primary-500/30 bg-tertiary-500/10 dark:bg-primary-500/10"
		>
			<div class="flex items-center justify-between mb-4">
				<div>
					<h2 class="text-lg font-bold text-surface-900 dark:text-white">Choose Your Workspace</h2>
					<p class="text-sm text-surface-500 dark:text-surface-400 mt-1">
						Pick a preset that matches your workflow. Customize later in Design System (themes,
						overrides, and live preview).
					</p>
				</div>
				<Button variant="ghost" size="sm" onclick={skip} aria-label="Skip workspace setup"
					>Skip</Button
				>
			</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				{#each presets as preset (preset.id)}
					...
					>
						<!-- Preset accent dot -->
						<div
							class="h-3 w-3 rounded-full {preset.accent} ring-2 {preset.ring} ring-offset-2 ring-offset-white dark:ring-offset-surface-900"
						></div>

						<iconify-icon
							icon={preset.icon}
							width="28"
							class="text-tertiary-500 dark:text-primary-500 group-hover:scale-110 transition-transform"
						></iconify-icon>
						...
						<div class="text-center">
							...
							<div class="flex items-center justify-center gap-2 mt-1">
								<span
									class="text-[10px] font-medium uppercase tracking-wider text-surface-400 bg-surface-500/10 dark:bg-surface-800 px-1.5 py-0.5 rounded"
									>{preset.density}</span
								>
								<span
									class="text-[10px] font-medium uppercase tracking-wider text-surface-400 bg-surface-500/10 dark:bg-surface-800 px-1.5 py-0.5 rounded"
