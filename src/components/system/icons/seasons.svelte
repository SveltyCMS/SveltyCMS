	...
	{#if seasonRegion === 'Western_Europe'}
		...
		{#if isValentine}
			...
			<div class="absolute -top-28 start-1/2 -translate-x-1/2 -translate-y-1/2">
				<iconify-icon icon="mdi:heart" width="40" class="absolute -start-[60px] -top-[10px] text-error-600"></iconify-icon>
		...
		{#if isEaster}
			...
			<div class="absolute -top-24 start-1/2 -translate-x-1/2 -translate-y-1/2">
				...
				<iconify-icon icon="game-icons:easter-egg" width="40" class="absolute -top-[25px] start-0 rotate-12 text-warning-500"></iconify-icon>
				<iconify-icon icon="game-icons:high-grass" width="40" class="absolute -top-[5px] end-10 -rotate-32 text-success-500"></iconify-icon>
				<iconify-icon icon="mdi:easter" width="70" class="absolute -top-[31px] start-8 rotate-32 text-error-500"></iconify-icon>
			</div>
		{/if}
	...
	{#if seasonRegion === 'East_Asia'}
		{#if isChineseNewYear}
			...
			<div class="absolute start-1/2 top-[-50px] -translate-x-1/2 justify-center">
				<iconify-icon icon="noto:lantern" width="40" class="absolute -start-[60px] -top-[20px] text-error-600"></iconify-icon>
				<iconify-icon icon="noto:dragon-face" width="40" class="absolute -end-[60px] -top-[20px]"></iconify-icon>
			</div>
			<p class="absolute start-[-40px] top-[-50px] justify-center whitespace-nowrap text-2xl font-bold text-error-600">{login_new_year()}</p>
		{/if}
	...
	{#if seasonRegion === 'South_Asia'}
		{#if isDiwali}
			...
			<div class="absolute start-1/2 top-[-50px] -translate-x-1/2 justify-center">
				...
				<iconify-icon icon="noto:sparkles" width="50" class="absolute -end-[160px] top-[120px] text-warning-500"></iconify-icon>
				<iconify-icon icon="noto:sparkles" width="50" class="absolute -end-[200px] top-[100px] rotate-90 text-warning-500"></iconify-icon>
			</div>
			<p class="absolute -start-[10px] top-[170px] justify-center whitespace-nowrap text-3xl font-bold italic text-warning-600">
				{login_happy_diwali()}
			</p>
		{/if}
		...
		{#if isHoli}
			...
			<div class="absolute start-1/2 top-[-50px] -translate-x-1/2 justify-center">
				...
				<iconify-icon icon="noto:balloon" width="50" class="absolute end-[60px] top-[20px] text-success-500"></iconify-icon>
