---
path: "src/widgets/custom/currency/currency.mdx"
...
<blockquote>
  **Currency input widget** that stores precise numeric values while displaying and accepting
  localized, formatted currency strings with proper symbols and thousand separators.
</blockquote>
...
| Below minimum               | "Amount must be at least {formatted min}" |
| Above maximum               | "Amount must not exceed {formatted max}"  |
| Invalid format during input | Auto-corrects on blur                     |

> **Note**: Error messages display formatted currency values (e.g., "Amount must be at least €10.00") for better UX.
...
The Currency widget is part of SveltyCMS and is licensed under the Business Source License 1.1.
