...
<blockquote>
  **Address input widget** with optional Mapbox integration for interactive mapping, geocoding, and
...
- ✅ **Structured Data**: Stores complete address object with all components
- ✅ **Mapbox Integration**: Optional interactive map with draggable markers
...
- ✅ **Validation**: Valibot schema ensures complete, valid addresses with required field checks
- ✅ **Unified Error Handler**: Uses `handleWidgetValidation` for consistent error management
- ✅ **Non-Translatable**: Addresses are universal (not language-specific)
- ✅ **Autocomplete**: HTML5 address autocomplete for better UX
...
// MongoDB Document
{
  ...
  "address": {
    ...
    "latitude": 34.0736,          // ✅ Number type
    "longitude": -118.4004        // ✅ Number type
  },
...
The widget now uses the **Google Maps JavaScript API** via the lightweight `@googlemaps/js-api-loader`.

1.  **Draggable Marker**: Click and drag to set exact location.
2.  **Place Autocomplete**: Search for addresses globally with Google Places.
3.  **Locate Me**: Use browser geolocation to find your current position.
4.  **Draggable Marker**: Updates coordinates automatically.

### Configuration

To enable map features, you must provide a **Google Maps API Key**:
...
- The map container will show a friendly "Map unavailable - API Key missing" message with an icon.
- Manual address input fields remain fully functional.
- Console warnings will alert developers to the missing key.
...
2. **Don't Mix Coordinate Order**
   ...
   coordinates: [34.0522, -118.2437]; // ❌ [lat, lng] WRONG!
   ```
...
- ✅ **Required Fields**: Street, city, postal code, country validation when required
- ✅ **Error Display**: Improved error messaging through `validationStore`

### Version 1.1.0 (2025-01-25)

- ✅ **Performance**: Implemented hybrid loading strategy (Core 14KB vs Full 160KB lazy loaded)
- ✅ **Search**: Added client-side country search
- ✅ **Multilingual**: Strict UI/Data separation using Dual-Language architecture

### Version 1.0.0 (2025-01-15)

- ✅ Initial release with structured address storage
- ✅ Optional Mapbox integration
- ✅ Geospatial query support
- ✅ Multilingual country selection
