...
<script module lang="ts">
	...
	const PLATFORMS = [
		...
		},
		{
			id: 'linkedin',
			icon: 'mdi:linkedin',
			color: 'text-tertiary-600 dark:text-primary-500',
			label: 'LinkedIn'
		},
...
<script lang="ts">
	...
	let displayTitle = $derived(
		activePlatform === 'twitter' ? twitterTitle || ogTitle || 'Page Title' : ogTitle || 'Page Title'
	);
...
<div class="card preset-tonal-surface p-4 rounded-lg mb-6">
		...
		{#each PLATFORMS as platform (platform.id)}
			<SystemTooltip title={platform.label}>
				<Button
					...
					class="p-0! min-w-0 transition-all {activePlatform === platform.id
						? ' ring-2 ring-surface-900 dark:ring-white scale-110'
						: ' hover:bg-surface-500/10 dark:hover:bg-surface-700'}"
				>
					<iconify-icon
						icon={platform.icon}
						width="24"
	...
	<!-- Preview Card Area -->
	<div
		class="bg-surface-500/10 dark:bg-surface-900 rounded p-4 md:p-8 flex justify-center border border-surface-500/30 dark:text-surface-50"
	>
		<!-- Dynamic Preview Styling based on Platform -->
		<div
			class="w-full max-w-125 bg-white text-black overflow-hidden shadow-lg rounded transition-all duration-300"
		>
			<!-- Image Area -->
			<div
			...
			<div class="p-3 bg-[#f0f2f5] border-t border-gray-200">
				...
				<div
					class="font-bold text-[16px] leading-snug text-[#050505] line-clamp-2 md:line-clamp-1 font-sans mb-1"
				>
					{displayTitle}
				</div>
