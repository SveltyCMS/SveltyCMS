...
<div class="relative mt-2 border-t border-surface-500 dark:border-white dark:bg-transparent">
	...
	<div class="absolute left-0 top-1 flex justify-between gap-2">
		<button
			...
			aria-label="Preview"
			class="{SeoPreviewToggle ? 'hidden' : 'block'} bg-tertiary-500 text-white btn btn-sm flex items-center justify-center"
		>
		...
		<button
			onclick={handleTogglePreview}
			aria-label="Preview"
			class="{SeoPreviewToggle ? 'block' : 'hidden'} bg-tertiary-500 text-white btn flex items-center justify-center"
		>
	...
	{#if SeoPreviewToggle}
		<!-- Mobile Preview -->
		<div class="min-h-30 card bg-secondary-500/50 backdrop-blur mx-auto mt-4 max-w-sm p-1 sm:p-2 md:p-4">
	...
	{:else}
		<!-- Desktop Preview -->
		<div class="card bg-secondary-500/50 backdrop-blur mt-4 p-1 sm:p-2 md:p-4">
