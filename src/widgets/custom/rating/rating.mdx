---
path: "src/widgets/custom/rating/rating.mdx"
title: "Rating Widget"
...
<blockquote>
  **Interactive star rating widget** that stores numeric ratings with customizable icons and maximum
  values.
</blockquote>

## Overview

The **Rating widget** provides an intuitive star-based rating system for capturing user feedback, product reviews, service quality scores, and any other numeric rating needs. It uses the native Rating component for professional interaction patterns with click and hover states.
...
- ✅ **Native Rating Component**: Professional UI component with built-in interactions
...
function calculateOverallRating(data: any) {
  ...
  return (
    data.qualityRating * weights.quality +
    data.valueRating * weights.value +
    data.serviceRating * weights.service
  );
}
```
...
- ✅ Initial release with native Rating component
