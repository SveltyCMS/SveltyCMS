...
> During the trial period, all features are available without a license key. After the trial expires, a valid license key from [marketplace.sveltycms.com](https://marketplace.sveltycms.com) is required to unlock premium platforms.

### Free Tier (No License)

| Feature            | Description                                                            |
| :----------------- | :--------------------------------------------------------------------- |
| YouTube            | Full metadata fetching, embedding, and display                         |
| Vimeo              | Full metadata fetching, embedding, and display                         |
| Automatic metadata | Title, thumbnail, duration, channel info auto-fetched                  |
| Server-side APIs   | API keys protected, CORS handled server-side                           |
| Rich preview       | Visual thumbnail and video details display                             |
| Structured data    | Comprehensive video object storage                                     |
| Trial period       | Full access to ALL platforms for 14 days from first admin registration |
...
| Twitch                 | Live stream and VOD embedding with metadata |
| TikTok                 | Short-form video embedding with metadata    |
| Platform expansion     | Access to new platforms as they are added   |
...
if (!status.active && !status.hasLicense) {
  // Premium platforms (Twitch, TikTok) are unavailable
  // YouTube and Vimeo remain functional
}
```
