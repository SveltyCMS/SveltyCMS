---
path: "src/widgets/custom/color-picker/colorpicker.mdx"
...
<blockquote>
  **Color selection widget** that provides an intuitive interface combining native HTML5 color
  picker with hex code input for precise color selection.
</blockquote>
...
<input
  ...
  aria-invalid="false"
  aria-describedby="brandColor-error"
/>
```
...
- ✅ Added unified `handleWidgetValidation` integration
- ✅ Extended hex format support (#RGB, #RRGGBB, #RGBA, #RRGGBBAA)
- ✅ Improved error messages with format examples
- ✅ Integration with `validationStore` for form-wide validation state
