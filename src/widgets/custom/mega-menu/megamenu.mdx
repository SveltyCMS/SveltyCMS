---
path: "src/widgets/custom/mega-menu/megamenu.mdx"
...
The **MegaMenu** is a powerful, builder widget designed for creating nested, hierarchical menu structures. It is the ideal solution for managing complex site navigation, multi-level documentation sidebars, or detailed footers. Its core strength lies in its flexibility, allowing developers to define exactly what fields are available at each level of the menu.
...
A clean, focused editing experience for each menu item using the CMS's built-in native modal system with proper context passing.

### ✅ **Robust Validation**

Employs a comprehensive validation schema to ensure data integrity throughout the entire nested structure.

### 🎛️ **UI Feature Toggles**
...
- **➕ Add Root Item**: Adds a new item to the top level of the menu
- **➕ Add Child Item**: Creates nested items under parent items
- **✏️ Edit**: Opens a modal with level-specific fields for editing
- **🗑️ Delete**: Removes items and all descendants (with confirmation)
- **⋮⋮ Drag Handle**: Click and drag to reorder items among siblings
- **⯆ Expand/Collapse**: Toggle visibility of child items for organization
    ...
    "children": [
      {
        ...
        "_fields": {
          ...
          "description": {
            ...
            "de": "Unsere neuesten Softwarelösungen."
          }
        },
        "children": []
      }
  ...
  <details class="menu-item" open={level === 0}>
    <summary class="menu-summary {isActive ? 'active' : ''}">
      <a href={item._fields.url} class="menu-link">
        {item._fields.title?.[app.contentLanguage] || 'Untitled'}
      </a>
    </summary>
    <ul class="submenu">
