...
description: 'Enterprise color selection widget with native HTML5 color picker and hex input for SveltyCMS'
order: 302
...
<blockquote>
	**Enterprise color selection widget** that provides an intuitive interface combining native HTML5 color picker with hex code input for precise color
	selection.
</blockquote>
...
- ✅ **Native Color Picker**: Uses HTML5 `<input type="color">` for system-native color selection
- ✅ **Hex Code Input**: Manual entry and display of 6-digit hex codes (#RRGGBB)
- ✅ **Bidirectional Sync**: Changes in either picker or text input update both
- ✅ **Format Validation**: Valibot ensures valid hex format (#[0-9A-F]{6})
- ✅ **Visual Preview**: Large color swatch shows selected color
- ✅ **Non-Translatable**: Color codes are universal (automatically sets `translated: false`)
- ✅ **Accessible**: Full keyboard navigation and screen reader support
- ✅ **Mobile Optimized**: Native color picker on iOS/Android devices
- ✅ **Monospace Font**: Hex input uses monospace for better readability
- ✅ **Default Black**: Smart default to `#000000` for empty values
...
| `helper`       | `string`  | ❌ No     | `undefined` | Helper text displayed below field                |
| `default`      | `string`  | ❌ No     | `#000000`   | Default hex color (must be valid #RRGGBB format) |
| `readonly`     | `boolean` | ❌ No     | `false`     | Make field read-only                             |
| `translated`   | `boolean` | 🔒 Locked | `false`     | Always false (colors are universal)              |
...
// - Must be a string
// - Must match regex: /^#[0-9a-f]{6}$/i
// - If required: must not be empty
// - If not required: can be empty or valid hex
```
...
// ❌ Invalid formats
'FF5733'; // Missing #
'#FFF'; // 3-digit shorthand (must expand to 6)
'#FF57330'; // 7 digits
'#GGGGGG'; // Invalid hex characters
'rgb(255, 87, 51)'; // RGB format not supported
...
// MongoDB Document
{
  "_id": "theme_123",
  "name": "Dark Theme",
  "primaryColor": "#2563EB",      // ✅ Hex string
...
1. **Don't Store Other Formats**
   ...
   ```typescript
   // ❌ Don't use RGB, HSL, or named colors
...
- ✅ Initial release with native HTML5 color picker
- ✅ Hex code input and validation
- ✅ Bidirectional sync between swatch and hex input
- ✅ Full accessibility support
- ✅ Mobile-optimized color selection

---

## License

The ColorPicker widget is part of SveltyCMS and is licensed under the MIT License.
