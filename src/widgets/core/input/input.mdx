...
| `encrypt`      | Toggles            | Store AES-256-GCM ciphertext at rest             |          |
...
Store API credentials, national IDs, or private notes as AES-256-GCM ciphertext.
The admin UI still shows plaintext; the database stores only a `v1:iv:tag:ciphertext` envelope.
...
- [PhoneNumber Widget](/src/widgets/custom/phone-number/phonenumber.mdx) - For phone numbers with E.164 validation
