...
@component
**GuiField** is a component that allows users to select a collection from a dropdown.
-->
	...
	let _value = $state<CollectionTypes>(
		typeof value === 'string'
			...
			: (Object.values(collections.value).find((entry) => {
					return typeof value !== 'string' && entry['name'] === value['name'];
				}) as CollectionTypes) || ('null' as CollectionTypes)
	);
