---
path: "src/widgets/core/date-time/date-time.mdx"
...
Overview
The DateTime field provides a user-friendly calendar interface for selecting a specific date. It can also be configured to include a time picker for situations where the exact time of day is important, such as for an event or a publication schedule.
...
- Date & Time Field: After selecting a date, an additional interface will appear allowing you to select the hour and minute.
  ...
  The DateTimeWidget is highly versatile. Its primary configuration option is the timePicker property, which toggles between a date-only and a date-time input.
  Example Configuration
  Here is how you would set up a "Publication Date" (date only) and an "Event Start" (date and time).
  import { DateTimeWidget } from '@src/widgets';
