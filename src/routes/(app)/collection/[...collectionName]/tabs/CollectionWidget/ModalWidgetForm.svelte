<script lang="ts">
	...
	let specificOptions = options.filter(
		(option) => !['label', 'display', 'db_fieldName', 'permissions', 'required', 'translated', 'icon', 'helper', 'width'].includes(option)
	);
		...
		<form class={cForm}>
			<TabGroup justify="justify-between lg:justify-start">
				...
				<svelte:fragment slot="panel">
					<svelte:component this={tabSet === 0 ? Default : tabSet === 1 ? Permission : Specific} bind:guiSchema />
				</svelte:fragment>
			</TabGroup>
		</form>
