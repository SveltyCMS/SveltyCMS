...
<script lang="ts">
	...
	const itemHeight = $derived(
		gridSize === 'tiny' ? 120 : gridSize === 'small' ? 180 : gridSize === 'medium' ? 280 : 400
	);
	const itemWidth = $derived(
		gridSize === 'tiny' ? 100 : gridSize === 'small' ? 160 : gridSize === 'medium' ? 240 : 360
	);
...
>
	<div class="w-full relative" style:height="{totalRows * itemHeight}px">
		<div
			...
			{#each visibleItems as file (file._id || file.filename)}
				...
				>
					...
					<div class="relative flex-1 bg-surface-500/10 dark:bg-surface-800 overflow-hidden">
						...
						<!-- Action Dock (Hover) -->
						<div
							class="absolute inset-e-2 top-2 z-30 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity"
						>
							<Button
								variant="ghost"
					...
					<div class="p-2 border-t border-surface-100 dark:border-surface-500/40">
						<div
							class="truncate text-[10px] font-bold uppercase tracking-tighter opacity-60 mb-0.5"
						>
							{file.filename}
						</div>
						<div class="text-[9px] font-mono opacity-40">
