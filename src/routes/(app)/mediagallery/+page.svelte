...
<script lang="ts">
	...
	function getMediaSavedViewSnapshot(): Omit<
		SmartTableSavedView,
	...
	const publishedMediaIds = $derived(
		new SvelteSet<string>((data as { publishedMediaIds?: string[] }).publishedMediaIds ?? [])
	);
	...
	const filteredFiles = $derived.by(() => {
		let result = files.filter((file) => {
			...
			if (searchCriteria) {
				...
				if (
					...
					!file.filename?.toLowerCase().includes(searchCriteria.filename.toLowerCase())
				)
					return false;
				if (searchCriteria.minSize && (file as StoredMediaBase).size < searchCriteria.minSize)
					return false;
				if (searchCriteria.maxSize && (file as StoredMediaBase).size > searchCriteria.maxSize)
					return false;
				if (searchCriteria.minWidth && (!img.width || img.width < searchCriteria.minWidth))
					return false;
				if (searchCriteria.maxWidth && (!img.width || img.width > searchCriteria.maxWidth))
					return false;
				if (searchCriteria.minHeight && (!img.height || img.height < searchCriteria.minHeight))
					return false;
				if (searchCriteria.maxHeight && (!img.height || img.height > searchCriteria.maxHeight))
					return false;
				if (
					searchCriteria.uploadedAfter &&
					new Date(file.createdAt || 0) < searchCriteria.uploadedAfter
				)
					return false;
				if (
					searchCriteria.uploadedBefore &&
					new Date(file.createdAt || 0) > searchCriteria.uploadedBefore
				)
					return false;
				if (
					searchCriteria.fileTypes &&
					searchCriteria.fileTypes.length > 0 &&
					!searchCriteria.fileTypes.some((t) => (file as any).mimeType?.includes(t))
				)
					return false;
				...
				if (
					...
					(!meta?.exif || (meta.exif as any).camera !== searchCriteria.camera)
				)
					return false;
				if (
					searchCriteria.location &&
					(!meta?.exif || (meta.exif as any).location !== searchCriteria.location)
				)
					return false;
				if (searchCriteria.dominantColor && meta?.dominantColor !== searchCriteria.dominantColor)
					return false;
		...
		result.sort((a, b) => {
			switch (sortBy) {
				...
				case 'size-desc':
					return (b as StoredMediaBase).size - (a as StoredMediaBase).size;
				case 'size-asc':
					return (a as StoredMediaBase).size - (b as StoredMediaBase).size;
				case 'newest':
				default:
	...
	onMount(() => {
		...
		registerHotkey(
			'mod+f',
			() => document.getElementById('media-gallery-search')?.focus(),
			'Focus Search'
		);
		registerHotkey(
			'mod+a',
	...
	async function uploadFiles(fileList: FileList | File[]) {
		...
		const handle = uploadMediaFilesHandle(list, {
			...
			onFileProgress: (fp) => {
				...
				uploadFileLabel =
					list.length > 1 ? `${fp.fileIndex + 1}/${fp.fileCount}: ${fp.fileName}` : fp.fileName;
			},
	...
	async function handleOpenFileDetails(file: any) {
		const { default: MediaDetailsModal } =
			await import('@src/components/media/media-details-modal.svelte');
...
>
	{#snippet actions()}
		<div class="flex items-center gap-1 sm:gap-1.5">
			...
			<span class="hidden h-4 w-px bg-surface-300 sm:block dark:bg-surface-700" aria-hidden="true"
			></span>
	...
	{#if isUploading}
		<div class="shrink-0 px-2 pb-1 sm:px-3">
			...
			>
				...
				{#if uploadFileLabel}
					<span
						class="hidden max-w-40 truncate text-surface-500 sm:inline dark:text-surface-400"
						title={uploadFileLabel}
					>
						{uploadFileLabel}
					</span>
				{/if}
				<span class="shrink-0 font-medium tabular-nums text-surface-600 dark:text-surface-400"
					>{uploadProgress}%</span
				>
				<Button
					variant="outline"
					...
					{@const dropOptions = {
						...
						callbacks: {
							onDrop: (state: DragDropState<MediaDragData>) =>
								handleBreadcrumbDrop(state, crumb.folderId, crumb.name)
						}
					}}
		...
		<div class="shrink-0 px-2 sm:px-3" data-testid="media-gallery-toolbar">
			...
			<div class="hidden items-center gap-2 py-2 sm:flex">
				...
				>
					...
					>
						<iconify-icon icon="mdi:format-list-bulleted" width="16" aria-hidden="true"
						></iconify-icon>
					</button>
				</div>
		...
		>
			...
			{:else}
				{#await import('./media-table.svelte')}
					<div class="flex flex-1 items-center justify-center py-16 text-sm text-surface-500">
						{mediagallery_loading_table()}
					</div>
				{:then mod}
					<mod.default
	...
	{#if showAdvancedSearch}
		<Portal>
			<div
				class="fixed inset-0 z-100 bg-surface-900/50 backdrop-blur-sm transition-all"
				aria-hidden="true"
			></div>
