	...
	{:else}
		{#each visibleFiles as file (file._id || file.filename)}
          ...
          {isSelected ? 'ring-1 ring-inset ring-primary-500/50' : ''}"
				...
				use:liftAndCarry={{
					...
					dragData: {
						...
						preview: {
							...
							url: file.type === 'image' ? mediaDisplayUrl(file, previewSize) : undefined,
							type: file.type
						}
					},
				...
				ontouchstart={(e) => {
					e.stopPropagation();
					touchActiveId = fileId;
				}}
			>
				{#if isSelected}
					<div
						class="absolute inset-y-0 inset-s-0 z-10 w-0.5 bg-primary-500"
						aria-hidden="true"
					></div>
				{/if}
          ...
          -->
					...
					>
						...
						<div
							class="pointer-events-none absolute inset-x-0 bottom-0 z-10 bg-linear-to-t from-black/75 via-black/30 to-transparent px-2.5 pb-2 pt-8 transition-opacity duration-200 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100 {touchActiveId ===
							fileId
								? 'opacity-100'
								: 'opacity-0'}"
						>
							<p
								class="truncate text-[11px] font-medium leading-tight text-white"
								title={file.filename}
							>
								{file.filename}
							</p>
					...
					<div
						class="absolute inset-e-2 top-2 z-20 flex flex-col gap-1 transition-opacity duration-200 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100 {touchActiveId ===
						fileId
							? 'opacity-100'
							: 'opacity-0'}"
						data-no-drag
					...
					>
						...
						>
							{#snippet children()}
								<span class="font-serif text-sm font-semibold italic leading-none text-primary-500"
									>i</span
								>
							{/snippet}
							{#snippet content()}
								<div
									class="flex flex-col text-[11px] text-surface-600 dark:text-surface-400 min-w-60"
								>
									<div
									...
									<table class="w-full text-start border-collapse mt-2">
										<thead>
											<tr
												class="border-b border-surface-500/30 dark:border-surface-500/40 text-primary-600 dark:text-primary-500"
											>
												<th
										...
										<tbody class="text-surface-600 dark:text-surface-400">
											<tr class="border-b border-surface-500/30 dark:border-surface-500/40">
												...
												<td
													...
													>{getDimensionsLabel(file) || '-'}</td
												>
												<td class="py-1 px-2 text-end tabular-nums" colspan="2"
													>{formatBytes((file as MediaImage).size)}</td
												>
											</tr>
											{#if file.thumbnails}
												...
												{#each deriveSizeKeys(file.thumbnails) as sizeKey (sizeKey)}
													...
													<tr
														class="border-b border-surface-500/30 dark:border-surface-500/40 last:border-0"
													>
														<td
															class="py-1 px-2 font-bold text-primary-600 dark:text-primary-500 border-e border-surface-500/30 dark:border-surface-500/40 text-start"
															>{sizeKey}</td
														...
														{#if hasWebp}
															<td
																class="py-1 px-2 text-end tabular-nums border-e border-surface-500/30 dark:border-surface-500/40"
															>
						...
						{#if file.type === 'image'}
							...
							>
								{#snippet children()}
									<iconify-icon
										icon={(file as MediaImage).metadata?.tags?.length ||
						...
						>
							{#snippet children()}
								<iconify-icon icon="mdi:trash-can-outline" width={15} class="text-error-500"
								></iconify-icon>
							{/snippet}
						</MediaGridActionTooltip>
					</div>
				...
				<div
					class="mt-1.5 flex items-baseline justify-between gap-2 border-b border-surface-500/30 px-1.5 pb-2 pt-0.5 sm:px-2 dark:border-surface-500/40"
				>
					<span
						class="shrink-0 font-mono text-[10px] font-semibold uppercase tracking-wide text-surface-500 sm:text-[11px] dark:text-surface-400"
					>
						{formatMimeType((file as MediaImage).mimeType)}
					</span>
					<span
						class="shrink-0 font-mono text-[10px] tabular-nums text-surface-400 sm:text-[11px] dark:text-surface-500"
					>
