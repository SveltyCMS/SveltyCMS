...
<div class="flex flex-wrap items-start gap-4 overflow-auto content-start p-1 min-h-[400px]" role="grid" aria-label="Media asset grid">
	{#if filteredFiles.length === 0}
		<div class="flex flex-col items-center justify-center w-full min-h-[300px] border-2 border-dashed border-surface-300 dark:border-surface-700 rounded-2xl bg-surface-50/50 dark:bg-surface-800/20" transition:scale={{ duration: 200 }}>
			<iconify-icon icon="mdi:cloud-upload-outline" width="64" class="text-surface-300 dark:text-surface-600 mb-4"></iconify-icon>
			<h3 class="text-xl font-semibold text-surface-600 dark:text-surface-300">No media found</h3>
			<p class="text-sm text-surface-400 dark:text-surface-500 mb-6 text-center max-w-xs">
				Drop files here or use the upload button to start building your library.
			</p>
			
			<label class="btn btn-lg preset-filled-primary-500 cursor-pointer shadow-lg hover:shadow-primary-500/20 transition-all">
				...
				<input 
					...
					onchange={(e) => {
						...
						if (input.files?.length) {
							const event = new CustomEvent('externalUpload', { detail: { files: input.files } });
