	...
	<div class="media-table-scroll min-h-0 flex-1 overflow-auto">
		...
		<div class="flex flex-col md:hidden" role="table" aria-label="Media files">
			...
			{#if filteredFiles.length === 0}
				<div class="flex flex-col items-center justify-center gap-2 py-12 text-center" role="row">
					<div role="cell">
						<iconify-icon icon="mdi:image-search-outline" width="32" class="text-surface-400 dark:text-surface-500"></iconify-icon>
						<div class="mt-2 space-y-1">
							<p class="text-sm font-semibold">No media found</p>
							<p class="text-xs text-surface-500 dark:text-surface-400">Try adjusting your search or filter.</p>
						</div>
					</div>
				</div>
			{/if}
		...
		<table class="media-table hidden w-full border-collapse text-sm md:table lg:table-fixed">
			...
			<tbody>
				...
				{#if filteredFiles.length === 0}
					<tr>
						<td colspan="6" class="px-2 py-16 text-center sm:px-3">
