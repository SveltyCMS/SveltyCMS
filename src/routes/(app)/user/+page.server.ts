...
async function loadUserSession(event) {
  const session_id = event.cookies.get(SESSION_COOKIE_NAME) as string;
  return auth.validateSession(new mongoose.Types.ObjectId(session_id));
}
...
export async function load(event) {
  try {
    const user = await loadUserSession(event);
...
async function saveAvatarImage(file: File, path: 'avatars' | string): Promise<string> {
  try {
    ...
    const format = ext === '.svg' ? 'svg' : publicEnv.MEDIA_OUTPUT_FORMAT_QUALITY.format === 'original' ? ext : publicEnv.MEDIA_OUTPUT_FORMAT_QUALITY.format;
    ...
    if (format === 'svg') {
      ...
      const result = await sharp(buffer)
        ...
        .toFormat(format as keyof import('sharp').FormatEnum, {
          quality: publicEnv.MEDIASERVER_URL
        })
...
export const actions: Actions = {
  addUser: async ({ request, cookies }) => {
    try {
      ...
      const user = await loadUserSession({ cookies });
