...
<div class="wrapper h-full max-h-screen space-y-6 overflow-y-auto p-4 pb-16">
	...
	{:else if error}
		<div class="rounded border border-error-500/30 bg-error-500/10 p-4 dark:bg-error-900/20">
			<div class="flex items-start gap-3">
				<iconify-icon icon="mdi:alert-circle" width="24" class="mt-1 text-xl text-error-500"></iconify-icon>
				<div>
					<h3 class="font-semibold text-error-500">Error Loading Widgets</h3>
					<p class="text-error-500">{error}</p>
					<button onclick={() => loadWidgets()} class="mt-2 rounded bg-error-500 px-3 py-1 text-sm text-white hover:bg-error-500">Retry</button>
				</div>
			</div>
		</div>
	{:else}
		<!-- Permission Notice -->
		{#if !canManageWidgets}
			<div class="rounded border border-warning-500/30 bg-warning-500/10 p-4 dark:bg-warning-900/20">
				<div class="flex items-start gap-3">
					<iconify-icon icon="mdi:information" width="24" class="mt-1 text-xl text-warning-500"></iconify-icon>
					<div>
						<h3 class="font-semibold text-warning-500">Limited Access</h3>
		...
		<div class="flex gap-2 border-b border-[var(--admin-border-default)]" role="tablist" aria-label="Widget Categories">
			...
			>
				<div class="flex items-center gap-2">
					...
					<span class="rounded-full bg-tertiary-500/10 px-2 py-0.5 text-xs font-medium text-tertiary-500 dark:bg-tertiary-900/20">
						Coming Soon
					</span>
				</div>
			</button>
			...
			<div class="grid grid-cols-2 gap-4 md:grid-cols-4" data-testid="widget-stats">
				...
				<div class="relative rounded bg-tertiary-500/10 p-4 shadow-sm transition-all hover:bg-tertiary-500/10 dark:bg-tertiary-900/20 dark:hover:bg-tertiary-900/20">
					<Button variant="ghost"
					 ...
					 class="p-0! min-w-0 absolute inset-e-2 top-2 text-tertiary-500">
						<iconify-icon icon="mdi:information" width="20"></iconify-icon>
					</Button>
					<div class="flex items-center gap-3">
						<iconify-icon icon="mdi:widgets" width="24" class="text-2xl text-tertiary-500"></iconify-icon>
						<div>
							<h3 class="font-semibold text-tertiary-500">Total</h3>
							<p class="text-2xl font-bold text-tertiary-500">{stats.total}</p>
						</div>
					</div>
				</div>
				...
				<div class="relative rounded bg-success-500/10 p-4 shadow-sm transition-all hover:bg-success-500/10 dark:bg-success-900/20 dark:hover:bg-success-900/20">
					...
					<div class="flex items-center gap-3">
						<iconify-icon icon="mdi:check-circle" width="24" class="text-2xl text-tertiary-500"></iconify-icon>
						<div>
							<h3 class="font-semibold text-tertiary-500">Active</h3>
							<p class="text-2xl font-bold text-tertiary-500">{stats.active}</p>
						</div>
					</div>
				</div>
				...
				<div class="relative rounded bg-tertiary-500/10 p-4 shadow-sm transition-all hover:bg-tertiary-500/10 dark:bg-tertiary-900/20 dark:hover:bg-tertiary-900/20">
					...
					<div class="flex items-center gap-3">
						<iconify-icon icon="mdi:puzzle" width="24" class="text-2xl text-tertiary-500"></iconify-icon>
						<div>
							<h3 class="font-semibold text-tertiary-500">Core</h3>
							<p class="text-2xl font-bold text-tertiary-500">{stats.core}</p>
						</div>
					</div>
				</div>
				...
				>
					<Button variant="ghost"
					 ...
					 class="p-0! min-w-0 absolute inset-e-2 top-2 text-warning-500">
						<iconify-icon icon="mdi:information" width="20"></iconify-icon>
					</Button>
					<div class="flex items-center gap-3">
						<iconify-icon icon="mdi:puzzle-plus" width="24" class="text-2xl text-warning-500"></iconify-icon>
						<div>
							<h3 class="font-semibold text-warning-500">Custom</h3>
							<p class="text-2xl font-bold text-warning-500">{stats.custom}</p>
						</div>
					</div>
				</div>
			</div>
						...
						<Button variant="tertiary"
							...
							<span
								class="rounded-full px-2 py-0.5 text-xs font-semibold {activeFilter === filter.value
									...
									: 'bg-surface-500/10 text-[var(--admin-text-muted)] dark:bg-surface-800'}"
							>
								{filter.count}
							</span>
						</Button>
		...
		{:else}
			...
			<div class="rounded border border-[var(--admin-border-default)] bg-surface-500/10 p-12 text-center dark:border-surface-500/40 dark:bg-surface-800">
				<div class="mx-auto max-w-md">
					<iconify-icon icon="mdi:store" width="64" class="mx-auto text-6xl text-tertiary-500"></iconify-icon>
					...
					<div class="mt-6 space-y-2 text-start">
						<div class="flex items-start gap-3 text-sm text-[var(--admin-text-muted)]">
							<iconify-icon icon="mdi:check" width="20" class="mt-0.5 text-tertiary-500"></iconify-icon>
					...
					<button
						disabled
						class="mt-6 cursor-not-allowed rounded bg-surface-500/30 px-6 py-3 font-medium text-[var(--admin-text-muted)] dark:bg-surface-800"
					>
