...
<script lang="ts">
	...
	function handleDragOver(e: DragEvent) {
		if (e.clientX && e.clientY) trackPointer(e.clientX, e.clientY);
	}
		...
		if (isActive) {
			...
			if (dragData?.itemId) {
				...
				const el = document.querySelector(
					`[data-item-id="${CSS.escape(dragData.itemId)}"] .truncate`
				);
	...
	<Portal>
		<div class="fixed inset-0 pointer-events-none z-9999" aria-hidden="true">
			<div
				...
				{style}
			>
				<span class="truncate font-medium text-sm text-surface-900 dark:text-surface-100"
					>{snapName}</span
				>
				{#if nestTargetName}
