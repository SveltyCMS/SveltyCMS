...
<script lang="ts">
	...
	function hashNodes(nodes: Array<{ _id?: unknown; parentId?: unknown; order?: number }>): string {
		return (
			nodes
				.map(
					(n) => `${String(n._id)}:${n.parentId == null ? '' : String(n.parentId)}:${n.order ?? 0}`
				)
				.sort()
				.join('|') + `#${nodes.length}`
		);
	...
	function isNodeVisible(node: TreeNode, search: string): boolean {
		...
		const matchesTag =
			!selectedTagFilter || collectionMetadata.getTags(node.id).includes(selectedTagFilter);
								...
								<Button
									...
									onclick={() => {
										...
										currentTagsInput = collectionMetadata
											.getTags(activeItemForTagging!.id)
											.join(', ');
									}}
