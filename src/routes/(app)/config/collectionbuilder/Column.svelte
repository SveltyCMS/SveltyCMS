...
<div class="my-0.5 w-full" style="padding-left: {paddingLeft}" role="listitem" aria-busy={isDragging || isUpdating}>
	{#if updateError}
		<div class="bg-error-500/10 text-error-500 mb-2 rounded-sm p-2" role="alert">
			{updateError}
		</div>
	{/if}

	{#if isCategory}
		<div class="bg-surface-300/10 flex items-center justify-between rounded-sm p-2">
			<div class="flex items-center gap-2">
				...
				<span class="text-tertiary-500 dark:text-primary-500 font-bold">{name}</span>
			</div>
