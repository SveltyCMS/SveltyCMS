...
<div class="flex w-full flex-col">
		...
		<VerticalList items={fields} {headers} {flipDurationMs} {handleDndConsider} {handleDndFinalize}>
			{#each fields as field (field.id)}
				...
				>
					...
					<button onclick={() => modalWidgetForm(field)} type="button" aria-label={m.button_edit()} class="bg-primary-500/10 text-primary-500 hover:bg-primary-500/20 btn-icon ml-auto">
	...
	<div>
		<div class="mt-2 flex items-center justify-center gap-3">
			<button onclick={() => modalSelectWidget(null)} class="bg-tertiary-500 text-white btn" aria-label={m.collection_widgetfield_addFields()}>
				{m.collection_widgetfield_addFields()}
			</button>
		</div>
		<div class=" flex items-center justify-between">
			<button onclick={() => tabSet.set(0)} type="button" aria-label={m.button_previous()} class="bg-secondary-500 text-white btn mt-2 justify-end">
				{m.button_previous()}
			</button>
			<button
				onclick={handleSave}
