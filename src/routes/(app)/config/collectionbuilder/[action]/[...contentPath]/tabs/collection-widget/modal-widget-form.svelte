...
<script lang="ts">
	...
	const { value, close, roles: rolesProp = [] }: Props = $props() as Props;
	...
	const guiSchema = $derived((widgetFn?.GuiSchema as Record<string, { widget: any }>) || {});
	...
	const specificKeys = $derived(Object.keys(guiSchema).filter((k) => !DEFAULT_KEYS.has(k)));
...
{#if local}
	<div class="flex flex-col w-full" style="min-width: 640px; max-width: 900px; min-height: 500px;">
		...
		<div class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
			...
			{:else if activeTab === 'permissions'}
				...
				<div class="space-y-5">
					<!-- Visibility toggle -->
					<div
						class="rounded-xl border border-surface-500/30 dark:border-surface-500/40 bg-surface-500/10 dark:bg-surface-900/20 p-4"
					>
						<h4
					...
					>
						<label
							class="flex cursor-pointer items-center gap-3 rounded-lg border border-surface-500/30 dark:border-surface-500/40 bg-white dark:bg-surface-800 p-3"
						>
							<input
								type="checkbox"
						...
						>
							...
							<div class="space-y-1">
								{#each rolesProp as role (role._id)}
									<div
										class="grid grid-cols-[1fr_60px_60px] gap-2 items-center rounded-lg px-3 py-2 hover:bg-surface-500/10 dark:hover:bg-surface-800/50 transition-colors"
									>
					...
					{:else}
						<div
							class="rounded-xl border border-dashed border-surface-500/30 dark:border-surface-500/40 bg-surface-500/30 dark:bg-surface-900/10 p-6 text-center"
						>
							<iconify-icon
		...
		<!-- Footer -->
		<div
			class="flex items-center justify-between border-t border-surface-500/30 dark:border-surface-500/40 px-6 py-4 bg-surface-500/10 dark:bg-surface-900/50"
		>
