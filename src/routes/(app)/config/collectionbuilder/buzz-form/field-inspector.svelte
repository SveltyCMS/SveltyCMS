...
<script lang="ts">
	...
	const guiSchema = $derived(
		(asAny<any>(availableWidgets)[widgetKey]?.GuiSchema || {}) as Record<string, { widget: Component<any> }>
	);
	...
	function tabClass(tab: string, disabled = false) {
		return `flex-1 border-b-2 px-1 py-3 text-[10px] font-bold uppercase tracking-widest transition-all ${
			activeTab === tab ? "border-primary-500 text-primary-500" : "border-transparent"
		} ${disabled ? "cursor-not-allowed opacity-40" : ""}`;
	}
</script>

<div class="flex h-full w-80 flex-col border-l border-surface-200-800 bg-surface-50-950">
	...
	{:else}
		<div class="border-b border-surface-200-800 bg-surface-100/50 p-4 dark:bg-surface-800/30">
			<div class="flex items-center gap-3">
				...
				<div class="flex-1 overflow-hidden">
					...
					<div class="flex items-center gap-1.5">
						<span class="rounded-full bg-surface-200 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-surface-600 dark:bg-surface-700 dark:text-surface-400">
				...
				<div class="flex flex-col gap-1">
					<button onclick={onDuplicate} class="btn-icon btn-icon-sm preset-ghost-tertiary-500 transition-all hover:preset-filled-tertiary-500" title="Duplicate Field">
						<iconify-icon icon="mdi:content-copy" width={18}></iconify-icon>
					</button>
					<button onclick={onDelete} class="btn-icon btn-icon-sm preset-ghost-error-500 transition-all hover:preset-filled-error-500" title="Delete Field">
		...
		<div class="flex items-center justify-between border-b border-surface-200-800 bg-surface-50 px-4 py-2 font-mono text-[10px] text-surface-500 dark:bg-surface-900">
			<span class="max-w-[150px] truncate">{target.db_fieldName || "no_name"}</span>
		...
		<div class="flex flex-1 flex-col overflow-hidden">
			<div class="flex border-b border-surface-200-800 bg-surface-100/30 px-2 dark:bg-surface-800/20" role="tablist" aria-label="Field inspector tabs">
				<button
					type="button"
					role="tab"
				...
				<button
					...
					class={tabClass("specific", specificProperties.length === 0)}
					onclick={() => {
						if (specificProperties.length > 0) activeTab = "specific";
					}}
				>
					Settings
				</button>
