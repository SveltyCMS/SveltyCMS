...
>
	...
	<AdminCard
		class="flex flex-wrap items-center justify-between gap-4 border border-surface-500/30 bg-white p-4 shadow-sm dark:border-surface-500/40 dark:bg-surface-900"
	...
	<div class="grid min-h-0 flex-1 grid-cols-1 gap-6 lg:grid-cols-4" data-testid="workflow-builder">
		<!-- Canvas Area -->
		<div
			class="relative overflow-hidden rounded-2xl border-2 border-dashed border-surface-500/30 bg-surface-500/10 p-12 lg:col-span-3 dark:border-surface-500/40 dark:bg-surface-900/50"
			...
			<div class="flex flex-wrap items-start justify-center gap-12">
				{#each states as state (state.id)}
					<div
						...
						onkeydown={(e) => {
							if (e.key === 'Enter' || e.key === ' ') selectNode(state.id);
						}}
					>
						{#if state.isInitial}
							<span
								class="absolute -top-3 inset-s-1/2 -translate-x-1/2 rounded-full bg-primary-500 px-2 py-0.5 text-[8px] font-bold uppercase text-white"
								>Initial</span
							>
						{/if}
						<div class="mb-2 flex items-center justify-between">
							...
							<button
								class="text-error-500 opacity-0 group-hover:opacity-100"
