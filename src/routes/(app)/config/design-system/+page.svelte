...
<script lang="ts">
	...
	const userPrefs = $derived(
		(data as any).user?.preferences?.theme as Record<string, any> | undefined
	);
	...
	let myDensity = $state<string>(untrack(() => userPrefs?.density || ''));
	let myVariant = $state<string>(untrack(() => userPrefs?.variant || ''));
	let myReducedMotion = $state(untrack(() => userPrefs?.reducedMotion ?? false));
	let myHighContrast = $state(untrack(() => userPrefs?.highContrast ?? false));
	...
	onMount(() => {
		...
		if (localPrefs) {
			...
			if (localPrefs.reducedMotion !== undefined && !myReducedMotion)
				myReducedMotion = localPrefs.reducedMotion;
			if (localPrefs.highContrast !== undefined && !myHighContrast)
				myHighContrast = localPrefs.highContrast;
			for (const key of USER_LAYOUT_PREF_KEYS) {
	...
	let collapsibleSidebar = $state(
		untrack(() => loadedTheme?.features?.collapsibleSidebar ?? false)
	);
	...
	let lockReducedMotion = $state(
		untrack(() => loadedTheme?.lockedSettings?.reducedMotion ?? false)
	);
...
>
	...
	{#if !loadingThemes}
		...
		{#if current}
			<AdminCard class="flex items-center gap-3 flex-wrap p-4">
				<iconify-icon
	...
	<AdminCard class="shadow-sm backdrop-blur-md">
		<div
			class="flex flex-wrap gap-1 p-2 border-b"
			style="border-color: var(--admin-border-default)"
			data-testid="appearance-tabs"
		>
			{#each visibleTabs as tab (tab.id)}
				<Button
					...
					class={activeTab === tab.id
						? 'bg-surface-500/10 text-tertiary-600 dark:bg-surface-800 dark:text-primary-500'
						: ''}
				>
					...
					>
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
							...
							<div class="flex flex-col justify-end">
								<Toggle
									bind:value={myReducedMotion}
									label="Reduced Motion"
									disabled={isPreferenceLocked(loadedTheme?.lockedSettings, 'reducedMotion')}
								/>
							</div>
							<div class="flex flex-col justify-end">
								<Toggle
									bind:value={myHighContrast}
									label="High Contrast"
									disabled={isPreferenceLocked(loadedTheme?.lockedSettings, 'highContrast')}
								/>
							</div>
						</div>
						...
						{:else}
							<div class="space-y-3 max-w-2xl" data-testid="appearance-theme-list">
								{#each themes as t (t.id)}
									...
									>
										<iconify-icon
											icon={t.isActive ? 'mdi:checkbox-marked-circle' : 'mdi:theme-light-dark'}
											class="text-xl {t.isActive
										...
										<div class="flex items-center gap-1">
											...
											{#if !t.isActive && !t.isDefault}
												<Button
													...
													data-testid={`appearance-theme-delete-${t.id}`}
												></Button>
											{/if}
										</div>
									</AdminCard>
					...
					<div class="space-y-6" in:fly={{ y: 10, duration: 200 }}>
						<div>
							...
							<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
								...
								<AdminCard
									class="cursor-pointer p-4 border-2 {density === 'spacious' &&
									variant === 'elevated'
										? 'border-tertiary-500 dark:border-primary-500'
										: 'border-surface-500/30 dark:border-surface-500/40'} hover:border-tertiary-500 dark:hover:border-primary-400 transition-colors"
									onclick={() => {
								...
								<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
									{#each marketplaceThemes as item (item.id)}
										<AdminCard class="p-4 border border-surface-500/30 dark:border-surface-500/40">
											...
											<p
												class="text-xs text-surface-500 dark:text-surface-400 mt-1 mb-3 line-clamp-2"
											>
												{item.description}
											</p>
						...
						<div class="space-y-6 max-w-2xl">
							...
							<div class="grid grid-cols-3 gap-4">
								{#each [{ d: 'compact', label: 'Compact', desc: 'Sidebar: 200px · Dense · Small text' }, { d: 'cozy', label: 'Cozy', desc: 'Sidebar: 240px · Balanced · Default' }, { d: 'spacious', label: 'Spacious', desc: 'Sidebar: 300px · Airy · Writer mode' }] as p (p.d)}
									<AdminCard
										class="p-3 border {density === p.d
											? 'border-tertiary-500 dark:border-primary-500 ring-2 ring-tertiary-500/20 dark:ring-primary-500/20'
				...
				{:else if activeTab === 'style'}
					<div class="space-y-6" in:fly={{ y: 10, duration: 200 }}>
						...
						<div class="space-y-6 max-w-2xl">
							...
							<div class="grid grid-cols-3 gap-4">
								<AdminCard
									class="p-6 {variant === 'flat'
										? 'border-tertiary-500 dark:border-primary-500 ring-2'
										: ''}"
									style="border-width: {variant === 'flat'
										? '0'
	...
	{#if hasChanges && isAdmin && activeTab !== 'overrides' && activeTab !== 'preview'}
		<div
			class="sticky bottom-0 z-30 -mx-6 -mb-6 px-6 py-4 border-t border-surface-500/30 dark:border-surface-500/40 bg-white/90 dark:bg-surface-900/90 backdrop-blur-md flex items-center justify-between"
		>
