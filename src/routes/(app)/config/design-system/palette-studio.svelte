...
<script lang="ts">
	...
	const expandedLonghand = $derived(
		expandShorthandPaletteProperties(seedRecord as Record<string, string>)
	);
...
<AdminCard class="p-5 space-y-4" data-testid="palette-studio">
		...
		{#each CORE_KEYS as row (row.key)}
			<div class="space-y-2">
				...
				>
					...
					<input
						id={`palette-color-${row.key}`}
						type="color"
						aria-label={`${row.label} color`}
						title={`${row.label} color`}
						class="h-9 w-12 cursor-pointer rounded border border-surface-500/30 dark:border-surface-600 bg-transparent p-0.5"
						value={seeds[row.key]}
						oninput={(e) => {
							seeds[row.key] = (e.currentTarget as HTMLInputElement).value;
						}}
					/>
				</div>
				<Input
	...
	{#if showExtras}
		<div
			class="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2 border-t"
				...
				<div class="space-y-2">
					...
					>
						...
						<input
							...
							aria-label={`${row.label} color`}
							title={`${row.label} color`}
							class="h-8 w-10 cursor-pointer rounded border border-surface-500/30 dark:border-surface-600 bg-transparent p-0.5"
							value={seeds[row.key]}
							oninput={(e) => {
								seeds[row.key] = (e.currentTarget as HTMLInputElement).value;
							}}
		...
		<div class="space-y-2">
			{#each Object.keys(seedRecord) as paletteKey (paletteKey)}
				<div class="flex flex-wrap items-center gap-2">
					<span class="w-20 text-xs font-semibold capitalize" style="color: var(--admin-text-body)"
						>{paletteKey}</span
					>
