...
<script lang="ts">
	...
	const filteredPermissions = $derived(
		permissionsList.filter(
			(permission) => permission._id?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false
		)
	);
	...
	const filterGroups = (permissions: Permission[], group: string): Permission[] => {
		...
		if (group === 'Collection Entries') {
			...
			filtered = permissions.filter(
				(cur) => cur.type === PermissionType.SYSTEM && cur._id.startsWith('system:')
			);
			...
			filtered = permissions.filter(
				(cur) => cur.type === PermissionType.SYSTEM && cur._id.startsWith('content:')
			);
		} else if (group === 'Media Management') {
			filtered = permissions.filter(
				(cur) => cur.type === PermissionType.SYSTEM && cur._id.startsWith('media:')
			);
		} else if (group === 'Admin') {
			filtered = permissions.filter(
				(cur) => cur.type === PermissionType.SYSTEM && cur._id.startsWith('admin:')
			);
		} else {
...
{:else}
	...
	<div class="sticky top-0 z-10 mb-6 flex items-center justify-between">
		<div class="relative w-full">
			<div
				class="absolute inset-y-0 inset-s-0 ps-3.5 flex items-center pointer-events-none text-surface-400"
			>
	...
	{#if filteredPermissions.length === 0}
		<div
			class="text-center py-8 border border-dashed border-surface-500/30 dark:border-surface-500/40 rounded bg-surface-500/10 dark:bg-surface-900"
		>
			<iconify-icon
	...
	{:else}
		...
		{#if adminRole}
			<div
				class="mb-4 flex items-center justify-center gap-1.5 text-xs text-surface-500 dark:text-surface-400 bg-surface-500/10 dark:bg-surface-900/20 border border-surface-500/30 dark:border-surface-500/40 py-2 px-4 rounded w-max mx-auto shadow-xs"
			>
		...
		{/if}
		<div
			class="permission overflow-x-auto rounded border border-surface-500/30 dark:border-surface-500/40 shadow-sm bg-white dark:bg-surface-900"
		>
			<table class="w-full text-start border-collapse table-auto">
				<!-- Header -->
				<thead
					class="bg-surface-500/10 dark:bg-surface-500/10 border-b border-surface-500/30 dark:border-surface-500/40"
				>
					<tr>
						<th
								...
								<th class="px-5 py-4 dark:text-surface-50 text-center" scope="col">
									<div class="flex flex-col items-center gap-1.5">
										...
										<div class="flex items-center gap-1.5 mt-0.5">
											<input
												type="checkbox"
												aria-label={`Select all filtered permissions for ${role.name}`}
											...
											<td class="px-5 py-3 text-center">
												<input
													...
													checked={role.permissions.some(
														(p) => String(p) === String(permission._id)
													)}
