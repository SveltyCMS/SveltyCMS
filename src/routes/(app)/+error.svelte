...
{#if page}
	...
	>
		...
		<a
			href="#error-content"
			class="sr-only focus:not-sr-only focus:absolute focus:inset-s-4 focus:top-4 focus:z-50 focus:rounded-md focus:bg-white focus:px-4 focus:py-2 focus:text-surface-900 focus:outline-none focus:ring-2 focus:ring-primary-500"
		>
			{error_skip_content()}
		</a>
		...
		<div class="relative mb-12 grid place-items-center" style="width: {size}px; height: {size}px;" aria-hidden="true">
			...
			<div class="animate-[spin_20s_linear_infinite] absolute inset-0 flex items-center justify-center" style="font-size: {font}em;">
				{#each array as char, index (index)}
					<div
						class="absolute inset-s-1/2 top-0 h-full -translate-x-1/2 text-center font-bold uppercase leading-none"
		...
		<div id="error-content" class="flex flex-col items-center space-y-6 text-center">
			...
			<div role="alert" aria-live="assertive" class="relative">
				...
				<!-- Error URL Banner -->
				<div
					class="mt-4 rounded-md bg-error-600/90 px-4 py-2 text-sm font-semibold text-white shadow-lg sm:absolute sm:inset-s-1/2 sm:top-1/2 sm:mt-0 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rotate-12"
