...
<div class="content flex min-h-0 flex-1 flex-col">
	...
	{#if isSavingDraft}
		<div
			class="fixed inset-e-4-4 top-20 z-50 flex items-center gap-2 rounded bg-warning-500 px-4 py-2 text-white shadow-lg"
		>
			<div
	...
	{#if !collections.active}
		...
		>
			...
			<h3 class="mb-2 text-xl font-bold text-error-600 dark:text-error-500">
				{collection_not_loaded()}
			</h3>
	...
	{:else if ['edit', 'create'].includes(collections.mode)}
		<div
			id="fields_container"
			class="fields max-h-[calc(100vh-100px)] overflow-y-auto overflow-x-visible max-md:max-h-[calc(100vh-120px)] space-y-6"
		>
