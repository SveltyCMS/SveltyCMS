<!--
@file src/routes/(app)/dashboard/widgets/security/index.svelte
@component
					...
					<div class="flex items-center justify-between text-xs px-1 w-full h-full min-h-9">
						<div class="flex items-center gap-2">
							<div class="relative flex h-2.5 w-2.5">
								<span
									class="absolute inline-flex h-full w-full rounded-full opacity-75 animate-ping {overallThreatLevel ===
										'critical' || overallThreatLevel === 'high'
										? 'bg-error-400'
								...
								></span>
								<span
									class="relative inline-flex rounded-full h-2.5 w-2.5 {overallThreatLevel ===
										'critical' || overallThreatLevel === 'high'
										? 'bg-error-500'
										: overallThreatLevel === 'medium'
											? 'bg-warning-500'
					...
					<div class="flex h-full flex-col gap-4">
						...
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-3">
								<div class="relative shrink-0">
									<div
										class="h-6 w-6 rounded-full flex items-center justify-center {overallThreatLevel ===
										'critical'
											? 'bg-error-500'
									...
									>
										<iconify-icon
											icon={overallThreatLevel === 'safe' ? 'mdi:shield-check' : 'mdi:shield-alert'}
											class="text-white text-sm"
										></iconify-icon>
									</div>
									<div
										class="absolute inset-0 h-6 w-6 rounded-full {overallThreatLevel === 'critical'
											? 'bg-error-500'
											: overallThreatLevel === 'high'
												? 'bg-warning-500'
												: overallThreatLevel === 'medium'
													? 'bg-warning-500'
													: 'bg-success-500'} animate-ping opacity-40"
									></div>
								</div>
								<div>
									...
									<svg viewBox="0 0 100 28" class="w-full h-full overflow-visible">
										<defs>
											<linearGradient id="secThreatGrad" x1="0" y1="0" x2="0" y2="1">
												<stop
													offset="0%"
													stop-color={sparklineStyle.stopColor}
													stop-opacity="0.2"
												/>
												<stop
													offset="100%"
						...
						{#if isLicensed && size.h >= 3}
							<div class="flex-1 flex flex-col min-h-0">
								...
								{#if activeIncidents.length > 0}
									<div class="space-y-2 overflow-y-auto pe-1 flex-1 custom-scroll max-h-40">
										{#each activeIncidents as incident (incident.id)}
											<div
												class="rounded border-s-4 p-2.5 text-xs bg-surface-500/10 dark:bg-surface-800/40 border-surface-500/30 dark:border-surface-500/40
											...
											>
												<div class="flex justify-between items-center">
													<div class="font-mono font-medium text-surface-600 dark:text-surface-400">
														{incident.clientIp}
													</div>
													<div
														class="capitalize font-semibold text-[10px] px-1.5 py-0.5 rounded-full
														{incident.threatLevel === 'critical'
								...
								{:else}
									<div
										class="flex flex-1 items-center justify-center rounded-2xl bg-surface-500/10 dark:bg-surface-800/30 border border-dashed border-surface-500/30 dark:border-surface-500/40 text-center py-6"
									>
										<div>
											<iconify-icon
