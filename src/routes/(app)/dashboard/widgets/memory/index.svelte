	...
	{#snippet children({ data })}
		...
		{#if data?.memoryInfo?.total}
			<div
				...
				role="region"
				aria-label={dashboard_memory_aria()}
			>
				{#if size.h === 1}
					...
					<div class="flex items-center justify-between text-xs px-1 w-full h-full min-h-9">
						<div class="flex items-center gap-2">
							<div class="relative flex h-2.5 w-2.5">
								<span
									class="absolute inline-flex h-full w-full rounded-full opacity-75 animate-ping {mem.level ===
									'high'
										? 'bg-error-400'
										: mem.level === 'medium'
											? 'bg-warning-400'
								...
								></span>
								<span
									class="relative inline-flex rounded-full h-2.5 w-2.5 {mem.level === 'high'
										? 'bg-error-500'
										: mem.level === 'medium'
											? 'bg-warning-500'
											: 'bg-success-500'}"
							...
							<span
								class="text-gray-400 dark:text-gray-500 font-semibold uppercase tracking-wider text-[10px]"
								>RAM</span
							>
						</div>
						<div class="flex items-center gap-2 text-end">
							<span class="font-semibold text-gray-700 dark:text-gray-300 tabular-nums"
								>{mem.usedGB.toFixed(1)} / {mem.totalGB.toFixed(0)} GB</span
							>
							{#if mem.swapPercent !== null}
								<span class="text-gray-400 dark:text-gray-500"
									>| {dashboard_memory_swap()}
									<span class="font-semibold text-gray-600 dark:text-gray-400"
										>{mem.swapPercent.toFixed(0)}%</span
									></span
								>
							{/if}
						</div>
				...
				{:else}
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-3">
							<div class="relative">
								<div
									class="h-4 w-4 rounded-full {mem.level === 'high'
										? 'bg-error-500'
										: mem.level === 'medium'
											? 'bg-warning-500'
											: 'bg-success-500'}"
								></div>
								<div
									class="absolute inset-0 h-4 w-4 rounded-full {mem.level === 'high'
										? 'bg-error-500'
										: mem.level === 'medium'
											? 'bg-warning-500'
						...
						>
							...
							>
								...
								<circle
									...
									class="transition-all duration-700 ease-out {mem.level === 'high'
										? 'stroke-error-500'
										: mem.level === 'medium'
											? 'stroke-warning-500'
											: 'stroke-success-500'}"
								...
								{#if mem.swapPercent !== null}
									...
									<circle
										...
										stroke-linecap="round"
										class="transition-all duration-700 ease-out {mem.swapLevel === 'high'
											? 'stroke-error-500'
											: mem.swapLevel === 'medium'
												? 'stroke-warning-500'
												: 'stroke-tertiary-500'}"
									/>
								{/if}
							...
							>
								<span
									class="text-3xl font-semibold tabular-nums {theme === 'dark'
										? 'text-white'
										: 'text-gray-900'}"
								>
								...
								<span
									class="text-[9px] font-bold tracking-widest uppercase {theme === 'dark'
										? 'text-gray-400'
										: 'text-gray-500'}">{dashboard_memory_ram_used()}</span
								>
							</div>
						</div>
					</div>
					...
					<div class="space-y-4">
						...
						>
							...
							<div>
								...
								<div
									class="font-semibold tabular-nums mt-0.5 {mem.level === 'high'
										? 'text-error-500'
										: mem.level === 'medium'
											? 'text-warning-500'
											: 'text-success-500'}"
						...
						{#if mem.swapPercent !== null}
							<div
								class="border-t pt-2.5 {theme === 'dark' ? 'border-gray-800' : 'border-gray-150'}"
							>
								<div
									class="flex justify-between text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 px-1"
								>
									<span>{dashboard_memory_swap_memory()}</span>
									<span class="tabular-nums"
										>{mem.swapPercent.toFixed(0)}{dashboard_memory_pct_used()}</span
									>
								</div>
								<div class="grid grid-cols-3 gap-2 text-center text-xs">
									<div>
										...
										<div class="font-medium text-gray-600 dark:text-gray-300 tabular-nums">
											{mem.swapTotalGB.toFixed(1)} GB
										</div>
									</div>
									<div>
										<div class="text-[10px] text-gray-400 dark:text-gray-500">
		...
		{:else}
			...
			>
				<div
					class="h-8 w-8 animate-spin rounded-full border-2 border-success-500 border-t-transparent"
				></div>
