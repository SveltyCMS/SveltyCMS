...
<script lang="ts">
	...
	function severityCls(m: SysMessage): string {
		...
		if (lvl === 'critical' || lvl === 'error')
			return 'border-s-error-500 bg-error-500/20 dark:bg-error-900/20';
		if (lvl === 'high' || lvl === 'warning')
			return 'border-s-warning-500 bg-warning-500/20 dark:bg-warning-900/20';
		if (lvl === 'medium' || lvl === 'warn')
			return 'border-s-warning-500 bg-warning-500/20 dark:bg-warning-900/20';
	...
	function extractLinks(text: string): Array<{ url: string; text: string }> {
		...
		return matches
			? matches.map((url) => ({ url, text: url.length > 50 ? url.slice(0, 47) + '...' : url }))
			: [];
	}
</script>
		...
		{:else if isCompact}
			<div class="flex h-full items-center gap-2 overflow-hidden">
				<span class="shrink-0 text-xs font-semibold text-surface-500"
					>{widget_sysmsg_count({ count: messages.length })}</span
				>
				...
				<div class="flex flex-1 items-center gap-1.5 overflow-x-auto scrollbar-none">
					{#each messages.slice(0, 8) as msg (msg.id || msg.timestamp + msg.title)}
						<div
							class="flex shrink-0 items-center gap-1 rounded-full border-s-2 {severityCls(
								msg
							)} px-2 py-0.5"
							title="{severityLabel(msg)}: {msgText(msg)}"
						>
				...
				<div class="flex-1 overflow-y-auto space-y-2 pe-0.5 custom-scroll">
					{#each messages.slice(0, 5) as msg (msg.id || msg.timestamp + msg.title)}
						...
						<button
							...
							class="w-full text-start rounded-2xl border-s-3 {severityCls(
								msg
							)} px-4 py-3 transition-colors hover:opacity-90"
						>
							<div class="flex items-start justify-between gap-3">
								<div class="flex items-center gap-2 shrink-0">
									<iconify-icon
										icon={severityIcon(msg)}
										class="text-lg {severityDot(msg).replace('bg-', 'text-')}"
									></iconify-icon>
									<span
										class="text-[10px] font-semibold uppercase tracking-wider {severityDot(
											msg
										).replace('bg-', 'text-')}"
									>
										{severityLabel(msg)}
