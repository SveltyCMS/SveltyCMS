	...
	{#snippet children({ data })}
		{const cpu = {
			...
			average: data?.cpuInfo?.historicalLoad?.usage?.length
				? Number(
						data.cpuInfo.historicalLoad.usage.reduce((a: number, b: number) => a + b, 0) /
							data.cpuInfo.historicalLoad.usage.length
					)
				: 0,
			level:
				Number(data?.cpuInfo?.historicalLoad?.usage?.at(-1) ?? 0) > 80
					? 'high'
					: Number(data?.cpuInfo?.historicalLoad?.usage?.at(-1) ?? 0) > 50
						? 'medium'
						: 'low',
		...
		{#if data?.cpuInfo}
			...
			{const points = cpu.usage.map((val: number, i: number) => ({
				...
				y:
					chartHeight -
					(val / 100) * (chartHeight - (size.h === 1 ? 4 : 20)) -
					(size.h === 1 ? 2 : 10)
			}))}
			...
			{const areaPath = points.length
				? `${linePath} L ${points.at(-1)!.x.toFixed(2)} ${chartHeight} L 0 ${chartHeight} Z`
				: ''}
			...
			<div class="flex h-full flex-col justify-between {size.h === 1 ? 'space-y-1' : 'space-y-3'}">
				{#if size.h === 1}
					...
					<div class="flex items-center justify-between text-xs px-1">
						<div class="flex items-center gap-1.5">
							<div
								class="h-2 w-2 rounded-full {cpu.level === 'high'
									? 'bg-error-500'
									: cpu.level === 'medium'
										? 'bg-warning-500'
										: 'bg-success-500'}"
						...
						<span class="text-[10px] text-gray-400 dark:text-gray-500 tabular-nums"
							>{dashboard_cpu_avg()} {cpu.average.toFixed(1)}%</span
						>
					</div>
				{:else}
					<!-- Header Stats -->
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-2">
							<div class="relative">
								<div
									class="h-3 w-3 rounded-full {cpu.level === 'high'
										? 'bg-error-500'
										: cpu.level === 'medium'
											? 'bg-warning-500'
											: 'bg-success-500'}"
								></div>
								<div
									class="absolute inset-0 h-3 w-3 rounded-full {cpu.level === 'high'
										? 'bg-error-500'
										: cpu.level === 'medium'
											? 'bg-warning-500'
					...
					<!-- Progress Bar -->
					<div
						class="relative h-2 overflow-hidden rounded-full {theme === 'dark'
							? 'bg-gray-800'
							: 'bg-gray-100'}"
					>
						<div
				...
				<div
					class="relative grow"
					style="min-height: {size.h === 1 ? '40px' : size.h >= 2 ? '160px' : '130px'}"
				>
					<svg
					...
					{#if size.h !== 1 && activeIndex !== null && points[activeIndex]}
						...
						{const time = formatTime(cpu.timestamps[activeIndex], {
							hour: '2-digit',
							minute: '2-digit'
						})}
						<div
							...
							style="left: {Math.max(
								20,
								Math.min(points[activeIndex].x - 45, 220)
							)}px; top: {points[activeIndex].y - 58}px;"
						>
							<div
								class="font-mono font-semibold text-lg leading-none tabular-nums text-gray-900 dark:text-white"
							>
								{val.toFixed(1)}%
							</div>
				...
				{#if size.h !== 1 && (size.w >= 2 || size.h >= 2)}
					<div
						class="flex justify-between text-xs pt-1 {theme === 'dark'
							? 'text-gray-400'
							: 'text-gray-500'} border-t border-gray-100/50 dark:border-gray-800/50"
					>
						<span
							>{dashboard_cpu_cores()}
							<span class="font-medium text-gray-700 dark:text-gray-300"
								>{data?.cpuInfo?.cores?.count ?? '—'}</span
							></span
						>
						<span class="text-end truncate max-w-85">
							{data?.cpuInfo?.cores?.perCore?.[0]?.model?.split(' ').slice(0, 3).join(' ') ??
								'Unknown'}
						</span>
					</div>
				{/if}
