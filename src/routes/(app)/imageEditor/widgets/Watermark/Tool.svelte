<!-- 
@file: src/routes/(app)/imageEditor/components/Watermark.svelte
@component
**This component manages Watermark overlays on an image (logic only)**
Handles Konva interactions for adding, transforming, and managing Watermark stickers.
UI is in WatermarkToolPanel.svelte
...
<script lang="ts">
	...
	let { stage, layer, imageNode, onStickerChange } = $props<{
		stage: Konva.Stage;
		layer: Konva.Layer;
		imageNode: Konva.Image;
		...
		if (activeState === 'watermark') {
			...
			imageEditorStore.setToolbarControls({
				...
				props: {
					...
					hasSelection: !!selectedSticker,
					onAddSticker: () => openFileDialog(),
					onDeleteSelected: () => deleteSelectedSticker(),
					onDeleteAll: () => deleteAllStickers(),
					onApply: () => {
