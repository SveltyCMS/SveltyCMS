...
<style lang="postcss">
	...
	.label {
		@apply text-nowrap text-sm font-medium ; color: var(--color-surface-200);
	}

	.slider {
		@apply h-2 w-32 cursor-pointer appearance-none rounded-full ; background-color: var(--color-surface-600);
	}
	...
	.value {
		@apply min-w-[2rem] text-center text-sm font-semibold ; color: var(--color-surface-200);
	}
	...
	/* Segmented Control */
	.segment-group {
		@apply flex items-center rounded-lg  p-0.5; background-color: var(--color-surface-700);
	}
	.segment-btn {
	...
	.segment-btn.active {
		@apply bg-white  shadow-sm; background-color: var(--color-surface-900); color: var(--color-primary-600);
	}
</style>
