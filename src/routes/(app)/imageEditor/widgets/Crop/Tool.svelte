...
@component
**Crop tool - Konva canvas logic**
Handles crop, rotate, scale transformations on the Konva canvas.
Registers controls with MasterToolbar via imageEditorStore.
...
<script lang="ts">
	...
	$effect(() => {
		if (!mounted && stage && layer && imageNode && container) {
			mounted = true;
			logger.debug('Crop tool mounted');
			return () => {
	...
	// React to active tool state: register controls and init only when active
	$effect(() => {
		...
		if (activeState === 'crop') {
			imageEditorStore.setToolbarControls({
				...
				props: {
					...
					cropShape,
					onCropShapeChange: (shape: 'rectangle' | 'square' | 'circular') => setCropShape(shape),
					onAspectRatio: (ratio: string) => {
						aspectRatio = ratio;
						initCropTool();
					},
					onApply: apply
			...
			// Ensure Konva elements are initialized when activating crop
			if (!cropTool) {
				setTimeout(() => initCropTool(), 10);
			}
		} else {
			// Deactivate and clear only if Crop owns the toolbar
			cleanupCropTool();
			cleanupRotationGrid();
			if (imageEditorStore.state.toolbarControls?.component === CropControls) {
				imageEditorStore.setToolbarControls(null);
