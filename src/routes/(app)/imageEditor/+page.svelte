<script lang="ts">
	...
	// import { saveData } from '@src/utils/utils';
	...
	// 	name: z.string({ required_error: 'Name Required' }).max(24, { message: 'max 24 Characteres' }).trim()
	// };
...
<input
	...
	class="w-fulxl block cursor-pointer rounded-lg border border-gray-300 bg-gray-50 text-sm text-gray-900 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-gray-400 dark:placeholder-gray-400"
	type="file"
/>
<Cropper
	bind:SCALE={scale}
...
<!-- <label class="my-2" for="#input-text">Name</label>

<input class="input" placeholder="Image Name" type="text" id="input-text" bind:value={name} /> -->

<!-- Image Name-->
<div class="group relative z-0 mb-6 w-full">
	<iconify-icon icon="bi:images" width="24" class="absolute left-0 top-3.5 text-primary-500" />
	...
	<input
		...
		type="text"
		name="name"
		class="peer block w-full appearance-none !rounded-none !border-0 !border-b-2 !border-surface-300 !bg-transparent px-6 py-2.5 text-surface-500 focus:border-tertiary-600 focus:outline-none focus:ring-0 dark:border-surface-600 dark:text-white dark:focus:border-tertiary-500"
		placeholder=" "
	/>
	<label
		for="name"
		class="absolute left-6 top-3 -z-10 origin-[0] -translate-y-6 scale-75 transform text-sm duration-300 peer-placeholder-shown:translate-y-0 peer-placeholder-shown:scale-100 peer-focus:left-0 peer-focus:-translate-y-6 peer-focus:scale-75 peer-focus:text-tertiary-600 dark:text-surface-400 peer-focus:dark:text-tertiary-500"
	>
