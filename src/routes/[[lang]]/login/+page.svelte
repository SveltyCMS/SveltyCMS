...
<div class="body" style="background:{background} ">
	<Signin signInError={form?.type === 'SIGN_IN_ERROR' ? form.message: undefined} {active} on:click={() => (active = 0)} on:pointerenter={() => (background = '#242728')} />
	<SignUp singUpError={form?.type === 'SIGN_UP_ERROR' ? form.message: undefined} {active} on:click={() => (active = 1)} on:pointerenter={() => (background = 'white')} />
	{#if active == undefined}
