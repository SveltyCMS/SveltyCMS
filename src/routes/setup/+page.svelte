...
<script lang="ts">
  ...
  onMount(() => {
    ...
    // ✨ SECURITY: If config is already locked (private.ts exists), jump to admin creation
    if (data.configLocked && wizard.currentStep < 1) {
      wizard.currentStep = 1;
      if (wizard.highestStepReached < 1) {
        wizard.highestStepReached = 1;
      }
    }

    const welcomeShown = sessionStorage.getItem(
      "sveltycms_welcome_modal_shown",
    );
    if (!welcomeShown) {
  ...
  const steps = $derived([
    {
      label: setup_step_database(),
      shortDesc: data.configLocked
        ? "Database configuration is locked"
        : setup_step_database_desc(),
      locked: data.configLocked,
    },
  ...
  async function prevStep() {
    const minStep = data.configLocked ? 1 : 0;
    if (wizard.currentStep > minStep) {
      wizard.currentStep--;
      wizard.errorMessage = "";
      await focusStepContent();
    }
  ...
  async function handleCompleteSetup() {
    ...
    const success = await completeSetup((redirectPath: string) => {
      console.log(
        "[SetupPage] Setup successful, redirecting to:",
        redirectPath,
      );
      initialDataSnapshot = JSON.stringify(wizard);
      goto(redirectPath);
...
<div class="bg-surface-50-900 min-h-screen w-full transition-colors">
  ...
  <div
    class="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8"
    role="main"
    aria-label="SveltyCMS Setup Wizard"
  >
    <SetupHeader
          ...
          {#if (wizard.successMessage || wizard.errorMessage) && wizard.lastDbTestResult}
            ...
            >
              ...
              >
                ...
                <div class="flex-1">
                  {wizard.successMessage || wizard.errorMessage}
                </div>
                <button
                  type="button"
              ...
              {#if wizard.showDbDetails && wizard.lastDbTestResult}
                <div
                  class="border-t border-surface-200 bg-secondary-50 text-xs dark:border-surface-600 dark:bg-surface-700"
                >
                  <div
                    class="grid grid-cols-2 gap-x-4 gap-y-2 p-3 sm:grid-cols-6"
                  >
                    <div
                      class="sm:col-span-1 flex items-center gap-1 whitespace-nowrap"
                    >
                      <span class="font-semibold"
                        >{setup_db_test_latency()}:</span
                      >
                      <span
                        class="text-tertiary-500 dark:text-primary-500 font-semibold"
                    ...
                    {#if wizard.lastDbTestResult.classification}
                      <div
                        class="sm:col-span-2 flex items-center gap-1 whitespace-nowrap"
                      >
                        <span class="font-semibold">Code:</span>
                        <span
                          class="text-tertiary-500 dark:text-primary-500 font-semibold"
                          >{wizard.lastDbTestResult.classification}</span
                        >
                      </div>
                    {/if}
                  </div>
                  {#if !wizard.lastDbTestResult.success}
                    <div
                      class="border-t border-surface-200 p-3 dark:border-surface-600"
                    >
                      {#if wizard.lastDbTestResult.userFriendly}
                        ...
                        <div
                          class="mb-3 rounded bg-red-50 p-2 text-sm text-red-700 dark:bg-error-900/20 dark:text-white"
                        >
