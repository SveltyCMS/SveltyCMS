			...
			<div class="grid grid-cols-1 gap-8 md:grid-cols-2">
				<div>
					<h3 class="mb-3 flex items-center font-semibold tracking-tight">
						<iconify-icon icon="mdi:database" width="24" class="mr-2 text-primary-500 dark:text-tertiary-500" aria-hidden="true"></iconify-icon>
					...
					<dl class="grid grid-cols-[9em_1fr] gap-x-3 gap-y-1 text-sm">
						...
						<dd class="text-primary-500 dark:text-tertiary-500">{dbConfig.type}:</dd>
						{#if dbConfig.host}<dt class="font-medium">{m.label_host?.() || 'Host'}:</dt>
							<dd class="text-primary-500 dark:text-tertiary-500">{dbConfig.host}</dd>{/if}
						{#if dbConfig.port}<dt class="font-medium">{m.label_port?.() || 'Port'}:</dt>
							<dd class="text-primary-500 dark:text-tertiary-500">{dbConfig.port}</dd>{/if}
						{#if dbConfig.name}<dt class="font-medium">{m.label_database?.() || 'Database'}:</dt>
							<dd class="text-primary-500 dark:text-tertiary-500">{dbConfig.name}</dd>{/if}
						{#if dbConfig.user}<dt class="font-medium">{m.form_username()}:</dt>
							<dd class="text-primary-500 dark:text-tertiary-500">{dbConfig.user}</dd>{/if}
					</dl>
				</div>

				<div>
					<h3 class="mb-3 flex items-center font-semibold tracking-tight">
						<iconify-icon icon="mdi:account" width="24" class="mr-2 text-primary-500 dark:text-tertiary-500" aria-hidden="true"></iconify-icon>
					...
					<dl class="grid grid-cols-[9rem_1fr] gap-x-3 gap-y-1 text-sm">
						...
						<dd class="text-primary-500 dark:text-tertiary-500">{adminUser.username}</dd>
						<dt class="font-medium">{m.form_email()}:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">{adminUser.email}</dd>
						<dt class="font-medium">{m.form_password()}:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">••••••••</dd>
					</dl>
				</div>

				<div>
					<h3 class="mb-3 flex items-center font-semibold tracking-tight">
						<iconify-icon icon="mdi:cog" width="24" class="mr-2 text-primary-500 dark:text-tertiary-500" aria-hidden="true"></iconify-icon>
						{m.setup_review_section_system?.() || 'System Settings'}
					</h3>
					<dl class="grid grid-cols-[9rem_1fr] gap-x-3 gap-y-1 text-sm">
						<dt class="font-medium">CMS Name:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">{systemSettings.siteName}</dd>
						<dt class="font-medium">Production URL:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">{systemSettings.hostProd}</dd>
						<dt class="font-medium">{m.setup_review_label_default_system_lang?.() || 'Default System Lang'}:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">{systemSettings.defaultSystemLanguage}</dd>
						<dt class="font-medium">{m.setup_review_label_system_languages?.() || 'System Languages'}:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">{systemSettings.systemLanguages.join(', ')}</dd>
						<dt class="font-medium">{m.setup_review_label_default_content_lang?.() || 'Default Content Lang'}:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">{systemSettings.defaultContentLanguage}</dd>
						<dt class="font-medium">{m.setup_review_label_content_languages?.() || 'Content Languages'}:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">{systemSettings.contentLanguages.join(', ')}</dd>
						<dt class="font-medium">{m.setup_review_label_timezone?.() || 'Timezone'}:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">{systemSettings.timezone}</dd>
					</dl>
				</div>

				<div>
					<h3 class="mb-3 flex items-center font-semibold tracking-tight">
						<iconify-icon icon="mdi:folder" width="24" class="mr-2 text-primary-500 dark:text-tertiary-500" aria-hidden="true"></iconify-icon>
						{m.setup_review_section_media?.() || 'Media Storage'}
					</h3>
					<dl class="grid grid-cols-[9rem_1fr] gap-x-3 gap-y-1 text-sm">
						<dt class="font-medium">Storage Type:</dt>
						<dd class="text-primary-500 dark:text-tertiary-500">
						...
						<dd class="text-primary-500 dark:text-tertiary-500">{systemSettings.mediaFolder}</dd>
					</dl>
				</div>
			</div>
		</div>
