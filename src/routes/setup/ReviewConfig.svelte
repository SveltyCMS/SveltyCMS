...
<div class="fade-in space-y-6">
	<div class="review-title mb-4 flex items-center gap-3 border-b border-white/10 pb-4 text-lg font-bold text-white">
		...
		{m.setup_step_complete() || 'Review Configuration'}
	</div>
	...
	<div
		...
		{selectedPresetData
			? 'border-tertiary-500/30 bg-tertiary-500/5'
	...
	>
		...
		<div class="flex-1">
			...
			<div class="text-xs text-white/40">
				{selectedPresetData ? selectedPresetData.description : 'Collections will be created manually'}
			</div>
		</div>
	...
	<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
		...
		<div class="review-section space-y-3">
			<h4 class="flex items-center gap-2 text-xs font-bold tracking-widest text-white/50 uppercase">
				...
				{m.setup_review_section_admin() || 'Administrator'}
			</h4>
			<div class="dl space-y-2">
				<div class="dl-row flex items-baseline justify-between gap-4 text-sm">
					<span class="dl-label text-white/45">{m.form_username() || 'Username'}</span>
					<span class="dl-value font-medium text-tertiary-500">{adminUser.username}</span>
				</div>
				<div class="dl-row flex items-baseline justify-between gap-4 text-sm">
					<span class="dl-label text-white/45">{m.form_email() || 'Email'}</span>
					<span class="dl-value font-medium text-tertiary-500">{adminUser.email}</span>
				</div>
				<div class="dl-row flex items-baseline justify-between gap-4 text-sm">
					<span class="dl-label text-white/45">{m.form_password() || 'Password'}</span>
		...
		<div class="review-section space-y-3">
			<h4 class="flex items-center gap-2 text-xs font-bold tracking-widest text-white/50 uppercase">
				...
				{m.setup_review_section_system() || 'System'}
			</h4>
			<div class="dl space-y-2">
				<div class="dl-row flex items-baseline justify-between gap-4 text-sm">
					<span class="dl-label text-white/45">{m.setup_system_site_name() || 'CMS Name'}</span>
					<span class="dl-value font-medium text-tertiary-500">{systemSettings.siteName}</span>
				</div>
				<div class="dl-row flex items-baseline justify-between gap-4 text-sm">
					<span class="dl-label text-white/45">{m.setup_system_host_prod() || 'Production URL'}</span>
					<span class="dl-value font-medium text-tertiary-500">{systemSettings.hostProd}</span>
				</div>
				<div class="dl-row flex items-baseline justify-between gap-4 text-sm">
					<span class="dl-label text-white/45">{m.setup_system_timezone() || 'Timezone'}</span>
					<span class="dl-value font-medium text-tertiary-500">{systemSettings.timezone}</span>
				</div>
				<div class="dl-row flex items-baseline justify-between gap-4 text-sm">
					<span class="dl-label text-white/45">{m.setup_system_multi_tenant() || 'Multi-Tenant'}</span>
					{#if systemSettings.multiTenant}
						<span class="status-pill rounded-full bg-emerald-500/15 px-2 py-0.5 text-[10px] font-bold text-emerald-400 border border-emerald-500/30">ENABLED</span>
					{:else}
		...
		<div class="review-section space-y-3">
			...
			<div class="dl space-y-2">
				...
				<div class="dl-row flex items-baseline justify-between gap-4 text-sm">
					<span class="dl-label text-white/45">{m.setup_review_label_path() || 'Media Folder'}</span>
