...
<script lang="ts">
	...
	const filteredLanguages = $derived(
		sortedLanguages.filter((lang: AvailableLanguage) =>
...
<div class="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center p-4">
	<div class="w-full max-w-md">
		<!-- Welcome Card -->
		<div class="rounded-xl bg-white p-8 shadow-lg border border-slate-100">
			<!-- Logo/Icon -->
			<div class="mb-8 text-center">
				...
				<div class="space-y-6">
					<div>
						<label for="language-search" class="block text-sm font-medium text-slate-700 mb-2">
						...
						<div class="relative mb-4">
							<input
								...
								class="w-full rounded-md border border-slate-200 bg-white px-4 py-2 pl-9 text-slate-800 placeholder-slate-400 transition-all duration-200 focus:border-[#ff3e00] focus:outline-none focus:ring-1 focus:ring-[#ff3e00]/20 hover:border-slate-300"
							/>
						...
						<div class="max-h-64 overflow-y-auto rounded-md border border-slate-200 bg-white shadow-sm">
							{#each filteredLanguages as lang (lang)}
								<button
									class="flex w-full items-center justify-between px-4 py-2.5 text-left transition-all duration-200 hover:bg-slate-50 {selectedLanguage === lang ? 'bg-[#ff3e00]/5 text-[#ff3e00]' : 'text-slate-700'} border-b border-slate-100 last:border-b-0"
					...
					<button
						...
						class="w-full rounded-md bg-[#ff3e00] px-4 py-2.5 font-medium text-white transition-all duration-200 hover:bg-[#ff3e00]/90 focus:outline-none focus:ring-2 focus:ring-[#ff3e00]/20 mt-2"
					>
		...
		<div class="mt-5 text-center">
			<p class="text-xs text-slate-400">
				SveltyCMS Multi-Language Setup
			</p>
		</div>
	</div>
</div>
