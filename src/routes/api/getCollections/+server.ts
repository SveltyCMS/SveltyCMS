...
// Constants
const systemCollectionsFolder = path.join(import.meta.env.root, 'src', 'collections');
const userCollectionsFolder = path.join(import.meta.env.root, 'config', 'collections');
const compiledCollectionsFolder = path.join(import.meta.env.root, 'collections');
// Cache configuration
        ...
        if (fileName.endsWith('.ts')) {
           ...
           const matches = content.match(/export\s+default\s+({[\s\S]*?});/);
            if (!matches || !matches[1]) {
             ...
             const exportedObj = matches[1].trim();
