...
const loadServerModules = async (): Promise<ServerModules | null> => {
	...
	try {
		...
		moduleCache = {
			...
			path,
			crypto,
			compiledCollectionsFolder: path.join(import.meta.env.root, 'compiledCollections') // Only compiled collections folder
		};
		return moduleCache;
	} catch (error) {
...
export async function getCollectionFiles(user_id?: string): Promise<string[]> {
	if (browser) {
	...
	try {
		...
		// If user_id is provided, filter files specific to the user
		if (user_id) {
			const userSpecificFiles = filteredFiles.filter((file) => {
				// Assuming the file name contains the user_id
				return file.includes(user_id);
			});
