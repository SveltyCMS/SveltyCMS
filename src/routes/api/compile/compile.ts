				...
				if (!code || recompile) {
					...
					const transpileOutput = ts.transpileModule(content, {
						compilerOptions: {
							...
							outDir: collectionsFolderJS, // Set the output directory for incremental compilation
							tsBuildInfoFile: `${collectionsFolderJS}/.tsbuildinfo` // Specify the build info file path
						}
					});
