...
export const PATCH = apiHandler(async ({ locals, params, request }) => {
	...
	if (entries && Array.isArray(entries) && entries.length > 0) {
		...
		if (!result.success) throw new AppError(result.error.message, 500, 'DB_UPDATE_ERROR');
		...
		if (!verify.success || !verify.data) throw new AppError('Entry not found or access denied', 404, 'NOT_FOUND');
