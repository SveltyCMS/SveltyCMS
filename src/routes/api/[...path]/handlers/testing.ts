...
export async function handleTestingRoutes(
  ...
  try {
    ...
    if (action === "reset") {
      ...
      // 🚀 HARDENING: Re-provision system tables so the next 'seed' doesn't fail
      if (typeof (cms.db as any).provision === "function") {
        await (cms.db as any).provision();
      }
    ...
    logger.error(`[TestingHandler] Critical Error during action:`, err);
    return rawResponse(
      event,
      {
        success: false,
