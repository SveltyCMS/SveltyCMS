...
import { forgotFormSchema, resetFormSchema } from "@utils/schemas";
...
import { isSecureConnection, limiter, signInInternal, signUpInternal } from "./auth";
...
async function forgotPWInternal(event: RequestEvent, input: any) {
  ...
  try {
    ...
    if (user?._id) {
      ...
      resetToken = await auth.createToken({
        user_id: user._id,
      ...
      const resetLink = `${baseUrl}/login?token=${resetToken}&email=${encodeURIComponent(result.output.email)}`;
  ...
  return {
    ...
    smtpConfigured,
    // In test mode, return the token so the UI can auto-populate the reset form
    ...(process.env.TEST_MODE === "true" && result?.output?.email && resetToken
      ? { token: resetToken }
      : {}),
  };
}
