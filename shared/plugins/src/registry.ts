...
class PluginRegistry implements IPluginService {
	...
	async runMigrations(
		...
		try {
			...
			const appliedMigrationIds = new Set(
				appliedMigrationsResult.data?.map((m: PluginMigrationRecord) => m.migrationId) || []
			);
	...
	async runAllMigrations(dbAdapter: IDBAdapter, tenantId: string): Promise<DatabaseResult<void>> {
		try {
			...
			for (const plugin of plugins) {
				...
				if (!result.success) {
					logger.error(`Failed to run migrations for ${plugin.metadata.id}`, { error: result.error });
	 ...
	 */
	getSSRHooks(collectionId: string): PluginSSRHook[] {
		...
		for (const entry of this.plugins.values()) {
			...
			// Skip if plugin doesn't have SSR hook
			if (!plugin.ssrHook) {
				continue;
			}

			// Check if plugin is enabled for this collection
			if (!this.isEnabledForCollection(plugin.metadata.id, collectionId)) {
				continue;
			}
	 ...
	 */
	isEnabledForCollection(pluginId: string, collectionId: string): boolean {
	...
	private async ensureMigrationTable(dbAdapter: IDBAdapter): Promise<void> {
		...
		const createResult = await dbAdapter.crud.insert('plugin_migrations', {
			pluginId: '__INIT__',
