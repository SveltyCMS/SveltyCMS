/**
 * @file shared/stores/src/system/types.ts
...
// Granular initialization stages
export type InitializationStage =
	| 'starting'
	| 'db_connecting'
	| 'auth_initializing'
	| 'api_ready' // System can serve API requests
	| 'services_starting' // Background/Non-critical services
	| 'active'; // Fully operational

// Individual service health statuses
export type ServiceHealth = 'healthy' | 'unhealthy' | 'initializing' | 'breaker_tripped';
...
export interface ServiceCircuitBreaker {
	...
	lastFailure: number;
	nextRetry: number;
	isOpen: boolean; // true = tripped (blocks requests)
}
...
export interface SystemStateStore {
	...
	initializationStage: InitializationStage; // Granular stage tracking
