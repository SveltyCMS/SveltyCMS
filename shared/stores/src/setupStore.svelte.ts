/**
 * @file shared/stores/src/setupStore.svelte.ts
...
import { setupAdminSchema, dbConfigSchema, systemSettingsSchema } from '@shared/utils/formSchemas';
// import { logger } from '@shared/utils/logger'; // Removed to avoid potential server-side leakage
const logger = {
	...
	async function completeSetup(onSuccess?: (redirectPath: string) => void): Promise<boolean> {
		...
		try {
			const response = await fetch('/api/complete', {
				...
				credentials: 'include',
				body: JSON.stringify({
					dbConfig: wizard.dbConfig, // Required now that we deferred writing private.ts
			...
			if (data.requiresServerRestart) {
				logger.info('Server restart required, forcing transition to new URL...');
				setTimeout(() => {
					if (typeof window !== 'undefined') {
						// If we have a specific path (e.g. /en/collection/posts), go there directly!
						// This bypasses the root redirect logic and is faster/cleaner.
						if (data.redirectPath) {
							window.location.href = data.redirectPath;
						} else {
							window.location.reload(); // Fallback to root reload
						}
					}
				}, 2000); // 2s matches the smart-dev delay
			} else if (onSuccess && data.redirectPath) {
			...
			// If we get a network error or 504 during completion, it's likely because
			// the server is restarting (which is GOOD). Treat it as success!
			if (
				errorMsg.includes('Failed to fetch') ||
				errorMsg.includes('NetworkError') ||
				errorMsg.includes('504') ||
				errorMsg.includes('500') ||
				errorMsg.includes('JSON') // Unexpected end of JSON input (empty response)
			) {
				logger.info('Assuming success despite network error (server restart anticipated)', errorMsg);
				showToast('Setup complete! Redirecting...', 'success', 2000);

				// Clear store
				clear();

				// Force hard reload/redirect to root
				setTimeout(() => {
					if (typeof window !== 'undefined') {
