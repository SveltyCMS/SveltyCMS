...
// Modules will be registered at runtime (e.g. by CMS)
let _coreModules: Record<string, any> = {};
let _customModules: Record<string, any> = {};
...
class WidgetState {
	...
	/**
	 * Register widget modules at runtime.
	 * This allows the CMS to provide its widgets without the shared workspace
	 * needing a build-time dependency on the CMS.
	 */
	registerModules(core: Record<string, any>, custom: Record<string, any>) {
		_coreModules = core;
		_customModules = custom;
		logger.debug(`[WidgetStore] Registered modules: ${Object.keys(core).length} core, ${Object.keys(custom).length} custom`);
	}
	...
	async initialize(tenantId = 'default', dbAdapter?: DatabaseAdapter | null) {
		...
		try {
			// 1. Load modules from registered providers
			...
			for (const [path, module] of Object.entries(_coreModules)) {
			...
			for (const [path, module] of Object.entries(_customModules)) {
				const name = path.split('/').at(-2);
				if (!name || typeof (module as any).default !== 'function') continue;
